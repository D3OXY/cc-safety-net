import{n,a,De,Ke,R,rt,Ye,l,i,Je,P,_,Le,it,f,Ze,u,o,S,s,D,r,y,ue,G,at,lt,c,pe,fe,ct,O,Y,ie,x,dt,A,w,me,T,d,H,Xe,Qe,N,L,I,J,ge,Ne,ut,m,e,g,Me,ke,et,tt,se,Ee,ae,$e,Z,W,Fe,he,Re,U,nt,Pe,_e,Ae,Te,ye,ve,B,X,Q,le,h,z,je,He,We,k,Ue,Ie,Be,v,t,ee,be,b,C,E,pt,ze,Ve,p,V}from"./chunks/index-nzf0366w.js";import{ne,q,qe,M,F}from"./chunks/index-q2xz9c8h.js";var iu=["-h","--help"];function Dt(j,K){let te=Object.entries(j.booleans??{}),re=Object.entries(j.values??{}),oe=Object.entries(j.lists??{}),ce=Object.fromEntries(te.map(([Ge])=>[Ge,!1])),de={},we=Object.fromEntries(oe.map(([Ge])=>[Ge,[]])),xe=[],Ce=[],Se=!1,Oe=-1;for(let[Ge,st]of K.entries()){if(Ge<=Oe)continue;if(st==="--"){xe.push(...K.slice(Ge+1));break}if(iu.includes(st)){Se=!0;continue}let ft=te.find(([,yt])=>yt.includes(st));if(ft){ce[ft[0]]=!0;continue}let ot=re.find(([,yt])=>yt.includes(st));if(ot){let yt=K[Ge+1];if(yt===void 0||yt.startsWith("-")){Ce.push(`${st} requires a value`);continue}de[ot[0]]=yt,Oe=Ge+1;continue}let mt=oe.find(([,yt])=>yt.includes(st));if(mt){let yt=K.slice(Ge+1),ht=yt.findIndex((wt)=>wt.startsWith("-")),gt=yt.slice(0,ht===-1?yt.length:ht);if(gt.length===0){Ce.push(`${st} requires at least one value`);continue}we[mt[0]]=[...we[mt[0]]??[],...gt],Oe=Ge+gt.length;continue}if(st.startsWith("-")){Ce.push(`Unknown option for ${j.label}: ${st}`);continue}if(j.positionals==="tail"){xe.push(...K.slice(Ge));break}xe.push(st)}if(j.positionals!=="list"&&j.positionals!=="tail")Ce.push(...xe.map((Ge)=>`Unexpected argument for ${j.label}: ${Ge}`));return{flags:ce,values:de,lists:we,positionals:xe,help:Se,errors:Ce}}function Wt(j){for(let K of j)console.error(K);return j.length>0}import{readdirSync as pu,statSync as Xi,unlinkSync as fu}from"node:fs";import{basename as Qi,dirname as mu,isAbsolute as gu,join as yu,relative as hu,resolve as vu,sep as bu}from"node:path";import{existsSync as su,readdirSync as au,readFileSync as lu}from"node:fs";import{join as cu}from"node:path";var Ki=(j)=>{let K=Date.now()-new Date(j).getTime();if(!Number.isFinite(K))return"";let te=Math.floor(K/60000),re=Math.floor(te/60),oe=Math.floor(re/24);if(oe>0)return`${oe}d ago`;if(re>0)return`${re}h ago`;if(te>0)return`${te}m ago`;return"just now"},cr=(j)=>{let K=(j??"").trim().split(/\s+/).filter((oe)=>oe&&!/^[A-Za-z_][A-Za-z0-9_]*=/.test(oe)),te=K[0]?.split("/").pop();if(!te)return null;let re=K[1];return re&&/^[a-z][a-z0-9-]*$/.test(re)?`${te} ${re}`:te};function sn(j,K){try{return au(j,{withFileTypes:!0,encoding:"utf8"}).flatMap((te)=>{let re=cu(j,te.name);if(te.isDirectory())return sn(re,K);if(te.name.endsWith(".jsonl"))return[re];return[]})}catch{if(K&&su(j))K.count++;return[]}}function Yi(j){let K=(oe)=>`${oe.sessionId}
${cr(oe.segment||oe.command)}`,te=j.filter((oe)=>oe.decision!=="allow"),re=te.filter((oe)=>oe.sessionId).reduce((oe,ce)=>oe.set(K(ce),(oe.get(K(ce))??0)+1),new Map);return new Set(te.filter((oe)=>oe.failureStage||(re.get(K(oe))??0)>=2))}var du=["segment","reason","sessionId","decision","agent","ruleId","failureStage"];function uu(j){if(!j||typeof j!=="object"||Array.isArray(j))return!1;let K=j;if(typeof K.ts!=="string"||typeof K.command!=="string")return!1;return du.every((te)=>K[te]===void 0||typeof K[te]==="string")}function Ln(j,K){try{return lu(j,"utf-8").split(`
`).filter(Boolean).flatMap((te)=>{try{let re=JSON.parse(te);if(!uu(re)){if(K)K.count++;return[]}return[re]}catch{if(K)K.count++;return[]}})}catch{if(K)K.count++;return[]}}function At(j){return Array.from(j,(K)=>{let te=K.charCodeAt(0);if(te<=31||te>=127&&te<=159)return`\\x${te.toString(16).padStart(2,"0")}`;return K}).join("")}function Lu(j,K){let te=ne(j),re=Dt({label:"logs",booleans:{all:["--all"],suspect:["--suspect"],json:["--json"],pruneLegacy:["--prune-legacy"],dryRun:["--dry-run"]},values:{id:["--id"],limit:["--limit"],since:["--since"],agent:["--agent"],rule:["--rule"],session:["--session"],project:["--project"]}},K);if(Wt(re.errors))return null;if(re.values.id!==void 0&&!/^[a-f0-9]{16}$/.test(re.values.id))return console.error("--id must be 16 hexadecimal characters"),null;let oe=re.values.limit===void 0?20:Zi(re.values.limit);if(oe===null)return console.error("--limit must be a positive number"),null;let ce=re.values.since===void 0?Math.min(30,te):Zi(re.values.since);if(ce===null||ce>te)return console.error(`--since must be a positive number of days no greater than ${te}`),null;let de={limit:oe,limitExplicit:re.values.limit!==void 0,since:ce,sinceExplicit:re.values.since!==void 0,all:re.flags.all,json:re.flags.json,suspect:re.flags.suspect,pruneLegacy:re.flags.pruneLegacy,dryRun:re.flags.dryRun,id:re.values.id,agent:re.values.agent,rule:re.values.rule,session:re.values.session,project:re.values.project===void 0?void 0:vu(re.values.project)};if(de.id&&(de.agent!==void 0||de.rule!==void 0||de.session!==void 0||de.project!==void 0||de.suspect||de.sinceExplicit||de.limitExplicit))return console.error("--id cannot be combined with --agent, --rule, --session, --project, --suspect, --since, or --limit"),null;if(de.pruneLegacy&&(de.id!==void 0||de.agent!==void 0||de.rule!==void 0||de.session!==void 0||de.project!==void 0||de.suspect||de.all||de.sinceExplicit||de.limitExplicit))return console.error("--prune-legacy cannot be combined with --id, --agent, --rule, --session, --project, --suspect, --all, --since, or --limit"),null;if(de.dryRun&&!de.pruneLegacy)return console.error("--dry-run requires --prune-legacy"),null;return de}async function es(j,K,te={}){let re=Lu(j,K);if(!re)return 1;let oe=te.logsDir??M(j);if(re.pruneLegacy)return wu(oe,re.json,re.dryRun);if(!oe)return console.log(re.json?"[]":re.id?`No retained audit log entry found for id ${At(re.id)}.`:"No audit log entries found."),0;q(j,oe);let ce={count:0},de=sn(oe,ce).flatMap((Oe)=>Ln(Oe,ce).map((Ge)=>({entry:Ge,file:Oe})));if(ce.count>0)console.error(`warning: ${ce.count} audit log ${ce.count===1?"source":"sources"} could not be read; these results are incomplete`);if(re.id)return Su(de,re,te.timeZone);let we=Date.now()-re.since*24*60*60*1000,xe=de.filter((Oe)=>Ru(Oe,re,oe,we)),Ce=re.suspect?Yi(xe.map((Oe)=>Oe.entry)):null,Se=(Ce?xe.filter((Oe)=>Ce.has(Oe.entry)):xe).sort((Oe,Ge)=>Date.parse(Ge.entry.ts)-Date.parse(Oe.entry.ts)).slice(0,re.limit);if(re.json)return console.log(JSON.stringify(Se.map((Oe)=>Oe.entry),null,2)),0;if(Se.length===0)return console.log("No audit log entries found."),0;for(let Oe of Se)console.log(Du(Oe.entry,te.timeZone));return 0}function wu(j,K,te){let re=j?xu(j).map((we)=>yu(j,we)):[];if(te)return ku(re,K);let oe=[],ce=0,de=0;for(let we of re){let xe=Xi(we,{throwIfNoEntry:!1})?.size??0,Ce=Cu(we);if(Ce){oe.push(`${Qi(we)}: ${Ce}`);continue}ce++,de+=xe}if(K)return console.log(JSON.stringify({removedFiles:ce,removedBytes:de,failedFiles:oe.length})),oe.length===0?0:1;console.log(ce===0&&oe.length===0?"No legacy audit log files found.":`Removed ${ce} legacy audit log ${ce===1?"file":"files"} (${ts(de)}).`);for(let we of oe)console.error(`Could not remove ${At(we)}`);if(console.log("Nested v2 audit logs were not changed."),ce>0)console.log("This deletion cannot be undone.");return oe.length===0?0:1}function ku(j,K){let te=j.reduce((re,oe)=>re+(Xi(oe,{throwIfNoEntry:!1})?.size??0),0);if(K)return console.log(JSON.stringify({dryRun:!0,files:j.length,bytes:te})),0;if(console.log(j.length===0?"No legacy audit log files found.":`Would remove ${j.length} legacy audit log ${j.length===1?"file":"files"} (${ts(te)}).`),console.log("Nested v2 audit logs are not included."),j.length>0)console.log("Run the same command without --dry-run to delete them.");return 0}function xu(j){try{return pu(j,{withFileTypes:!0}).filter((K)=>K.isFile()&&K.name.endsWith(".jsonl")).map((K)=>K.name)}catch{return[]}}function Cu(j){try{return fu(j),null}catch(K){return K instanceof Error?K.message:String(K)}}function ts(j){let K=["B","KiB","MiB","GiB"],te=Math.min(Math.floor(Math.log2(Math.max(j,1))/10),K.length-1);return`${Math.round(j/1024**te*10)/10} ${K[te]}`}function Su(j,K,te){let re=j.filter((ce)=>ce.entry.id===K.id);if(re.length>1)return console.error(`Multiple audit log entries found for id ${At(K.id??"")}.`),1;if(K.json)return console.log(JSON.stringify(re.map((ce)=>ce.entry),null,2)),0;let oe=re[0];if(!oe)return console.log(`No retained audit log entry found for id ${At(K.id??"")}.`),0;return console.log(Au(oe.entry,te)),0}function Ru(j,K,te,re){if(!K.all&&j.entry.decision==="allow")return!1;if(Date.parse(j.entry.ts)<re)return!1;if(K.agent!==void 0&&j.entry.agent!==K.agent)return!1;if(K.rule!==void 0&&j.entry.ruleId!==K.rule)return!1;if(K.session!==void 0&&!Pu(j,te,K.session))return!1;if(K.project!==void 0&&!Eu(j.entry.cwd,K.project))return!1;return!0}function Pu(j,K,te){if(j.entry.sessionId===te)return!0;return mu(j.file)===K&&Qi(j.file,".jsonl")===te}function Eu(j,K){if(!j)return!1;let te=hu(K,j);return te!==".."&&!te.startsWith(`..${bu}`)&&!gu(te)}function Du(j,K){let te=At(j.id??"-"),re=At(j.decision??"deny"),oe=j.cwd?`  [${At(j.cwd)}]`:"",ce=j.segment||j.command,de=ce===j.command?"":"↳ ",we=ce.length>50?`${ce.slice(0,50)}…`:ce;return`${te.padEnd(16)}  ${At(ns(j.ts,K))}  ${re.padEnd(5)}  ${At(j.agent??"-").padEnd(15)}  ${At(j.ruleId??"-").padEnd(20)}  ${de}${At(we)}${oe}`}function Au(j,K){let te=(oe)=>At(oe===void 0||oe===null||oe===""?"-":oe),re=j.shape?`${j.agent??"-"} (shape: ${j.shape})`:j.agent??"-";return[`id:        ${te(j.id)}`,`ts:        ${te(ns(j.ts,K))}`,`decision:  ${te(j.decision)}`,`agent:     ${te(re)}`,`level:     ${te(j.level)}`,`tool:      ${te(j.toolName)}`,`rule:      ${te(j.ruleId)}`,`intent:    ${te(j.intent)}`,`stage:     ${te(j.failureStage)}`,`error:     ${te(j.errorCode)}`,`session:   ${te(j.sessionId)}`,`cwd:       ${te(j.cwd)}`,`version:   ${te(j.v)}`,`truncated: ${te(j.truncated===!0?"yes":void 0)}`,`reason:    ${te(j.reason)}`,`command:   ${te(j.command)}`,`segment:   ${te(j.segment)}`].join(`
`)}function ns(j,K){let te=new Date(j);if(Number.isNaN(te.getTime()))return j;return new Intl.DateTimeFormat("sv-SE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23",timeZone:K}).format(te)}function Zi(j){let K=Number(j);return Number.isFinite(K)&&K>0?K:null}var rs={name:"doctor",aliases:["--doctor"],description:"Run diagnostic checks to verify installation and configuration",usage:"doctor [options]",options:[{flags:"--json",description:"Output diagnostics as JSON"},{flags:"--skip-update-check",description:"Skip npm registry version check"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net doctor","cc-safety-net doctor --json","cc-safety-net doctor --skip-update-check"]};var os={name:"explain",description:"Show step-by-step analysis trace of how a command would be analyzed",usage:"explain [options] <command>",argument:"<command>",options:[{flags:"--json",description:"Output analysis as JSON"},{flags:"--cwd",argument:"<path>",description:"Use custom working directory"},{flags:"-h, --help",description:"Show this help"}],examples:['cc-safety-net explain "git reset --hard"','cc-safety-net explain --json "rm -rf /"','cc-safety-net explain --cwd /tmp "git status"']};var is={name:"gui",description:"Open the local policy editor GUI",usage:"gui [options]",options:[{flags:"--no-open",description:"Print the URL without opening a browser"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net gui","cc-safety-net gui --no-open"]};var Fn=[{id:"antigravity-cli",displayName:"Antigravity CLI",doctorOrder:3,runtime:{order:1,flags:["-ac","--agy-cli"],description:"Run as Antigravity CLI PreToolUse hook",legacyTopLevelFlags:[]},install:{order:2,flag:"--agy-cli",artifactKind:"hook config",probeCommand:["agy","--version"]}},{id:"claude-code",displayName:"Claude Code",doctorOrder:1,runtime:{order:2,displayName:"Coding CLI",flags:["-cc","--coding-cli"],legacyFlags:["--claude-code"],description:"Run as Coding CLI PreToolUse hook",legacyTopLevelFlags:["-cc","--claude-code"]},install:{order:3,flag:"--claude-code",artifactKind:"plugin",probeCommand:["claude","--version"]}},{id:"codex",displayName:"Codex",doctorOrder:4,runtime:{order:3,flags:["-cx","--codex"],description:"Run as a Codex PreToolUse hook",legacyTopLevelFlags:[]},install:{order:4,flag:"--codex",artifactKind:"plugin",probeCommand:["codex","--version"]}},{id:"copilot-cli",displayName:"GitHub Copilot CLI",doctorOrder:7,runtime:{order:6,flags:["-cp","--copilot-cli"],description:"Run as GitHub Copilot CLI PreToolUse hook",legacyTopLevelFlags:["-cp","--copilot-cli"]},install:{order:7,flag:"--copilot-cli",artifactKind:"plugin",probeCommand:["copilot","--binary-version"]}},{id:"gemini-cli",displayName:"Gemini CLI",doctorOrder:6,runtime:{order:5,flags:["-gc","--gemini-cli"],description:"Run as Gemini CLI BeforeTool hook",legacyTopLevelFlags:["-gc","--gemini-cli"]},install:{order:6,flag:"--gemini-cli",artifactKind:"extension",probeCommand:["gemini","--version"]}},{id:"grok-build",displayName:"Grok Build",doctorOrder:8,runtime:{order:7,flags:["-gb","--grok-build"],description:"Run as Grok Build PreToolUse hook",legacyTopLevelFlags:[]},install:{order:8,flag:"--grok-build",artifactKind:"hook config",probeCommand:["grok","--version"]}},{id:"hermes-agent",displayName:"Hermes Agent",doctorOrder:9,runtime:{order:8,flags:["-ha","--hermes-agent"],description:"Run as Hermes Agent pre_tool_call hook",legacyTopLevelFlags:[]},install:{order:9,flag:"--hermes-agent",artifactKind:"plugin",probeCommand:["hermes","--version"]}},{id:"kimi-code",displayName:"Kimi Code",doctorOrder:10,runtime:{order:9,flags:["-kc","--kimi-code"],description:"Run as Kimi Code PreToolUse hook",legacyTopLevelFlags:[]},install:{order:10,flag:"--kimi-code",artifactKind:"hook config",probeCommand:["kimi","--version"]}},{id:"openclaw",displayName:"OpenClaw",doctorOrder:11,install:{order:11,flag:"--openclaw",artifactKind:"plugin",probeCommand:["openclaw","--version"]}},{id:"opencode",displayName:"OpenCode",doctorOrder:12,install:{order:12,flag:"--opencode",artifactKind:"plugin",probeCommand:["opencode","--version"]}},{id:"pi",displayName:"Pi",doctorOrder:13,install:{order:13,flag:"--pi",artifactKind:"package",probeCommand:["pi","--version"]}},{id:"cursor",displayName:"Cursor",doctorOrder:5,runtime:{order:4,flags:["-cu","--cursor"],description:"Run as Cursor preToolUse hook",legacyTopLevelFlags:[]},install:{order:5,flag:"--cursor",artifactKind:"hook config",probeCommand:["cursor","--version"]}},{id:"amp",displayName:"Amp Code",doctorOrder:2,install:{order:1,flag:"--amp",artifactKind:"plugin",probeCommand:["amp","--version"]}}],dr=Fn.slice().sort((j,K)=>j.doctorOrder-K.doctorOrder).map((j)=>j.id),Nn=Fn.filter((j)=>("runtime"in j)).slice().sort((j,K)=>j.runtime.order-K.runtime.order).map((j)=>({id:j.id,displayName:"displayName"in j.runtime?j.runtime.displayName:j.displayName,flags:j.runtime.flags,legacyFlags:"legacyFlags"in j.runtime?j.runtime.legacyFlags:[],description:j.runtime.description,legacyTopLevelFlags:j.runtime.legacyTopLevelFlags})),Ft=Fn.slice().sort((j,K)=>j.install.order-K.install.order).map((j)=>({id:j.id,...j.install})).map(({order:j,...K})=>K),l0=Object.fromEntries(Fn.map((j)=>[j.id,j.displayName]));function Ct(j){return Fn.find((K)=>K.id===j)?.displayName??j}var _u=Nn.map((j)=>({flags:j.flags.join(", "),description:j.description})),Tu=Nn.flatMap((j)=>j.flags.map((K)=>`cc-safety-net hook ${K}`)),ss={name:"hook",description:"Run as an agent CLI hook (reads JSON from stdin)",usage:"hook INTEGRATION_FLAG",options:[..._u,{flags:"-h, --help",description:"Show this help"}],examples:Tu};var as={name:"install",description:"Install CC Safety Net into a coding agent CLI",usage:"install [TARGET_FLAG]",options:[...Ft.map((j)=>({flags:j.flag,description:`Install ${Ct(j.id)} ${j.artifactKind}`})),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net install",...Ft.map((j)=>`cc-safety-net install ${j.flag}`)]},ls={name:"uninstall",description:"Uninstall CC Safety Net from a coding agent CLI",usage:"uninstall [TARGET_FLAG]",options:[...Ft.map((j)=>({flags:j.flag,description:`Uninstall ${Ct(j.id)} ${j.artifactKind}`})),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net uninstall",...Ft.map((j)=>`cc-safety-net uninstall ${j.flag}`)]},cs={name:"update",description:"Update every installed CC Safety Net integration to the latest version",usage:"update",options:[{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net update"]};var ds={name:"logs",description:"Browse audit log entries recorded by hooks",usage:"logs [options]",options:[{flags:"--id",argument:"<id>",description:"Show one entry from retained history by its 16-character id (not guaranteed once it is older than the configured retention)"},{flags:"--limit",argument:"<n>",description:"Maximum entries to print",default:"20"},{flags:"--since",argument:"<days>",description:"Only include entries newer than this many days (max: the configured audit retention, 1-365)",default:"30"},{flags:"--agent",argument:"<name>",description:"Filter by agent name"},{flags:"--rule",argument:"<ruleId>",description:"Filter by rule id"},{flags:"--session",argument:"<id>",description:"Filter by session id"},{flags:"--project",argument:"<path>",description:"Filter by project path"},{flags:"--suspect",description:"Only denials that look like false positives"},{flags:"--all",description:"Include allow entries"},{flags:"--prune-legacy",description:"Permanently delete all legacy root-level logs; nested logs are untouched"},{flags:"--dry-run",description:"With --prune-legacy, report what would be deleted and delete nothing"},{flags:"--json",description:"Output entries as JSON"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net logs --id 3fa9c2d1a70e8b42","cc-safety-net logs --agent claude-code","cc-safety-net logs --project . --since 7","cc-safety-net logs --suspect --since 7","cc-safety-net logs --json","cc-safety-net logs --prune-legacy --dry-run","cc-safety-net logs --prune-legacy"]};var ur={name:"policy",description:"Check and apply project or user policy proposals",usage:"policy <subcommand>",subcommands:[{usage:"check <file>",description:"Validate a policy proposal and print its diff"},{usage:"apply <file>",description:"Apply a proposal after confirming in a terminal"}],options:[{flags:"-g, --global",description:"Use the user-scope policy instead of the project one"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net policy check proposal.json","cc-safety-net policy apply proposal.json","cc-safety-net policy apply proposal.json --global"]};var mo=[{flags:"--ref",argument:"<ref>",description:"Use a branch, tag, or commit"},{flags:"--only",argument:"<rulebook...>",description:"Add only these repository rulebooks"},{flags:"-g, --global",description:"Use user-scope rule config"},{flags:"-h, --help",description:"Show this help"}],go=["cc-safety-net rule add project-rules","cc-safety-net rule add acme/safety-rules","cc-safety-net rule add acme/safety-rules --only aws gcloud","cc-safety-net rule add acme/safety-rules --ref v2 --only aws","cc-safety-net rule add --only terraform aws"],wn={name:"rule",description:"Manage CC Safety Net rule config and rulebook sources",usage:"rule <subcommand>",subcommands:[{usage:"init [--example]",description:"Create inert rule config"},{usage:"add [source] [--ref <ref>] [--only <rulebook...>]",description:"Add rulebook sources and sync"},{usage:"remove <source>",description:"Remove a rulebook source and sync"},{usage:"update [source]",description:"Re-fetch and vendor remote rulebooks"},{usage:"sync",description:"Deprecated: migrate lock and cache leftovers"},{usage:"list",description:"List active rulebooks"},{usage:"wrapper add <command>",description:"Trust a transparent command wrapper"},{usage:"wrapper remove <command>",description:"Remove a transparent command wrapper"},{usage:"wrapper list",description:"List transparent command wrappers"},{usage:"migrate [--cleanup]",description:"Migrate legacy inline rules"},{usage:"doc",description:"Print the rulebook authoring guide"},{usage:"verify",description:"Validate rule config files"}],options:[{flags:"-g, --global",description:"Use user-scope rule config"},{flags:"--cleanup",description:"Delete legacy files after rule migrate verifies them"},{flags:"--delete-source",description:"Delete clean local source directory on remove"},{flags:"--example",description:"Create an inactive example rulebook with rule init"},...mo.slice(0,2),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net rule init","cc-safety-net rule init --example","cc-safety-net rule wrapper add rtk",...go,"cc-safety-net rule update","cc-safety-net rule migrate --cleanup","cc-safety-net rule verify"]};var us={name:"status",description:"Show what the runtime is enforcing right now",usage:"status",options:[{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net status"]};var ps={name:"statusline",description:"Print status line with mode indicators for shell integration",usage:"statusline --claude-code",options:[{flags:"-cc, --claude-code",description:"Print status line for Claude Code"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net statusline -cc","cc-safety-net statusline --claude-code"]};var pr=[us,rs,ds,os,wn,ur,as,cs,ls,ss,is,ps];function Iu(j){return j.aliases??[]}function fr(j){let K=j.toLowerCase();return pr.find((te)=>te.name.toLowerCase()===K||Iu(te).some((re)=>re.toLowerCase()===K))}import{basename as $u}from"node:path";function mr(j,K=7,te=M(j)){let re=Date.now()-K*24*60*60*1000,oe=[],ce=new Set,de=0,we,xe,Ce,Se;if(te)q(j,te);let Oe={count:0},Ge=te?sn(te,Oe):[];for(let ft of Ge)for(let ot of Ln(ft,Oe)){if(ot.decision==="allow")continue;let mt=new Date(ot.ts).getTime();if(mt>=re){if(de++,ce.add(ot.sessionId??$u(ft,".jsonl")),xe===void 0||mt<=xe)we=ot.ts,xe=mt;if(Se===void 0||mt>Se)Ce=ot.ts,Se=mt;Ou(oe,ot,mt)}}let st=oe.map((ft)=>({timestamp:ft.ts,command:ft.command,reason:ft.reason,relativeTime:Ki(new Date(ft.ts))}));return{totalBlocked:de,sessionCount:ce.size,recentEntries:st,oldestEntry:we,newestEntry:Ce,unreadable:Oe.count}}function Ou(j,K,te){let re=j.findIndex((oe)=>te>new Date(oe.ts).getTime());if(re===-1){if(j.length<3)j.push(K);return}if(j.splice(re,0,K),j.length>3)j.pop()}import{dirname as Ju}from"node:path";import{dirname as ju,join as Fu,resolve as Nu}from"node:path";var Mu="config.json";function Tt(j,K,te,re){y(Hu(j),`${JSON.stringify(K,null,2)}
`,te,re)}function Hu(j){return typeof j==="string"?D(j):j}function ho(j){return{errors:se(Gu(j),": "," "),ruleNames:new Set($e(j).map((K)=>K.toLowerCase()))}}var Uu="must match pattern (letters, numbers, hyphens, underscores; max 64 chars)",fs="must match pattern (letters, numbers, hyphens, underscores)";function Gu(j){if(!ms(j))return[e([],"Config must be an object")];return[...j.version===1?[]:[e(["version"],"must be 1")],...Bu(j.rules)]}function Bu(j){if(j===void 0)return[];if(!Array.isArray(j))return[e(["rules"],"must be an array")];return[...j.flatMap((K,te)=>ms(K)?qu(K,["rules",te]):[e(["rules",te],"must be an object")]),...Me(j)]}function qu(j,K){return[...yo(j.name,[...K,"name"],"required string",c,Uu),...yo(j.command,[...K,"command"],"required string",L,fs),...j.subcommand===void 0?[]:yo(j.subcommand,[...K,"subcommand"],"must be a string if provided",L,fs),...Vu(j.block_args,[...K,"block_args"]),...zu(j.reason,[...K,"reason"]),...j.intent===void 0||Ee(j.intent)?[]:[e([...K,"intent"],ke)]]}function yo(j,K,te,re,oe){if(typeof j!=="string")return[e(K,te)];return re.test(j)?[]:[e(K,oe)]}function Vu(j,K){if(!Array.isArray(j))return[e(K,"required array")];if(j.length===0)return[e(K,"must have at least one element")];return j.flatMap((te,re)=>{if(typeof te!=="string")return[e([...K,re],"must be a string")];return te===""?[e([...K,re],"must not be empty")]:[]})}function zu(j,K){if(typeof j!=="string")return[e(K,"required string")];if(j==="")return[e(K,"must not be empty")];return j.length>I?[e(K,`must be at most ${I} characters`)]:[]}function ms(j){return!!j&&typeof j==="object"&&!Array.isArray(j)}function vo(j){let K=gs(j);if(!K.ok)return K.result;return ho(K.parsed)}function gs(j){let K=[],te=new Set;try{let re=typeof j==="string"?D(j):j,oe=r(re);if(oe===null)return K.push(`File not found: ${re.path}`),{ok:!1,result:{errors:K,ruleNames:te}};if(!oe.trim())return K.push("Config file is empty"),{ok:!1,result:{errors:K,ruleNames:te}};return{ok:!0,parsed:JSON.parse(oe)}}catch(re){if(re instanceof o)return K.push(re.message),{ok:!1,result:{errors:K,ruleNames:te}};let oe=re instanceof Error?re.message:String(re);return K.push(re instanceof SyntaxError?"Invalid JSON":oe),{ok:!1,result:{errors:K,ruleNames:te}}}}function ys(j){return Nu(j,".safety-net.json")}function en(j){let K=gs(j);if(!K.ok)return K.result;let te=et(K.parsed);return{errors:te.errors,ruleNames:te.sources}}function gr(j,K={}){return Fu(ju(me(j,K)),Mu)}function hs(j,K,te){let re;try{if(r(K)===null)return{path:j,exists:!1,valid:!1,ruleCount:0};re=en(K),re.errors.push(...W(j,te))}catch(oe){if(!(oe instanceof o))throw oe;re={errors:[oe.message],ruleNames:new Set}}return{path:j,exists:!0,valid:re.errors.length===0,ruleCount:re.ruleNames.size,...re.errors.length>0?{errors:re.errors}:{}}}function Wu(j,K){return{source:K,name:j.name,command:j.command,subcommand:j.subcommand,blockArgs:[...j.block_args],reason:j.reason}}function vs(j,K,te){let re=te?.userConfigPath??T(j),oe=te?.projectConfigPath??A(K),ce=Ju(re),de=Z(j,{cwd:K,userConfigPath:re,projectConfigPath:oe,userConfigDir:ce}),we=H(j,{cwd:K,userConfigPath:re,projectConfigPath:oe,userConfigDir:ce}),xe=new Map(de.rulebooks.flatMap((Ce)=>Ce.rules.map((Se)=>[Se,Ce.source])));return{userConfig:hs(re,we.userConfigTarget,we.userScope),projectConfig:hs(oe,we.projectConfigTarget,we.projectScope),effectiveRules:de.rules.map((Ce)=>Wu(Ce,xe.get(Ce.name)??"project")),shadowedRules:[]}}var Ku=[{flag:i.level,description:"Safety level preset: standard, strict, or paranoid",defaultBehavior:"standard"},{flag:i.strict,description:"Legacy; equivalent to safety.overrides.fail_closed",defaultBehavior:"permissive"},{flag:i.paranoid,description:"Legacy; equivalent to safety.overrides.paranoid_rm and paranoid_interpreters",defaultBehavior:"off"},{flag:i.paranoidRm,description:"Legacy; equivalent to safety.overrides.paranoid_rm",defaultBehavior:"off"},{flag:i.paranoidInterpreters,description:"Legacy; equivalent to safety.overrides.paranoid_interpreters",defaultBehavior:"off"},{flag:i.worktree,description:"Allow local git discards in linked worktrees",defaultBehavior:"off"},{flag:i.debug,description:"Print diagnostic messages to stderr",defaultBehavior:"off"},{flag:i.auditScope,description:"Command decisions recorded: all, or blocked (privacy-minimizing, denials only)",defaultBehavior:"all"}];function bs(j){return[...Ku.map((K)=>({name:K.flag.name,value:Le(K.flag,j.env),isSet:it(K.flag,j.env),legacyName:K.flag.legacyName,legacyValue:K.flag.legacyName?j.env.get(K.flag.legacyName):void 0,legacyIsSet:K.flag.legacyName?j.env.get(K.flag.legacyName)!==void 0:void 0,description:K.description,defaultBehavior:K.defaultBehavior})),{name:"CC_SAFETY_NET_HOME",value:j.env.get("CC_SAFETY_NET_HOME"),isSet:j.env.get("CC_SAFETY_NET_HOME")!==void 0,description:"Override user-scope config/cache directory",defaultBehavior:"~/.cc-safety-net"}]}var Ls={error:0,warning:1,info:2},Yu=["policy","config","audit"];function Zu(j){return j.map((K)=>{if(K==="ownership")return"is not owned by the current user";if(K==="permissions")return"has unsafe permissions";if(K==="symlink")return"is a symbolic link";return"is not a directory"}).join(" and ")}var Xu=[{derive:(j)=>j.hooks.length>0&&j.hooks.every((K)=>!K.configured)?[{checkId:"integration.none-configured",severity:"error",title:"No integration configured",detail:"CC Safety Net is not connected to any supported coding-agent integration.",fixHint:"Run `cc-safety-net install` and configure at least one integration."}]:[]},{derive:(j)=>j.hooks.filter((K)=>K.inspectionStatus==="failed").map((K)=>{let te=Ct(K.platform);return{checkId:"integration.inspection-failed",severity:"error",title:`${te} inspection failed`,detail:`Doctor could not verify the ${te} integration configuration.`,fixHint:`Correct the reported ${te} configuration error, then run \`cc-safety-net doctor\` again.`,integration:K.platform}})},{derive:(j)=>j.userConfig.exists&&!j.userConfig.valid?[{checkId:"config.user-invalid",severity:"error",title:"User configuration is invalid",detail:"Doctor could not load a valid user rules configuration.",fixHint:"Run `cc-safety-net rule verify`, correct the reported error, then rerun doctor.",path:j.userConfig.path}]:[]},{derive:(j)=>j.projectConfig.exists&&!j.projectConfig.valid?[{checkId:"config.project-invalid",severity:"error",title:"Project configuration is invalid",detail:"Doctor could not load a valid project rules configuration.",fixHint:"Run `cc-safety-net rule verify`, correct the reported error, then rerun doctor.",path:j.projectConfig.path}]:[]},{derive:(j)=>j.configState.state==="degraded"?[{checkId:"config.runtime-degraded",severity:"warning",title:"Runtime is enforcing a fallback configuration",detail:`The rejected candidate configuration is not active: ${j.configState.reason}`,fixHint:"Fix the file named in the reason, or run `cc-safety-net rule update` to vendor a remote source, then rerun doctor."}]:[]},{derive:(j)=>j.v2Leftovers&&j.v2Leftovers.length>0?[{checkId:"config.v2-leftovers",severity:"info",title:"Rulebook lock and cache leftovers detected",detail:`Files an earlier version left behind are no longer read: ${j.v2Leftovers.join(", ")}.`,fixHint:"Run `cc-safety-net rule sync` (add `--global` for user scope) to migrate them, then rerun doctor."}]:[]},{derive:(j)=>{let K=j.environment.find((te)=>te.name==="CC_SAFETY_NET_AUDIT_SCOPE");return Je(K?.value)==="invalid"?[{checkId:"environment.audit-scope-invalid",severity:"warning",title:"Audit scope value is invalid",detail:"CC_SAFETY_NET_AUDIT_SCOPE is not `all` or `blocked`, so allowed command decisions are not recorded.",fixHint:"Set CC_SAFETY_NET_AUDIT_SCOPE to `all` or `blocked`, then restart the integration."}]:[]}},...Yu.map((j)=>({derive:(K)=>K.posture.directories.filter((te)=>te.kind===j&&te.status==="unsafe").map((te)=>({checkId:`posture.${j}-directory-unsafe`,severity:"error",title:`${j[0]?.toUpperCase()}${j.slice(1)} directory is unsafe`,detail:`The ${j} directory ${Zu(te.issues)}.`,fixHint:"Ensure this is a real directory owned by the current user with no group or other write access, then rerun doctor.",...te.path?{path:te.path}:{}}))})),{derive:(j)=>{let K=[...j.effectiveSafety.weakenedRuleOverrides].sort();return K.length>0?[{checkId:"posture.rule-overrides-weaken-preset",severity:"warning",title:"Rule overrides weaken the selected preset",detail:`Explicit overrides disable rules the resolved preset would enable: ${K.join(", ")}.`,fixHint:`Remove these \`off\` overrides or set them to \`on\`: ${K.join(", ")}.`}]:[]}}];function ws(j){return Xu.flatMap((K,te)=>K.derive(j).map((re,oe)=>({finding:re,catalogOrder:te,occurrence:oe}))).sort((K,te)=>Ls[K.finding.severity]-Ls[te.finding.severity]||K.catalogOrder-te.catalogOrder||K.occurrence-te.occurrence).map((K)=>K.finding)}function Gt(){return Boolean(process.stdout.isTTY&&!process.env.NO_COLOR)}var Qu=(j)=>Gt()?`\x1B[32m${j}\x1B[0m`:j,ep=(j)=>Gt()?`\x1B[33m${j}\x1B[0m`:j,tp=(j)=>Gt()?`\x1B[34m${j}\x1B[0m`:j,np=(j)=>Gt()?`\x1B[35m${j}\x1B[0m`:j,rp=(j)=>Gt()?`\x1B[36m${j}\x1B[0m`:j,op=(j)=>Gt()?`\x1B[31m${j}\x1B[0m`:j,ip=(j)=>Gt()?`\x1B[2m${j}\x1B[0m`:j,sp=(j)=>Gt()?`\x1B[1m${j}\x1B[0m`:j,vt={green:Qu,yellow:ep,blue:tp,magenta:np,cyan:rp,red:op,dim:ip,bold:sp},ap="\x1B[0m",lp=[39,82,198,226,208,51,196,46,201,214,93,154,220,27,49,190,200,33,129,227,45,160,63,118,123,202];function cp(j){let K=j;return()=>(K=(K*1664525+1013904223)%4294967296,K/4294967296)}function dp(j){let K=[...lp],te=cp(j);for(let re=K.length-1;re>0;re--){let oe=Math.floor(te()*(re+1)),ce=K[re];K[re]=K[oe],K[oe]=ce}return K}function up(j,K=0){if(!Gt())return"";let te=dp(K);return`\x1B[38;5;${te[j%te.length]}m`}function ks(j,K,te=0){if(!Gt())return`"${j}"`;return`${up(K,te)}"${j}"${ap}`}function yr(j){return j==="default"?"built-in default":`${j} policy`}var pp=new RegExp("\x1B\\[[0-9;]*m","g"),bo=(j)=>j.replace(pp,"").length;function an(j){let K=(j.headers??j.rows[0]??[]).map((de,we)=>{let xe=Math.max(...j.rows.map((Ce)=>bo(Ce[we]??"")));return Math.max(bo(de),xe)}),te=(de,we)=>de+" ".repeat(Math.max(0,we-bo(de))),re=(de,we)=>we[0]+K.map((xe)=>de.repeat(xe+2)).join(we[1])+we[2],oe=(de)=>`│ ${de.map((we,xe)=>te(we,K[xe]??0)).join(" │ ")} │`,ce=j.headers?[`   ${oe(j.headers)}`,`   ${re("─",["├","┼","┤"])}`]:[];return[`   ${re("─",["┌","┬","┐"])}`,...ce,...j.rows.map((de)=>`   ${oe(de)}`),`   ${re("─",["└","┴","┘"])}`].join(`
`)}function xs(j){let K=[];K.push("Hook Integration"),K.push(fp(j));let te=[],re=[];for(let oe of j){let ce=Ct(oe.platform);if(oe.errors&&oe.errors.length>0)for(let de of oe.errors)if(oe.configured)te.push({platform:ce,message:de});else re.push({platform:ce,message:de})}for(let oe of te)K.push(`   Warning (${oe.platform}): ${oe.message}`);for(let oe of re)K.push(vt.red(`   Error (${oe.platform}): ${oe.message}`));return K.join(`
`)}function fp(j){let K=["Platform","Discovery","Configuration","Inspection"],te=j.map((re)=>{let oe=Ct(re.platform);if(re.inspectionStatus==="not-inspected"){let xe=vt.dim("Not inspected");return[oe,xe,xe,xe]}let ce=re.detected?vt.green("Detected"):re.inspectionStatus==="failed"?vt.red("Unknown"):vt.dim("Not detected"),de=re.configured?vt.green("Configured"):re.detected?vt.yellow("Not configured"):re.inspectionStatus==="failed"?vt.red("Unknown"):vt.dim("Not applicable"),we=re.inspectionStatus==="verified"?vt.green("Verified"):re.inspectionStatus==="failed"?vt.red("Failed"):vt.dim("Not applicable");return[oe,ce,de,we]});return an({headers:K,rows:te})}function Cs(j){let te=["Guard Engine Verification",`   Synthetic self-test: ${j.failed>0?vt.red(`${j.passed}/${j.total} FAIL`):vt.green(`${j.passed}/${j.total} passed`)}`],re=j.results.filter((oe)=>!oe.passed);if(re.length>0){te.push(""),te.push(vt.red("   Failures:"));for(let oe of re)te.push(vt.red(`   • ${oe.description}`)),te.push(vt.red(`     expected ${oe.expected}, got ${oe.actual}`))}return te.join(`
`)}function mp(j){if(j.length===0)return"   (no custom rules)";let K=["Source","Name","Command","Block Args"],te=j.map((re)=>[re.source,re.name,re.subcommand?`${re.command} ${re.subcommand}`:re.command,re.blockArgs.join(", ")]);return an({headers:K,rows:te})}function Ss(j){let K=[];if(K.push("Configuration"),K.push(gp(j.userConfig,j.projectConfig)),K.push(""),j.effectiveRules.length>0)K.push(`   Effective rules (${j.effectiveRules.length} total):`),K.push(mp(j.effectiveRules));else K.push("   Effective rules: (none - using built-in rules only)");for(let te of j.shadowedRules)K.push(""),K.push(`   Note: Project rule "${te.name}" shadows user rule with same name`);return K.join(`
`)}function gp(j,K){let te=["Scope","Status"],re=(ce)=>{if(!ce.exists)return vt.dim("N/A");if(!ce.valid)return vt.red(`Invalid (${ce.errors?.[0]??"unknown error"})`);return vt.green("Configured")},oe=[["User",re(j)],["Project",re(K)]];return an({headers:te,rows:oe})}function Rs(j){let K=[];return K.push("Environment"),K.push(yp(j)),K.join(`
`)}function Ps(j){let K=j.effectiveSafety.policyScopes,te=["Effective Safety",`   Selected preset: ${j.effectiveSafety.selectedPreset}${K?` (${yr(K.levelScope)})`:""}`,`   Effective: ${j.effectiveSafety.level}`],re=[["fail_closed","fail_closed"],["paranoid_rm","paranoid_rm"],["paranoid_interpreters","paranoid_interpreters"]];for(let[oe,ce]of re){let de=j.effectiveSafety.capabilities[oe],we=de.enabled?vt.green("ON"):vt.dim("OFF"),xe=de.sources.length>0?` (${de.sources.join(", ")})`:"";te.push(`   ${ce}: ${we} via ${de.source}${xe}`)}if(K&&K.weakenings.length>0){te.push("   Project policy deltas:");for(let oe of K.weakenings)te.push(`      ${oe}`)}te.push(`   Stored rule customizations: ${j.effectiveSafety.ruleCounts.stored}`),te.push(`   Effective rule customizations: ${j.effectiveSafety.ruleCounts.effective}`);for(let[oe,ce]of Object.entries(j.effectiveSafety.ruleOverrides))te.push(`   ${oe}: ${ce}`);return te.join(`
`)}function Es(j){let K=["Findings"];if(j.length===0)return K.push("   No findings from inspected doctor facts."),K.join(`
`);for(let te of j){let re=`[${te.severity.toUpperCase()}] ${te.checkId}: ${At(te.title)}`,oe=te.severity==="error"?vt.red:te.severity==="warning"?vt.yellow:vt.blue;if(K.push(`   ${oe(re)}`),K.push(`      ${At(te.detail)}`),te.path)K.push(`      Path: ${At(te.path)}`);if(te.fixHint)K.push(`      Fix: ${At(te.fixHint)}`)}return K.join(`
`)}function yp(j){let K=["Variable","Status","Legacy"],te=j.map((re)=>{let oe=re.isSet?vt.green("✓"):vt.dim("✗"),ce=re.legacyName&&re.legacyIsSet?`${re.legacyName} ${vt.green("✓")}`:re.legacyName??"";return[re.name,oe,ce]});return an({headers:K,rows:te})}function Ds(j){let K=[];if(j.totalBlocked===0)K.push("Recent Activity"),K.push("   No blocked commands in the last 7 days"),K.push("   Tip: This is normal for new installations");else K.push(`Recent Activity · last 7 days (${j.totalBlocked} blocked / ${j.sessionCount} sessions)`),K.push(hp(j.recentEntries));if(j.unreadable>0)K.push(`   Warning: ${j.unreadable} audit log ${j.unreadable===1?"source":"sources"} could not be read; this summary is incomplete`);return K.join(`
`)}function hp(j){let K=["Time","Command"],te=j.map((re)=>{let oe=At(re.command.replace(/\r\n|\r|\n/g," ↵ ").replace(/\t/g," ")),ce=oe.length>40?`${oe.slice(0,37)}...`:oe;return[re.relativeTime,ce]});return an({headers:K,rows:te})}function As(j){let K=[];if(K.push("Update Check"),j.latestVersion===null&&!j.error)return K.push(hr([["Status",vt.dim("Skipped")],["Installed",j.currentVersion]])),K.join(`
`);if(j.error)return K.push(hr([["Status",`${vt.yellow("⚠")} Error`],["Installed",j.currentVersion],["Error",vt.dim(j.error)]])),K.join(`
`);if(j.updateAvailable)return K.push(hr([["Status",`${vt.yellow("⚠")} Update Available`],["Current",j.currentVersion],["Latest",vt.green(j.latestVersion??"")]])),K.push(""),K.push("   Run: bunx cc-safety-net@latest doctor"),K.push("   Or:  npx cc-safety-net@latest doctor"),K.join(`
`);return K.push(hr([["Status",`${vt.green("✓")} Up to date`],["Version",j.currentVersion]])),K.join(`
`)}function hr(j){return an({rows:j})}function _s(j){let K=[];return K.push("System Info"),K.push(vp(j)),K.join(`
`)}function vp(j){let K=["Component","Version"],te=(ce)=>{if(ce===null)return vt.dim("not found");return ce},oe=[{label:"cc-safety-net",value:j.version},...dr.map((ce)=>({label:Ct(ce),value:j.versions[ce]??null})),{label:"Node.js",value:j.nodeVersion},{label:"npm",value:j.npmVersion},{label:"Bun",value:j.bunVersion},{label:"Platform",value:j.platform}].map((ce)=>[ce.label,te(ce.value)]);return an({headers:K,rows:oe})}function Ts(j){if(j.findings.length===0)return vt.green(`
...
${j.findings.length} ${re}: ${te.join(", ")}.`;if(K.error>0)return vt.red(oe);if(K.warning>0)return vt.yellow(oe);return vt.blue(oe)}import{lstatSync as bp}from"node:fs";import{dirname as Lo}from"node:path";function wo(j,K){try{let te=bp(K);if(te.isSymbolicLink())return{kind:j,path:K,status:"unsafe",issues:["symlink"]};if(!te.isDirectory())return{kind:j,path:K,status:"unsafe",issues:["not-directory"]};if(process.platform==="win32"||typeof process.getuid!=="function")return{kind:j,path:K,status:"unknown",issues:[]};let re=[...te.uid!==process.getuid()?["ownership"]:[],...(te.mode&18)!==0?["permissions"]:[]];return{kind:j,path:K,status:re.length>0?"unsafe":"safe",issues:re}}catch(te){if(typeof te==="object"&&te!==null&&"code"in te&&te.code==="ENOENT")return{kind:j,path:K,status:"not-applicable",issues:[]};return{kind:j,path:K,status:"unknown",issues:[]}}}function Is(j,K){let te=M(j);return{directories:[wo("policy",Lo(Lo(K))),wo("config",Lo(K)),...te?[wo("audit",te)]:[{kind:"audit",status:"unknown",issues:[]}]]}}import{spawn as Lp}from"node:child_process";import{existsSync as $s}from"node:fs";import{delimiter as wp,extname as kp,join as xp}from"node:path";import{stripVTControlCharacters as Os}from"node:util";var Fs="2.4.1",Cp=5000,Sp="_CC_SAFETY_NET_TEST_SPAWN_PLATFORM";function Pt(){return Fs}function ko(j,K){let te=j[K];if(te)return te;let re=Object.keys(j).find((oe)=>oe.toLowerCase()===K.toLowerCase()&&!!j[oe]);return re?j[re]:te}function Rp(j){return(ko(j,"PATHEXT")||".COM;.EXE;.BAT;.CMD").split(";").filter((K)=>K.length>0)}function Pp(j,K){let te=kp(j)?[j]:[...Rp(K).map((re)=>`${j}${re}`),j];if(j.includes("/")||j.includes("\\"))return te.find((re)=>$s(re))??j;return(ko(K,"PATH")??"").split(wp).flatMap((re)=>te.map((oe)=>xp(re,oe))).find((re)=>$s(re))??j}function js(j){if(!/[\s"&|<>^]/.test(j))return j;return`"${j.replace(/"/g,'""')}"`}function ln(j,K){let[te,...re]=j,oe=K[Sp]==="win32"?"win32":process.platform;if(!te||oe!=="win32")return{cmd:te??"",args:re};let ce=Pp(te,K);if(!/\.(?:bat|cmd)$/i.test(ce))return{cmd:ce,args:re};return{cmd:ko(K,"COMSPEC")??"cmd.exe",args:["/d","/c",["call",js(ce),...re.map(js)].join(" ")]}}var kn=async(j,K=Cp)=>{let te=await Ep(j,{timeoutMs:K});if(te.code!==0)return null;return Os(te.stdout).trim()||Os(te.stderr).trim()||null};function Ep(j,K){let[te,...re]=j;if(!te)return Promise.resolve({code:null,stdout:"",stderr:""});return new Promise((oe)=>{try{let ce=ln([te,...re],process.env),de=Lp(ce.cmd,ce.args,{stdio:["ignore","pipe","pipe"]}),we=!1,xe="",Ce="";de.stdout.on("data",(Ge)=>{xe+=Ge.toString()}),de.stderr.on("data",(Ge)=>{Ce+=Ge.toString()});let Se=(Ge)=>{if(we)return;we=!0,clearTimeout(Oe),oe(Ge)},Oe=setTimeout(()=>{de.kill(),Se({code:null,stdout:xe,stderr:Ce})},K.timeoutMs);de.on("close",(Ge)=>{Se({code:Ge,stdout:xe,stderr:Ce})}),de.on("error",()=>{Se({code:null,stdout:xe,stderr:Ce})})}catch{oe({code:null,stdout:"",stderr:""})}})}function vr(j){if(!j)return null;let K=/Claude Code\s+(\d+\.\d+\.\d+)/i.exec(j);if(K)return K[1]??null;let te=/v?(\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?)/i.exec(j);if(te)return te[1]??null;return j.split(`
`)[0]?.trim()||null}async function Mn(j=kn){let[K,te,re,oe,ce,de]=await Promise.all([Promise.all(Ft.map(async(we)=>[we.id,vr(await j([...we.probeCommand]))])),j(["codex","plugin","list"],30000),j(["amp","plugins","list"],30000),j(["node","--version"]),j(["npm","--version"]),j(["bun","--version"])]);return{version:Fs,versions:Object.fromEntries(K),codexPluginListOutput:te,ampPluginListOutput:re,nodeVersion:vr(oe),npmVersion:vr(ce),bunVersion:vr(de),platform:`${process.platform} ${process.arch}`}}function xo(j,K){if(K==="dev")return!1;let te=j.split(".").map(Number),re=K.split(".").map(Number),[oe=0,ce=0,de=0]=te,[we=0,xe=0,Ce=0]=re;if(oe!==we)return oe>we;if(ce!==xe)return ce>xe;return de>Ce}async function tn(){let j=Pt(),K=new AbortController,te=setTimeout(()=>K.abort(),3000);try{let re=await fetch("https://registry.npmjs.org/cc-safety-net/latest",{signal:K.signal});if(!re.ok)return{currentVersion:j,latestVersion:null,updateAvailable:!1,error:`npm registry returned ${re.status}`};let oe=await re.json(),ce=xo(oe.version,j);return{currentVersion:j,latestVersion:oe.version,updateAvailable:ce}}catch(re){return{currentVersion:j,latestVersion:null,updateAvailable:!1,error:re instanceof Error?re.message:"Network error"}}finally{clearTimeout(te)}}import*as Vs from"node:readline";var Us=(j)=>`\x1B[${j}B`,Dp=(j)=>`\x1B[${j}A`;var Ns=["░","▒","▓","╱","╲","┃","━","┏","┓","┗","┛","╋"];function Ap(j){return new Promise((K)=>setTimeout(K,j))}function _p(j,K,te){if(!te)return K(j);if(te.aborted)return Promise.resolve();return new Promise((re,oe)=>{let ce=()=>te.removeEventListener("abort",de),de=()=>{ce(),re()};te.addEventListener("abort",de,{once:!0}),K(j).then(()=>{ce(),re()},(we)=>{ce(),oe(we)})})}function Hn(j,K){return j&&j>0?j:K}function br(j){return Math.max(0,Math.min(1,j))}function xn(j){return Math.max(0,Math.min(255,Math.round(j)))}function Co(j){return j<=0.0031308?12.92*j:1.055*j**0.4166666666666667-0.055}function Tp(j,K,te){let re=te*Math.PI/180,oe=K*Math.cos(re),ce=K*Math.sin(re),de=(j+0.3963377774*oe+0.2158037573*ce)**3,we=(j-0.1055613458*oe-0.0638541728*ce)**3,xe=(j-0.0894841775*oe-1.291485548*ce)**3;return{blue:xn(Co(br(-0.0041960863*de-0.7034186147*we+1.707614701*xe))*255),green:xn(Co(br(-1.2684380046*de+2.6097574011*we-0.3413193965*xe))*255),red:xn(Co(br(4.0767416621*de-3.3077115913*we+0.2309699292*xe))*255)}}function So(j,K){let te=(K*j*180/Math.PI%360+360)%360;return Tp(0.72,0.15,te)}function Gs(j,K=0.1){let te=So(K,j);return`\x1B[38;2;${te.red};${te.green};${te.blue}m`}function Ip(j,K){return{blue:xn(j.blue+(255-j.blue)*K),green:xn(j.green+(255-j.green)*K),red:xn(j.red+(255-j.red)*K)}}function Bs(j,K,te){let re=Math.imul(j+2654435769,2246822507)^Math.imul(K+3266489909,668265263)^Math.imul(te+374761393,2654435761),oe=re^re>>>15,ce=Math.imul(oe,739982445),de=ce^ce>>>12,we=Math.imul(de,695872825);return((we^we>>>15)>>>0)/4294967296}function $p(j,K,te){let re=Math.floor(Bs(j,K,te)*Ns.length);return Ns[re]??"░"}function Ms(j){let K=br(j);return K*K*K*(K*(K*6-15)+10)}function Op(j){if(j.length===0)return"";let K=[],te=!1,re="";for(let oe of j){let ce=`${oe.red};${oe.green};${oe.blue}`;if(oe.bold!==te)K.push(oe.bold?"\x1B[1m":"\x1B[22m"),te=oe.bold;if(ce!==re)K.push(`\x1B[38;2;${ce}m`),re=ce;K.push(oe.character)}return`${K.join("")}\x1B[22m\x1B[39m`}function jp(j,K,te,re,oe){return j.map((ce,de)=>({...So(te,re+K+de/oe),bold:!1,character:ce}))}function Fp(j,K,te,re,oe,ce,de,we){let xe=Math.max(1,re*0.75),Ce=Math.min(1,te/xe),Se=oe*Ms(Ce),Oe=Math.max(0,(te-xe)/Math.max(1,re-xe)),Ge=(1-Ms(te/re))*we*2,st=0.35*Math.max(0,1-Oe*2),ft=Ce>=1,ot=Math.min(j.length,Math.ceil(Se+2+1));return j.slice(0,ot).map((mt,yt)=>{let ht=So(ce,de+K+yt/we+Ge),gt=yt+Bs(K,yt,7919)*2-1;if(gt>Se+2)return{...ht,bold:!1,character:" "};let wt=Se-gt,bt=0.8*Math.exp(-(wt*wt)/12.5),xt=Math.min(0.9,bt+st),on=!ft&&gt>Se-4;return{...Ip(ht,xt),bold:xt>0.3,character:on?$p(K,yt,te):mt}})}function Hs(j){return`\x1B[?2026h${j.map((K,te)=>`\x1B8${te>0?Us(te):""}${Op(K)}`).join("")}\x1B[?2026l`}async function Ro(j,K={}){if(!j)return;let te=K.output??process.stdout,re=K.sleep??Ap,oe=Hn(K.frequency,0.1),ce=K.seed??0,de=Hn(K.speed,40),we=Hn(K.spread,3),xe=Hn(K.frameRate,60),Ce=Math.max(1,Math.floor(Hn(K.duration,12))),Se=j.split(`
`).map((ot)=>Array.from(ot)),Oe=Math.max(...Se.map((ot)=>ot.length)),Ge=1000*Ce*Se.filter((ot)=>ot.length>0).length/de,st=Oe>0?Math.max(1,Math.ceil(Ge/(1000/xe))):0,ft=st>0?Ge/st:0;te.write(`\x1B[?25l${Se.length>1?`${`
`.repeat(Se.length-1)}${Dp(Se.length-1)}`:""}\x1B7`);try{for(let ot=1;ot<=st;ot+=1){if(K.signal?.aborted)break;te.write(Hs(Se.map((mt,yt)=>Fp(mt,yt,ot,st,Oe,oe,ce,we)))),await _p(ft,re,K.signal)}}finally{if(te.write(Hs(Se.map((ot,mt)=>jp(ot,mt,oe,ce,we)))),te.write("\x1B8"),Se.length>1)te.write(Us(Se.length-1));te.write(`
\x1B[0m\x1B[?25h`)}}var qs=["┏━┛┏━┛  ┏━┛┏━┃┏━┛┏━┛━┏┛┃ ┃  ┏━ ┏━┛━┏┛","┃  ┃    ━━┃┏━┃┏━┛┏━┛ ┃ ━┏┛  ┃ ┃┏━┛ ┃ ","━━┛━━┛  ━━┛┛ ┛┛  ━━┛ ┛  ┛   ┛ ┛━━┛ ┛ "].join(`
`);function Np(j){return Boolean(j.isTTY)}async function Un(j={}){let K=j.output??process.stdout;if(!Np(K))return;let te=j.input??process.stdin,re={duration:j.duration,frequency:j.frequency,output:K,seed:j.seed??Math.random()*8192,sleep:j.sleep,speed:j.speed,spread:j.spread};if(!te.isTTY||typeof te.setRawMode!=="function"){await Ro(qs,re);return}let oe=new AbortController,ce=te.readableFlowing===!0,de=te.isRaw===!0,we=!1,xe=(Ce,Se)=>{if(Se.ctrl&&Se.name==="c")we=!0;if(we||Se.name==="return"||Se.name==="enter")oe.abort()};Vs.emitKeypressEvents(te),te.on("keypress",xe),te.setRawMode(!0),te.resume();try{await Ro(qs,{...re,signal:oe.signal})}finally{if(te.off("keypress",xe),te.setRawMode(de),!ce)te.pause()}if(!we)return;if(j.onInterrupt){j.onInterrupt();return}process.kill(process.pid,"SIGINT")}import{createHash as Bp}from"node:crypto";import{existsSync as Ws}from"node:fs";import{dirname as Lr,join as Ks}from"node:path";import{dirname as zs,join as Mp,resolve as Hp}from"node:path";var Up="rule.lock";function Gp(j){return Mp(zs(j),Up)}function Js(j={}){return Hp(j.cwd??process.cwd(),".safety-net.json")}function It(j,K){let te=K.global?K.userConfigPath??T(j,K):K.projectConfigPath??A(K.cwd??process.cwd()),re=K.global?Xe(j,K):Qe(te,K.cwd??process.cwd()),oe=Gp(te);return{configDir:zs(te),configPath:te,lockPath:oe,filesystemScope:re,configTarget:s(re,te),lockTarget:s(re,oe)}}var qp="`cc-safety-net rule sync` is deprecated: rulebooks are live files that need no synchronization. This run only migrates the lock and cache an earlier version left behind.",Vp="cache",zp="rulebooks";function Ys(j,K={}){let te=It(j,K),re=s(te.filesystemScope,Xs(te.configDir)),oe=r(te.lockTarget);if(console.log(qp),oe===null&&!Ws(re.path))return console.log(`No v2 lock or cache leftovers found in ${Lr(te.configDir)}; nothing to migrate.`),0;let ce=Zp(oe),de=m(te.configTarget);if(!de.config&&(r(te.configTarget)!==null||ce.size>0))return console.error(`Cannot migrate: the rules config in ${Lr(te.configDir)} is missing or unreadable while v2 leftovers remain. Restore rule.json, then re-run rule sync.`),1;let we=de.config?.rules??[];for(let xe of we.flatMap((Ce)=>Jp(Ce,ce,te,re,K.global===!0)))console.log(xe);return G(te.lockTarget),at(re),console.log(`Removed the v2 lock and cache under ${Lr(te.configDir)}.`),0}function Zs(j,K){return[...new Set([{cwd:K},{cwd:K,global:!0}].flatMap((te)=>{let re=It(j,te);return[re.lockPath,Xs(re.configDir)]}))].filter((te)=>Ws(te))}function Jp(j,K,te,re,oe){if(!x(j))return[];let ce=O(j).name,de=s(te.filesystemScope,N(te.configDir,ce)),we=r(de);if(we!==null&&Wp(we,ce))return[];let xe=K.get(j),Ce=xe?Kp(xe,ce,re.path,te.filesystemScope):null;if(Ce===null)return[`Could not migrate ${j} from the v2 cache. Run \`cc-safety-net rule update ${j}${oe?" --global":""}\` to vendor it.`];if(y(de,Ce),we!==null)return[`Restored ${j} from the v2 cache over an invalid file.`];return[`Vendored ${j} from the v2 cache.`]}function Wp(j,K){let te=he(j);return!("problem"in te)&&te.rulebook.name===K}function Kp(j,K,te,re){let oe=Ks(te,zp,`${Yp(j)}--${j.digest.replace("sha256:","").slice(0,12)}`,pe),ce=r(s(re,oe));if(ce===null||e2(ce)!==j.digest)return null;let de=he(ce);if("problem"in de||de.rulebook.name!==K)return null;return ce}function Xs(j){return Ks(Lr(j),Vp)}function Yp(j){return([j.owner,j.repo,j.display_ref,j.name].every((re)=>typeof re==="string"&&re!=="")?`${j.owner}/${j.repo}#${j.display_ref}/${j.name}`:j.spec).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")||"rulebook"}function Zp(j){let K=j===null?null:Qp(j),te=Qs(K)&&Array.isArray(K.rulebooks)?K.rulebooks:[];return new Map(te.filter(Xp).map((re)=>[re.spec,re]))}function Xp(j){return Qs(j)&&typeof j.spec==="string"&&typeof j.digest==="string"}function Qs(j){return!!j&&typeof j==="object"}function Qp(j){try{return JSON.parse(j)}catch{return null}}function e2(j){return`sha256:${Bp("sha256").update(j).digest("hex")}`}var ea="\r\x1B[2K",t2="\x1B[?25l",n2="\x1B[39m",r2="\x1B[?25h",o2=100,i2=0.55,s2=80,ta=["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"];function a2(j){return new Promise((K)=>setTimeout(K,j))}async function wr(j,K={}){let te=K.output??process.stdout;if(!te.isTTY)return j;let re=K.sleep??a2,oe=!1,ce=j.then((we)=>(oe=!0,we),(we)=>{throw oe=!0,we});if(await Promise.race([ce.then(()=>!0),re(o2).then(()=>!1)]))return ce;te.write(t2);try{for(let we=0;!oe;we+=1)te.write(`${ea}${Gs(we*i2)}${ta[we%ta.length]}${n2} ${K.loadingMessage??"Loading…"}`),await Promise.race([ce,re(s2)]);return await ce}finally{te.write(`${ea}${r2}`)}}async function Gn(j,K,te,re={}){let oe=K();if(j)await te();if(j&&oe.ready)await wr(oe.ready,re);return oe.finish()}import{stripVTControlCharacters as l2}from"node:util";var kr="amp plugins list",c2=/^\s*[✓✗]\s+cc-safety-net(?:\.ts)?\s+\(User Plugins\)\s+(\S+)\s*$/;function na(j){if(!j.ampPluginListOutput)return{platform:"amp",status:"n/a"};let K=l2(j.ampPluginListOutput).split(`
`).map((te)=>c2.exec(te)?.[1]).find((te)=>te!==void 0);if(!K)return{platform:"amp",status:"n/a"};if(K!=="active")return{platform:"amp",status:"disabled",method:kr,configPath:kr,errors:[`Amp personal plugin cc-safety-net is ${K}; run "plugins: reload" in Amp or reinstall with install --amp`]};return{platform:"amp",status:"configured",method:kr,configPath:kr}}import{existsSync as p2,readFileSync as f2}from"node:fs";import{isAbsolute as yv,join as u2,relative as hv}from"node:path";function Bn(j){return u2(j,".gemini","config","hooks.json")}var m2=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*(?:--agy-cli|-ac)(\s|["']|$)/;function g2(j){if(!j||typeof j!=="object"||Array.isArray(j))return[];return Object.values(j).flatMap((K)=>{if(!K||typeof K!=="object"||Array.isArray(K))return[];let te=K,re=te.PreToolUse;if(!Array.isArray(re))return[];return re.flatMap((oe)=>{if(!oe||typeof oe!=="object"||Array.isArray(oe))return[];let ce=oe.hooks;if(!Array.isArray(ce))return[];return ce.flatMap((de)=>{if(!de||typeof de!=="object"||Array.isArray(de))return[];let we=de.command;if(typeof we!=="string"||!m2.test(we))return[];return[{command:we,enabled:te.enabled!==!1}]})})})}function ra(j){let K=Bn(j.environment.home);if(!p2(K))return{platform:"antigravity-cli",status:"n/a",configPath:K};let te;try{te=g2(JSON.parse(f2(K,"utf-8")))}catch(re){return{platform:"antigravity-cli",status:"n/a",configPath:K,errors:[`Failed to parse Antigravity hooks config ${K}: ${re instanceof Error?re.message:String(re)}`]}}if(te.some((re)=>re.enabled))return{platform:"antigravity-cli",status:"configured",method:"hook config",configPath:K};if(te.length>0)return{platform:"antigravity-cli",status:"disabled",method:"hook config",configPath:K};return{platform:"antigravity-cli",status:"n/a",configPath:K}}import{join as oa}from"node:path";import{existsSync as y2,lstatSync as h2,readFileSync as v2}from"node:fs";function Bt(j,K=(te)=>te){if(!y2(j))return{kind:"missing"};try{return{kind:"ok",value:JSON.parse(K(v2(j,"utf-8")))}}catch{return{kind:"unreadable"}}}function Rt(j){try{return h2(j)}catch{return}}function xr(j,K){let te=Rt(K);if(!te)return{platform:j,status:"n/a",configPath:K};if(!te.isSymbolicLink()&&te.isDirectory())return;return{platform:j,status:"n/a",configPath:K,errors:[`${K} is a symlink or not a directory; move or remove it before installing`]}}function Lt(j,K){return typeof j==="object"&&j!==null?j[K]:void 0}var Po="cc-safety-net@cc-marketplace";function ia(j){return oa(j.home,".claude","plugins","installed_plugins.json")}function sa(j,K){let te=Lt(Lt(j,"plugins"),K);return Array.isArray(te)&&te.length>0}function Cr(j,K){let te=Bt(ia(j));return te.kind==="ok"&&sa(te.value,K)}function Eo(j){let K=ia(j),te=Bt(K);if(te.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(te.kind==="missing")return{platform:"claude-code",status:"n/a"};if(!sa(te.value,Po))return{platform:"claude-code",status:"n/a"};let re=oa(j.home,".claude","settings.json"),oe=Bt(re);if(oe.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(!(oe.kind==="ok"&&Lt(Lt(oe.value,"enabledPlugins"),Po)===!0))return{platform:"claude-code",status:"disabled",method:"plugin config",configPath:re,errors:[`${Po} is installed but not enabled in Claude Code`]};return{platform:"claude-code",status:"configured",method:"plugin config",configPath:K}}function aa(j){return Eo(j.environment)}function la(j){if(!j.codexPluginListOutput)return{platform:"codex",status:"n/a"};let K=j.codexPluginListOutput.split(`
`).find((te)=>te.includes("https://github.com/kenryu42/cc-safety-net.git"));if(!K)return{platform:"codex",status:"n/a"};if(!K.includes("installed,"))return{platform:"codex",status:"n/a"};if(!K.includes("installed, enabled"))return{platform:"codex",status:"disabled",method:"codex plugin list",configPath:"codex plugin list",errors:["Codex plugin line for https://github.com/kenryu42/cc-safety-net.git must contain installed, enabled."]};return{platform:"codex",status:"configured",method:"codex plugin list",configPath:"codex plugin list"}}import{existsSync as Ar,readdirSync as b2,readFileSync as L2}from"node:fs";import{join as _t}from"node:path";function Ot(j){let K="",te=0,re=!1,oe=!1,ce=-1;while(te<j.length){let de=j[te],we=j[te+1];if(oe){K+=de,oe=!1,te++;continue}if(de==='"'&&!re){re=!0,ce=-1,K+=de,te++;continue}if(de==='"'&&re){re=!1,K+=de,te++;continue}if(de==="\\"&&re){oe=!0,K+=de,te++;continue}if(re){K+=de,te++;continue}if(de==="/"&&we==="/"){while(te<j.length&&j[te]!==`
`)te++;continue}if(de==="/"&&we==="*"){te+=2;while(te<j.length-1){if(j[te]==="*"&&j[te+1]==="/"){te+=2;break}te++}continue}if(de===","){ce=K.length,K+=de,te++;continue}if(de==="}"||de==="]"){if(ce!==-1){let xe=K.slice(ce+1);if(/^\s*$/.test(xe))K=K.slice(0,ce)+xe}ce=-1,K+=de,te++;continue}if(!/\s/.test(de))ce=-1;K+=de,te++}return K}function Do(j,K,te){let re=K+1,oe=!1;while(re<j.length){if(oe){oe=!1,re++;continue}if(j[re]==="\\"){oe=!0,re++;continue}if(j[re]==='"')return re+1;re++}throw Error(te)}function Ao(j,K,te){let re=j[K],oe=re==="["?"]":"}",ce=0,de=K;while(de<j.length){let we=te.skipComment?.(j,de)??de;if(we!==de){de=we;continue}if(j[de]==='"'){de=Do(j,de,te.stringError);continue}if(j[de]===re)ce++;if(j[de]===oe){if(ce--,ce===0)return de}de++}throw Error(te.bracketError)}function da(j,K){let te=j.lastIndexOf(`
`,K)+1;return/^[ \t]*/.exec(j.slice(te))?.[0]??""}function Rr(j,K){let te=K.end+(/^\s*/.exec(j.slice(K.end))?.[0].length??0);if(j[te]===","){let de=j[te+1]===`
...
`,re-1),ce=oe!==-1&&/^\s*$/.test(j.slice(oe+1,re))?oe:re;return`${j.slice(0,ce)}${j.slice(K.end)}`}function Sr(j,K){if(j.startsWith("//",K)){let te=j.indexOf(`
`,K+2);return te===-1?j.length:te+1}if(j.startsWith("/*",K)){let te=j.indexOf("*/",K+2);return te===-1?j.length:te+2}return K}function ca(j,K){let te=K;while(te<j.length){if(/\s/.test(j[te]??"")){te++;continue}let re=Sr(j,te);if(re===te)return te;te=re}return te}function ua(j,K,te){let re=0,oe=0;while(oe<j.length){let ce=Sr(j,oe);if(ce!==oe){oe=ce;continue}if(j[oe]==='"'){let de=Do(j,oe,te.stringError);if(re===1&&JSON.parse(j.slice(oe,de))===K){let we=ca(j,de),xe=ca(j,we+1);if(j[we]===":"&&j[xe]==="[")return{start:xe,end:Ao(j,xe,{skipComment:Sr,...te})}}oe=de;continue}if(j[oe]==="{"||j[oe]==="[")re++;if(j[oe]==="}"||j[oe]==="]")re--;oe++}return}function pa(j,K,te){let re=[],oe=K.start+1;while(oe<K.end){let ce=Sr(j,oe);if(ce!==oe){oe=ce;continue}if(j[oe]==='"'){let de=Do(j,oe,te),we=JSON.parse(j.slice(oe,de));if(typeof we==="string")re.push({range:{start:oe,end:de},value:we});oe=de;continue}oe++}return re}var qt="cc-safety-net@cc-marketplace",Pr=["cc-marketplace","cc-safety-net"],fa=["_direct","copilot-safety-net"],ma=["cc-marketplace","safety-net"],ga="safety-net@cc-marketplace";function Er(j,K){let te=K.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(`(^|[^a-z0-9-])${te}([^a-z0-9-]|$)`,"m").test(j??"")}function ya(j){return Er(j,"cc-safety-net@cc-marketplace")}function ha(j){return Er(j,"cc-marketplace")}function va(j){return Er(j,"copilot-safety-net")}function ba(j){return Er(j,"safety-net@cc-marketplace")}function _o(j){if(!j?.includes("cc-safety-net"))return!1;return/(^|\s)hook\s+(?:[^\s]+\s+)*(--copilot-cli|-cp)(\s|$)/.test(j)}function wa(j,K){if(!j)return null;let te=j.match(/(\d+)\.(\d+)\.(\d+)/);if(!te)return null;let re=[Number(te[1]),Number(te[2]),Number(te[3])];for(let oe=0;oe<K.length;oe++){let ce=re[oe]??0,de=K[oe]??0;if(ce!==de)return ce>de}return!0}function w2(j){return wa(j,[0,0,422])}function k2(j){return wa(j,[1,0,8])}function qn(j){return j.env.get("COPILOT_HOME")||_t(j.home,".copilot")}function To(j){return(j.hooks?.preToolUse??[]).some((te)=>{if(te.type!=="command")return!1;return _o(te.command)||_o(te.bash)||_o(te.powershell)})}function Dr(j){return j===void 0||typeof j==="string"}function x2(j){if(!j||typeof j!=="object"||Array.isArray(j))return!1;let K=j;if(K.disableAllHooks!==void 0&&typeof K.disableAllHooks!=="boolean")return!1;if(K.hooks===void 0)return!0;if(!K.hooks||typeof K.hooks!=="object"||Array.isArray(K.hooks))return!1;let te=K.hooks.preToolUse;if(te===void 0)return!0;return Array.isArray(te)&&te.every((re)=>re!==null&&typeof re==="object"&&!Array.isArray(re)&&Dr(re.type)&&Dr(re.command)&&Dr(re.bash)&&Dr(re.powershell))}function Io(j,K){try{let te=JSON.parse(Ot(L2(j,"utf-8")));if(!x2(te)){K?.push(`Invalid hook config ${j}: hooks.preToolUse must be an array of hook objects`);return}return te}catch(te){K?.push(`Failed to parse ${j}: ${te instanceof Error?te.message:String(te)}`);return}}function ka(j,K){try{return b2(j).filter((te)=>te.endsWith(".json")).sort((te,re)=>te.localeCompare(re))}catch(te){return K?.push(`Failed to read ${j}: ${te instanceof Error?te.message:String(te)}`),[]}}function C2(j,K){if(!Ar(j))return[];let te=[];for(let re of ka(j,K)){let oe=_t(j,re),ce=Io(oe,K);if(ce&&To(ce))te.push(oe)}return te}function Cn(j,K){if(!Ar(j))return;let te=Io(j,K);if(!te)return;return{path:j,config:te}}function La(j,K,te,re){if(K){j.push(`GitHub Copilot CLI ${K} does not support ${te}; requires ${re}+`);return}j.push(`GitHub Copilot CLI version unavailable; skipping ${te} because it requires ${re}+`)}function S2(j){for(let K of j){if(K?.config.disableAllHooks===!0)return K.path;if(K?.config.disableAllHooks===!1)return}return}function R2(j,K,te,re){let oe=qn(j),ce=_t(K,".github","hooks"),de=_t(oe,"hooks"),we=_t(K,".github","copilot"),xe=_t(K,".claude"),Ce=k2(te),Se=Ce===!0?re:void 0,Oe=[Cn(_t(we,"settings.local.json"),Se),Cn(_t(we,"settings.json"),Se),Cn(_t(xe,"settings.local.json"),Se),Cn(_t(xe,"settings.json"),Se)],Ge=[Cn(_t(oe,"settings.json"),Se),Cn(_t(oe,"config.json"),Se)];if(Ce!==!1){let wt=S2([...Oe,...Ge]);if(wt){if(Ce===null)re.push(`GitHub Copilot CLI version unavailable; treating disableAllHooks in ${wt} as active`);return{activeConfigPaths:[],disabledBy:wt}}}let st=C2(ce,re),ft=w2(te),ot=ft===!0?re:void 0,mt=Ar(de)?ka(de,ot):[],yt=[];for(let wt of mt){let bt=_t(de,wt),xt=Io(bt,ot);if(xt&&To(xt))yt.push(bt)}if(ft!==!0&&yt.length>0)La(re,te,`user hook files in ${de}`,"0.0.422"),yt.length=0;let ht=[];for(let wt of[...Oe,...Ge]){if(!wt)continue;if(!To(wt.config))continue;if(Ce===!0){ht.push(wt);continue}La(re,te,"inline hook definitions in Copilot config files","1.0.8");break}let gt=(wt)=>wt.filter((bt)=>!!bt&&ht.includes(bt)).map((bt)=>bt.path);return{activeConfigPaths:[...gt(Oe),...st,...gt(Ge),...yt]}}function xa(j){let K=[],te=R2(j.environment,j.cwd,j.copilotCliVersion,K);if(te.disabledBy)return{platform:"copilot-cli",status:"disabled",method:"hook config",configPath:te.disabledBy,configPaths:[te.disabledBy],errors:K.length>0?K:void 0};let re=qn(j.environment),oe=_t(re,"installed-plugins",...Pr),ce=Ar(oe),de=_t(re,"settings.json"),we=Bt(de,Ot);if(ce&&we.kind==="unreadable")return{platform:"copilot-cli",status:"not-inspected"};if(ce&&we.kind==="ok"&&Lt(Lt(we.value,"enabledPlugins"),qt)===!1)return{platform:"copilot-cli",status:"disabled",method:"plugin config",configPath:de,errors:[`${qt} is installed but not enabled in Copilot CLI`]};if(ce||te.activeConfigPaths.length>0){let xe=ce,Ce=te.activeConfigPaths[0];return{platform:"copilot-cli",status:"configured",method:xe?"plugin config":"hook config",configPath:Ce??(xe?oe:void 0),configPaths:te.activeConfigPaths.length>0?te.activeConfigPaths:void 0,errors:K.length>0?K:void 0}}return{platform:"copilot-cli",status:"n/a",errors:K.length>0?K:void 0}}import{existsSync as j2,readFileSync as F2}from"node:fs";import{existsSync as Ca,mkdirSync as D2,readFileSync as A2}from"node:fs";import{dirname as _2,join as T2}from"node:path";import{renameSync as P2,writeFileSync as E2}from"node:fs";function Et(j,K){let te=`${j}.${process.pid}.tmp`;E2(te,K),P2(te,j)}var Vt=Object.fromEntries(Nn.map((j)=>[j.id,`npx -y cc-safety-net hook ${j.flags[1]}`]));var Vn=Vt.cursor,Sa=30;function Tr(j){return T2(j.home,".cursor","hooks.json")}function cn(j){return typeof j==="object"&&j!==null&&!Array.isArray(j)}function $o(){return{command:Vn,timeout:Sa,failClosed:!0}}function _r(j){return cn(j)&&j.command===Vn}function I2(j){return Object.keys(j).length===3&&j.command===Vn&&j.timeout===Sa&&j.failClosed===!0}function $2(j){try{return JSON.parse(A2(j,"utf-8"))}catch(K){if(K instanceof SyntaxError)throw Error(`Failed to parse Cursor hooks config ${j}: ${K.message}`);throw K}}function Ra(j){let K=$2(j);if(!cn(K))throw Error(`Cursor hooks config ${j} must be a JSON object`);if(K.version!==1)throw Error(`Cursor hooks config ${j} must set "version": 1`);if(K.hooks!==void 0&&!cn(K.hooks))throw Error(`Cursor hooks config ${j} "hooks" must be an object`);let te=cn(K.hooks)?K.hooks.preToolUse:void 0;if(te!==void 0&&!Array.isArray(te))throw Error(`Cursor hooks config ${j} "hooks.preToolUse" must be an array`);return K}function Pa(j){let K=cn(j.hooks)?j.hooks.preToolUse:void 0;return Array.isArray(K)?K:[]}function O2(j){if(!j.some(_r))return[...j,$o()];return j.reduce((K,te)=>{if(!_r(te))return K.result.push(te),K;if(!K.inserted)K.result.push($o()),K.inserted=!0;return K},{result:[],inserted:!1}).result}function Ea(j,K,te){let re=cn(K.hooks)?K.hooks:{},oe={...K,hooks:{...re,preToolUse:te}};Et(j,`${JSON.stringify(oe,null,2)}
`)}function Da(j){let K=Tr(j);if(!Ca(K))return D2(_2(K),{recursive:!0}),Et(K,`${JSON.stringify({version:1,hooks:{preToolUse:[$o()]}},null,2)}
`),{path:K,alreadyInstalled:!1};let te=Ra(K),re=Pa(te),oe=re.filter(_r);if(cn(te.hooks)&&Array.isArray(te.hooks.preToolUse)&&oe.length===1&&oe[0]!==void 0&&I2(oe[0]))return{path:K,alreadyInstalled:!0};return Ea(K,te,O2(re)),{path:K,alreadyInstalled:!1}}function Aa(j){let K=Tr(j);if(!Ca(K))return{path:K,alreadyInstalled:!1};let te=Ra(K),re=Pa(te),oe=re.filter((ce)=>!_r(ce));if(oe.length===re.length)return{path:K,alreadyInstalled:!1};return Ea(K,te,oe),{path:K,alreadyInstalled:!0}}function N2(j){if(!j||typeof j!=="object"||Array.isArray(j))return[];let K=j.hooks;if(!K||typeof K!=="object"||Array.isArray(K))return[];let te=K.preToolUse;if(!Array.isArray(te))return[];return te.filter((re)=>!!re&&typeof re==="object"&&!Array.isArray(re)&&re.command===Vn)}function M2(j){let K=[];if(j.length>1)K.push("Multiple managed cc-safety-net hooks found; reinstall to collapse duplicates");let te=j[0];if(te&&te.failClosed!==!0)K.push('Managed hook is missing "failClosed": true; reinstall to repair');if(te&&te.timeout!==30)K.push('Managed hook "timeout" is not 30; reinstall to repair');return K}function _a(j){let K=Tr(j.environment);if(!j2(K))return{platform:"cursor",status:"n/a",configPath:K};let te;try{te=JSON.parse(F2(K,"utf-8"))}catch(ce){return{platform:"cursor",status:"n/a",configPath:K,errors:[`Failed to parse Cursor hooks config ${K}: ${ce instanceof Error?ce.message:String(ce)}`]}}let re=N2(te);if(re.length===0)return{platform:"cursor",status:"n/a",configPath:K};let oe=M2(re);return{platform:"cursor",status:"configured",method:"hook config",configPath:K,errors:oe.length>0?oe:void 0}}import{existsSync as H2}from"node:fs";import{join as Oo}from"node:path";var jo="gemini-safety-net";function Fo(j){let K=Oo(j.home,".gemini","extensions"),te=Oo(K,jo);if(!H2(te))return{platform:"gemini-cli",status:"n/a"};let re=Oo(K,"extension-enablement.json"),oe=Bt(re);if(oe.kind==="unreadable")return{platform:"gemini-cli",status:"not-inspected"};let ce=oe.kind==="ok"?Lt(Lt(oe.value,jo),"overrides"):void 0;if(Array.isArray(ce)&&ce.some((we)=>typeof we==="string"&&we.startsWith("!")))return{platform:"gemini-cli",status:"disabled",method:"extension config",configPath:re,errors:[`${jo} is disabled in Gemini CLI`]};return{platform:"gemini-cli",status:"configured",method:"extension config",configPath:te}}function Ta(j){return Fo(j.environment)}import{existsSync as q2,readFileSync as V2}from"node:fs";import{existsSync as $a,mkdirSync as U2,readFileSync as Oa,rmSync as G2}from"node:fs";import{dirname as B2,join as Ia}from"node:path";var zn=Vt["grok-build"],Or=30;function jr(j){return Ia(j.env.get("GROK_HOME")??Ia(j.home,".grok"),"hooks","cc-safety-net.json")}function dn(j){return typeof j==="object"&&j!==null&&!Array.isArray(j)}function Ir(){return{hooks:[{type:"command",command:zn,timeout:Or}]}}function ja(j){return dn(j)&&j.command===zn}function Fa(j){return j.flatMap((K)=>{if(!dn(K)||!Array.isArray(K.hooks))return[K];let te=K.hooks.filter((re)=>!ja(re));if(te.length===K.hooks.length)return[K];return te.length===0?[]:[{...K,hooks:te}]})}function Na(j){try{let K=JSON.parse(j);return dn(K)?K:null}catch{return null}}function Ma(j){let K=dn(j.hooks)?j.hooks.PreToolUse:void 0;return Array.isArray(K)?K:[]}function $r(j,K,te){let re=dn(K.hooks)?K.hooks:{};Et(j,`${JSON.stringify({...K,hooks:{...re,PreToolUse:te}},null,2)}
`)}function Ha(j){let K=jr(j);if(!$a(K))return U2(B2(K),{recursive:!0}),$r(K,{},[Ir()]),{path:K,alreadyInstalled:!1};let te=Na(Oa(K,"utf-8"));if(!te)return $r(K,{},[Ir()]),{path:K,alreadyInstalled:!1};let re=Ma(te),oe=re.filter((ce)=>dn(ce)&&Array.isArray(ce.hooks)&&ce.hooks.some(ja));if(oe.length===1&&JSON.stringify(oe[0])===JSON.stringify(Ir()))return{path:K,alreadyInstalled:!0};return $r(K,te,[...Fa(re),Ir()]),{path:K,alreadyInstalled:!1}}function Ua(j){let K=jr(j);if(!$a(K))return{path:K,alreadyInstalled:!1};let te=Na(Oa(K,"utf-8"));if(!te)return{path:K,alreadyInstalled:!1};let re=Ma(te),oe=Fa(re);if(JSON.stringify(oe)===JSON.stringify(re))return{path:K,alreadyInstalled:!1};let ce=dn(te.hooks)?te.hooks:{};if(oe.length===0&&Object.keys(te).length===1&&Object.keys(ce).length===1)return G2(K),{path:K,alreadyInstalled:!0};return $r(K,te,oe),{path:K,alreadyInstalled:!0}}function Jn(j){return!!j&&typeof j==="object"&&!Array.isArray(j)}function z2(j){if(!Jn(j)||!Jn(j.hooks))return[];let K=j.hooks.PreToolUse;if(!Array.isArray(K))return[];return K.filter((te)=>Jn(te)&&Array.isArray(te.hooks)&&te.hooks.some((re)=>Jn(re)&&re.command===zn))}function J2(j){let te=(Array.isArray(j.hooks)?j.hooks.filter(Jn):[]).find((re)=>re.command===zn);return[...j.matcher===void 0||j.matcher===""||j.matcher==="*"?[]:['Managed hook has a "matcher" that narrows coverage; reinstall to repair'],...te?.type==="command"?[]:['Managed hook "type" is not "command"; reinstall to repair'],...te?.timeout===Or?[]:[`Managed hook "timeout" is not ${Or}; reinstall to repair`]]}function Ga(j){let K=jr(j.environment);if(!q2(K))return{platform:"grok-build",status:"n/a",configPath:K};let te;try{te=JSON.parse(V2(K,"utf-8"))}catch(ce){return{platform:"grok-build",status:"n/a",configPath:K,errors:[`Failed to parse Grok Build hooks config ${K}: ${ce instanceof Error?ce.message:String(ce)}`]}}let re=z2(te)[0];if(!re)return{platform:"grok-build",status:"n/a",configPath:K};let oe=J2(re);return{platform:"grok-build",status:"configured",method:"hook config",configPath:K,errors:oe.length>0?oe:void 0}}import{readFileSync as Ya}from"node:fs";import{join as Za}from"node:path";var jt="cc-safety-net",No="# cc-safety-net managed Hermes Agent plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --hermes-agent",W2=30;function Ba(j){return`${No}
# version: ${j}
`}function K2(j){return`${Ba(j)}name: ${jt}
version: "${j}"
description: "Block destructive commands and secret-file access before Hermes runs a tool."
author: "cc-safety-net"
provides_hooks:
  - pre_tool_call
`}function Y2(j){return`${Ba(j)}"""CC Safety Net guard for Hermes Agent.
...
ANALYZER = [${Vt["hermes-agent"].split(" ").map((K)=>`"${K}"`).join(", ")}]
TIMEOUT_SECONDS = ${W2}
...
`}function Wn(j){return[{name:"__init__.py",content:Y2(j)},{name:"plugin.yaml",content:K2(j)}]}import{mkdirSync as Z2,readdirSync as X2,readFileSync as Q2,rmSync as Mo}from"node:fs";import{join as un}from"node:path";var ef="__pycache__";function Ho(j){let K=j.env.get("HERMES_HOME")?.trim();return K?K:un(j.home,".hermes")}function Uo(j){return un(Ho(j),"plugins",jt)}function Go(j){return j.startsWith(No)}function Bo(j,K){let te=Uo(j),re=Rt(te);if(re&&(re.isSymbolicLink()||!re.isDirectory()))throw Error(`Refusing to ${K} ${te}: not a regular directory. Move or remove it and rerun ${K==="install"?"install":"uninstall"} --hermes-agent.`);return te}function qa(j,K){let te=Rt(j);if(!te)return;if(te.isSymbolicLink()||!te.isFile())throw Error(`Refusing to ${K} ${j}: not a regular file. Move or remove it.`);let re=Q2(j,"utf-8");if(!Go(re))throw Error(`Refusing to ${K} unmanaged file at ${j}. Move or remove it.`);return re}function Va(j){let K=Bo(j,"install"),te=Wn(Pt());if(te.map((oe)=>qa(un(K,oe.name),"overwrite")).every((oe,ce)=>oe===te[ce]?.content))return{path:K,alreadyInstalled:!0};return Z2(K,{recursive:!0}),te.forEach((oe)=>{Et(un(K,oe.name),oe.content)}),{path:K,alreadyInstalled:!1}}function qo(j){let K=Bo(j,"remove");if(!Rt(K))return[];return Wn(Pt()).filter((te)=>qa(un(K,te.name),"remove")!==void 0)}function za(j){let K=Bo(j,"remove");if(!Rt(K))return{path:K,alreadyInstalled:!1};let te=qo(j);if(te.forEach((re)=>{Mo(un(K,re.name))}),Mo(un(K,ef),{recursive:!0,force:!0}),X2(K).length===0)Mo(K,{recursive:!0});return{path:K,alreadyInstalled:te.length>0}}var Fr="hermes-agent",Ja=/^([^\s#][^:]*):/,tf=/^\s+([A-Za-z_][\w-]*):/,Wa=/^\s+-\s*(.*)$/;function nf(j){return j.trim().replace(/^(["'])(.*)\1$/,"$2")}function rf(j){let K=j.split(/\r?\n/),te=K.findIndex((ce)=>Ja.exec(ce)?.[1]?.trim()==="plugins");if(te===-1)return[];let re=K.slice(te+1),oe=re.findIndex((ce)=>Ja.test(ce));return oe===-1?re:re.slice(0,oe)}function Ka(j,K){let te=rf(j),re=te.findIndex((de)=>tf.exec(de)?.[1]===K);if(re===-1)return[];let oe=te.slice(re+1),ce=oe.findIndex((de)=>!Wa.test(de));return(ce===-1?oe:oe.slice(0,ce)).map((de)=>nf(Wa.exec(de)?.[1]??""))}function of(j){try{return Ya(Za(Ho(j),"config.yaml"),"utf-8")}catch{return}}function Vo(j){let K=of(j)??"";return Ka(K,"enabled").includes(jt)&&!Ka(K,"disabled").includes(jt)}function Xa(j){return/^# version:\s*(.+)$/m.exec(j)?.[1]?.trim()}function sf(j,K){let te=Rt(j);if(!te)return{error:`${K.name} is missing from ${j}; run install --hermes-agent`};if(te.isSymbolicLink()||!te.isFile())return{error:`${j} is a symlink or not a regular file; move or remove it`};try{let re=Ya(j,"utf-8");if(!Go(re))return{error:`Unmanaged ${K.name} occupies ${j}; move or remove it`};if(Xa(re)===Pt()&&re!==K.content)return{error:`Modified ${K.name} occupies ${j}; run install --hermes-agent to restore it`};return{content:re}}catch(re){return{error:`Failed to read ${j}: ${re instanceof Error?re.message:String(re)}`}}}function Qa(j){let K=Uo(j.environment),te=xr(Fr,K);if(te)return te;let re=Wn(Pt()).map((we)=>sf(Za(K,we.name),we)),oe=re.flatMap((we)=>("error"in we)?[we.error]:[]);if(oe.length>0)return{platform:Fr,status:"n/a",configPath:K,errors:oe};let ce=re.some((we)=>("content"in we)&&Xa(we.content)!==Pt()),de=ce?["Installed Hermes Agent plugin is outdated; run install --hermes-agent to update"]:[];if(!Vo(j.environment))return{platform:Fr,status:"disabled",method:"plugin directory",configPath:K,errors:[`${jt} is not enabled in Hermes; run \`hermes plugins enable ${jt}\``,...de]};return{platform:Fr,status:"configured",method:"plugin directory",configPath:K,errors:ce?de:void 0}}import{existsSync as af,readFileSync as lf}from"node:fs";import{join as el}from"node:path";var cf=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*--kimi-code(\s|["']|$)/;function df(j){return el(j.env.get("KIMI_CODE_HOME")||el(j.home,".kimi-code"),"config.toml")}function Kn(j){let K=df(j.environment);if(!af(K))return{platform:"kimi-code",status:"n/a",configPath:K};try{if(!cf.test(lf(K,"utf-8")))return{platform:"kimi-code",status:"n/a",configPath:K}}catch(te){return{platform:"kimi-code",status:"n/a",configPath:K,errors:[`Failed to read ${K}: ${te instanceof Error?te.message:String(te)}`]}}return{platform:"kimi-code",status:"configured",method:"hook config",configPath:K}}import{readFileSync as cl}from"node:fs";import{join as Zn}from"node:path";var St="cc-safety-net",Nt="index.js",Sn="openclaw.plugin.json",Rn="package.json";var Nr="// cc-safety-net managed OpenClaw plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --openclaw";import{existsSync as ff,lstatSync as mf,readdirSync as gf,readFileSync as yf}from"node:fs";import{dirname as nl,join as Kt}from"node:path";import{fileURLToPath as hf}from"node:url";import{spawn as uf}from"node:child_process";function pf(j){return j.join(" ")}function zo(j,K,te){return[`Failed to run ${pf(j)}${K===null?"":` (exit ${K})`}.`,te.trim()].filter(Boolean).join(`
`)}function Jo(j){let K={stdout:"",stderr:""};return j.stdout.setEncoding("utf-8"),j.stderr.setEncoding("utf-8"),j.stdout.on("data",(te)=>{K.stdout+=te}),j.stderr.on("data",(te)=>{K.stderr+=te}),K}function zt(j,K){return new Promise((te,re)=>{let oe=ln([...j],process.env),ce=uf(oe.cmd,oe.args,{stdio:["ignore","pipe","pipe"]}),de=Jo(ce),we=()=>[de.stdout,de.stderr].filter(Boolean).join(`
`),xe=K?.timeoutMs??120000,Ce=setTimeout(()=>{ce.kill(),re(Error(zo(j,null,`Timed out after ${xe}ms.
${we()}`.trim())))},xe);ce.on("error",(Se)=>{clearTimeout(Ce),re(Error(zo(j,null,`${Se.message}
${we()}`.trim())))}),ce.on("close",(Se)=>{if(clearTimeout(Ce),Se!==0){re(Error(zo(j,Se,we())));return}te(K?.stdoutOnly?de.stdout:we())})})}async function Wo(j){for(let K of j)await zt(K)}async function tl(j){for(let K of j)try{await zt(K)}catch(te){console.warn(te instanceof Error?te.message:String(te))}}var Ko=Kt("openclaw",St),vf=[Nt,Sn,Rn];function Yo(j,K){if(j==="~")return K;if(j.startsWith("~/")||j.startsWith("~\\"))return Kt(K,j.slice(2));return j}function rl(j){let K=j.env.get("OPENCLAW_STATE_DIR")?.trim();if(K)return Yo(K,j.home);let te=j.env.get("OPENCLAW_CONFIG_PATH")?.trim();return te?nl(Yo(te,j.home)):Kt(j.home,".openclaw")}function ol(j){let K=j.env.get("OPENCLAW_CONFIG_PATH")?.trim();return K?Yo(K,j.home):Kt(rl(j),"openclaw.json")}function Zo(j){return Kt(rl(j),"extensions",St)}function bf(j){let K=gf(j);if(K.length===0)return!0;if(K.some((oe)=>!vf.includes(oe)))return!1;let te=Kt(j,Nt),re=Rt(te);return re!==void 0&&!re.isSymbolicLink()&&re.isFile()&&yf(te,"utf-8").startsWith(Nr)}function Xo(j){let K=Zo(j),te=Rt(K);if(!te)return;if(!te.isSymbolicLink()&&te.isDirectory()&&bf(K))return;throw Error(`Refusing to modify ${K}: it does not hold a cc-safety-net managed OpenClaw plugin. Move or remove it, then run the command again.`)}function il(){let j=nl(hf(import.meta.url));return[Kt(j,Ko),Kt(j,"..",Ko),Kt(j,"..","..","..","dist",Ko)]}function Qo(j=il()){return j.find((K)=>ff(K)&&mf(K).isDirectory())}function Lf(j=il()){let K=Qo(j);if(!K)throw Error("Packaged OpenClaw plugin directory not found. Reinstall cc-safety-net and try again.");return K}function sl(j=Lf()){return[["openclaw","plugins","install",j,"--force"],["openclaw","plugins","enable",St]]}function wf(j){let K=(()=>{try{return JSON.parse(j)}catch{return}})(),te=Lt(Lt(K,"plugin"),"status");return typeof te==="string"?te:void 0}async function al(){let j=wf(await zt(["openclaw","plugins","inspect",St,"--runtime","--json"],{stdoutOnly:!0}));if(j==="loaded")return;throw Error(`${j===void 0?`The ${St} plugin's load state could not be verified: OpenClaw's runtime inspect report was unreadable.`:`OpenClaw reports the ${St} plugin with status "${j}".`} Run \`openclaw plugins inspect ${St} --runtime\` for details.`)}var Mr="openclaw",Yn=`run \`openclaw plugins enable ${St}\``;function Pn(j,K){let te=Zn(j,K),re=Rt(te);if(!re)return{error:`${K} is missing from ${te}; run install --openclaw`};if(re.isSymbolicLink()||!re.isFile())return{error:`${te} is a symlink or not a regular file; move or remove it`};try{return{content:cl(te,"utf-8")}}catch(oe){return{error:`Failed to read ${te}: ${oe instanceof Error?oe.message:String(oe)}`}}}function dl(j){try{return JSON.parse(Ot(j))}catch{return}}function kf(j){let K=Pn(j,Sn);if("error"in K)return K.error;if(Lt(dl(K.content),"id")===St)return;return`${Zn(j,Sn)} is not a valid ${St} manifest; run install --openclaw`}function xf(j){let K=Pn(j,Rn);if("error"in K)return K.error;let te=Lt(Lt(dl(K.content),"openclaw"),"extensions");if(Array.isArray(te)&&te.includes(`./${Nt}`))return;return`${Zn(j,Rn)} does not point OpenClaw at ${Nt}; run install --openclaw`}function ll(j){return Array.isArray(j)?j.filter((K)=>typeof K==="string"):[]}function Cf(j){let K=ol(j);if(!Rt(K))return`${St} is not enabled; ${Yn}`;let te=(()=>{try{return JSON.parse(Ot(cl(K,"utf-8")))}catch{return}})();if(te===void 0)return`Failed to read ${K}; fix it, then ${Yn}`;let re=Lt(te,"plugins");if(Lt(re,"enabled")===!1)return`plugins.enabled is false in ${K}; no OpenClaw plugin loads`;let oe=Lt(Lt(Lt(re,"entries"),St),"enabled");if(ll(Lt(re,"deny")).includes(St)||oe===!1)return`${St} is disabled in ${K}; ${Yn}`;let ce=ll(Lt(re,"allow"));if(ce.length>0&&!ce.includes(St))return`plugins.allow in ${K} does not list ${St}; add it, then ${Yn}`;if(ce.includes(St)||oe===!0)return;return`${St} is not enabled; ${Yn}`}function ul(j){return/^\/\/ version:\s*(.+)$/m.exec(j)?.[1]?.trim()}function Sf(j,K,te){if(te===void 0)return[];let re=Pn(te,Nt);if("error"in re||ul(re.content)!==K)return[];return[Nt,Sn,Rn].flatMap((oe)=>{let ce=Pn(j,oe),de=Pn(te,oe);if("error"in ce||"error"in de||ce.content===de.content)return[];return[`Modified ${oe} occupies ${Zn(j,oe)}; run install --openclaw to restore it`]})}function pl(j){let K=Zo(j.environment),te=xr(Mr,K);if(te)return te;let re=Pn(K,Nt),ce=["error"in re?re.error:re.content.startsWith(Nr)?void 0:`Unmanaged ${Nt} occupies ${Zn(K,Nt)}; move or remove it`,kf(K),xf(K)].filter((Se)=>Se!==void 0),de="content"in re?ul(re.content):void 0,we=ce.length>0?ce:Sf(K,de,Qo());if(we.length>0)return{platform:Mr,status:"n/a",configPath:K,errors:we};let xe=de===Pt()?[]:["Installed OpenClaw plugin is outdated; run install --openclaw to update"],Ce=Cf(j.environment);if(Ce)return{platform:Mr,status:"disabled",method:"plugin directory",configPath:K,errors:[Ce,...xe]};return{platform:Mr,status:"configured",method:"plugin directory",configPath:K,errors:xe.length>0?xe:void 0}}import{existsSync as Tf,readFileSync as If}from"node:fs";import{join as $f}from"node:path";import{existsSync as ei,readFileSync as gl,rmSync as Rf}from"node:fs";import{join as Yt}from"node:path";import{pathToFileURL as Pf}from"node:url";var Hr="cc-safety-net",yl=`${Hr}@latest`,hl=["opencode.json","opencode.jsonc"],fl="CCSafetyNetPlugin",ml={stringError:"Unterminated string in OpenCode config",bracketError:"Unmatched plugin array in OpenCode config"};function Ur(j){return Yt(j.env.get("XDG_CONFIG_HOME")||Yt(j.home,".config"),"opencode")}function Ef(j){return Yt(Ur(j),hl[0])}function Df(j){return hl.map((K)=>Yt(Ur(j),K))}function vl(j){return Yt(j.env.get("XDG_CACHE_HOME")||Yt(j.home,".cache"),"opencode","packages",yl)}function ti(j){Rf(vl(j),{recursive:!0,force:!0})}async function bl(j){let K=Yt(vl(j),"node_modules",Hr),te=Yt(K,"package.json");if(!ei(te))throw Error(`The OpenCode plugin cache at ${K} is missing its package, so OpenCode would load nothing and fail open. Run \`opencode plugin -g -f ${yl}\` for details.`);let re=Lt(JSON.parse(gl(te,"utf-8")),"main");if(typeof re!=="string")throw Error(`The cached OpenCode plugin at ${K} declares no "main" entry.`);let oe=Yt(K,re);if(typeof(await import(Pf(oe).href))[fl]==="function")return;throw Error(`The cached OpenCode plugin at ${oe} does not export a callable ${fl}, so OpenCode would load nothing and fail open.`)}function Ll(j,K){try{return JSON.parse(Ot(j))}catch(te){if(te instanceof SyntaxError)throw Error(`Failed to parse OpenCode config ${K}: ${te.message}`);throw te}}function Af(j){if(!j||typeof j!=="object"||Array.isArray(j))return!1;let K=j.plugin;if(!Array.isArray(K))return!1;return K.some((te)=>typeof te==="string"&&te.includes(Hr))}function _f(j,K){let te=ua(j,"plugin",ml);if(!te)throw Error(`Failed to locate OpenCode plugin array in ${K}`);let re=pa(j,te,ml.stringError).filter((oe)=>oe.value.includes(Hr)).map((oe)=>oe.range).reverse().reduce(Rr,j);return Ll(re,K),re}function wl(j){ti(j);let K=Df(j),te=K.find((oe)=>ei(oe)),re=[];for(let oe of K){if(!ei(oe))continue;try{let ce=gl(oe,"utf-8");if(!Af(Ll(ce,oe)))continue;return Et(oe,_f(ce,oe)),{path:oe,alreadyInstalled:!0}}catch(ce){re.push(ce instanceof Error?ce.message:String(ce))}}if(re.length>0)throw Error(re.join(`
`));return{path:te??Ef(j),alreadyInstalled:!1}}function kl(j){let K=[],te=Ur(j.environment),re=["opencode.json","opencode.jsonc"];for(let oe of re){let ce=$f(te,oe);if(Tf(ce))try{let de=If(ce,"utf-8"),we=Ot(de);if((JSON.parse(we).plugin??[]).some((Oe)=>Oe.includes("cc-safety-net")))return{platform:"opencode",status:"configured",method:"plugin array",configPath:ce,errors:K.length>0?K:void 0}}catch(de){K.push(`Failed to parse ${oe}: ${de instanceof Error?de.message:String(de)}`)}}return{platform:"opencode",status:"n/a",errors:K.length>0?K:void 0}}import{join as Of}from"node:path";function ni(j){return Of(j.home,".pi","agent","settings.json")}function ri(j){if(typeof j!=="string")return!1;return j==="npm:cc-safety-net"||j.startsWith("npm:cc-safety-net@")}function xl(j){let K=ni(j.environment),te=Bt(K);if(te.kind==="unreadable")return{platform:"pi",status:"not-inspected"};if(te.kind==="missing")return{platform:"pi",status:"n/a"};let re=Lt(te.value,"packages");if(!Array.isArray(re))return{platform:"pi",status:"n/a"};let oe=re.find((we)=>ri(typeof we==="string"?we:Lt(we,"source")));if(oe===void 0)return{platform:"pi",status:"n/a"};let ce=Lt(oe,"extensions");if(Array.isArray(ce)&&ce.some((we)=>typeof we==="string"&&we.startsWith("-")))return{platform:"pi",status:"disabled",method:"package config",configPath:K,errors:["npm:cc-safety-net is installed but its extension is disabled in Pi settings"]};return{platform:"pi",status:"configured",method:"package config",configPath:K}}var jf={amp:na,"antigravity-cli":ra,"claude-code":aa,codex:la,"copilot-cli":xa,cursor:_a,"gemini-cli":Ta,"grok-build":Ga,"hermes-agent":Qa,"kimi-code":Kn,openclaw:pl,opencode:kl,pi:xl};function En(j,K,te){let re={...te,cwd:K,environment:j};return dr.map((oe)=>Ff(jf[oe](re)))}function Ff(j){if(j.status==="not-inspected")return{platform:j.platform,detected:!1,configured:!1,inspectionStatus:"not-inspected"};return{platform:j.platform,detected:j.status!=="n/a",configured:j.status==="configured",inspectionStatus:j.status!=="n/a"?"verified":j.errors&&j.errors.length>0?"failed":"not-applicable",method:j.method,configPath:j.configPath,configPaths:j.configPaths,errors:j.errors}}import{join as Nf}from"node:path";var Mf=Object.freeze([{command:"git reset --hard",description:"git reset --hard",expectBlocked:!0},{command:"rm -rf /",description:"rm -rf /",expectBlocked:!0},{command:"rm -rf ./node_modules",description:"rm in cwd (safe)",expectBlocked:!1}]),Hf=Object.freeze({state:"ready",diagnostics:Object.freeze([]),ruleMetadata:Object.freeze({}),policy:Object.freeze({rules:Object.freeze([]),transparentWrappers:Object.freeze([]),safety:Object.freeze({}),worktreeMode:!1,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:Object.freeze({}),destructiveCommandAllowPaths:Object.freeze([]),secretProtection:Object.freeze({enabled:!0,disabledRules:Object.freeze([]),denyPaths:Object.freeze([]),allowPaths:Object.freeze([])})})}),Uf={strict:!1,paranoidRm:!1,paranoidInterpreters:!1,worktreeMode:!1,effectiveLevel:"standard",capabilities:{fail_closed:{enabled:!1,source:"preset",sources:[]},paranoid_rm:{enabled:!1,source:"preset",sources:[]},paranoid_interpreters:{enabled:!1,source:"preset",sources:[]}}};function Cl(j){let K=Nf(j.tmpdir,"cc-safety-net-self-test"),te=Mf.map((re)=>{let oe=F(j,u("self-test",{command:re.command},{kind:"command",shell:"auto"},{configCwd:K,executionCwd:K},re.command),{guard:{dependencies:{loadPolicySnapshot:()=>Hf,getModes:()=>Uf,findPolicyMutation:()=>null}},audit:{agent:"self-test",getSessionId:()=>{return}}}),ce=re.expectBlocked?"blocked":"allowed",de=oe.decision.kind==="deny"?"blocked":"allowed";return{command:re.command,description:re.description,expected:ce,actual:de,passed:ce===de,reason:oe.decision.kind==="deny"?oe.decision.reason:void 0,ruleId:oe.decision.kind==="deny"?oe.decision.ruleId:void 0}});return{passed:te.filter((re)=>re.passed).length,failed:te.filter((re)=>!re.passed).length,total:te.length,results:te}}function oi(j){let K=Dt({label:"doctor",booleans:{json:["--json"],skipUpdateCheck:["--skip-update-check"]}},j);if(Wt(K.errors))return null;return{json:K.flags.json,skipUpdateCheck:K.flags.skipUpdateCheck}}async function Sl(j,K={}){let te=await Gn(!K.json,()=>{let re=Gf(j,K);return{ready:re,finish:()=>re}},()=>Un(),{loadingMessage:"Checking system status…"});if(K.json)console.log(JSON.stringify(te,null,2));else Bf(te);return te.engineSelfTest.failed>0||te.findings.some((re)=>re.severity==="error")?1:0}async function Gf(j,K){let te=K.cwd??process.cwd(),re=await Mn(),oe=En(j,te,{ampPluginListOutput:re.ampPluginListOutput,codexPluginListOutput:re.codexPluginListOutput,copilotCliVersion:re.versions["copilot-cli"]}),ce=vs(j,te),de=bs(j),we=k(j,{cwd:te}),xe=we.policy,Ce=P(xe,j.env),Se=B(xe,Ce.capabilities),Oe=mr(j,7),Ge=Zs(j,te),st=K.skipUpdateCheck?{currentVersion:Pt(),latestVersion:null,updateAvailable:!1}:await tn(),ft={hooks:oe,engineSelfTest:Cl(j),userConfig:ce.userConfig,projectConfig:ce.projectConfig,configState:Ue(we),effectiveRules:ce.effectiveRules,shadowedRules:ce.shadowedRules,environment:de,effectiveSafety:{selectedPreset:xe.safety.level??"standard",level:Ce.effectiveLevel,capabilities:Ce.capabilities,ruleOverrides:xe.destructiveCommandRuleOverrides,weakenedRuleOverrides:Object.entries(Se).filter(([,ot])=>ot.source==="rule_override"&&ot.override==="off"&&ot.inheritedEnabled&&ot.changesInherited).map(([ot])=>ot),ruleCounts:{stored:Object.keys(xe.destructiveCommandRuleOverrides).length,effective:Object.values(Se).filter((ot)=>ot.changesInherited).length},...we.policyScopes?{policyScopes:we.policyScopes}:{}},...Ge.length>0?{v2Leftovers:Ge}:{},posture:Is(j,ce.userConfig.path),activity:Oe,update:st,system:re};return{...ft,findings:ws(ft)}}function Bf(j){console.log(),console.log(xs(j.hooks)),console.log(),console.log(Cs(j.engineSelfTest)),console.log(),console.log(Ss(j)),console.log(),console.log(Rs(j.environment)),console.log(),console.log(Ps(j)),console.log(),console.log(Es(j.findings)),console.log(),console.log(Ds(j.activity)),console.log(),console.log(_s(j.system)),console.log(),console.log(As(j.update)),console.log(Ts(j))}import{existsSync as qf}from"node:fs";var Vf=/^[A-Za-z0-9_@%+=:,./-]+$/,Rl="Usage: cc-safety-net explain [--json] [--cwd <path>] <command>";function ii(j){let K=Dt({label:"explain",booleans:{json:["--json"]},values:{cwd:["--cwd"]},positionals:"tail"},j);if(Wt(K.errors))return console.error(Rl),console.error("Pass -- before a command that starts with dashes."),null;if(K.values.cwd!==void 0&&!qf(K.values.cwd))return console.error(`Error: --cwd path does not exist: ${K.values.cwd}`),null;let te=K.positionals.length===1?K.positionals[0]:K.positionals.map((re)=>Vf.test(re)?re:`'${re.replaceAll("'","'\\''")}'`).join(" ");if(!te)return console.error("Error: No command provided"),console.error(Rl),null;return{json:K.flags.json,cwd:K.values.cwd,command:te}}function Pl(j){if(j)return{dh:"=",dv:"|",dtl:"+",dtr:"+",dbl:"+",dbr:"+",h:"-",v:"|",tl:"+",tr:"+",bl:"+",br:"+",sh:"="};return{dh:"═",dv:"║",dtl:"╔",dtr:"╗",dbl:"╚",dbr:"╝",h:"─",v:"│",tl:"┌",tr:"┐",bl:"└",br:"┘",sh:"━"}}function El(j,K){let re=K-18;return[`${j.dtl}${j.dh.repeat(K)}${j.dtr}`,`${j.dv}  Command Analysis${" ".repeat(re)}${j.dv}`,`${j.dbl}${j.dh.repeat(K)}${j.dbr}`]}function si(j){return JSON.stringify(j)}function Dl(j,K=0){return`[${j.map((re,oe)=>ks(re,oe,K)).join(",")}]`}function Xn(j,K,te=70){let re=j.split(" "),oe=[],ce="";for(let de of re)if(ce&&ce.length+de.length+1>te)oe.push(ce),ce=de;else ce=ce?`${ce} ${de}`:de;if(ce)oe.push(ce);return oe.map((de,we)=>we===0?de:`${K}${de}`)}function Al(j,K,te){let re=[];switch(j.type){case"parse":return null;case"env-strip":return re.push(""),re.push(`STEP ${K} ${te.h} Strip environment variables`),re.push(`  Removed: ${j.envVars.map((oe)=>`${oe}=<redacted>`).join(", ")}`),re.push(`  Tokens:  ${si(j.output)}`),{lines:re,incrementStep:!0};case"leading-tokens-stripped":return re.push(""),re.push(`STEP ${K} ${te.h} Strip wrappers`),re.push(`  Removed: ${j.removed.join(", ")}`),re.push(`  Tokens:  ${si(j.output)}`),{lines:re,incrementStep:!0};case"shell-wrapper":return re.push(""),re.push(`STEP ${K} ${te.h} Detect shell wrapper`),re.push(`  Wrapper: ${j.wrapper} -c`),re.push(`  Inner:   ${j.innerCommand}`),{lines:re,incrementStep:!0};case"interpreter":{if(re.push(""),re.push(`STEP ${K} ${te.h} Detect interpreter`),re.push(`  Interpreter: ${j.interpreter}`),re.push(`  Code:        ${j.codeArg}`),j.paranoidBlocked)re.push("  Result:      ✗ BLOCKED (paranoid mode)");return{lines:re,incrementStep:!0}}case"busybox":return re.push(""),re.push(`STEP ${K} ${te.h} Busybox wrapper`),re.push(`  Subcommand: ${j.subcommand}`),{lines:re,incrementStep:!0};case"transparent-wrapper":return re.push(""),re.push(`STEP ${K} ${te.h} Transparent wrapper`),re.push(`  Wrapper: ${j.wrapper}`),re.push(`  Tokens:  ${si(j.output)}`),{lines:re,incrementStep:!0};case"recurse":return{lines:[],incrementStep:!1};case"rule-check":{if(re.push(""),re.push(`STEP ${K} ${te.h} Match rules`),re.push(`  Rule:   ${j.rule}()`),j.matched)re.push("  Result: MATCHED");else re.push("  Result: No match");return{lines:re,incrementStep:!0}}case"worktree-relaxation":return re.push(""),re.push(`STEP ${K} ${te.h} Worktree relaxation`),re.push(`  Mode:   ${i.worktree.name}`),re.push(`  Git cwd: ${j.gitCwd}`),re.push("  Result: Allowed local discard in linked worktree"),{lines:re,incrementStep:!0};case"tmpdir-check":return null;case"fallback-scan":{if(j.embeddedCommandFound)return re.push(""),re.push(`STEP ${K} ${te.h} Fallback scan`),re.push(`  Found: ${j.embeddedCommandFound}`),{lines:re,incrementStep:!0};return null}case"custom-rules-check":{if(j.rulesChecked){if(re.push(""),re.push(`STEP ${K} ${te.h} Custom rules`),j.matched)re.push("  Result: MATCHED");else re.push("  Result: No match");return{lines:re,incrementStep:!0}}return null}case"cwd-change":return null;case"dangerous-text":{if(j.matched)return re.push(""),re.push(`STEP ${K} ${te.h} Dangerous text check`),re.push(`  Token:  ${j.token}`),re.push("  Result: MATCHED"),{lines:re,incrementStep:!0};return null}case"strict-unparseable":return re.push(""),re.push(`STEP ${K} ${te.h} Strict mode check`),re.push(`  Command: ${j.rawCommand}`),re.push("  Result:  ✗ UNPARSEABLE"),{lines:re,incrementStep:!0};case"segment-skipped":return null;case"error":return re.push(""),re.push(`ERROR: ${j.message}`),{lines:re,incrementStep:!1};default:return j}}function ai(j,K){let te=Pl(K?.asciiOnly??!1),re=58,oe=[],ce=1;oe.push(...El(te,58)),oe.push("");let de=j.trace.steps.find((ft)=>ft.type==="error");if(de&&de.type==="error"){oe.push("ERROR"),oe.push(`  ${de.message}`),oe.push(""),oe.push("RESULT"),oe.push(`  Status: ${j.result==="blocked"?vt.red("BLOCKED"):vt.green("ALLOWED")}`),oe.push(""),oe.push("CONFIG");let ft=j.configSource??"none";return oe.push(`  Path: ${ft}`),oe.join(`
`)}let we=j.trace.steps.find((ft)=>ft.type==="parse");if(we&&we.type==="parse"){oe.push("INPUT"),oe.push(`  ${we.input}`),oe.push(""),oe.push(`STEP ${ce} ${te.h} Split shell commands`),ce++;for(let ft=0;ft<we.segments.length;ft++){let ot=we.segments[ft];if(ot){let mt=Math.random();oe.push(`  Segment ${ft+1}: ${Dl(ot,mt)}`)}}}let xe=j.trace.segments,Ce=xe.length>1;for(let ft of xe){if(Ce){oe.push("");let ht="";if(we&&we.type==="parse"){let fo=we.segments[ft.index];if(fo)ht=fo.join(" ")}let gt=54,wt=ht,bt=` Segment ${ft.index+1}: `,xt=" ";if(ht){if(bt.length+ht.length+xt.length>gt){let ou=gt-bt.length-xt.length;wt=`${ht.substring(0,ou-1)}…`}}let on=ht?`${bt}${wt}${xt}`:` Segment ${ft.index+1} `,Ut=ht?`${bt}${vt.cyan(wt)}${xt}`:on,Ji=58-on.length,Wi=Math.floor(Ji/2),ru=Ji-Wi;oe.push(`${te.sh.repeat(Wi)}${Ut}${te.sh.repeat(ru)}`)}if(ft.steps.find((ht)=>ht.type==="segment-skipped")){oe.push(""),oe.push("  (skipped — prior segment blocked)");continue}let mt=!1,yt=!1;for(let ht of ft.steps){let gt=Al(ht,ce,te);if(gt){if(yt=!0,ht.type==="recurse"){oe.push("");let wt=" RECURSING ",bt=58-wt.length-4;oe.push(`  ${te.tl}${te.h}${wt}${te.h.repeat(bt)}`),oe.push(`  ${te.v}`),mt=!0;continue}for(let wt of gt.lines)if(mt)oe.push(`  ${te.v} ${wt}`);else oe.push(wt);if(gt.incrementStep)ce++}}if(mt)oe.push(`  ${te.v}`),oe.push(`  ${te.bl}${te.h.repeat(56)}`),mt=!1;if(!yt)oe.push(""),oe.push(`  ${vt.green("✓")} Allowed (no matching rules)`)}if(oe.push(""),oe.push("RESULT"),j.result==="blocked"){if(oe.push(`  Status: ${vt.red("BLOCKED")}`),j.customRule){if(oe.push(`  Rule: ${j.customRule.id}`),j.customRule.rulebook)oe.push(`  Rulebook: ${j.customRule.rulebook.name} ${j.customRule.rulebook.version}`);if(j.customRule.source)oe.push(`  Source: ${j.customRule.source}`);if(j.customRule.override)oe.push(`  Override: reason ${j.customRule.override.reason}`)}if(j.reason){let ft=Xn(j.reason,"          ");oe.push(`  Reason: ${ft[0]}`);for(let ot=1;ot<ft.length;ot++)oe.push(ft[ot]??"")}}else oe.push(`  Status: ${vt.green("ALLOWED")}`);oe.push(""),oe.push("CONFIG");let Se=j.configSource??"none",Oe=j.configValid?"":" (invalid)";oe.push(`  Path: ${Se}${Oe}`);let Ge=j.safetyPresetScope;oe.push(`  Safety preset: ${j.selectedPreset??"standard"}${Ge?` (${yr(Ge)})`:""}`),oe.push(`  Effective capabilities: ${j.effectiveLevel}`);let st=Object.entries(j.destructiveCommandRuleOverrides??{});if(oe.push(`  Rule customizations: ${st.length}`),j.ruleActivation)oe.push(`  Rule activation: ${j.ruleActivation.id} — ${j.ruleActivation.enabled?"on":"off"} via ${j.ruleActivation.source}`);return oe.join(`
`)}function li(j){return JSON.stringify(j,null,2)}import{resolve as Zf}from"node:path";var zf=["AKIA","ASIA","ghp_","gho_","ghu_","ghs_","ghr_","github_pat_","glpat-","xox","npm_","pypi-","rk_","sk-","sk_","gsk_","xai-","pplx-","bastn_","tgp_v1_","flp_","wfr_","fw_","fwp_","tp-","psk-"];function _l(j){let K=0,te={allocateSegment(){return K++},getNextSegmentIndex(){return K},recordGlobal(re){j.record({kind:"step",scope:"global",step:re})},recordSegment(re,oe=te.currentSegmentIndex){if(oe===void 0)return;j.record({kind:"step",scope:"segment",segmentIndex:oe,step:re})}};return te}function Tl(j={}){let K=[],te=j.maxEvents??512,re={maxTextLength:j.maxTextLength??2048,maxListLength:j.maxListLength??128,maxObjectProperties:j.maxObjectProperties??j.maxListLength??128,maxDepth:j.maxDepth??16},oe=0,ce,de=new Set;return{record(we){if(ce)return;try{if(!we||K.length>=te){oe++;return}K.push(di(Jf(we,re,de)))}catch{oe++}},finish(we){if(ce)return ce;try{ce=di({events:Object.freeze(K),droppedEvents:oe,terminal:Wf(we,re,de)})}catch{oe++,ce=Object.freeze({events:Object.freeze(K),droppedEvents:oe,terminal:Object.freeze({result:"blocked",reason:"trace unavailable".slice(0,re.maxTextLength),segment:"trace unavailable".slice(0,re.maxTextLength)})})}return ce}}}function Jf(j,K,te){if(j.kind!=="step")throw TypeError("invalid trace event");let{scope:re,step:oe}=j;Gr(oe,te,K);let ce=Dn(oe,K,te);if(re==="global")return{kind:"step",scope:"global",step:ce};if(re!=="segment")throw TypeError("invalid trace event scope");return{kind:"step",scope:"segment",segmentIndex:j.segmentIndex,step:ce}}function Wf(j,K,te){let re=j.result;if(re==="allowed")return Object.freeze({result:"allowed"});if(re!=="blocked")throw TypeError("invalid trace terminal");let oe=j.ruleId;return Object.freeze({result:"blocked",reason:Dn(j.reason,K,te),segment:Dn(j.segment,K,te),...oe?{ruleId:Dn(oe,K,te)}:{}})}function Gr(j,K,te,re=0,oe=new WeakSet){if(typeof j==="string"){let we=j.slice(0,te.maxTextLength);if(!Ye(we))return;for(let xe of rt(we))for(let Ce of xe.match(/[^\s"'()$]+/g)??[])K.add(Il(Ce));return}if(!j||typeof j!=="object"||re>=te.maxDepth||oe.has(j))return;if(oe.add(j),Array.isArray(j)){let we=Math.min(j.length,te.maxListLength);for(let xe=0;xe<we;xe++)Gr(j[xe],K,te,re+1,oe);return}let ce=0,de=new Set;for(let we in j){if(!Object.hasOwn(j,we))continue;if(ce>=te.maxObjectProperties)break;ce++,Gr(we,K,te);let xe=ci(we,te,K);if(de.has(xe))continue;de.add(xe),Gr(j[we],K,te,re+1,oe)}}function Dn(j,K,te,re=0,oe=new WeakSet){if(typeof j==="string")return ci(j,K,te);if(!j||typeof j!=="object")return j;if(re>=K.maxDepth)return;if(oe.has(j))return;if(oe.add(j),Array.isArray(j)){let we=[],xe=Math.min(j.length,K.maxListLength);for(let Ce=0;Ce<xe;Ce++)we.push(Dn(j[Ce],K,te,re+1,oe));return we}let ce={},de=0;for(let we in j){if(!Object.hasOwn(j,we))continue;if(de>=K.maxObjectProperties)break;de++;let xe=ci(we,K,te);if(Object.hasOwn(ce,xe))continue;Object.defineProperty(ce,xe,{value:Dn(j[we],K,te,re+1,oe),enumerable:!0,configurable:!0,writable:!0})}return ce}function ci(j,K,te){let re=j.slice(0,K.maxTextLength),oe=Ye(re)?Ke(re):re,ce=te.size>0?Yf(oe,te):oe;return(Kf(ce)?De(ce):ce).slice(0,K.maxTextLength)}function Kf(j){return j.includes("PRIVATE KEY")||j.includes("://")||j.includes("eyJ")||j.includes(":")&&/(?:authorization|cookie|x-api-key|api-key|(?:^|\s)(?:-u|--user)(?:\s|=))/i.test(j)||j.length>=14&&zf.some((K)=>j.includes(K))||j.length>=49&&/\b[a-f0-9]{32}\.[A-Za-z0-9]{16}\b/.test(j)}function Yf(j,K){return j.replace(/[^\s"'()$]+/g,(te)=>K.has(Il(te))?"<redacted>":te)}function Il(j){let K=2166136261,te=2166136261;for(let re=0;re<j.length;re++)K=Math.imul(K^j.charCodeAt(re),16777619),te=Math.imul(te^j.charCodeAt(j.length-re-1),16777619);return`${K>>>0}:${te>>>0}:${j.length}`}function di(j){if(j&&typeof j==="object"&&!Object.isFrozen(j)){for(let K of Object.values(j))di(K);Object.freeze(j)}return j}function Qn(j,K={},te){let re=Zf(K.cwd??process.cwd()),oe=K.policySnapshot??k(te,{cwd:re,userConfigDir:K.userConfigDir}),ce=P(oe.policy,te.env),de=K.strict,we=Be({policySnapshot:oe,effectiveCapabilities:ce.capabilities,strict:de??ce.strict,paranoidRm:ce.paranoidRm,paranoidInterpreters:ce.paranoidInterpreters,worktreeMode:ce.worktreeMode}),xe={effectiveLevel:we.effectiveLevel,selectedPreset:oe.policy.safety.level??"standard",...oe.policyScopes?{safetyPresetScope:oe.policyScopes.levelScope}:{},effectiveCapabilities:we.effectiveCapabilities,destructiveCommandRuleOverrides:oe.policy.destructiveCommandRuleOverrides},{configSource:Ce,configValid:Se}=Qf(te,{cwd:re,userConfigDir:K.userConfigDir});if(!j||!j.trim())return{trace:{steps:[{type:"error",message:"No command provided"}],segments:[]},result:"allowed",configSource:Ce,configValid:Se,...xe};let Oe=v(j,"auto");if(Oe.status==="limited")throw new b;let Ge=Oe.dialect==="powershell"?v(j,"posix"):Oe,st=pt(Ge),ft=Tl(),ot=_l(ft);ot.recordGlobal({type:"parse",input:j,segments:st.map((Ut)=>[...Ut])});let mt=u("Bash",{command:j},{kind:"command",shell:"auto"},{configCwd:re,executionCwd:re},j),yt=V(mt,{environment:te,trace:ot,dependencies:{loadPolicySnapshot:()=>oe,...de===void 0?{}:{getModes:()=>({...ce,strict:de})}}}),ht=yt.decision.kind==="deny"?yt.decision:null;if(ht&&(yt.stage==="policy-protection"||yt.stage==="secret-protection")){let Ut=Xf(ht);return{trace:{steps:[],segments:[{index:0,steps:[{type:"rule-check",rule:Ut.rule,matched:!0,reason:ht.reason}]}]},result:"blocked",reason:R(ht.reason),segment:R(ui(ht,j)),...Ut.ruleId?{ruleId:R(Ut.ruleId)}:{},configSource:Ce,configValid:Se,...xe}}let gt=ot.getNextSegmentIndex();if(ht&&gt>0&&gt<st.length)ot.recordSegment({type:"segment-skipped",index:gt,reason:"prior-segment-blocked"},gt);let wt=ft.finish(ht?{result:"blocked",reason:ht.reason,segment:ui(ht,j),...ht.ruleId?{ruleId:ht.ruleId}:{}}:{result:"allowed"}),bt=ht?.ruleId??em(mt,oe,ce,te),xt=U.find((Ut)=>Ut.id===bt&&Ut.activationCapability),on=xt?we.policy.effectiveDestructiveCommandRules[xt.id]:void 0;return{trace:nm(wt),result:ht?"blocked":"allowed",reason:ht?R(ht.reason):void 0,segment:ht?R(ui(ht,j)):void 0,ruleId:ht?.ruleId?R(ht.ruleId):void 0,customRule:tm(rm(ht?.ruleId,oe)),configSource:Ce,configValid:Se,...xe,...xt&&on?{ruleActivation:{id:xt.id,...on}}:{}}}function ui(j,K){return j.evidence.find((te)=>te.kind==="command")?.segment??K}function Xf(j){if(j.reason===Ve)return{ruleId:"policy-protection",rule:"policy-protection:findPolicyConfigMutationTargetInSemanticFacts"};if(j.reason===ze)return{ruleId:"policy-apply-protection",rule:"policy-apply-protection:findPolicyApplyInvocationInSemanticFacts"};if(j.reason===C)return{ruleId:"git-metadata-protection",rule:"git-metadata-protection:findGitMetadataMutationTargetInSemanticFacts"};return{ruleId:j.ruleId,rule:"secret-protection:findSensitiveTargetInSemanticFacts"}}function Qf(j,K){let te=A(K.cwd),re=K.userConfigPath??T(j,K),oe=H(j,{cwd:K.cwd,userConfigDir:K.userConfigDir,userConfigPath:K.userConfigPath});try{if(r(oe.projectConfigTarget)!==null){if(en(oe.projectConfigTarget).errors.length===0)return{configSource:te,configValid:!0};return{configSource:te,configValid:!1}}}catch(ce){if(ce instanceof o)return{configSource:te,configValid:!1};throw ce}try{if(r(oe.userConfigTarget)!==null){let ce=en(oe.userConfigTarget);return{configSource:re,configValid:ce.errors.length===0}}return{configSource:null,configValid:!0}}catch(ce){if(ce instanceof o)return{configSource:re,configValid:!1};throw ce}}function em(j,K,te,re){let oe=K.policy,ce=Ie({...oe,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:{...oe.destructiveCommandRuleOverrides,...Object.fromEntries(U.flatMap((we)=>we.activationCapability?[[we.id,"on"]]:[]))}},K.state==="degraded"?{diagnostics:K.diagnostics,reason:K.reason}:void 0),de=V(j,{environment:re,dependencies:{loadPolicySnapshot:()=>ce,getModes:()=>({...te,strict:!0,paranoidRm:!0,paranoidInterpreters:!0}),findSensitiveTarget:()=>null}});return de.decision.kind==="deny"?de.decision.ruleId:void 0}function tm(j){if(!j)return;return{id:R(j.id),...j.rulebook?{rulebook:{name:R(j.rulebook.name),version:R(j.rulebook.version)}}:{},...j.source?{source:R(j.source)}:{},...j.override?{override:{type:"reason",reason:R(j.override.reason)}}:{}}}function nm(j){let K=j.events.flatMap((re)=>re.kind==="step"&&re.scope==="global"?[re.step]:[]),te=new Map;for(let re of j.events){if(re.kind!=="step"||re.scope!=="segment")continue;let oe=te.get(re.segmentIndex)??{index:re.segmentIndex,steps:[]};oe.steps.push(re.step),te.set(re.segmentIndex,oe)}return{steps:K,segments:[...te.values()]}}function rm(j,K){let te=j?.replace(/^custom\./,"");if(!te||!K.policy.rules.some((re)=>re.name===te))return;return K.ruleMetadata[te]??Object.freeze({id:te})}function $l(j){return new Promise((K)=>{process.stdout.write(`${j}
`,()=>K())})}async function Ol(j,K){let te=ii(K);if(!te)return 1;try{let re=Qn(te.command,{cwd:te.cwd},j),oe=!!process.env.NO_COLOR||!process.stdout.isTTY;return await $l(te.json?li(re):ai(re,{asciiOnly:oe})),0}catch(re){let oe=om(re instanceof p?re.cause:re);if(oe===void 0)throw re;if(te.json)return await $l(JSON.stringify({error:oe})),1;return console.error(oe),1}}function om(j){if(j instanceof b)return j.message;if(j instanceof f)return j.message;if(j instanceof a&&n[j.kind].errorCode==="path-canonicalization-limit")return"Path canonicalization work limit exceeded.";return}var jl="2.4.1",Mt="  ",pn="cc-safety-net";function Fl(j){return j.argument?`${j.flags} ${j.argument}`:j.flags}function im(j){return Math.max(...j.map((K)=>Fl(K).length))}function sm(j){return Math.max(...j.map((K)=>K.usage.length))}function am(j){return Math.max(...j.map((K)=>`${pn} ${K.usage}`.length))}function lm(j,K){let te=`${pn} ${j.usage}`;return`${Mt}${te.padEnd(K+2)}${j.description}`}function Zt(j,K){return`${Mt}${j.padEnd(Math.max(40,j.length+2))}${K}`}function An(j,K=console.log){let te=[];if(te.push(`${pn} ${j.name}`),te.push(""),te.push(`${Mt}${j.description}`),te.push(""),te.push("USAGE:"),te.push(`${Mt}${pn} ${j.usage}`),te.push(""),j.subcommands&&j.subcommands.length>0){te.push("SUBCOMMANDS:");let re=sm(j.subcommands);for(let oe of j.subcommands)te.push(`${Mt}${oe.usage.padEnd(re+2)}${oe.description}`);te.push("")}if(j.options.length>0){te.push("OPTIONS:");let re=im(j.options);for(let oe of j.options){let ce=Fl(oe),de=oe.default?`${oe.description} (default: ${oe.default})`:oe.description;te.push(`${Mt}${ce.padEnd(re+2)}${de}`)}te.push("")}if(j.examples&&j.examples.length>0){te.push("EXAMPLES:");for(let re of j.examples)te.push(`${Mt}${re}`)}K(te.join(`
`))}function pi(){let j=am(pr),K=[];K.push(`${pn} v${jl}`),K.push(""),K.push("Blocks destructive commands and secret access."),K.push(""),K.push("COMMANDS:");for(let te of pr)K.push(lm(te,j));K.push(""),K.push("GLOBAL OPTIONS:"),K.push(`${Mt}-h, --help       Show help (use with command for command-specific help)`),K.push(`${Mt}-V, --version    Show version`),K.push(""),K.push("HELP:"),K.push(`${Mt}${pn} help <command>     Show help for a specific command`),K.push(`${Mt}${pn} <command> --help   Show help for a specific command`),K.push(""),K.push("ENVIRONMENT VARIABLES:"),K.push(Zt(`${i.level.name}=standard|strict|paranoid`,"Set session safety level")),K.push(Zt(`${i.worktree.name}=1`,"Allow local git discards in linked worktrees")),K.push(Zt(`${i.debug.name}=1`,"Print diagnostic messages to stderr")),K.push(Zt(`${i.auditScope.name}=all|blocked`,"Record all command decisions, or denials only")),K.push(Zt("CC_SAFETY_NET_HOME","Override rule config home directory")),K.push(""),K.push("LEGACY ENVIRONMENT VARIABLES (STILL SUPPORTED):"),K.push(Zt(`${i.strict.name}=1`,"Force safety.overrides.fail_closed on")),K.push(Zt(`${i.paranoid.name}=1`,"Force paranoid_rm and paranoid_interpreters on")),K.push(Zt(`${i.paranoidRm.name}=1`,"Force safety.overrides.paranoid_rm on")),K.push(Zt(`${i.paranoidInterpreters.name}=1`,"Force safety.overrides.paranoid_interpreters on")),K.push(""),K.push("Documentation:        https://ccsafetynet.com/docs"),console.log(K.join(`
`))}function Nl(){console.log(jl)}function er(j,K=console.log){let te=fr(j);if(!te)return!1;if(te.name.toLowerCase()!==j.toLowerCase())return!1;return An(te,K),!0}import{existsSync as Si,readFileSync as $c}from"node:fs";import{join as xi}from"node:path";import*as nn from"node:readline";function cm(j){return j==="install"?"Install":"Uninstall"}function dm(j){return j==="install"?"Installing":"Uninstalling"}function um(j){return j==="install"?"into":"from"}function Ul(j){return j?.available===!0}function pm(j,K){let te=new Set(K);return j.filter((re)=>te.has(re.target)).map((re)=>re.target)}function Ml(j,K,te){if(j.length===0||j.every((re)=>!re.available))return K;return Array.from({length:j.length},(re,oe)=>oe+1).map((re)=>(K+re*te+j.length)%j.length).find((re)=>Ul(j[re]))}function fm(j,K,te){if(te.ctrl&&te.name==="c")return"interrupt";if(te.name==="escape"||K==="q")return"abort";if(j==="install"&&(K==="u"||K==="U"))return"update";if(te.name==="up"||K==="k")return"up";if(te.name==="down"||K==="j")return"down";if(te.name==="space"||K===" ")return"toggle";if(te.name==="return"||te.name==="enter")return"confirm";return null}function mm(j){return{cursor:j.findIndex((K)=>K.available),selected:[]}}function gm(j,K,te){if(te==="confirm"||te==="update"||te==="abort"||te==="interrupt")return{state:j,done:te};if(te==="up")return{state:{...j,cursor:Ml(K,j.cursor,-1)}};if(te==="down")return{state:{...j,cursor:Ml(K,j.cursor,1)}};let re=K[j.cursor];if(!Ul(re))return{state:j};let oe=j.selected.includes(re.target)?j.selected.filter((ce)=>ce!==re.target):pm(K,[...j.selected,re.target]);return{state:{...j,selected:oe}}}var Gl="◉",Bl="◯",ql=">",Vl=" ";function ym(j,K,te,re={}){let oe=re.color!==!1,ce=oe?vt.dim:(xe)=>xe,de=oe?vt.green:(xe)=>xe,we=oe?vt.bold:(xe)=>xe;return["",`${cm(j)} CC Safety Net ${um(j)}:`,"",...K.map((xe,Ce)=>{let Se=te.selected.includes(xe.target),Oe=Ce===te.cursor,Ge=Se?Gl:Bl,st=Oe?ql:Vl,ft=xe.available?"":` (${xe.unavailableReason??"not installed"})`,ot=`${Ge} ${xe.label}${ft}`,mt=!xe.available?ce(ot):Se?de(ot):Oe?we(ot):ot;return`${st} ${mt}`}),"",j==="install"?"Space: select  Enter: confirm  u: update installed  Up/Down: move  q/Esc: cancel":K.some((xe)=>xe.available)?"Space: select  Enter: confirm  Up/Down: move  q/Esc: cancel":`No selectable integrations found for ${j}. q/Esc: close`].join(`
`)}var Hl=["global-hook","plugin"];function hm(j,K,te={}){let re=te.color!==!1?vt.bold:(ce)=>ce;return["","Install the Kimi Code integration as:","",...[`Global hook — ${K?"already installed; selecting it reports the current state":"write the hook into ~/.kimi-code/config.toml now"}`,"Native Kimi plugin — print the steps to run inside Kimi Code"].map((ce,de)=>{let we=de===j,xe=`${we?Gl:Bl} ${ce}`;return`${we?ql:Vl} ${we?re(xe):xe}`}),"","Enter: confirm  Up/Down: move  q/Esc: cancel"].join(`
`)}function zl(j){let{input:K,output:te}=j;nn.emitKeypressEvents(K);let re=K.isRaw===!0;K.setRawMode(!0),K.resume();let oe=0,ce=()=>{if(oe===0)return;nn.moveCursor(te,0,-oe),nn.cursorTo(te,0),nn.clearScreenDown(te)},de=()=>{ce();let we=j.render();te.write(`${we}
`),oe=we.split(`
`).length};return new Promise((we)=>{let xe=(Se)=>{K.off("keypress",Ce),K.setRawMode(re),K.pause(),ce(),we(Se)};function Ce(Se,Oe){j.onKey(Se,Oe,{finish:xe,draw:de})}K.on("keypress",Ce),de()})}function Jl(j={}){let K=0;return zl({input:j.input??process.stdin,output:j.output??process.stdout,render:()=>hm(K,j.globalHookInstalled===!0),onKey:(te,re,oe)=>{if(re.ctrl&&re.name==="c"){oe.finish(null),(j.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return}if(re.name==="escape"||te==="q")return oe.finish(null);if(re.name==="return"||re.name==="enter")return oe.finish(Hl[K]);if(re.name==="up"||re.name==="down"||te==="k"||te==="j")K=(K+1)%Hl.length,oe.draw()}})}function fi(j=process.stdin,K=process.stdout){return Boolean(j.isTTY&&K.isTTY&&typeof j.setRawMode==="function")}function Wl(j,K,te={}){let re=te.output??process.stdout,oe=mm(K);return zl({input:te.input??process.stdin,output:re,render:()=>ym(j,K,oe),onKey:(ce,de,we)=>{let xe=fm(j,ce,de);if(!xe)return;let Ce=gm(oe,K,xe);if(oe=Ce.state,Ce.done==="interrupt"){we.finish(null),(te.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return}if(Ce.done==="abort")return we.finish(null);if(Ce.done==="update")return we.finish("update");if(Ce.done==="confirm"){if(oe.selected.length===0){re.write("\x07"),we.draw();return}we.finish([...oe.selected]),re.write(`${dm(j)} selected integrations...
`);return}we.draw()}})}import{existsSync as Kl,lstatSync as bm,mkdirSync as Lm,mkdtempSync as wm,readdirSync as km,readFileSync as Tn,rmSync as qr}from"node:fs";import{basename as xm,dirname as Cm,join as $t}from"node:path";import{fileURLToPath as Sm}from"node:url";var mi="// cc-safety-net managed Amp plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --amp",fn="cc-safety-net",mn="cc-safety-net/index.ts";import{spawn as vm}from"node:child_process";var gi=(j,K)=>{let te=ln([...j],process.env);return new Promise((re)=>{let oe=vm(te.cmd,te.args,{cwd:K,stdio:["ignore","pipe","pipe"]}),ce=Jo(oe),de=!1,we=setTimeout(()=>{de=!0,oe.kill()},120000);oe.on("error",(xe)=>{clearTimeout(we),re({status:null,errorCode:xe.code,stdout:ce.stdout,stderr:[xe.message,ce.stderr].filter(Boolean).join(`
`)})}),oe.on("close",(xe)=>{clearTimeout(we),re({status:de?null:xe,errorCode:de?"ETIMEDOUT":void 0,stdout:ce.stdout,stderr:ce.stderr})})})};var _n="cc-safety-net.ts",yi=$t("amp",mn);function Rm(j){return $t(j.home,".config","amp","plugins","cc-safety-net.ts")}function Pm(){let j=Cm(Sm(import.meta.url));return[$t(j,yi),$t(j,"..",yi),$t(j,"..","..","..","dist",yi)]}function Em(j=Pm()){let K=j.find((te)=>Kl(te)&&bm(te).isFile());if(!K)throw Error("Packaged Amp plugin artifact not found. Reinstall cc-safety-net and try again.");return K}function Yl(j){try{return JSON.parse(j)}catch{return}}function Vr(j){return j.subarray(0,Buffer.byteLength(mi)).toString("utf-8")===mi}async function tr(j,K,te){let re=await j(K,te);if(re.status===0)return re;throw Error([`Failed to run ${K.join(" ")}${re.status===null?"":` (exit ${re.status})`}.`,[re.stdout,re.stderr].filter(Boolean).join(`
`).trim()].filter(Boolean).join(`
`))}async function Zl(j){let K=await j(["amp","plugins","repositories","--json"]);if(K.status===null)throw Error(`${K.errorCode==="ENOENT"?'Amp CLI not found. Install the amp CLI, sign in with "amp login", and rerun install --amp.':`amp plugins repositories --json did not finish (${K.errorCode??"terminated"}). Check that the amp CLI responds and rerun install --amp.`}
...
`)}`.trim());let te=Yl(K.stdout),re=(Array.isArray(te)?te:[]).filter((oe)=>Lt(oe,"scope")==="user"&&Lt(oe,"exists")===!0&&Lt(oe,"viewerCanWrite")===!0).map((oe)=>Lt(oe,"cloneRef")).find((oe)=>typeof oe==="string"&&oe.length>0);if(!re)throw Error('Your Amp account has no writable Personal Plugins repository. Sign in with "amp login", open Amp once to create it, and rerun install --amp.');return re}async function Xl(j,K,te){let re=wm($t(K.tmpdir,"cc-safety-net-amp-"));try{return await tr(j,["amp","clone","user-plugins",re]),await te(re)}finally{qr(re,{recursive:!0,force:!0})}}function hi(j){return`rerun ${j==="overwrite"?"install":"uninstall"} --amp`}function Ql(j,K,te){let re=$t(j,K),oe=Rt(re);if(!oe)return;if(oe.isSymbolicLink()||!oe.isFile())throw Error(`Refusing to ${te} ${K} in your Amp personal plugins repository: not a regular file. Remove it there and ${hi(te)}.`);let ce=Tn(re);if(Vr(ce))return ce;throw Error(`Refusing to ${te} unmanaged file ${K} in your Amp personal plugins repository. Remove it there and ${hi(te)}.`)}function ec(j,K){let te=$t(j,fn),re=Rt(te);if(!re)return;if(re.isSymbolicLink()||!re.isDirectory())throw Error(`Refusing to ${K} ${fn} in your Amp personal plugins repository: not a regular directory. Remove it there and ${hi(K)}.`);return Ql(j,mn,K)}function Dm(j){let K=$t(j,_n),te=Rt(K);if(!te||te.isSymbolicLink()||!te.isFile())return;let re=Tn(K);return Vr(re)?re:void 0}async function tc(j,K,te,re){if(await tr(j,te,K),(await tr(j,["git","status","--porcelain"],K)).stdout.trim()==="")return!1;return await tr(j,["git","-c","commit.gpgsign=false","-c","user.name=cc-safety-net","-c","user.email=cc-safety-net@localhost","commit","-m",re],K),await tr(j,["git","push","origin","HEAD"],K),!0}function Br(j,K){Am(j,K),_m(j,K)}function nc(j,K){if(K==="keep")return;throw Error(`Local Amp plugin ${j} is not a managed copy and masks the personal plugin. Remove it and rerun install --amp.`)}function Am(j,K){let te=Rm(j),re=Rt(te);if(!re)return;if(!re.isSymbolicLink()&&re.isFile()&&Vr(Tn(te))){qr(te);return}nc(te,K)}function _m(j,K){let te=$t(j.home,".config","amp","plugins",fn),re=Rt(te);if(!re)return;if(!re.isSymbolicLink()&&re.isDirectory()&&Tm(te)){qr(te,{recursive:!0});return}nc(te,K)}function Tm(j){let K=xm(mn);if(km(j).join("\x00")!==K)return!1;let te=$t(j,K),re=Rt(te);return!!re&&!re.isSymbolicLink()&&re.isFile()&&Vr(Tn(te))}function Im(j){let K=d(j);if(!Kl(K))return"";let te=Yl(Tn(K,"utf-8"));if(!te||typeof te!=="object"||Array.isArray(te))return"";return`;globalThis.__CC_SAFETY_NET_EMBEDDED_POLICY__ = ${JSON.stringify(h(te,j.home))};
`}async function rc(j,K=Em(),te=gi){let re=Buffer.concat([Tn(K),Buffer.from(Im(j),"utf-8")]),oe=await Zl(te);return Xl(te,j,async(ce)=>{let de=`${oe}/${fn}`,we=ec(ce,"overwrite"),xe=Ql(ce,_n,"overwrite");if(we?.equals(re)&&!xe)return Br(j,"fail"),{path:de,alreadyInstalled:!0};if(Lm($t(ce,fn),{recursive:!0}),Et($t(ce,mn),re),xe)qr($t(ce,_n));let Ce=await tc(te,ce,["git","add","--",mn,...xe?[_n]:[]],`chore: update cc-safety-net plugin to v${Pt()}`);return Br(j,"fail"),{path:de,alreadyInstalled:!Ce}})}async function oc(j,K=gi){let te=await Zl(K);return Xl(K,j,async(re)=>{let oe=ec(re,"remove"),ce=Dm(re),de=`${te}/${ce&&!oe?_n:fn}`;if(!oe&&!ce)return Br(j,"keep"),{path:de,alreadyInstalled:!1};return await tc(K,re,["git","rm","--",...oe?[mn]:[],...ce?[_n]:[]],`chore: remove cc-safety-net plugin v${Pt()}`),Br(j,"keep"),{path:de,alreadyInstalled:!0}})}import{existsSync as ic,mkdirSync as $m,readFileSync as Om}from"node:fs";import{dirname as jm}from"node:path";var vi=Vt["antigravity-cli"],gn="cc-safety-net";function yn(j){return Boolean(j)&&typeof j==="object"&&!Array.isArray(j)}function Jr(){return{PreToolUse:[{hooks:[{type:"command",command:vi,timeout:30}]}]}}function sc(j){try{let K=JSON.parse(Om(j,"utf-8"));if(!K||typeof K!=="object"||Array.isArray(K))throw Error("Antigravity hooks config must be a JSON object");return K}catch(K){if(K instanceof SyntaxError)throw Error(`Failed to parse Antigravity hooks config ${j}: ${K.message}`);throw K}}function ac(j){let K=j[gn];if(K===void 0){let re=Jr();return j[gn]=re,{definition:re,preToolUse:re.PreToolUse??[]}}if(!yn(K))throw Error(`Antigravity hooks config entry "${gn}" must be an object`);let te=Array.isArray(K.PreToolUse)?K.PreToolUse:[];return K.PreToolUse=te,{definition:K,preToolUse:te}}function lc(j){if(!Array.isArray(j.PreToolUse))return!1;return j.PreToolUse.some((K)=>yn(K)&&Array.isArray(K.hooks)&&K.hooks.some((te)=>yn(te)&&te.command===vi))}function Fm(j){return Object.values(j).some((K)=>yn(K)&&K.enabled!==!1&&lc(K))}function Nm(j){if(j[gn]===void 0)return!1;let K=ac(j);if(K.definition.enabled!==!1||!lc(K.definition))return!1;return K.definition.enabled=!0,!0}function Mm(j){if(j[gn]===void 0){j[gn]=Jr();return}let K=ac(j);K.definition.enabled=!0,K.preToolUse.push(Jr().PreToolUse?.[0]??{hooks:[]})}function Hm(j){let K=!1;for(let te of Object.values(j)){if(!yn(te)||!Array.isArray(te.PreToolUse))continue;te.PreToolUse=te.PreToolUse.flatMap((re)=>{if(!yn(re)||!Array.isArray(re.hooks))return[re];let oe=re.hooks.filter((ce)=>!yn(ce)||ce.command!==vi);if(oe.length!==re.hooks.length)K=!0;return oe.length===0?[]:[{...re,hooks:oe}]})}return K}function zr(j,K){Et(j,`${JSON.stringify(K,null,2)}
`)}function cc(j){let K=Bn(j.home);if($m(jm(K),{recursive:!0}),!ic(K))return zr(K,{[gn]:Jr()}),{path:K,alreadyInstalled:!1};let te=sc(K);if(Fm(te))return{path:K,alreadyInstalled:!0};if(Nm(te))return zr(K,te),{path:K,alreadyInstalled:!1};return Mm(te),zr(K,te),{path:K,alreadyInstalled:!1}}function dc(j){let K=Bn(j.home);if(!ic(K))return{path:K,alreadyInstalled:!1};let te=sc(K);if(!Hm(te))return{path:K,alreadyInstalled:!1};return zr(K,te),{path:K,alreadyInstalled:!0}}import{existsSync as Um,readdirSync as Gm,rmSync as Bm}from"node:fs";import{join as qm}from"node:path";function uc(j,K=process.platform,te){if(!Um(j))return;let re=K==="win32"?/^bunx-\d+-cc-safety-net@/:new RegExp(`^bunx-${process.getuid?.()??0}-cc-safety-net@`);Gm(j).filter((oe)=>oe!==te&&re.test(oe)).forEach((oe)=>{Bm(qm(j,oe),{recursive:!0,force:!0})})}import{spawn as Vm}from"node:child_process";var Jt=Ft.map((j)=>({target:j.id,flag:j.flag,label:Ct(j.id),probeCommand:j.probeCommand}));function bi(j){let K=new Set(j);return Jt.map((te)=>te.target).filter((te)=>K.has(te))}async function pc(j,K){for(let te of j)await K(te)}var zm=5000;function Li(j,K=zm){return new Promise((te)=>{let re=ln([...j],process.env),oe=Vm(re.cmd,re.args,{env:process.env,stdio:"ignore"}),ce=!1,de=(xe)=>{if(ce)return;ce=!0,clearTimeout(we),te(xe)},we=setTimeout(()=>{oe.kill(),de(!1)},K);oe.on("error",()=>de(!1)),oe.on("close",(xe)=>de(xe===0))})}function fc(j=Li,K={}){let te=new Set(K.configuredTargets??[]);return Promise.all(Jt.map(async(re)=>({target:re.target,flag:re.flag,label:re.label,...gc(K.action,await j(re.probeCommand),te.has(re.target))})))}function mc(j,K){let te=new Set(K.configuredTargets??[]);return j.map((re)=>({...re,...gc(K.action,re.available,te.has(re.target))}))}function gc(j,K,te){if(j==="uninstall")return te?{available:!0}:{available:!1,unavailableReason:"not installed"};if(j==="install"&&te)return{available:!1,unavailableReason:"already installed"};if(!K)return{available:!1,unavailableReason:"CLI not installed"};return{available:!0}}import{existsSync as yc,readdirSync as Jm,rmSync as Wm}from"node:fs";import{join as In}from"node:path";function Wr(j,K=process.platform){let te=In(j.env.get("npm_config_cache")||(K==="win32"?In(j.env.get("LOCALAPPDATA")||In(j.home,"AppData","Local"),"npm-cache"):In(j.home,".npm")),"_npx");if(!yc(te))return;Jm(te).filter((re)=>yc(In(te,re,"node_modules","cc-safety-net"))).forEach((re)=>{Wm(In(te,re),{recursive:!0,force:!0})})}import{existsSync as kc,mkdirSync as Ym,readFileSync as xc}from"node:fs";import{dirname as Zm,join as wc}from"node:path";function Km(j,K){if(j[K]!=="#")return K;let te=j.indexOf(`
`,K+1);return te===-1?j.length:te+1}function wi(j,K,te){let re=new RegExp(`^(\\s*)${K}\\s*=\\s*\\[`),oe=0;for(let ce of j.split(`
`)){if(/^\s*\[/.test(ce))return;let de=re.exec(ce);if(de){let we=oe+de[0].lastIndexOf("[");return{start:we,end:Ao(j,we,{skipComment:Km,...te})}}oe+=ce.length+1}return}function hc(j,K,te){let re=j.slice(0,K.end).trimEnd(),oe=da(j,K.end),ce=oe===""?"     ":`${oe}  `,de=!re.endsWith("[")&&!re.endsWith(",");return`${re}${de?",":""}
${ce}${te}${j.slice(K.end)}`}function vc(j,K,te){let re=j.indexOf(te,K.start);if(re===-1||re>K.end)return j;return Rr(j,{start:re,end:re+te.length})}function bc(j,K){let te=new RegExp(`^\\s*${K}\\s*=\\s*\\[\\s*]\\s*(?:#.*)?$`),re=j.split(`
`),oe=re.findIndex((we)=>/^\s*\[/.test(we)),ce=oe===-1?re:re.slice(0,oe),de=oe===-1?[]:re.slice(oe);return[...ce.filter((we)=>!te.test(we)),...de].join(`
`)}function Lc(j,K,te){let re=new RegExp(`^\\s*\\[\\[${K}]]\\s*$`,"m");return j.split(/(?=^\s*\[)/m).filter((oe)=>!re.test(oe)||!oe.includes(te)).join("").trimEnd()}var nr=Vt["kimi-code"],ki=`[[hooks]]
event = "PreToolUse"
command = "${nr}"`,Cc=`{ event = "PreToolUse", command = "${nr}" }`,Sc={stringError:"Unterminated string in Kimi Code config",bracketError:"Unmatched hooks array in Kimi Code config"};function Rc(j){return wc(j.env.get("KIMI_CODE_HOME")??wc(j.home,".kimi-code"),"config.toml")}function Xm(j){let K=wi(j,"hooks",Sc);if(K&&j.slice(K.start+1,K.end).trim())return hc(j,K,Cc);let te=bc(j,"hooks").trimEnd();if(te==="")return`${ki}
`;return`${te}

${ki}
`}function Pc(j){let K=Rc(j);if(Ym(Zm(K),{recursive:!0}),!kc(K))return Et(K,`${ki}
`),{path:K,alreadyInstalled:!1};let te=xc(K,"utf-8");if(te.includes(nr))return{path:K,alreadyInstalled:!0};return Et(K,Xm(te)),{path:K,alreadyInstalled:!1}}function Ec(j){let K=Rc(j);if(!kc(K))return{path:K,alreadyInstalled:!1};let te=xc(K,"utf-8");if(!te.includes(nr))return{path:K,alreadyInstalled:!1};let re=wi(te,"hooks",Sc),oe=re?vc(te,re,Cc):`${Lc(te,"hooks",nr)}
`;return Et(K,oe),{path:K,alreadyInstalled:!0}}var Ci="safety-net@cc-marketplace",Dc=new Set(["claude-code","codex","copilot-cli","gemini-cli","hermes-agent","openclaw","opencode","pi"]),Ac=new Set(["antigravity-cli","cursor","grok-build","hermes-agent","kimi-code"]);function Ri(j){return/^\s*safety-net@cc-marketplace[^a-z0-9-][^\n]*installed,/m.test(j??"")}function Oc(j){return/^\s*cc-safety-net[^a-z0-9-][^\n]*installed,/m.test(j??"")}function Qm(j){return/^Marketplace `cc-marketplace`\s*$/m.test(j??"")}var jc={"claude-code":{installCommands:(j)=>{let K=Cr(j,"cc-safety-net@cc-marketplace");return{commands:[...K?[["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","update","cc-safety-net@cc-marketplace"]]:[["claude","plugin","marketplace","add","kenryu42/cc-marketplace"],["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","install","cc-safety-net@cc-marketplace"]],...Eo(j).status==="disabled"?[["claude","plugin","enable","cc-safety-net@cc-marketplace"]]:[]],cleanupCommands:Cr(j,Ci)?[["claude","plugin","uninstall",Ci]]:[],update:K}},uninstallCommands:[["claude","plugin","uninstall","cc-safety-net@cc-marketplace"],["claude","plugin","marketplace","remove","cc-marketplace"]]},codex:{installCommands:async(j,K)=>{let te=K??await zt(["codex","plugin","list"]),re=Oc(te);return{commands:[re||Qm(te)?["codex","plugin","marketplace","upgrade","cc-marketplace"]:["codex","plugin","marketplace","add","kenryu42/cc-marketplace"],["codex","plugin","add","cc-safety-net@cc-marketplace"]],cleanupCommands:Ri(te)?[["codex","plugin","remove","safety-net@cc-marketplace"]]:[],update:re}},uninstallCommands:[["codex","plugin","remove","cc-safety-net@cc-marketplace"],["codex","plugin","marketplace","remove","cc-marketplace"]],postInstallMessage:"Start Codex, open `/hooks`, select the cc-safety-net PreToolUse hook, and press `t` to trust it."},"copilot-cli":{installCommands:async()=>{let j=await zt(["copilot","plugin","list"]),K=[...va(j)?[["copilot","plugin","uninstall","copilot-safety-net"]]:[],...ba(j)?[["copilot","plugin","uninstall",ga]]:[]];if(ya(j))return{commands:[["copilot","plugin","marketplace","update","cc-marketplace"],["copilot","plugin","update",qt]],cleanupCommands:K,update:!0};return{commands:[ha(await zt(["copilot","plugin","marketplace","list"]))?["copilot","plugin","marketplace","update","cc-marketplace"]:["copilot","plugin","marketplace","add","kenryu42/cc-marketplace"],["copilot","plugin","install",qt]],cleanupCommands:K}},uninstallCommands:[["copilot","plugin","uninstall","cc-safety-net@cc-marketplace"],["copilot","plugin","marketplace","remove","cc-marketplace"]]},"gemini-cli":{installCommands:(j)=>{let K=Fo(j);if(K.status==="configured")return{commands:[["gemini","extensions","update","gemini-safety-net"]],update:!0};if(K.status==="disabled")return{commands:[["gemini","extensions","update","gemini-safety-net"],["gemini","extensions","enable","gemini-safety-net"]],update:!0};return{commands:[["gemini","extensions","install","https://github.com/kenryu42/gemini-safety-net","--consent"]]}},uninstallCommands:[["gemini","extensions","uninstall","gemini-safety-net"]]},openclaw:{beforeInstall:Xo,installCommands:()=>({commands:sl()}),uninstallCommands:[["openclaw","plugins","uninstall",St,"--force"]],postInstallMessage:["Restart the OpenClaw Gateway to apply the change.","If plugins.allow is set in openclaw.json, it must also list cc-safety-net."].join(`
`)},opencode:{beforeInstall:ti,installCommands:[["opencode","plugin","-g","-f","cc-safety-net@latest"]]},pi:{installCommands:[["pi","install","npm:cc-safety-net"]],uninstallCommands:[["pi","uninstall","npm:cc-safety-net"]]}};function Fc(j,K=(te)=>te){try{let te=JSON.parse(K($c(j,"utf-8")));if(!te||typeof te!=="object"||Array.isArray(te))throw Error(`Settings file ${j} must be a JSON object`);return te}catch(te){if(te instanceof SyntaxError)throw Error(`Failed to parse ${j}: ${te.message}`);throw te}}function eg(j){let K=xi(qn(j),"settings.json");if(!Si(K))return;let te=Fc(K,Ot),re=te.enabledPlugins;if(!re||typeof re!=="object"||Array.isArray(re))return;if(re[qt]!==!1)return;let oe=$c(K,"utf-8"),ce=oe.replace(new RegExp(`("${qt}"\\s*:\\s*)false`),"$1true");return re[qt]=!0,Et(K,ce!==oe?ce:`${JSON.stringify(te,null,2)}
`),`Enabled ${qt} plugin in ${K}`}function tg(j){let K=ni(j);if(!Si(K))return;let te=Fc(K);if(!Array.isArray(te.packages))return;let re=te.packages.find((oe)=>!!oe&&typeof oe==="object"&&!Array.isArray(oe)&&ri(oe.source)&&("extensions"in oe));if(!re)return;return delete re.extensions,Et(K,`${JSON.stringify(te,null,2)}
`),`Enabled npm:cc-safety-net extensions in ${K}`}function _c(j,K){let te=Dt({label:K,booleans:Object.fromEntries(Jt.map((ce)=>[ce.target,[ce.flag]]))},j),re=te.errors[0];if(re)throw Error(re);let oe=Jt.filter((ce)=>te.flags[ce.target]).map((ce)=>ce.target);if(oe.length!==1)throw Error(`Choose exactly one ${K} target: ${Jt.map((ce)=>ce.flag).join(", ")}`);return oe[0]}async function Nc(j,K=kn){let[te,re,oe]=await Promise.all([K(["amp","plugins","list"],30000),K(["codex","plugin","list"],30000),K(["copilot","--binary-version"])]);return{codexPluginListOutput:re,hooks:En(j,process.cwd(),{ampPluginListOutput:te,codexPluginListOutput:re,copilotCliVersion:oe})}}async function ng(j,K,te=kn){let re=await Nc(j,te);return re.hooks.filter((oe)=>K==="install"?oe.configured:oe.detected||oe.inspectionStatus==="not-inspected").filter((oe)=>oe.platform!=="codex"||!Ri(re.codexPluginListOutput)||Oc(re.codexPluginListOutput)).map((oe)=>oe.platform)}function rg(j,K,te,re){if(te.length>0)return{finish:async()=>[_c(te,K)]};if(!re.selectTargets&&!fi(re.input,re.output))return{finish:async()=>[_c(te,K)]};let oe=re.detectConfiguredTargets??(()=>ng(j,K,re.fetchVersion)),ce=Promise.all([fc(re.probeTargets),oe()]);return{ready:ce,finish:async()=>{let[de,we]=await ce,xe=mc(de,{action:K,configuredTargets:we}),Ce=re.selectTargets?await re.selectTargets(K,Ic(K,xe)):await Wl(K,Ic(K,xe),{input:re.input,output:re.output});if(Ce==="update")return Ce;if(!Ce||Ce.length===0)return null;return bi(Ce)}}}async function hn(j,K,te=!1,re){let oe=jc[j];oe.beforeInstall?.(K);let ce=typeof oe.installCommands==="function"?await oe.installCommands(K,re):{commands:oe.installCommands};return await Wo(ce.commands),await tl(ce.cleanupCommands??[]),[`${ce.update||te?"Updated":"Installed"} ${Ct(j)} integration`,oe.postInstallMessage].filter(Boolean).join(`
`)}async function $n(j){let K=jc[j];if(!K.uninstallCommands)throw Error(`${Ct(j)} uninstall is not supported`);return await Wo(K.uninstallCommands),`Uninstalled ${Ct(j)} integration`}function og(j){let K=wl(j);return K.alreadyInstalled?`Uninstalled OpenCode plugin from ${K.path}`:`OpenCode plugin not installed in ${K.path}`}var ig={"antigravity-cli":{install:cc,uninstall:dc},cursor:{install:Da,uninstall:Aa},"grok-build":{install:Ha,uninstall:Ua},"kimi-code":{install:Pc,uninstall:Ec}};function rn(j,K,te,re=!1){if(j==="install"&&!re)Wr(te);let oe=ig[K][j](te),ce=Ct(K),de=j!=="install"?"Uninstalled":re?"Updated":"Installed";return j==="install"&&oe.alreadyInstalled?re?`${ce} hook up to date in ${oe.path}`:`${ce} hook already installed in ${oe.path}`:j==="uninstall"&&!oe.alreadyInstalled?`${ce} hook not installed in ${oe.path}`:`${de} ${ce} hook ${j==="install"?"in":"from"} ${oe.path}`}var sg={amp:{install:rc,uninstall:oc,restartNote:'Amp personal plugins apply to every Amp session, including Orb threads. Restart Amp or run "plugins: reload" to apply the change.'},"hermes-agent":{install:Va,uninstall:za,afterInstall:async(j)=>{let K=Vo(j);return await zt(["hermes","plugins","enable",jt,"--no-allow-tool-override"]),!K},beforeUninstall:async(j)=>{qo(j);try{await zt(["hermes","plugins","disable",jt])}catch(K){console.warn(`${K instanceof Error?K.message:String(K)}
Removing the plugin files anyway; ${jt} may still be listed in the Hermes config.`)}},restartNote:"Restart Hermes to apply the change."}};async function Kr(j,K,te,re=!1){let oe=sg[K];if(j==="uninstall")await oe.beforeUninstall?.(te);let ce=j==="install"?await oe.install(te):await oe.uninstall(te),de=j==="install"&&await oe.afterInstall?.(te),we=Ct(K),xe=!de&&(j==="install"&&ce.alreadyInstalled||j==="uninstall"&&!ce.alreadyInstalled);return[xe?j==="install"?`${we} plugin ${re?"up to date":"already installed"} at ${ce.path}`:`${we} plugin not installed at ${ce.path}`:`${j!=="install"?"Uninstalled":re?"Updated":"Installed"} ${we} plugin ${j==="install"?"at":"from"} ${ce.path}`,xe?void 0:oe.restartNote].filter(Boolean).join(`
`)}var ag={amp:{install:(j,K)=>Kr("install","amp",j,K),uninstall:(j)=>Kr("uninstall","amp",j)},"antigravity-cli":{install:(j,K)=>rn("install","antigravity-cli",j,K),uninstall:(j)=>rn("uninstall","antigravity-cli",j)},"claude-code":{install:(j,K)=>hn("claude-code",j,K),uninstall:()=>$n("claude-code")},codex:{install:(j,K,te)=>hn("codex",j,K,te),uninstall:()=>$n("codex")},"copilot-cli":{install:async(j,K)=>[await hn("copilot-cli",j,K),eg(j)].filter(Boolean).join(`
`),uninstall:()=>$n("copilot-cli")},cursor:{install:(j,K)=>rn("install","cursor",j,K),uninstall:(j)=>rn("uninstall","cursor",j)},"gemini-cli":{install:(j,K)=>hn("gemini-cli",j,K),uninstall:()=>$n("gemini-cli")},"grok-build":{install:(j,K)=>rn("install","grok-build",j,K),uninstall:(j)=>rn("uninstall","grok-build",j)},"hermes-agent":{install:(j,K)=>{if(!K)Wr(j);return Kr("install","hermes-agent",j,K)},uninstall:(j)=>Kr("uninstall","hermes-agent",j)},"kimi-code":{install:(j,K)=>rn("install","kimi-code",j,K),uninstall:(j)=>rn("uninstall","kimi-code",j)},openclaw:{install:async(j,K)=>{let te=await hn("openclaw",j,K);return await al(),te},uninstall:(j)=>(Xo(j),$n("openclaw"))},opencode:{install:async(j,K)=>{let te=await hn("opencode",j,K);return await bl(j),te},uninstall:(j)=>og(j)},pi:{install:async(j,K)=>[await hn("pi",j,K),tg(j)].filter(Boolean).join(`
`),uninstall:()=>$n("pi")}},Tc=["Install CC Safety Net as a native Kimi Code plugin:","","  1. Start Kimi Code and run: /plugins install https://github.com/kenryu42/cc-safety-net","     Confirm the trust prompt; it defaults to cancel.","  2. Run /reload, or start a new session.","","Note: Kimi Code hooks are fail-open. When the hook process cannot start, crashes, or times","out, Kimi Code allows the tool call."].join(`
`);function lg(j){if(Kn({environment:j,cwd:process.cwd()}).status!=="configured")return Tc;return[Tc,"",vt.red(["CAUTION: the global Kimi Code hook is installed and will run alongside the plugin.","After the plugin is active, remove it with: cc-safety-net uninstall --kimi-code"].join(`
`))].join(`
`)}function Ic(j,K){return K.map((te)=>j==="install"&&te.target==="kimi-code"&&te.unavailableReason==="already installed"?{...te,available:!0,unavailableReason:void 0,label:`${te.label} (global hook installed)`}:te)}function cg(j,K){if(j.selectKimiInstallMethod)return j.selectKimiInstallMethod();if(!fi(j.input,j.output))return Promise.resolve("global-hook");return Jl({input:j.input,output:j.output,globalHookInstalled:Kn({environment:K,cwd:process.cwd()}).status==="configured"})}async function Mc(j,K,te,re=!1,oe){return ag[K][j](te,re,oe)}function dg(j){let K=Dt({label:"update"},j).errors[0];if(K)throw Error(K)}async function ug(j,K=kn){let te=await Nc(j,K),re=xi(qn(j),"installed-plugins");return{targets:bi([...te.hooks.filter((ce)=>ce.platform!=="copilot-cli"&&ce.detected).map((ce)=>ce.platform),...[Pr,ma,fa].flatMap((ce)=>Si(xi(re,...ce))?["copilot-cli"]:[]),...Cr(j,Ci)?["claude-code"]:[],...Ri(te.codexPluginListOutput)?["codex"]:[]]),codexPluginListOutput:te.codexPluginListOutput}}async function pg(j){let K=l(),te=j.output??process.stdout,re=(j.scriptPath??process.argv[1]??"").split(/[\\/]/),oe=re.find((st)=>/^bunx-\d+-/.test(st)),ce=oe!==void 0||re.includes("_npx")?null:(j.checkLatestVersion??tn)(),de=async()=>{let st=ce&&await ce;if(st?.updateAvailable)te.write(`
Update available: cc-safety-net ${st.currentVersion} → ${st.latestVersion}. Update this CLI with your package manager, e.g. \`npm i -g cc-safety-net@latest\` for a global install.
`)},we=ug(K,j.fetchVersion??kn).then(async(st)=>{let ft=new Set(st.targets);return{targets:st.targets,codexPluginListOutput:st.codexPluginListOutput,available:new Map(await Promise.all(Jt.filter((ot)=>ft.has(ot.target)&&Dc.has(ot.target)).map(async(ot)=>[ot.target,await Li(ot.probeCommand)])))}}),xe=await Gn(j.showBanner??!0,()=>({ready:we,finish:()=>we}),()=>Un({input:j.input??process.stdin,output:te}),{loadingMessage:"Checking installed integrations…",output:te}),Ce=await Promise.resolve().then(()=>(uc(K.tmpdir,process.platform,oe),null)).catch((st)=>rr(st));if(xe.targets.length===0){if(te.write("No installed integrations found. Run `cc-safety-net install` to set one up.\n"),Ce!==null)console.error(Ce);return await de(),Ce===null?0:1}let Se=xe.targets.some((st)=>Ac.has(st))?await Promise.resolve().then(()=>(Wr(K),null)).catch((st)=>rr(st)):null,Oe=await wr(Promise.all(xe.targets.map((st)=>{if(Dc.has(st)&&!xe.available.get(st))return Promise.resolve({message:`${Ct(st)} not found; skipped`,failed:!1});if(Se!==null&&Ac.has(st))return Promise.resolve({message:Se,failed:!0});return Mc("install",st,K,!0,xe.codexPluginListOutput).then((ft)=>({message:ft,failed:!1}),(ft)=>({message:rr(ft),failed:!0}))})),{loadingMessage:`Updating ${xe.targets.length} integration${xe.targets.length===1?"":"s"}…`,output:te}),Ge=Ce===null?Oe:[...Oe,{message:Ce,failed:!0}];return Ge.forEach((st)=>{st.failed?console.error(st.message):te.write(`${st.message}
`)}),await de(),Ge.some((st)=>st.failed)?1:0}function Pi(j,K={}){return Promise.resolve().then(()=>dg(j)).then(()=>pg(K)).catch((te)=>(console.error(rr(te)),1))}async function or(j,K,te={}){try{let re=l(),oe=await Gn(!0,()=>rg(re,j,K,te),()=>Un({input:te.input??process.stdin,output:te.output??process.stdout}),{loadingMessage:j==="install"?"Checking available integrations…":"Checking installed integrations…",output:te.output??process.stdout});if(!oe)return(te.output??process.stdout).write(`Cancelled: nothing was ${j}ed.
`),0;if(oe==="update")return(te.runUpdate??(()=>Pi([],{fetchVersion:te.fetchVersion,input:te.input,output:te.output,showBanner:!1})))();let ce=te.output??process.stdout;return await pc(oe,async(de)=>{if(de==="kimi-code"&&j==="install"){let xe=await cg(te,re);if(xe===null){ce.write(`Cancelled: Kimi Code integration was not installed.
`);return}if(xe==="plugin"){ce.write(`${lg(re)}
`);return}}let we=await wr(Mc(j,de,re),{loadingMessage:`${j==="install"?"Installing":"Uninstalling"} ${Ct(de)} integration…`,output:ce});ce.write(`${we}
`)}),0}catch(re){return console.error(rr(re)),1}}function rr(j){let K=j instanceof Error?j.message:String(j),te=typeof j==="object"&&j!==null&&"code"in j?j.code:null;if(te==="EACCES"||te==="EPERM")return`${K}
...
Check that every parent path component is a directory.`;return K}import{mkdirSync as kg}from"node:fs";import{dirname as xg}from"node:path";import{createInterface as Cg}from"node:readline";import{existsSync as zc,readFileSync as hg}from"node:fs";var Gc=["version","safety","workflow","destructive_command_protection","secret_protection","audit"],fg=["standard","strict","paranoid"],Bc="must be an object if provided",mg=`must be an integer between ${ye} and ${ve}`;function Ht(j,K){return se(tt(gg(j,K),Gc,(te)=>te.kind==="custom")," "," ")}function gg(j,K){if(!Di(j))return[e([],"Config must be an object")];return[...j.version===1?[]:[e(["version"],"must be 1")],...On(j.safety,["safety"],["level","overrides"],(te)=>[...te.level===void 0||typeof te.level==="string"&&fg.includes(te.level)?[]:[e(["safety","level"],'must be "standard", "strict", or "paranoid"')],...On(te.overrides,["safety","overrides"],Hc,(re)=>Hc.flatMap((oe)=>Yr(re[oe],["safety","overrides",oe])))]),...On(j.workflow,["workflow"],["worktree_mode"],(te)=>Yr(te.worktree_mode,["workflow","worktree_mode"])),...On(j.destructive_command_protection,["destructive_command_protection"],["enabled","overrides","allow_paths"],(te)=>[...Yr(te.enabled,["destructive_command_protection","enabled"]),...Uc(te.overrides,["destructive_command_protection","overrides"],Re,"destructive command"),...Ei(te.allow_paths,["destructive_command_protection","allow_paths"],_e,K)]),...On(j.secret_protection,["secret_protection"],["enabled","overrides","deny_paths","allow_paths"],(te)=>[...Yr(te.enabled,["secret_protection","enabled"]),...Uc(te.overrides,["secret_protection","overrides"],Pe,"secret protection"),...Ei(te.deny_paths,["secret_protection","deny_paths"],Ae,K),...Ei(te.allow_paths,["secret_protection","allow_paths"],Te,K)]),...On(j.audit,["audit"],["retention_days"],(te)=>yg(te.retention_days)?[]:[e(["audit","retention_days"],mg)]),...Object.keys(j).filter((te)=>!Gc.includes(te)).map((te)=>qc([],te))]}var Hc=["fail_closed","paranoid_rm","paranoid_interpreters"];function On(j,K,te,re){if(j===void 0)return[];if(!Di(j))return[e(K,Bc)];return[...re(j),...Object.keys(j).filter((oe)=>!te.includes(oe)).map((oe)=>qc(K,oe))]}function Yr(j,K){return j===void 0||typeof j==="boolean"?[]:[e(K,"must be a boolean")]}function Uc(j,K,te,re){if(j===void 0)return[];if(!Di(j))return[e(K,Bc)];return[...Object.keys(j).filter((oe)=>!te.has(oe)).map((oe)=>({path:[...K,oe],message:`unknown ${re} rule id "${oe}"`,kind:"key"})),...Object.keys(j).filter((oe)=>j[oe]!=="on"&&j[oe]!=="off").map((oe)=>e([...K,oe],'must be "on" or "off"'))]}function Ei(j,K,te,re){if(j===void 0)return[];if(!Array.isArray(j))return[e(K,"must be an array of paths")];return j.flatMap((oe,ce)=>{if(typeof oe!=="string")return[e([...K,ce],"must be a non-empty path string")];let de=te(oe,re);return de===null?[]:[g([...K,ce],de)]})}function yg(j){if(j===void 0)return!0;if(typeof j!=="number"||!Number.isInteger(j))return!1;return j>=ye&&j<=ve}var qc=(j,K)=>({path:j,message:K,kind:"unknownKeys"});function Di(j){return!!j&&typeof j==="object"&&!Array.isArray(j)}function Vc(j,K){return{"safety.level":j.safety.level,...Ai("safety.overrides",j.safety.overrides),"workflow.worktree_mode":String(j.workflow.worktree_mode),"destructive_command_protection.enabled":String(j.destructive_command_protection.enabled),...Ai("destructive_command_protection.overrides",j.destructive_command_protection.overrides),"destructive_command_protection.allow_paths":_i(j.destructive_command_protection.allow_paths),"secret_protection.enabled":String(j.secret_protection.enabled),...Ai("secret_protection.overrides",j.secret_protection.overrides),"secret_protection.deny_paths":_i(j.secret_protection.deny_paths),"secret_protection.allow_paths":_i(j.secret_protection.allow_paths),...K?{"audit.retention_days":String(j.audit.retention_days)}:{}}}function Zr(j,K,te){let re=Vc(j,te),oe=Vc(K,te);return[...new Set([...Object.keys(re),...Object.keys(oe)])].flatMap((ce)=>re[ce]===oe[ce]?[]:[{field:ce,before:re[ce],after:oe[ce]}])}function ir(j,K){let te=d(j,K);if(!zc(te))return{baseline:h(globalThis.__CC_SAFETY_NET_EMBEDDED_POLICY__,j.home),diagnostics:[]};let re=vn(te);return{baseline:h(re.value,j.home),diagnostics:re.errors.length>0?re.errors:Ht(re.value,j.home)}}function vn(j){if(!zc(j))return{errors:[`${j}: file not found`]};try{return{value:JSON.parse(hg(j,"utf-8")),errors:[]}}catch(K){let te=K instanceof Error?K.message:String(K);return{errors:[`${j}: ${K instanceof SyntaxError?`Invalid JSON: ${te}`:te}`]}}}function Xr(j,K){let te=vg(j)?j:{};return{version:K.version,...Object.fromEntries(["safety","workflow","destructive_command_protection","secret_protection"].filter((re)=>te[re]!==void 0).map((re)=>[re,te[re]]))}}function Ai(j,K){return Object.fromEntries(Object.entries(K).flatMap(([te,re])=>re===void 0?[]:[[`${j}.${te}`,String(re)]]))}function _i(j){return j.length===0?"(none)":j.join(", ")}function vg(j){return!!j&&typeof j==="object"&&!Array.isArray(j)}import{chmodSync as bg,existsSync as Jc,mkdirSync as Lg,readFileSync as Wc}from"node:fs";import{dirname as wg}from"node:path";function Kc(j,K={}){let te=d(j,K);if(!Jc(te))return{path:te,exists:!1,raw:"",policy:z(),errors:[]};let re=Wc(te,"utf-8");if(!re.trim())return{path:te,exists:!0,raw:re,policy:z(),errors:["Config file is empty"]};try{let oe=JSON.parse(re),ce=Ht(oe,j.home);return{path:te,exists:!0,raw:re,policy:h(oe,j.home),errors:ce}}catch(oe){return{path:te,exists:!0,raw:re,policy:z(),errors:[`Invalid JSON: ${oe instanceof Error?oe.message:String(oe)}`]}}}function Xt(j,K,te={}){let re=d(j,te),oe=Ht(K,j.home),ce=oe.length>0?z():h(K,j.home);if(oe.length>0)return{path:re,policy:ce,errors:oe};return Lg(wg(re),{recursive:!0,mode:448}),y(D(re),`${JSON.stringify(ce,null,2)}
`,384),bg(re,384),{path:re,policy:ce,errors:[]}}function Yc(j,K){let te=Ht(K,j.home);if(te.length>0)return{errors:te};return{preview:je(h(K,j.home),j.env),errors:[]}}function Zc(j,K={}){let te=d(j,K);if(!Jc(te))return Xt(j,Q,K);let re=Wc(te,"utf-8");if(!re.trim())return Xt(j,Q,K);try{return Xt(j,h(JSON.parse(re),j.home),K)}catch{return Xt(j,Q,K)}}var Xc=new Set(["check","apply"]),Qc="(unset)";async function td(j,K,te={}){let re=Dt({label:"policy",booleans:{global:["-g","--global"]},positionals:"list"},K),oe=re.positionals[0],ce=[...re.errors,...oe&&!Xc.has(oe)?[`Unknown policy subcommand: ${oe}`]:[],...oe&&Xc.has(oe)&&!re.positionals[1]?[`policy ${oe} requires a file`]:[],...re.positionals.slice(2).map((ft)=>`Unexpected policy argument: ${ft}`)];if(ce.length>0){for(let ft of ce)console.error(ft);return 1}let de=re.positionals[1];if(!oe||!de)return An(ur,console.error),1;let we=re.flags.global?d(j):w(te.cwd??process.cwd()),xe=vn(de),Ce=[...xe.errors,...Ht(xe.value,j.home).map((ft)=>`${de}: ${ft}`),...!re.flags.global&&Pg(xe.value)&&xe.value.audit!==void 0?[`${de}: audit settings are user scope only; remove the audit section from a project proposal`]:[]];if(Ce.length>0){for(let ft of Ce)console.error(ft);return 1}let Se=h(xe.value,j.home);if(console.log(`Scope: ${re.flags.global?"user":"project"} (${we})`),console.log(`Proposal: ${de}`),re.flags.global)ed(h(vn(we).value,j.home),Se,!0);if(!re.flags.global){let ft=ir(j).baseline;console.log("Effective policy (user + project merged):"),ed(X(ft,le(vn(we).value,j.home).policy).policy,X(ft,le(xe.value,j.home).policy).policy,!1)}if(oe==="check")return 0;let Oe=te.input??process.stdin,Ge=te.output??process.stdout;if(!Oe.isTTY||!Ge.isTTY)return console.error("policy apply confirms interactively; run this yourself in a terminal:"),console.error(`  cc-safety-net policy apply ${de}${re.flags.global?" --global":""}`),1;if(!await Sg(`Apply this policy to ${we}? [y/N] `,Oe,Ge))return console.log("Cancelled; nothing was written."),0;return Rg(j,we,xe.value,Se,re.flags.global),console.log(`Policy applied: ${we}`),0}function Sg(j,K,te){let re=Cg({input:K,output:te,terminal:!1});return new Promise((oe)=>{re.once("close",()=>oe(!1)),re.question(j,(ce)=>{oe(/^y(es)?$/i.test(ce.trim())),re.close()})})}function Rg(j,K,te,re,oe){if(oe){Xt(j,re);return}kg(xg(K),{recursive:!0}),Tt(K,Xr(te,re))}function ed(j,K,te){let re=Zr(j,K,te);if(re.length===0){console.log("No changes.");return}console.log(`Changes (${re.length}):`);for(let oe of re)console.log(`  ${oe.field}: ${oe.before??Qc} -> ${oe.after??Qc}`)}function Pg(j){return!!j&&typeof j==="object"&&!Array.isArray(j)}import{join as Hy}from"node:path";var nd="# Custom Rules Reference\n\nAgent reference for generating CC Safety Net rulebook configuration.\n\n## Config Locations\n\n| Scope | Config path | Rulebook path | Priority |\n|-------|-------------|---------------|----------|\n| User | `~/.cc-safety-net/rules/rule.json` | `~/.cc-safety-net/rules/<rulebook-name>/rulebook.json` | First |\n| Project | `.cc-safety-net/rules/rule.json` | `.cc-safety-net/rules/<rulebook-name>/rulebook.json` | Second |\n| GitHub source | Listed in a local `rule.json` | Vendored into the consumer's `<rulebook-name>/rulebook.json` by `rule add` | Source order |\n\nEvery rulebook is a live file: the runtime reads it on each tool call, so an edit applies to the next command with no publishing step.\n\nUser scope is evaluated before project scope; within a scope, sources apply in `rules` array order. A duplicate active rulebook name keeps the first claim and ignores the later rulebook with a warning, so a user-scoped name shadows a project-scoped one.\n\nUse `cc-safety-net rule init` to create an inert local config. Use `--global` for user scope. Use `cc-safety-net rule init --example` to also create an inactive example rulebook. `CC_SAFETY_NET_HOME` overrides the `~/.cc-safety-net` user root.\n\nLegacy inline `.safety-net.json` and `~/.cc-safety-net/config.json` files are not loaded at runtime. Convert them with `cc-safety-net rule migrate`.\n\n## rule.json Schema\n\n```json\n{\n  \"version\": 1,\n  \"rules\": [\"project-rules\", \"owner/repo#main/team-rules\"],\n  \"overrides\": {\n    \"project-rules/block-docker-system-prune\": {\n      \"reason\": \"Use targeted Docker cleanup commands.\"\n    },\n    \"team-rules/block-npm-global\": \"off\"\n  },\n  \"transparent_wrappers\": [\"rtk\"]\n}\n```\n\n- `version`: Required. Must be `1`.\n- `$schema`: Optional. `cc-safety-net rule verify` inserts it into a valid `rule.json` that lacks it.\n- `rules`: Optional array of rulebook source strings. Missing `rules` is treated as `[]`.\n- `overrides`: Optional object keyed by `<rulebook-name>/<rule-name>`.\n- `overrides` values are either `\"off\"` to disable a rule or an object with a required `reason` (replacement block reason) and an optional `intent` (one of `hard_stop`, `use_alternative`, `scope_down`, `manual_only`, `stop_and_explain`).\n- A project override cannot target a user-scoped rule: only that override is ignored, the user rule keeps its configured state, and `rule verify` reports the diagnostic as a failure.\n- `transparent_wrappers`: Optional array of command names that transparently execute a visible child command.\n- Transparent wrappers have no built-in defaults. Configure only wrappers you intentionally trust, such as `\"rtk\"`.\n- Use `cc-safety-net rule wrapper add rtk` to configure RTK without manually editing `rule.json`.\n\n## Rulebook Sources\n\n- Local sources are bare rulebook names such as `project-rules`; the rulebook file is `.cc-safety-net/rules/project-rules/rulebook.json`.\n- Run `cc-safety-net rule add owner/repo` to add every rulebook currently present on the repository's default branch.\n- Use `--only` to select one or more rulebooks while preserving their order: `cc-safety-net rule add owner/repo --only aws gcloud`.\n- Use `--ref` to select a branch, tag, or commit instead of the default branch: `cc-safety-net rule add owner/repo --ref v2 --only aws`.\n- GitHub sources are stored in canonical form as `owner/repo#ref/<rulebook-name>`. That form remains valid in `rule.json` and as direct CLI input.\n- GitHub refs may contain `/`-separated path segments, such as `feature/rulebook-v2`.\n- The GitHub source name, the repository directory name, and the rulebook `name` must match exactly.\n- Rulebook source strings must be unique in a config.\n\n## rulebook.json Schema\n\n```json\n{\n  \"rulebook_version\": 1,\n  \"name\": \"project-rules\",\n  \"version\": \"1.0.0\",\n  \"description\": \"Project-specific CC Safety Net rules.\",\n  \"author\": \"project\",\n  \"allowed_commands\": [\"docker\"],\n  \"rules\": [\n    {\n      \"name\": \"block-docker-system-prune\",\n      \"command\": \"docker\",\n      \"subcommand\": \"system\",\n      \"block_args\": [\"prune\"],\n      \"reason\": \"Use targeted cleanup instead.\"\n    }\n  ],\n  \"tests\": [\n    {\n      \"command\": \"docker system prune\",\n      \"expect\": \"blocked\",\n      \"rule\": \"block-docker-system-prune\"\n    },\n    {\n      \"command\": \"docker ps\",\n      \"expect\": \"allowed\"\n    }\n  ]\n}\n```\n\n### Rulebook Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `rulebook_version` | Yes | Must be `1` or `2` |\n| `name` | Yes | `^[a-zA-Z][a-zA-Z0-9_-]{0,63}$` |\n| `version` | Yes | Non-empty string |\n| `description` | No | Free text; not type-checked at runtime |\n| `author` | No | Free text; not type-checked at runtime |\n| `allowed_commands` | Yes | Unique command names matching `^[a-zA-Z][a-zA-Z0-9_-]*$` |\n| `rules` | Yes | Array of rule objects |\n| `tests` | No | Array of fixtures |\n\n### Rule Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `name` | Yes | Unique within the rulebook (case-insensitive); same pattern as rulebook `name` |\n| `command` | Yes | Must be listed in `allowed_commands`; basename only, not path |\n| `subcommand` | No | Same pattern as `command`; omit to match any subcommand |\n| `intent` | No | One of `hard_stop`, `use_alternative`, `scope_down`, `manual_only`, `stop_and_explain` |\n| `block_args` | Yes | Non-empty array of non-empty strings |\n| `reason` | Yes | Non-empty string, max 256 chars |\n\n### Rule Fields (`rulebook_version` 2)\n\nVersion 2 replaces `subcommand` and `block_args` with an exact-token `match` object. Version 1 rulebooks keep their fields and their behavior; a client that does not support version 2 rejects the rulebook instead of applying broader version 1 semantics.\n\n```json\n{\n  \"name\": \"block-terraform-apply-destroy\",\n  \"command\": \"terraform\",\n  \"match\": {\n    \"command_path\": [\"apply\"],\n    \"any_args\": [\"-destroy\", \"--destroy\"]\n  },\n  \"reason\": \"Review a destroy plan first with 'terraform plan -destroy'.\",\n  \"intent\": \"use_alternative\"\n}\n```\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `name` | Yes | Same as version 1 |\n| `command` | Yes | Same as version 1 |\n| `match.command_path` | Yes | Non-empty array of non-empty command words |\n| `match.any_args` | No | Non-empty array of unique non-empty argument tokens |\n| `match.exclude_args` | No | Non-empty array of unique non-empty argument tokens |\n| `intent` | No | Same as version 1 |\n| `reason` | Yes | Same as version 1 |\n\n### Matching Behavior (`rulebook_version` 2)\n\n- **Command**: Normalized to lowercase basename, as in version 1.\n- **Command path**: After recognized global options and their values are skipped, the next command words must equal `command_path` exactly. AWS, gcloud, and Azure CLI value-taking global options are built in; Terraform's `-chdir=dir` is `=`-joined and is skipped with its own token.\n- **Unrecognized options**: A token starting with `-` that is not a recognized global option is skipped without consuming a value, so an unlisted value-taking option with a separate value (`--newflag value`) makes the rule miss. This fails open deliberately; document such gaps in the rulebook.\n- **`any_args`**: At least one listed token must appear literally among the arguments.\n- **`exclude_args`**: Any listed token appearing literally among the arguments prevents the match, which is how a safe preview such as `aws s3 rm --dryrun` stays allowed.\n- **No short-option expansion**: Arguments compare as exact tokens, so list every accepted spelling (`\"-destroy\"` and `\"--destroy\"`).\n- **Literal and case-sensitive**: No regex, glob, or substring matching. The first matching rule wins.\n- Release channels are separate rules: `gcloud beta compute instances delete` needs its own `command_path`.\n\n### Test Fixture Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `command` | Yes | Non-empty shell command string |\n| `expect` | Yes | `\"blocked\"` or `\"allowed\"` |\n| `rule` | Required for blocked fixtures | Rule name expected to block the command |\n\nFixtures are optional documentation of intended behavior. Version 1 fixtures are shape-validated only. Version 2 fixtures are evaluated against the rulebook's own rules when a source is fetched by `rule add` or `rule update`, and by `rule verify`; a failing fixture rejects that source before it is written. Loading a rulebook does not re-evaluate fixtures. CC Safety Net never executes fixture commands; they are analyzer inputs only.\n\n## Matching Behavior\n\nThe subcommand, argument, and option rules below describe `rulebook_version` 1 rules; version 2 rules match as described in Matching Behavior (`rulebook_version` 2). Execution order and transparent wrappers apply to both.\n\n- **Command**: Normalized to lowercase basename with any trailing `.exe` removed (`/usr/bin/git` → `git`).\n- **Subcommand**: The first command token after recognized Git and Docker global options and their values; `--` ends option parsing. An unrecognized option without `=` may consume the following token as its value.\n- **Arguments**: Each `block_args` value is compared literally against every command token, including expanded short options. The command is blocked if **any** item matches.\n- **Short options**: Expanded (`-Ap` matches `-A`).\n- **Long options**: Exact match (`--all-files` does not match `--all`).\n- **Execution order**: Built-in rules first, then custom rulebooks. Custom rules only add restrictions.\n- **Transparent wrappers**: A configured wrapper such as `rtk` lets `rtk git commit` be analyzed as `git commit` only when `git` is protected by built-in analyzers or active custom rules. `rtk -- git commit` is also supported.\n\n## Workflow\n\n1. Run `cc-safety-net rule init` or create `rule.json` manually.\n2. Optionally run `cc-safety-net rule init --example` to create an inactive example rulebook.\n3. Use `cc-safety-net rule wrapper add rtk` for trusted transparent wrappers.\n4. Run `cc-safety-net rule add <source>` after creating or choosing a rulebook source; add `--only <rulebook...>` or `--ref <ref>` for repository selection. The command adds the selected sources and syncs them.\n5. Edit a local rulebook whenever you like: the edit is enforced on the next command, so there is nothing to run afterwards.\n6. Run `cc-safety-net rule update [source]` to re-fetch remote sources and rewrite the vendored copies; the command prints what changed. A source with an ordinary update failure keeps its vendored copy while the other selected sources still update. Resource-limit failures remain fatal for the whole update.\n7. Run `cc-safety-net rule verify` to validate config, local rulebooks, and shareable GitHub-source rulebook directories in the current repository (it does not fetch remote content).\n8. Run `cc-safety-net rule list` to inspect active rulebooks and transparent wrappers.\n\nA missing or invalid rulebook file makes that source inactive, and an unreadable or invalid `rule.json` makes every source in its scope inactive. Inactive sources stop applying their rules while other custom rules and all built-in protections stay active. Fix the file named in the diagnostic, or run `cc-safety-net rule update` when a remote source has not been vendored yet. Run `cc-safety-net status` to see degraded sources.\n";function Qr(j,K){if(!j.ok){ad(j);return}id(j,K)}function od(j,K,te){if(j.ok)console.log(te);if(!j.add){Qr(j,`Added rulebook source: ${K}`);return}if(!j.ok){ad(j);return}if(j.add.added.length>0)console.log(`Added ${j.add.added.length} ${j.add.added.length===1?"rulebook":"rulebooks"} from ${j.add.source} at ${j.add.ref}:`),j.add.added.forEach((re)=>{console.log(`  - ${re}`)});if(j.add.alreadyConfigured.length>0)console.log(`Rulebooks already configured from ${j.add.source} at ${j.add.ref}: ${j.add.alreadyConfigured.join(", ")}`);if(j.add.commits.length>0)console.log(`Vendored at ${j.add.commits.map((re)=>re.slice(0,7)).join(", ")}.`);id(j,"Rule config updated.")}function id(j,K){for(let te of j.changes??[])console.log(te);console.log(K),console.log(""),Eg(j.entries)}function Eg(j){if(j.length===0){console.log("Active rulebooks: (none)");return}console.log(`Active rulebooks (${j.length}):`);for(let K of j)console.log(`  - ${K.name} ${K.version} (${Dg(K.ruleCount)})`),console.log(`    Source: ${K.spec}`)}function Dg(j){return`${j} ${j===1?"rule":"rules"}`}function sd(j){bn("Active sources",j.rulebooks,(K)=>[`[${K.source}] ${K.name} ${K.version}`,`  Source: ${K.spec}`]),bn("Active rules",j.rules,(K)=>[`[${_g(j,K.name)}] ${K.name}`,...Ag(K),`  Reason: ${K.reason}`]),bn("Disabled rules",rd(j,"off"),(K)=>[K.key]),bn("Reason overrides",rd(j,"reason"),(K)=>[K.key,`  Reason: ${K.value.reason}`]),bn("Transparent wrappers",j.transparent_wrappers,(K)=>[K]),bn("Issues",j.errors,(K)=>[K]),bn("Warnings",j.warnings,(K)=>[K])}function bn(j,K,te){if(K.length===0){console.log(`${j}: (none)`);return}console.log(`${j} (${K.length}):`);for(let re of K){let[oe,...ce]=te(re);console.log(`  - ${oe}`);for(let de of ce)console.log(`    ${de}`)}}function Ag(j){if(!j.match)return[`  Command: ${j.subcommand?`${j.command} ${j.subcommand}`:j.command}`,`  Block args: ${j.block_args.join(", ")}`];return[`  Command: ${[j.command,...j.match.command_path].join(" ")}`,...j.match.any_args?[`  Any args: ${j.match.any_args.join(", ")}`]:[],...j.match.exclude_args?[`  Exclude args: ${j.match.exclude_args.join(", ")}`]:[]]}function _g(j,K){return j.rulebooks.find((te)=>te.rules.includes(K))?.source??"project"}function rd(j,K){return Object.entries({...j.userConfig?.overrides??{},...j.projectConfig?.overrides??{}}).filter((te)=>{if(K==="off")return te[1]==="off";return!!te[1]&&typeof te[1]==="object"}).map(([te,re])=>({key:te,value:re}))}function ad(j){for(let K of j.errors)console.error(K)}import{dirname as Ad,join as ao}from"node:path";import{join as Fi,resolve as Bg}from"node:path";function Ti(j){let K=m(j);if(K.errors.length>0)return{ok:!1,result:{ok:!1,errors:K.errors,entries:[]}};return{ok:!0,config:K.config??ut}}function ld(j,K=[]){Tt(j,{version:1,rules:K,overrides:{},transparent_wrappers:[]})}function cd(j,K="project-rules"){Tt(j,{rulebook_version:1,name:K,version:"1.0.0",description:K==="project-rules"?"Project-specific CC Safety Net rules.":"User-specific CC Safety Net rules.",author:K==="project-rules"?"project":"user",allowed_commands:["docker"],rules:[{name:"block-docker-system-prune",command:"docker",subcommand:"system",block_args:["prune"],reason:"Use targeted cleanup instead."}],tests:[{command:"docker system prune",expect:"blocked",rule:"block-docker-system-prune"}]})}import{dirname as ro}from"node:path";var Tg="custom.";function eo(j){if(j.rulebook_version!==2)return[];let K=j.rules.map((te)=>({name:te.name,command:te.command,block_args:[],match:te.match,reason:te.reason,intent:te.intent}));return(j.tests??[]).flatMap((te,re)=>{let oe=Ii(v(te.command));if(oe.length===0)return[`tests[${re}]: could not parse fixture command: ${te.command}`];let ce=oe.reduce((de,we)=>de??E(we,K)?.id.slice(Tg.length),void 0);if(te.expect==="blocked"){if(ce===te.rule)return[];let de=ce?`"${ce}" matched first`:"no rule matched";return[`tests[${re}]: expected "${te.rule}" to block "${te.command}" but ${de}`]}return ce?[`tests[${re}]: expected "${te.command}" to be allowed but "${ce}" matched`]:[]})}function Ii(j){return j.nodes.flatMap((K)=>{if(K.kind==="group"||K.kind==="function")return Ii(K.body);if(K.kind!=="command")return[];let te=be(ee(K.dialect,K.words)).words.map(t);return[...te.length>0?[te]:[],...K.nested.flatMap((re)=>Ii(re))]})}var $i="Rule synchronization exceeds CC Safety Net's safe resource limits.",to=Object.freeze({maxSources:J,concurrency:4,maxRequests:131,maxResponseBytes:67108864});function no(j={}){return{requests:0,responseBytes:0,maxRequests:j.maxRequests??to.maxRequests,maxResponseBytes:j.maxResponseBytes??to.maxResponseBytes}}function Qt(j){return{controller:new AbortController,budget:no(),resolveUrl:j}}function dd(j){return j instanceof Error&&j.message===$i}function ud(j){if(j.requests>=j.maxRequests)throw Error($i);j.requests++}function pd(j,K){if(K>j.maxResponseBytes-j.responseBytes)throw j.responseBytes+=K,Error($i);j.responseBytes+=K}var gd=Object.freeze({timeoutMs:15000,metadataBytes:524288,commitBytes:262144,treeBytes:16777216,rawBytes:4194304});async function fd(j,K,te=S(ro(ro(K)),"rules policy"),re=Qt()){if(x(j))return jg(j,re);return Og(j,K,te)}async function yd(j,K,te,re,oe,ce){if(!x(j))return fd(j,K,te,re);let de=oe?null:Ig(j,K,te);if(de)return de;if(!oe&&!ce)throw Error(`${j} is not vendored; run rule update ${j} to vendor it`);return fd(j,K,te,re)}function Ig(j,K,te=S(ro(ro(K)),"rules policy")){let re=O(j),oe=N(K,re.name),ce=r(s(te,oe));if(ce===null)return null;let de=ae(Oi(ce,`Invalid rulebook ${oe}.`));if(de.name!==re.name)throw Error(`rulebook name "${de.name}" in ${oe} must match "${re.name}"`);return{spec:j,rulebook:de,content:ce}}async function hd(j,K={}){if(!Y(j))throw Error(`Invalid GitHub repository source: ${j}`);let[te,re]=j.split("/");if(!te||!re)throw Error(`Invalid GitHub repository source: ${j}`);if(K.ref!==void 0&&!ie(K.ref))throw Error(`GitHub rulebook refs must use valid path segments: ${K.ref}`);let oe=K.operation??Qt(),ce=K.ref??await $g(te,re,j,oe),de=await bd(te,re,ce,j,oe),we=await oo(`https://api.github.com/repos/${te}/${re}/git/trees/${de}?recursive=1`,"tree",oe),xe=we.response;if(!xe.ok)throw Error(`Failed to inspect ${j}: GitHub tree returned ${xe.status}`);let Ce=JSON.parse(we.content);if(!Array.isArray(Ce?.tree))throw Error(`Failed to inspect ${j}: unexpected GitHub tree response`);let Se=Ce.tree,Oe=[...new Set(Se.flatMap((Ge)=>{if(!Ge||typeof Ge!=="object")return[];let st=Ge;if(st.type!=="blob"||typeof st.path!=="string")return[];let ft=st.path.match(ct);return ft?.[1]?[ft[1]]:[]}))].sort();if(Oe.length===0)throw Error(`No rulebooks found in ${j} under ${fe}/`);return{source:j,owner:te,repo:re,ref:ce,commit:de,names:Oe}}async function $g(j,K,te,re){let oe=await oo(`https://api.github.com/repos/${j}/${K}`,"metadata",re),ce=oe.response;if(!ce.ok)throw Error(`Failed to inspect ${te}: GitHub returned ${ce.status}`);let we=JSON.parse(oe.content)?.default_branch;if(typeof we!=="string"||we==="")throw Error(`Failed to inspect ${te}: missing default branch`);if(!ie(we))throw Error(`GitHub returned an invalid default branch: ${we}`);return we}function Og(j,K,te){dt(j);let re=N(K,j),oe=r(s(te,re));if(oe===null)throw Error(`Rulebook source not found: ${j}`);let ce=vd(Oi(oe,"Invalid local rulebook source."));if(ce.name!==j)throw Error(`rulebook name "${ce.name}" must match local source "${j}"`);return{spec:j,rulebook:ce,content:oe}}async function jg(j,K){let te=O(j),re=await bd(te.owner,te.repo,te.ref,j,K),oe=await oo(`https://raw.githubusercontent.com/${te.owner}/${te.repo}/${re}/${te.path}`,"raw",K),ce=oe.response;if(!ce.ok)throw Error(`Failed to fetch ${j}: GitHub raw returned ${ce.status}`);let de=oe.content,we=vd(Oi(de,"Invalid GitHub rulebook response."));if(we.name!==te.name)throw Error(`rulebook name "${we.name}" must match GitHub source "${te.name}"`);return{spec:j,rulebook:we,content:de}}function vd(j){let K=ae(j),te=eo(K);if(te.length>0)throw Error(te.join("; "));return K}function Oi(j,K){try{return JSON.parse(j)}catch{throw Error(K)}}async function bd(j,K,te,re,oe){let ce=await oo(`https://api.github.com/repos/${j}/${K}/commits/${encodeURIComponent(te)}`,"commit",oe),de=ce.response;if(!de.ok)throw Error(`Failed to resolve ${re}: GitHub returned ${de.status}`);let we=JSON.parse(ce.content);if(typeof we?.sha!=="string"||we.sha==="")throw Error(`Failed to resolve commit for ${re}`);return we.sha}async function Fg(j,K,te={}){if(te.signal?.aborted)throw te.signal.reason;let re=te.budget??no(),oe=new AbortController,ce=()=>oe.abort(te.signal?.reason);te.signal?.addEventListener("abort",ce,{once:!0});let de=!1,we=setTimeout(()=>{if(oe.signal.aborted)return;de=!0,oe.abort()},te.timeoutMs??gd.timeoutMs);try{if(te.signal?.aborted)throw te.signal.reason;ud(re);let xe=await(te.fetch??fetch)(j,{signal:oe.signal,redirect:"error"});if(!xe.ok)return Ld(xe),{response:xe,content:""};return{response:xe,content:await Ng(xe,K,re,()=>oe.abort())}}catch(xe){if(de)throw Error("GitHub request timed out",{cause:xe});if(te.signal?.aborted)throw te.signal.reason;throw xe}finally{clearTimeout(we),te.signal?.removeEventListener("abort",ce)}}function oo(j,K,te){return Fg(te.resolveUrl?.(j)??j,K,{budget:te.budget,signal:te.controller.signal})}async function Ng(j,K,te=no(),re){let oe=gd[`${K}Bytes`],ce=Number(j.headers.get("content-length"));if(Number.isFinite(ce)&&ce>oe)throw Ld(j),Error(`GitHub ${K} response exceeds ${oe} bytes`);if(!j.body)return"";let de=j.body.getReader(),we=[],xe=0;while(!0){let Ce=await de.read();if(Ce.done)break;try{pd(te,Ce.value.byteLength)}catch(Se){throw re?.(),md(de),Se}if(xe+=Ce.value.byteLength,xe>oe)throw re?.(),md(de),Error(`GitHub ${K} response exceeds ${oe} bytes`);we.push(Buffer.from(Ce.value))}return Buffer.concat(we,xe).toString("utf-8")}function Ld(j){if(!j.body)return;wd(()=>j.body?.cancel())}function md(j){wd(()=>j.cancel())}function wd(j){try{Promise.resolve(j()).catch(()=>{})}catch{}}var Mg=/^([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+)#(.+)$/;function kd(j,K){let te=Sd(j.rules,K);if(te.length>0)return{ok:!0,specs:te};return Cd(j.rules,K)}function xd(j,K){let te=Sd(j,K);if(te.length>0)return{ok:!0,specs:te};let re=Ug(j,K);if(re.length>0)return{ok:!0,specs:re};let oe=Gg(j,K);if(!oe.ok)return oe;if(oe.specs.length>0)return{ok:!0,specs:oe.specs};return Cd(j,K)}function Cd(j,K){let te=j.filter((re)=>ji(re)?.name===K);if(te.length===1)return{ok:!0,specs:te};return Hg(K,te)}function Hg(j,K){return{ok:!1,result:{ok:!1,errors:K.length===0?[`No configured rulebook matches ${j}`]:[`Ambiguous rulebook match ${j}: ${K.join(", ")}`],entries:[]}}}function Sd(j,K){return j.filter((te)=>te===K)}function Ug(j,K){let te=K.match(Mg),re=te?.[1],oe=te?.[2],ce=te?.[3];if(!re||!oe||!ce||!ie(ce))return[];return Rd(j,(de)=>de.owner===re&&de.repo===oe&&de.ref===ce)}function Gg(j,K){if(!Y(K))return{ok:!0,specs:[]};let[te,re]=K.split("/"),oe=Rd(j,(de)=>de.owner===te&&de.repo===re);if(new Set(oe.map((de)=>ji(de)?.ref).filter((de)=>!!de)).size<2)return{ok:!0,specs:oe};return{ok:!1,result:{ok:!1,errors:[`Multiple refs are configured for ${K}. Use an explicit ref:`,`  cc-safety-net rule remove ${K}#<ref>`],entries:[]}}}function ji(j){try{return O(j)}catch{return null}}function Rd(j,K){return j.filter((te)=>{let re=ji(te);return re?K(re):!1})}async function so(j,K={}){let te=Ni(K);return qg(j,te,await io(j,te,Qt()))}function qg(j,K,te){if(!te.ok)return te;let re=It(j,K),oe=[...new Set(W(re.configPath,re.filesystemScope))];if(oe.length===0)return te;return{ok:!1,errors:oe,entries:te.entries}}async function io(j,K,te,re={},oe=new Set,ce=new Set){try{let de=It(j,K),we=Ti(de.configTarget);if(!we.ok)return we.result;let xe=we.config,Ce=K.only?kd(xe,K.only):{ok:!0,specs:xe.rules};if(!Ce.ok)return Ce.result;let Se=new Set([...K.refresh?Ce.specs:[],...oe]),Oe=(bt)=>yd(bt,de.configDir,de.filesystemScope,te,Se.has(bt),!K.refresh||Se.has(bt)),Ge=await ty(xe.rules,K.refresh?(bt)=>Oe(bt).then((xt)=>({ok:!0,item:xt})).catch((xt)=>{if(dd(xt))throw xt;return{ok:!1,spec:bt,message:xt instanceof Error?xt.message:String(xt)}}):async(bt)=>({ok:!0,item:await Oe(bt)}),te),st=Ge.filter((bt)=>!bt.ok),ft=Ge.filter((bt)=>bt.ok).map((bt)=>bt.item),ot=ft.flatMap((bt)=>Vg(bt,xe.rules)),mt=ft.flatMap((bt)=>zg(bt,ce,de)),yt=new Set([...ot,...mt].map((bt)=>bt.spec)),ht=[...st,...ot,...mt],gt=[],wt=Wg(gt,()=>ft.flatMap((bt)=>yt.has(bt.spec)||ht.length>0&&ce.has(bt.spec)?[]:Jg(bt,de,re,gt)));return{ok:ht.length===0,errors:ht.map((bt)=>`Failed to update ${bt.spec}: ${bt.message}`),entries:ft.map(Yg),changes:wt}}catch(de){return ar(de)}}function Vg(j,K){if(!x(j.spec))return[];let te=Fe(j.spec),re=K.filter((oe)=>oe!==j.spec&&Fe(oe).toLowerCase()===te.toLowerCase());if(re.length===0)return[];return[{ok:!1,spec:j.spec,message:`rulebook name "${te}" is also claimed by ${re.join(", ")}; rename one of them`}]}function zg(j,K,te){if(!K.has(j.spec)||!x(j.spec))return[];let re=N(te.configDir,j.rulebook.name),oe=r(s(te.filesystemScope,re));if(oe===null||oe===j.content)return[];return[{ok:!1,spec:j.spec,message:`${re} already exists and no configured source claims it; remove or rename the file, then re-run rule add`}]}function Jg(j,K,te,re){if(!x(j.spec))return[];let oe=N(K.configDir,j.rulebook.name),ce=s(K.filesystemScope,oe),de=r(ce);if(de===j.content)return[];return re?.push({target:ce,previous:de}),y(ce,j.content,void 0,te._testAfterPolicyRename),Kg(j,de)}function Wg(j,K){try{return K()}catch(te){for(let re of[...j].reverse()){if(re.previous===null){G(re.target);continue}y(re.target,re.previous)}throw te}}function Kg(j,K){if(K===null)return[`Vendored ${j.spec} (${j.rulebook.version})`];let te=he(K),re="problem"in te?null:te.rulebook,oe=new Map(re?.rules.map((de)=>[de.name,JSON.stringify(de)])??[]),ce=new Set(j.rulebook.rules.map((de)=>de.name));return[`Updated ${j.spec} (${re?.version??"unreadable"} -> ${j.rulebook.version})`,...[...ce].filter((de)=>!oe.has(de)).map((de)=>`  + ${de}`),...[...oe.keys()].filter((de)=>!ce.has(de)).map((de)=>`  - ${de}`),...j.rulebook.rules.filter((de)=>{let we=oe.get(de.name);return we!==void 0&&we!==JSON.stringify(de)}).map((de)=>`  ~ ${de.name}`)]}function Yg(j){return{spec:j.spec,name:j.rulebook.name,version:j.rulebook.version,ruleCount:j.rulebook.rules.length}}async function Pd(j,K,te={}){return Zg(j,K,ry(te),Qt())}async function Zg(j,K,te,re,oe={}){let ce=null,de=!1;try{let we=It(j,te),xe=r(we.configTarget);ce={target:we.configTarget,content:xe};let Ce=Ti(we.configTarget);if(!Ce.ok)return Ce.result;let Se=Ce.config,Oe=Y(K);Xg(K,te,Oe);let Ge=Oe?await hd(K,{ref:te.ref,operation:re}):null,st=Ge?Qg(Ge,te.rulebooks):[],ft=Ge?st.map((gt)=>ey(Se.rules,Ge,gt)??`${K}#${Ge.ref}/${gt}`):[K],ot=ft.filter((gt)=>!Se.rules.includes(gt)),mt=[...Se.rules,...ot];if(mt.length>J)return ny();if(mt.length!==Se.rules.length)de=!0,Tt(we.configTarget,{version:1,rules:mt,overrides:Se.overrides??{},transparent_wrappers:Se.transparent_wrappers??[]},void 0,oe._testAfterPolicyRename);let yt=await io(j,te,re,oe,new Set(ot),new Set(ot));if(!yt.ok)sr(we.configTarget,xe);if(!yt.ok||!Ge)return yt;let ht=st.filter((gt,wt)=>ot.includes(ft[wt]??""));return{...yt,add:{source:K,ref:Ge.ref,selected:st,added:ht,alreadyConfigured:st.filter((gt)=>!ht.includes(gt)),commits:ot.length>0?[Ge.commit]:[]}}}catch(we){if(de&&ce)try{sr(ce.target,ce.content)}catch(xe){return ar(xe)}return ar(we)}}function Xg(j,K,te){if(!te&&K.rulebooks!==void 0)throw Error("--only can only select rulebooks from an owner/repo source");if(!te&&K.ref)throw Error(`--ref can only select a ref for an owner/repo source: ${j}`);if(K.rulebooks?.length===0)throw Error("--only requires at least one rulebook name");let re=K.rulebooks?.filter((oe)=>!c.test(oe))??[];if(re.length>0)throw Error(`Invalid rulebook names: ${re.join(", ")}`)}function Qg(j,K){let te=K?[...new Set(K)]:j.names,re=te.filter((oe)=>!j.names.includes(oe));if(re.length>0)throw Error(`Rulebooks not found in ${j.source} at ${j.ref}: ${re.join(", ")}
Available rulebooks: ${j.names.join(", ")}`);return te}function ey(j,K,te){let re=`${K.source}#${K.ref}/${te}`;if(j.includes(re))return re;let oe=`${K.source}#${K.commit}/${te}`;return j.find((ce)=>ce===oe)}async function ty(j,K,te=Qt()){if(j.length>J)throw Error(ge);let re=Array(j.length),oe=0,ce,de=Array.from({length:Math.min(j.length,to.concurrency)},async()=>{while(!ce){let we=oe;if(we>=j.length)return;oe++;try{re[we]=await K(j[we],we,te.controller.signal)}catch(xe){if(!ce)ce={value:xe},oe=j.length,te.controller.abort(xe);return}}});if(await Promise.all(de),ce)throw ce.value;return re}function ny(){return{ok:!1,errors:[ge],entries:[]}}function Ni(j){return{cwd:j.cwd,userConfigDir:j.userConfigDir,userConfigPath:j.userConfigPath,projectConfigPath:j.projectConfigPath,global:j.global,only:j.only,refresh:j.refresh}}function ry(j){return{...Ni(j),ref:j.ref,rulebooks:j.rulebooks}}function oy(j){return{...Ni(j),deleteSource:j.deleteSource}}async function Ed(j,K,te={}){try{return await iy(j,K,oy(te),{})}catch(re){return ar(re)}}async function iy(j,K,te,re){let oe=It(j,te),ce=m(oe.configTarget);if(ce.errors.length>0)return{ok:!1,errors:ce.errors,entries:[]};if(!ce.config)return{ok:!1,errors:[`No config found at ${oe.configPath}`],entries:[]};let de=xd(ce.config.rules,K);if(!de.ok)return de.result;let we=te.deleteSource?sy(oe.configDir,de.specs,oe.filesystemScope):{ok:!0,dirs:[]};if(!we.ok)return we.result;let xe=r(oe.configTarget);if(xe===null)return ar(Error("Rules config is unavailable."));try{Tt(oe.configTarget,{version:1,rules:ce.config.rules.filter((Oe)=>!de.specs.includes(Oe)),overrides:ce.config.overrides??{},transparent_wrappers:ce.config.transparent_wrappers??[]},void 0,re._testAfterPolicyRename)}catch(Oe){throw sr(oe.configTarget,xe),Oe}let Ce=await io(j,te,Qt(),re);if(!Ce.ok)return sr(oe.configTarget,xe),Ce;let Se=ay(we.dirs,re,oe.filesystemScope);if(!Se.ok){sr(oe.configTarget,xe);let Oe=await io(j,te,Qt(),re);if(!Oe.ok)return{ok:!1,errors:[...Se.result.errors,...Oe.errors],entries:Oe.entries};return Se.result}return Ce}function sy(j,K,te){let re=K.flatMap((we)=>c.test(we)?[]:["--delete-source can only delete local rulebook sources"]),oe=K.map((we)=>Fi(j,we)),ce=re.length>0?[]:oe.flatMap((we)=>Dd(we,te)),de=[...re,...ce];return de.length>0?{ok:!1,result:{ok:!1,errors:de,entries:[]}}:{ok:!0,dirs:oe}}function Dd(j,K){let te=Bg(j),re=s(K,te),oe=ue(re);if(!oe)return[`Local rulebook source directory not found: ${j}`];let ce=oe.find((de)=>de.name==="rulebook.json");if(!ce)return[`Local rulebook source directory is missing rulebook.json: ${j}`];if(ce.kind!=="file")throw new o(K.label);if(r(s(K,Fi(te,"rulebook.json"))),oe.length>1)return[`Local rulebook source directory contains extra files: ${j}. delete manually if you really want to remove the directory.`];return[]}function ay(j,K,te){let re=j.flatMap((oe)=>{try{if(!ue(s(te,oe)))return[];let ce=Dd(oe,te);if(ce.length>0)return ce;return ly(oe,K,te),[]}catch(ce){return[`Failed to delete local rulebook source ${oe}: ${ce instanceof Error?ce.message:String(ce)}`]}});return re.length>0?{ok:!1,result:{ok:!1,errors:re,entries:[]}}:{ok:!0}}function ly(j,K,te){if(K._testDeleteLocalSourceDir){K._testDeleteLocalSourceDir(j);return}G(s(te,Fi(j,pe))),lt(s(te,j))}function sr(j,K){if(K===null){G(j);return}y(j,K)}function ar(j){return{ok:!1,errors:[j instanceof Error?j.message:String(j)],entries:[]}}var cy=".safety-net.json",dy="~/.cc-safety-net/config.json";async function Id(j,K){return[await _d(j,{legacyPath:Js({cwd:K.cwd}),configPath:A(K.cwd),defaultRulebookName:"project-rules",migratedFrom:cy,cleanup:K.cleanup,syncOptions:{cwd:K.cwd}}),await _d(j,{legacyPath:gr(j),configPath:T(j),defaultRulebookName:"user-rules",migratedFrom:dy,cleanup:K.cleanup,syncOptions:{cwd:K.cwd,global:!0}})].every((re)=>re)?0:1}async function _d(j,K){let te=It(j,K.syncOptions),re=s(te.filesystemScope,K.legacyPath),oe=r(re);if(oe===null)return console.log(`No legacy config found at ${K.legacyPath}`),!0;let ce=py(oe);if(!ce.ok){for(let st of ce.errors)console.error(st);return!1}let de=m(te.configTarget);if(de.errors.length>0){for(let st of de.errors)console.error(st);return!1}let we=de.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},xe=fy(Ad(K.configPath),we.rules,K.defaultRulebookName,K.migratedFrom,te.filesystemScope),Ce=ao(Ad(K.configPath),xe,"rulebook.json"),Se=s(te.filesystemScope,Ce),Oe=[Td(te.configTarget),Td(Se)],Ge=await uy(j,K,te.configTarget,Se,xe,ce.config.rules,we.rules.includes(xe)?we.rules:[...we.rules,xe],we.overrides??{},we.transparent_wrappers??[]);if(!Ge.ok){yy(Oe);for(let st of Ge.errors)console.error(st);return!1}if(!K.cleanup)return console.log(`Migrated legacy config at ${K.legacyPath}. Legacy file is no longer used.`),!0;if(!gy(te.configTarget,Se,xe,K.migratedFrom,ce.config.rules))return console.error(`Migration cleanup verification failed for ${K.legacyPath}`),!1;return G(re),console.log(`Deleted legacy config at ${K.legacyPath}`),!0}async function uy(j,K,te,re,oe,ce,de,we,xe){try{return Tt(te,{version:1,rules:de,overrides:we,transparent_wrappers:xe}),Tt(re,my(oe,K.migratedFrom,ce)),await so(j,K.syncOptions)}catch(Ce){return{ok:!1,errors:[Ce instanceof Error?Ce.message:String(Ce)]}}}function py(j){try{let K=JSON.parse(j),te=ho(K);if(te.errors.length>0)return{ok:!1,errors:te.errors};return{ok:!0,config:{version:1,rules:K.rules??[]}}}catch{return{ok:!1,errors:["Invalid JSON"]}}}function fy(j,K,te,re,oe){let ce=K.find((de)=>hy(s(oe,ao(j,de,"rulebook.json")))===re);if(ce)return ce;if(r(s(oe,ao(j,te,"rulebook.json")))===null)return te;for(let de=2;;de++){let we=`${te}-${de}`;if(r(s(oe,ao(j,we,"rulebook.json")))===null)return we}}function my(j,K,te){return{rulebook_version:1,name:j,version:"1.0.0",description:"Migrated CC Safety Net rules.",author:"project",migrated_from:K,allowed_commands:[...new Set(te.map((re)=>re.command))],rules:te,tests:te.map((re)=>({command:[re.command,re.subcommand,re.block_args[0]].filter(Boolean).join(" "),expect:"blocked",rule:re.name}))}}function gy(j,K,te,re,oe){if(!m(j).config?.rules.includes(te))return!1;try{let de=r(K);if(de===null)return!1;let we=JSON.parse(de);return we.migrated_from===re&&JSON.stringify(we.rules)===JSON.stringify(oe)}catch{return!1}}function Td(j){return{target:j,content:r(j)}}function yy(j){for(let K of j){if(K.content===null){G(K.target);continue}y(K.target,K.content)}}function hy(j){let K=r(j);if(K===null)return null;try{let te=JSON.parse(K);return typeof te.migrated_from==="string"?te.migrated_from:null}catch{return null}}import{mkdir as vy,readFile as by,writeFile as Ly}from"node:fs/promises";import{dirname as wy,join as ky}from"node:path";var xy=86400000,Cy=604800000;async function Od(j,K=Date.now()){if(j.env.get("CC_SAFETY_NET_NO_UPDATE_CHECK"))return null;let te=qe(j);if(!te)return null;let re=ky(te,".cc-safety-net","update-check.json"),oe=await Sy(re,K);if(!oe.lastCheck||K-oe.lastCheck>xy){let we=await tn();if(oe.lastCheck=K,we.latestVersion)oe.latestVersion=we.latestVersion;if(!await $d(re,oe))return null;if(we.error)return null}let ce=oe.latestVersion,de=Pt();if(!ce||!xo(ce,de))return null;if(oe.notifiedVersion===ce&&oe.notifiedAt!==void 0&&K-oe.notifiedAt<Cy)return null;if(oe.notifiedVersion=ce,oe.notifiedAt=K,!await $d(re,oe))return null;return`UPDATE_AVAILABLE: cc-safety-net v${ce} is available (running v${de}). Ask the user once whether to run \`npx -y cc-safety-net@latest update\`; continue the current task either way and do not raise this again.`}async function Sy(j,K){let te=await by(j,"utf8").then((ce)=>JSON.parse(ce)).catch(()=>{return});if(!te||typeof te!=="object"||Array.isArray(te))return{};let re=te,oe=(ce)=>typeof ce==="number"&&Number.isFinite(ce)&&ce<=K?ce:void 0;return{lastCheck:oe(re.lastCheck),latestVersion:typeof re.latestVersion==="string"?re.latestVersion:void 0,notifiedVersion:typeof re.notifiedVersion==="string"?re.notifiedVersion:void 0,notifiedAt:oe(re.notifiedAt)}}async function $d(j,K){return vy(wy(j),{recursive:!0,mode:448}).then(()=>Ly(j,JSON.stringify(K),{mode:384})).then(()=>!0).catch(()=>!1)}import{join as Ry,resolve as Mi}from"node:path";var jd="CC Safety Net Config",Py="═".repeat(jd.length),Ey="https://raw.githubusercontent.com/kenryu42/cc-safety-net/main/assets/cc-safety-net.schema.json",Dy=new Set(["rule.json","rule.lock","cache"]);function Fd(j,K={}){try{return Ay(j,K)}catch(te){if(te instanceof o)return console.error(te.message),1;throw te}}function Ay(j,K){let te=K.cwd??process.cwd(),re=K.userConfigPath??T(j),oe=K.projectConfigPath??A(te),ce=K.legacyUserConfigPath??gr(j),de=K.legacyProjectConfigPath??ys(te),we=Mi(te,fe),xe=H(j,{cwd:te,userConfigPath:re,projectConfigPath:oe}),Ce=H(j,{cwd:te}),Se=s(xe.userScope,re),Oe=s(xe.projectScope,oe),Ge=K.legacyUserConfigPath?D(K.legacyUserConfigPath,"user policy"):s(Ce.userScope,ce),st=K.legacyProjectConfigPath?D(K.legacyProjectConfigPath,"project policy"):s(Ce.projectScope,de),ft=!1,ot=!1,mt=[],yt=[],ht=_y(s(Ce.projectScope,we));if(Iy(),r(Se)!==null){let gt=en(Se);if(gt.errors.push(...W(re,xe.userScope)),mt.push({scope:"User",path:re,result:gt,schema:"rules",target:Se}),gt.errors.length>0)ft=!0}if(r(Ge)!==null)if(ot=!0,r(Se)!==null)yt.push(lo("user","cleanup"));else{let gt=vo(Ge);if(mt.push({scope:"User",path:ce,result:gt,schema:"legacy",inactive:!0,target:Ge}),yt.push(lo("user",gt.errors.length>0?"fix-or-delete":"migrate")),gt.errors.length>0)ft=!0}if(r(Oe)!==null){let gt=en(Oe);if(gt.errors.push(...W(oe,xe.projectScope)),mt.push({scope:"Project",path:Mi(oe),result:gt,schema:"rules",target:Oe}),gt.errors.length>0)ft=!0;if(r(st)!==null)ot=!0,yt.push(lo("project","cleanup"))}else if(r(st)!==null){ot=!0,ft=!0;let gt=vo(st);mt.push({scope:"Project",path:Mi(de),result:gt,schema:"legacy",inactive:!0,target:st}),yt.push(lo("project",gt.errors.length>0?"fix-or-delete":"migrate"))}if(ht?.result.errors.length)ft=!0;if(mt.length===0&&!ht)return console.log(`
No config files found. Using built-in rules only.`),0;for(let gt of mt)if(gt.inactive)Oy(gt.scope,gt.path,gt.result);else if(gt.result.errors.length>0)jy(gt.scope,gt.path,gt.result.errors);else{if(gt.schema==="rules"&&My(gt.target))console.log(`
Added $schema to ${gt.scope.toLowerCase()} config.`);$y(gt.scope,gt.path,gt.result,gt.schema)}for(let gt of yt)console.error(`
${vt.red(gt)}`);if(ht)if(ht.result.errors.length>0)Ny(ht.path,ht.result.errors);else Fy(ht.path,ht.result);if(ft)return console.error(`
Config validation failed.`),1;return console.log(ot?`
Configs valid with warnings.`:`
All configs valid.`),0}function lo(j,K){let te=`legacy ${j} config`;if(K==="cleanup")return`Warning: Legacy ${j} config is no longer needed. Run \`npx -y cc-safety-net rule migrate --cleanup\` to clean it up safely.`;if(K==="migrate")return`Warning: Legacy ${j} config is ignored by CC Safety Net. Run \`npx -y cc-safety-net rule migrate\`.`;return`Warning: Legacy ${j} config is no longer supported. Fix or delete the ${te}, then run \`npx -y cc-safety-net rule migrate\`.`}function _y(j){if(ue(j)===null)return null;let K=Ty(j);if(K.ruleNames.size===0&&K.errors.length===0)return null;return{path:j.path,result:K}}function Ty(j){let K=[],te=new Set,re=(ue(j)??[]).filter((oe)=>!Dy.has(oe.name)).sort((oe,ce)=>oe.name.localeCompare(ce.name));if(re.length===0)return{errors:K,ruleNames:te};for(let oe of re){if(!c.test(oe.name)){K.push(`rulebook directory names must match ${c}: ${oe.name}`);continue}if(oe.kind!=="directory"){K.push(`${oe.name} must be a rulebook directory`);continue}let ce=s(j.scope,Ry(j.path,oe.name,"rulebook.json")),de=r(ce);if(de===null){K.push(`${oe.name}/rulebook.json is required`);continue}try{let we;try{we=JSON.parse(de)}catch{K.push(`${oe.name}/rulebook.json: invalid JSON`);continue}let xe=ae(we);if(xe.name!==oe.name){K.push(`rulebook name "${xe.name}" must match folder "${oe.name}"`);continue}let Ce=eo(xe);if(Ce.length>0){K.push(...Ce.map((Se)=>`${oe.name}/rulebook.json: ${Se}`));continue}te.add(oe.name)}catch(we){K.push(we instanceof Error?`${oe.name}/rulebook.json: ${we.message}`:`${oe.name}/rulebook.json: ${String(we)}`)}}return{errors:K,ruleNames:te}}function Iy(){console.log(jd),console.log(Py)}function $y(j,K,te,re){if(console.log(`
✓ ${j} config: ${K}`),console.log(`  Schema: ${re==="rules"?"rulebook sources":"legacy inline rules"}`),te.ruleNames.size>0){console.log(`  ${re==="rules"?"Sources":"Rules"}:`);let oe=1;for(let ce of te.ruleNames)console.log(`    ${oe}. ${ce}`),oe++}else console.log(`  ${re==="rules"?"Sources":"Rules"}: (none)`)}function Oy(j,K,te){if(console.error(`
✗ Legacy ${j.toLowerCase()} config: ${K}`),console.error("  Schema: legacy inline rules"),console.error("  Status: ignored by CC Safety Net"),te.errors.length>0){console.error("  Errors:");let re=1;for(let oe of te.errors)for(let ce of oe.split("; "))console.error(`    ${re}. ${ce}`),re++;return}if(te.ruleNames.size>0){console.error("  Rules:");let re=1;for(let oe of te.ruleNames)console.error(`    ${re}. ${oe}`),re++;return}console.error("  Rules: (none)")}function jy(j,K,te){Nd(`${j} config`,K,te)}function Fy(j,K){console.log(`
✓ GitHub source rules: ${j}`),console.log("  Rulebooks:");let te=1;for(let re of K.ruleNames)console.log(`    ${te}. ${re}`),te++}function Ny(j,K){Nd("GitHub source rules",j,K)}function Nd(j,K,te){console.error(`
✗ ${j}: ${K}`),console.error("  Errors:");let re=1;for(let oe of te)for(let ce of oe.split("; "))console.error(`    ${re}. ${ce}`),re++}function My(j){try{let K=r(j);if(K===null)return!1;let te=JSON.parse(K);if(te.$schema)return!1;return y(j,JSON.stringify({$schema:Ey,...te},null,2)),!0}catch(K){if(K instanceof o)throw K;return!1}}var Md=new Set(["init","add","remove","update","sync","list","wrapper","migrate","doc","verify"]),Uy=new Set(["add","remove","list"]),Gy="cc-safety-net/rulebooks";async function Hd(j,K){try{return await By(j,K)}catch(te){if(te instanceof o)return console.error(te.message),1;throw te}}async function By(j,K){let te=Vy(K),re=te.help?qy(te.positionals):null;if(re)return An(re),0;if(te.errors.length>0){for(let we of te.errors)console.error(we);return 1}let oe=te.positionals[0];if(!oe)return An(wn,console.error),1;let ce=te.positionals[1],de={global:te.global};if(oe==="init"){let we=It(j,de);Ky(we.configTarget);let xe=Hy(we.configDir,"example-rules","rulebook.json"),Ce=s(we.filesystemScope,xe);if(te.example&&r(Ce)===null)cd(Ce,"example-rules");let Se=W(we.configPath,we.filesystemScope);for(let Oe of Se)console.error(Oe);if(Se.length>0)return 1;return console.log("Rule config initialized."),0}if(oe==="add"){let we=Ud(te);if(!we)return console.error("rule add requires a source (pass --only <rulebook...> to select from cc-safety-net/rulebooks)"),1;let xe=It(j,de),Ce=await Pd(j,we,{...de,ref:te.ref,rulebooks:te.only.length>0?te.only:void 0});return od(Ce,we,`Scope: ${te.global?"user":"project"} (${xe.configDir})`),Ce.ok?0:1}if(oe==="remove"){if(!ce)return console.error("rule remove requires a source"),1;let we=await Ed(j,ce,{...de,deleteSource:te.deleteSource});return Qr(we,`Removed rulebook source: ${ce}`),we.ok?0:1}if(oe==="update"){let we=await so(j,{...de,only:ce,refresh:!0});return Qr(we,"Rule config updated."),we.ok?0:1}if(oe==="sync")return Ys(j,{global:te.global});if(oe==="list"){let we=Z(j,{cwd:process.cwd()});return sd(we),we.errors.length>0?1:0}if(oe==="wrapper")return Yy(j,te);if(oe==="migrate")return Id(j,{cleanup:te.cleanup,cwd:process.cwd()});if(oe==="doc"){console.log(nd);let we=await Od(j);if(we)console.error(we);return 0}if(oe==="verify")return Fd(j);return 1}function qy(j){if(j.length===0)return wn;let K=wn.subcommands.filter((re)=>re.usage.split(" ")[0]===j[0]);if(K.length===0)return null;if(j.length===1&&K.length>1)return{name:`rule ${j[0]}`,description:`Subcommands of rule ${j[0]}`,usage:`rule ${j[0]} <subcommand>`,subcommands:K,options:[]};let te=j.length===1?K[0]:K.find((re)=>re.usage.split(" ")[1]===j[1]);if(!te)return null;return{name:`rule ${j[0]}`,description:te.description,usage:`rule ${te.usage}`,options:j[0]==="add"?mo:[],examples:j[0]==="add"?go:void 0}}function Vy(j){let K=Dt({label:"rule",booleans:{global:["-g","--global"],check:["--check"],cleanup:["--cleanup"],deleteSource:["--delete-source"],example:["--example"]},values:{ref:["--ref"]},lists:{only:["--only"]},positionals:"list"},j),te={...K.flags,ref:K.values.ref,only:K.lists.only??[],help:K.help,positionals:K.positionals,errors:K.errors};return zy(te),te}function zy(j){let[K]=j.positionals;if(K&&!Md.has(K))j.errors.push(`Unknown rule subcommand: ${K}`);if(j.deleteSource&&K!=="remove")if(K&&Md.has(K))j.errors.push(`Unknown option for rule ${K}: --delete-source`);else j.errors.push("--delete-source is only valid with 'rule remove'");if(j.check&&K)j.errors.push(jn(K,"--check"));if(j.cleanup&&K!=="migrate")j.errors.push(jn(K,"--cleanup"));if(j.example&&K!=="init")j.errors.push(jn(K,"--example"));if(j.ref&&K!=="add")j.errors.push(jn(K,"--ref"));if(j.only.length>0&&K!=="add")j.errors.push(jn(K,"--only"));if(K==="add")Jy(j);if(K==="migrate"){if(j.global)j.errors.push(jn(K,"--global"));if(j.positionals.length>1)j.errors.push(`Unexpected rule migrate argument: ${j.positionals[1]}`)}else if(K==="wrapper")Wy(j);else if(j.positionals.length>2)j.errors.push(`Unexpected rule argument: ${j.positionals[2]}`);if(K==="list"&&j.global)j.errors.push("Unknown option for rule list: --global")}function Ud(j){if(j.positionals[1])return j.positionals[1];if(j.ref||j.only.length>0)return Gy;return}function Jy(j){let K=Ud(j);if(!K)return;if((j.ref||j.only.length>0)&&!Y(K)){if(j.ref)j.errors.push(`--ref can only select a ref for an owner/repo source: ${K}`);if(j.only.length>0)j.errors.push("--only can only select rulebooks from an owner/repo source");return}if(j.ref&&!ie(j.ref))j.errors.push(`--ref must use valid path segments: ${j.ref}`);let te=j.only.filter((re)=>!c.test(re));if(te.length>0)j.errors.push(`Invalid rulebook names: ${te.join(", ")}`)}function jn(j,K){return j?`Unknown option for rule ${j}: ${K}`:`Unknown option for rule: ${K}`}function Wy(j){let K=j.positionals[1],te=j.positionals[2];if(!K){j.errors.push("rule wrapper requires add, remove, or list");return}if(!Uy.has(K)){j.errors.push(`Unknown rule wrapper action: ${K}`);return}if(K==="list"){if(te)j.errors.push(`Unexpected rule wrapper argument: ${te}`);return}if(!te){j.errors.push(`rule wrapper ${K} requires a command`);return}if(j.positionals.length>3)j.errors.push(`Unexpected rule wrapper argument: ${j.positionals[3]}`)}function Ky(j){if(r(j)===null){ld(j);return}let K=m(j);if(!K.config)return;Tt(j,{version:1,rules:K.config.rules,overrides:K.config.overrides??{},transparent_wrappers:K.config.transparent_wrappers??[]})}async function Yy(j,K){let te=K.positionals[1],re=K.positionals[2],oe=It(j,{global:K.global}).configTarget;if(te==="list"){let xe=m(oe);if(xe.errors.length>0){for(let Ce of xe.errors)console.error(Ce);return 1}return Zy(xe.config?.transparent_wrappers??[]),0}if(!re||!L.test(re))return console.error("transparent wrapper must match command pattern"),1;if(Ne(re))return console.error(`reserved command "${re}" cannot be a wrapper`),1;let ce=m(oe);if(ce.errors.length>0){for(let xe of ce.errors)console.error(xe);return 1}let de=ce.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},we=te==="add"?[...new Set([...de.transparent_wrappers??[],re])]:(de.transparent_wrappers??[]).filter((xe)=>xe!==re);return Tt(oe,{version:1,rules:de.rules,overrides:de.overrides??{},transparent_wrappers:we}),console.log(te==="add"?`Added transparent wrapper: ${re}`:`Removed transparent wrapper: ${re}`),0}function Zy(j){if(j.length===0){console.log("Transparent wrappers: (none)");return}console.log(`Transparent wrappers (${j.length}):`);for(let K of j)console.log(`  - ${K}`)}import{sep as rh}from"node:path";import{existsSync as Xy,readFileSync as Qy}from"node:fs";import{join as eh}from"node:path";async function th(j){if(j.isTTY)return null;return(await Ze(j).catch(()=>null))?.trim()||null}function nh(j){let K=j.env.get("CLAUDE_SETTINGS_PATH");if(K)return K;return eh(j.home,".claude","settings.json")}function Hi(j){let K=nh(j);if(!Xy(K))return!1;try{let te=Qy(K,"utf-8"),re=JSON.parse(te);if(!re.enabledPlugins)return!1;let oe="cc-safety-net@cc-marketplace";if(!(oe in re.enabledPlugins))return!1;return re.enabledPlugins[oe]===!0}catch(te){if(_(i.debug,j.env))console.error(`CC Safety Net debug: failed to read Claude settings: ${K}: ${te instanceof Error?te.message:String(te)}`);return!1}}async function Ui(j,K=process.stdin){let te=Hi(j),re;if(!te)re="\uD83D\uDEE1️ CC Safety Net ❌";else{let ce=k(j,{cwd:process.cwd()}),de=ce.policy,we=P(de,j.env),xe=Object.values(B(de,we.capabilities)).some((Oe)=>Oe.changesInherited),Ce={standard:"✅",strict:"\uD83D\uDD12",paranoid:"\uD83D\uDC41️",custom:"\uD83D\uDD27"}[xe?"custom":we.effectiveLevel],Se=(ce.policyScopes?.weakenings.length??0)>0?"\uD83D\uDD3B":"";re=`\uD83D\uDEE1️ CC Safety Net ${Ce}${we.worktreeMode?"\uD83C\uDF33":""}${Se}${ce.state==="degraded"?"⚠️":""}`}let oe=await th(K);if(oe&&!oe.startsWith("{"))console.log(`${oe} | ${re}`);else console.log(re)}function Gd(j){let K=k(j,{cwd:process.cwd()}),te=K.policy,re=P(te,j.env),oe=!!process.env.NO_COLOR||!process.stdout.isTTY,ce=Math.min(process.stdout.columns||80,100),de=oe?"ok":"✔",we=oe?"OFF":"✘",xe=(ot,mt)=>{let yt=`  ${ot.padEnd(13)}${mt}`;return(yt.length>ce?`${yt.slice(0,ce-1)}…`:yt).replaceAll(we,vt.red(we))},Ce=Object.values(B(te,re.capabilities)).some((ot)=>ot.changesInherited),Se=(ot)=>ot===j.home||ot.startsWith(`${j.home}${rh}`)?`~${ot.slice(j.home.length)}`:ot,Oe={ready:vt.green,degraded:vt.yellow}[K.state],Ge=K.policyScopes?.weakenings??[],st=[...Hi(j)?[]:["plugin cc-safety-net@cc-marketplace is disabled in Claude Code; nothing is enforced in Claude Code until it is re-enabled. Other integrations are not affected."],...K.diagnostics],ft=oe?"-":"·";console.log([`${oe?"":"\uD83D\uDEE1️  "}CC Safety Net — ${Oe(K.state)}`,"",xe("Protection",`destructive ${te.destructiveCommandProtectionEnabled?de:we}   secrets ${te.secretProtection.enabled?de:we}`),xe("Level",Ce?`${re.effectiveLevel} (customised)`:re.effectiveLevel),xe("Rules",te.rules.length===0?"none active":`${te.rules.length} active`),xe("Policy",Se(d(j))),...K.policyScopes?[xe("Project",Se(w(process.cwd())))]:[],...re.worktreeMode?[xe("Worktree","relaxations active")]:[],"",...Ge.length===0?[]:["  Project policy",...Ge.flatMap((ot)=>Xn(ot,"      ",ce-6).map((mt,yt)=>yt===0?`    ${mt}`:mt)),""],...st.length===0?["  Everything configured is active."]:["  Not active",...st.flatMap((ot)=>Xn(ot,"      ",ce-6).map((mt,yt)=>yt===0?`    ${ft} ${mt}`:mt)),"","  Full report: cc-safety-net doctor"]].join(`
`))}import{spawn as Qd}from"node:child_process";import{randomBytes as fh}from"node:crypto";import{existsSync as mh}from"node:fs";import{createServer as gh}from"node:http";import{Writable as yh}from"node:stream";var co=500;function oh(j){let K=j.filter((oe)=>oe.decision!=="allow"),te=j.filter((oe)=>oe.decision==="allow"),re=Math.min(K.length,Math.max(co-te.length,Math.ceil(co/2)));return[...K.slice(0,re),...te.slice(0,co-re)]}function Bd(j,K,te=M(j)){if(te)q(j,te);let re=(mt)=>new Date(mt.getFullYear(),mt.getMonth(),mt.getDate()).getTime(),oe=re(new Date),ce=new Date(oe);ce.setDate(ce.getDate()-(K-1));let de=ce.getTime(),we=[],xe={count:0};for(let mt of te?sn(te,xe):[])for(let yt of Ln(mt,xe)){if(!yt||typeof yt.ts!=="string"||typeof yt.command!=="string")continue;let ht=new Date(yt.ts).getTime();if(!Number.isFinite(ht))continue;if(ht>=de)we.push(yt)}we.sort((mt,yt)=>new Date(yt.ts).getTime()-new Date(mt.ts).getTime());let Ce=Array.from({length:K},()=>0),Se=Array.from({length:K},()=>0),Oe={},Ge={},st={},ft=0,ot=0;for(let mt of we){let yt=mt.agent||"unknown";Oe[yt]=(Oe[yt]??0)+1;let ht=Math.round((oe-re(new Date(mt.ts)))/86400000),gt=K-1-ht,wt=ht>=0&&ht<K;if(wt)Se[gt]=(Se[gt]??0)+1;if(mt.decision!=="allow"){if(ft++,mt.ruleId)Ge[mt.ruleId]=(Ge[mt.ruleId]??0)+1;let bt=cr(mt.segment||mt.command);if(bt)st[bt]=(st[bt]??0)+1;if(mt.failureStage)ot++;if(wt)Ce[gt]=(Ce[gt]??0)+1}}return{days:K,logsDir:te,homeDir:j.home,totalInWindow:we.length,truncated:we.length>co,unreadable:xe.count,counts:{blocked:ft,allowed:we.length-ft,agents:Oe,blockedByDay:Ce,analyzedByDay:Se,rules:Ge,commands:st,errors:ot},entries:oh(we).sort((mt,yt)=>new Date(yt.ts).getTime()-new Date(mt.ts).getTime())}}import{spawn as ih}from"node:child_process";import{existsSync as sh,statSync as qd}from"node:fs";import{delimiter as ah,join as lh}from"node:path";var ch=120000,uo="Choose the project folder",dh=`try
  return POSIX path of (choose folder with prompt "${uo}")
on error number -128
  return ""
end try`,uh=`Add-Type -AssemblyName System.Windows.Forms
$dialog = New-Object System.Windows.Forms.FolderBrowserDialog
$dialog.Description = '${uo}'
if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { [Console]::Out.Write($dialog.SelectedPath) }`,Vd=[{binary:"zenity",args:["--file-selection","--directory",`--title=${uo}`]},{binary:"kdialog",args:["--getexistingdirectory",".","--title",uo]}],zd=(j,K)=>(K.PATH??"").split(ah).some((te)=>{if(te.length===0)return!1;try{let re=qd(lh(te,j));return re.isFile()&&(re.mode&73)!==0}catch{return!1}});function Gi(j,K){if(j==="darwin"||j==="win32")return!0;if(j!=="linux")return!1;if(!K.DISPLAY&&!K.WAYLAND_DISPLAY)return!1;return Vd.some((te)=>zd(te.binary,K))}function ph(j,K){if(j==="darwin")return{cmd:"osascript",args:["-e",dh]};if(j==="win32")return{cmd:"powershell.exe",args:["-NoProfile","-STA","-Command",uh]};let te=Vd.find((re)=>zd(re.binary,K));return te?{cmd:te.binary,args:te.args}:null}function Bi(j=process.platform,K=process.env){let te=ph(j,K);if(!te)return Promise.resolve({error:"No folder dialog is available on this system"});return new Promise((re)=>{let oe=ih(te.cmd,te.args,{env:K,stdio:["ignore","pipe","pipe"]}),ce="",de=!1,we=(Ce)=>{if(de)return;de=!0,clearTimeout(xe),re(Ce)},xe=setTimeout(()=>{oe.kill(),we({error:"The folder dialog timed out"})},ch);oe.stdout.on("data",(Ce)=>{ce+=Ce.toString()}),oe.on("error",()=>we({error:`Could not open the folder dialog (${te.cmd})`})),oe.on("close",()=>{let Ce=ce.trim().replace(/\/+$/,"");if(!Ce)return we({cancelled:!0});if(!sh(Ce)||!qd(Ce).isDirectory())return we({error:"That selection is not a folder on disk"});we({path:Ce})})})}var Jd=`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
`;var Wd='<script id="ccsn-data" type="application/json">';function Kd(j){return Jd.replace(Wd,()=>Wd+JSON.stringify({token:j}).replaceAll("<","\\u003c"))}var po="kenryu42/cc-safety-net",hh=`https://github.com/${po}`,zi=1e4,vh=7,bh="The project draft directory changed; reload the draft before applying.",Lh="audit settings are user scope only; remove the audit section from a project proposal";async function eu(j,K={}){let te=Dt({label:"gui",booleans:{noOpen:["--no-open"]}},j),re=K.log??console.log,oe=K.error??console.error;if(te.errors.length>0){for(let de of te.errors)oe(de);return oe("Usage: cc-safety-net gui [--no-open]"),1}let ce=await wh(l,K);if(re(`CC Safety Net policy GUI: ${ce.url}`),!te.flags.noOpen)try{await(K.openBrowser??Th)(ce.url)}catch(de){oe(`Failed to open browser: ${de instanceof Error?de.message:String(de)}`),oe(`Open this URL manually: ${ce.url}`)}if(K.keepAlive===!1)return await ce.close(),0;return await _h(ce),0}async function wh(j,K={}){let te=fh(24).toString("base64url"),re={dir:null,revision:0},oe=gh((we,xe)=>{kh(j,we,xe,te,K,re)});await new Promise((we,xe)=>{oe.once("error",xe),oe.listen(0,"127.0.0.1",()=>{oe.off("error",xe),we()})});let de=`http://127.0.0.1:${oe.address().port}`;return{origin:de,token:te,url:`${de}/?token=${encodeURIComponent(te)}`,close:()=>Ah(oe)}}async function kh(j,K,te,re,oe,ce){let de=j(),we=new URL(K.url??"/","http://127.0.0.1");if(K.method==="GET"&&we.pathname==="/favicon.ico"){te.writeHead(204,{"cache-control":"no-store"}),te.end();return}if(!Ph(K,we,re)){kt(te,403,{error:"Forbidden"});return}if(K.method==="GET"&&we.pathname==="/"){Dh(te,Kd(re));return}if(K.method==="GET"&&we.pathname==="/api/policy"){let xe=Kc(de,oe),Ce=k(de,qi(oe));kt(te,200,{...xe,configState:Ue(Ce),...Ce.policyScopes?{projectPolicy:{path:w(oe.cwd??process.cwd()),weakenings:Ce.policyScopes.weakenings}}:{},destructiveCommandRules:U,secretPatterns:nt,version:Pt(),preview:xe.errors.length>0?null:je(xe.policy,de.env)});return}if(K.method==="POST"&&we.pathname==="/api/policy/preview"){let xe=await lr(K);if(!xe.ok){kt(te,xe.status,{errors:[xe.error]});return}let Ce=Yc(de,xe.value);kt(te,Ce.errors.length>0?400:200,Ce);return}if(K.method==="POST"&&we.pathname==="/api/policy/explain"){let xe=await lr(K);if(!xe.ok){kt(te,xe.status,{errors:[xe.error]});return}let Ce=xe.value;if(Ce===null||typeof Ce.command!=="string"){kt(te,400,{errors:["command must be a string"]});return}let Se=Ht(Ce.policy,de.home);if(Se.length>0){kt(te,400,{errors:Se});return}kt(te,200,Sh(de,Ce.command,Ce.policy,oe));return}if(K.method==="POST"&&we.pathname==="/api/policy"){let xe=await lr(K);if(!xe.ok){kt(te,xe.status,{errors:[xe.error]});return}let Ce=Xt(de,xe.value,oe);kt(te,Ce.errors.length>0?400:200,Ce);return}if(K.method==="POST"&&we.pathname==="/api/reset"){kt(te,200,Xt(de,Q,oe));return}if(K.method==="POST"&&we.pathname==="/api/repair"){kt(te,200,Zc(de,oe));return}if(K.method==="POST"&&we.pathname==="/api/policy/project/choose-directory"){let xe=await(oe.chooseDirectory??Bi)();if("path"in xe)ce.dir=xe.path,ce.revision+=1;kt(te,200,{cancelled:"cancelled"in xe,..."error"in xe?{error:xe.error}:{}});return}if(K.method==="GET"&&we.pathname==="/api/policy/project"){let xe=tu(ce,oe),Ce=Yd(xe,de.home),Se=ir(de,oe);kt(te,200,{dir:xe,path:w(xe),revision:ce.revision,baseline:Se.baseline,userPolicyDiagnostics:Se.diagnostics,projection:Ce.projection,projectionDiagnostics:Ce.diagnostics,canPickDirectory:Gi(process.platform,process.env)});return}if(K.method==="POST"&&we.pathname==="/api/policy/project/diff"){let xe=await Zd(de,K,te,ce,oe);if(!xe)return;let Ce=Yd(xe.dir,de.home),Se=ir(de,oe).baseline,Oe=X(Se,le(xe.proposal,de.home).policy);kt(te,200,{rows:Zr(X(Se,Ce.projection).policy,Oe.policy,!1),weakenings:Oe.weakenings,existingFileDiagnostics:Ce.diagnostics,errors:[]});return}if(K.method==="POST"&&we.pathname==="/api/policy/project/apply"){let xe=await Zd(de,K,te,ce,oe);if(!xe)return;let Ce=Ch(xe.dir,xe.proposal,de.home);kt(te,Ce.errors.length>0?500:200,Ce);return}if(K.method==="GET"&&we.pathname==="/api/activity"){let xe=ne(de,oe),Ce=Rh(we.searchParams.get("days"),xe);if(Ce===null){kt(te,400,{error:`days must be an integer between 1 and ${xe}`});return}kt(te,200,Bd(de,Ce,oe.activityLogsDir));return}if(K.method==="POST"&&we.pathname==="/api/rules/choose-directory"){kt(te,200,await Bi());return}if(K.method==="GET"&&we.pathname==="/api/rules"){let xe=Z(de,qi(oe)),Ce=new Map(xe.rules.map((Se)=>[Se.name,Se]));kt(te,200,{projectPath:oe.cwd??process.cwd(),canPickDirectory:Gi(process.platform,process.env),rulebooks:xe.rulebooks.map((Se)=>({source:Se.source,spec:Se.spec,name:Se.name,version:Se.version,rules:Se.rules.flatMap((Oe)=>{let Ge=Ce.get(Oe);if(!Ge)return[];return[{name:Ge.name,command:Ge.command,subcommand:Ge.subcommand,block_args:Ge.block_args,reason:Ge.reason}]})})),errors:xe.errors,warnings:xe.warnings});return}if(K.method==="GET"&&we.pathname==="/api/star/context"){kt(te,200,await(oe.fetchStarContext??(()=>Fh(de,{logsDir:oe.activityLogsDir})))());return}if(K.method==="POST"&&we.pathname==="/api/star"){let xe=await(oe.starRepo??Ih)();kt(te,200,xe.ok?{ok:!0}:{ok:!1,fallbackUrl:hh});return}if(K.method==="GET"&&we.pathname==="/api/integrations"){kt(te,200,await(oe.fetchIntegrations??(()=>$h(de)))());return}if(K.method==="GET"&&we.pathname==="/api/health"){kt(te,200,await(oe.fetchHealth??(()=>Oh(de)))());return}if(K.method==="POST"&&(we.pathname==="/api/install"||we.pathname==="/api/uninstall")){let xe=await lr(K);if(!xe.ok){kt(te,xe.status,{errors:[xe.error]});return}let Ce=xe.value?.target;if(typeof Ce!=="string"||!Jt.some((Oe)=>Oe.target===Ce)){kt(te,400,{error:"unknown target"});return}let Se=we.pathname==="/api/install"?"install":"uninstall";kt(te,200,await(oe.runIntegration??jh)(Se,Ce));return}kt(te,404,{error:"Not found"})}function qi(j){return{...j,cwd:j.cwd??process.cwd()}}function tu(j,K){return j.dir??K.cwd??process.cwd()}function Yd(j,K){let te=w(j),re=mh(te)?vn(te):{value:void 0,errors:[]},oe=le(re.value,K);return{projection:oe.policy,diagnostics:[...re.errors,...oe.diagnostics]}}async function Zd(j,K,te,re,oe){let ce=tu(re,oe),de=re.revision,we=await lr(K);if(!we.ok)return kt(te,we.status,{errors:[we.error]}),null;let xe=we.value;if(typeof xe?.revision!=="number")return kt(te,400,{errors:["revision must be a number"]}),null;if(xe.revision!==de)return kt(te,409,{errors:[bh]}),null;let Ce=xh(xe.proposal,j.home);if(Ce.length>0)return kt(te,400,{errors:Ce}),null;return{dir:ce,proposal:xe.proposal}}function xh(j,K){let te=Ht(j,K);if(te.length>0)return te;return j?.audit===void 0?[]:[Lh]}function Ch(j,K,te){let re=w(j),oe=Xr(K,h(K,te));try{return y(s(S(j,"project policy"),re),`${JSON.stringify(oe,null,2)}
`),{path:re,errors:[]}}catch(ce){return{path:re,errors:[ce instanceof Error?ce.message:String(ce)]}}}function Sh(j,K,te,re){let oe=h(te,j.home),ce=k(j,qi(re)),de=Ie({rules:ce.policy.rules,transparentWrappers:ce.policy.transparentWrappers,safety:We(oe.safety),worktreeMode:oe.workflow.worktree_mode,destructiveCommandProtectionEnabled:oe.destructive_command_protection.enabled,destructiveCommandRuleOverrides:oe.destructive_command_protection.overrides,destructiveCommandAllowPaths:oe.destructive_command_protection.allow_paths,secretProtection:{enabled:oe.secret_protection.enabled,disabledRules:He(oe.secret_protection.overrides),denyPaths:oe.secret_protection.deny_paths,allowPaths:oe.secret_protection.allow_paths}});return Qn(K,{policySnapshot:de,cwd:re.cwd,userConfigDir:re.userConfigDir},j)}function Rh(j,K){if(j===null)return Math.min(vh,K);let te=Number(j);if(!Number.isInteger(te)||te<1||te>K)return null;return te}function Ph(j,K,te){if(K.searchParams.get("token")!==te)return!1;if(j.method!=="POST")return!0;return j.headers["x-cc-safety-net-token"]===te}var Eh=1048576;async function lr(j){let K=[],te=0;for await(let re of j){let oe=re;if(te+=oe.byteLength,te>Eh)return{ok:!1,status:413,error:"Request body is too large"};K.push(oe)}try{return{ok:!0,value:JSON.parse(Buffer.concat(K).toString("utf-8")||"{}")}}catch(re){return{ok:!1,status:400,error:`Invalid JSON: ${re instanceof Error?re.message:String(re)}`}}}function Dh(j,K){j.writeHead(200,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}),j.end(K)}function kt(j,K,te){j.writeHead(K,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}),j.end(JSON.stringify(te))}function Ah(j){return new Promise((K,te)=>{j.close((re)=>re?te(re):K())})}function _h(j){return new Promise((K)=>{let te=()=>{process.off("SIGINT",re),process.off("SIGTERM",re)},re=()=>{te(),j.close().then(K)};process.once("SIGINT",re),process.once("SIGTERM",re)})}function Th(j){let K=process.platform==="darwin"?"open":process.platform==="win32"?"cmd":"xdg-open",te=process.platform==="win32"?["/c","start","",j]:[j];return new Promise((re,oe)=>{let ce=Qd(K,te,{detached:!0,stdio:"ignore"}),de=(xe)=>{ce.off("spawn",we),oe(xe)},we=()=>{ce.off("error",de),ce.unref(),re()};ce.once("error",de),ce.once("spawn",we)})}async function Ih(j="gh",K=zi){return{ok:await Vi(j,["api","-X","PUT",`/user/starred/${po}`],K)===0}}async function $h(j,K={}){let te=await Mn(K.fetcher),re=nu(j,te);return{targets:Ft.map((oe)=>{let ce=re.find((de)=>de.platform===oe.id);return{target:oe.id,label:Ct(oe.id),version:te.versions[oe.id]??null,status:ce?.configured?"active":ce?.detected?"disabled":ce?.inspectionStatus==="not-inspected"?"not-inspected":"not-installed"}}),system:{version:te.version,nodeVersion:te.nodeVersion,platform:te.platform}}}function nu(j,K){return En(j,process.cwd(),{ampPluginListOutput:K.ampPluginListOutput,codexPluginListOutput:K.codexPluginListOutput,copilotCliVersion:K.versions["copilot-cli"]})}async function Oh(j,K={}){let[te,re]=await Promise.all([Mn(K.fetcher),(K.checkUpdates??tn)()]);return{hooks:nu(j,te).filter((oe)=>oe.detected).map((oe)=>({platform:oe.platform,label:Ct(oe.platform),configured:oe.configured})),update:{currentVersion:re.currentVersion,latestVersion:re.latestVersion??null,updateAvailable:re.updateAvailable}}}var Xd=Promise.resolve();function jh(j,K,te={}){let re=async()=>{let ce=[],{log:de,error:we}=console;console.log=(...xe)=>ce.push(xe.map(String).join(" ")),console.error=console.log;try{return{ok:await or(j,[],{selectTargets:async()=>[K],output:new yh({write(Ce,Se,Oe){ce.push(String(Ce).replace(/\n$/,"")),Oe()}}),...te})===0,output:ce.join(`
`)}}finally{console.log=de,console.error=we}},oe=Xd.then(re);return Xd=oe.then(()=>{return},()=>{return}),oe}async function Fh(j,K={}){let[te,re,oe]=await Promise.all([Nh(K.command),Mh(K.fetchRepo),Promise.resolve(mr(j,ne(j),K.logsDir).totalBlocked)]);return{starred:te,starCount:re,blockedTotal:oe}}async function Nh(j="gh",K=zi){if(await Vi(j,["auth","status"],K)!==0)return null;let te=await Vi(j,["api",`/user/starred/${po}`],K);if(te===0)return!0;if(te===null)return null;return!1}function Vi(j,K,te){return new Promise((re)=>{let oe=Qd(j,K,{stdio:"ignore",windowsHide:!0}),ce=!1,de,we=(xe)=>{if(ce)return;if(ce=!0,de)clearTimeout(de);re(xe)};oe.once("error",()=>we(null)),oe.once("close",we),de=setTimeout(()=>{oe.kill(),we(null)},te)})}async function Mh(j=fetch){try{let K=await j(`https://api.github.com/repos/${po}`,{headers:{accept:"application/vnd.github+json"},signal:AbortSignal.timeout(zi)});if(!K.ok)return null;let te=await K.json();return typeof te.stargazers_count==="number"?te.stargazers_count:null}catch{return null}}function Hh(j){if(j[0]!=="help")return!1;let K=j[1];if(!K)pi(),process.exit(0);if(er(K))process.exit(0);console.error(`Unknown command: ${K}`),console.error("Run 'cc-safety-net --help' for available commands."),process.exit(1)}var Uh={hook:async()=>{console.error("hook requires exactly one integration flag. Try: cc-safety-net hook --kimi-code"),er("hook",console.error),process.exit(1)},install:async(j)=>{process.exit(await or("install",j))},update:async(j)=>{process.exit(await Pi(j))},uninstall:async(j)=>{process.exit(await or("uninstall",j))},rule:async(j)=>{process.exit(await Hd(l(),j))},policy:async(j)=>{process.exit(await td(l(),j))},status:async(j)=>{if(Wt(Dt({label:"status"},j).errors))process.exit(1);Gd(l())},statusline:async(j)=>{let K=Dt({label:"statusline",booleans:{claudeCode:["-cc","--claude-code"]}},j);if(K.errors.length===0&&K.flags.claudeCode){await Ui(l());return}if(Wt(K.errors),!K.flags.claudeCode)console.error("statusline requires --claude-code (-cc)");er("statusline",console.error),process.exit(1)},doctor:async(j)=>{let K=oi(j);if(!K)process.exit(1);let te=await Sl(l(),{json:K.json,skipUpdateCheck:K.skipUpdateCheck});process.exit(te)},logs:async(j)=>{process.exit(await es(l(),j))},gui:async(j)=>{process.exit(await eu(j))},explain:async(j)=>{process.exit(await Ol(l(),j))}};async function Gh(j){let K=Dt({label:"cc-safety-net",booleans:{version:["-V","--version"]},positionals:"list"},j);if(Hh(j))return;let te=j[0],re=te?fr(te):void 0;if(K.help&&re&&re.name!=="rule")er(re.name),process.exit(0);if(!te||K.help&&!re)pi(),process.exit(0);if(K.flags.version)Nl(),process.exit(0);if(re){await Uh[re.name](j.slice(1));return}if(te==="--statusline"){await Ui(l());return}console.error(te.startsWith("-")?`Unknown option: ${te}`:`Unknown command: ${te}`),console.error("Run 'cc-safety-net --help' for usage."),process.exit(1)}export{Gh as runCli};
