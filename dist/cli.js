import{a,s,ke,Ue,k,Ze,Ge,c,o,He,L,R,ve,Qe,f,We,d,r,v,i,re,n,g,le,N,tt,nt,l,ue,de,rt,A,q,oe,S,ot,F,b,Se,M,u,K,Be,ze,I,w,T,pe,fe,Re,it,m,e,Te,we,Ve,qe,ie,Ce,se,_e,J,j,Pe,me,U,Ke,G,Y,Z,ae,Je,Ye,C,H,Ae,Oe,De,E,Ne,Ee,Fe,h,t,X,ge,y,x,_,st,Me,$e,p,W}from"./chunks/index-26getn55.js";import{ee,B,je,O,D}from"./chunks/index-00kqt7kr.js";var Bd=["-h","--help"];function xt(P,V){let z=Object.entries(P.booleans??{}),Q=Object.entries(P.values??{}),te=Object.entries(P.lists??{}),ne=Object.fromEntries(z.map(([Xe])=>[Xe,!1])),ce={},ye=Object.fromEntries(te.map(([Xe])=>[Xe,[]])),he=[],be=[],Le=!1,Ie=-1;for(let[Xe,et]of V.entries()){if(Xe<=Ie)continue;if(et==="--"){he.push(...V.slice(Xe+1));break}if(Bd.includes(et)){Le=!0;continue}let xe=z.find(([,lt])=>lt.includes(et));if(xe){ne[xe[0]]=!0;continue}let at=Q.find(([,lt])=>lt.includes(et));if(at){let lt=V[Xe+1];if(lt===void 0||lt.startsWith("-")){be.push(`${et} requires a value`);continue}ce[at[0]]=lt,Ie=Xe+1;continue}let ct=te.find(([,lt])=>lt.includes(et));if(ct){let lt=V.slice(Xe+1),pt=lt.findIndex((gt)=>gt.startsWith("-")),mt=lt.slice(0,pt===-1?lt.length:pt);if(mt.length===0){be.push(`${et} requires at least one value`);continue}ye[ct[0]]=[...ye[ct[0]]??[],...mt],Ie=Xe+mt.length;continue}if(et.startsWith("-")){be.push(`Unknown option for ${P.label}: ${et}`);continue}if(P.positionals==="tail"){he.push(...V.slice(Xe));break}he.push(et)}if(P.positionals!=="list"&&P.positionals!=="tail")be.push(...he.map((Xe)=>`Unexpected argument for ${P.label}: ${Xe}`));return{flags:ne,values:ce,lists:ye,positionals:he,help:Le,errors:be}}function Ht(P){for(let V of P)console.error(V);return P.length>0}import{readdirSync as Yd,statSync as Mi,unlinkSync as Zd}from"node:fs";import{basename as Hi,dirname as Xd,isAbsolute as Qd,join as eu,relative as tu,resolve as nu,sep as ru}from"node:path";var Fi=(P)=>{let V=Date.now()-new Date(P).getTime();if(!Number.isFinite(V))return"";let z=Math.floor(V/60000),Q=Math.floor(z/60),te=Math.floor(Q/24);if(te>0)return`${te}d ago`;if(Q>0)return`${Q}h ago`;if(z>0)return`${z}m ago`;return"just now"},to=(P)=>{let V=(P??"").trim().split(/\s+/).filter((te)=>te&&!/^[A-Za-z_][A-Za-z0-9_]*=/.test(te)),z=V[0]?.split("/").pop();if(!z)return null;let Q=V[1];return Q&&/^[a-z][a-z0-9-]*$/.test(Q)?`${z} ${Q}`:z};function ji(P){let V=(te)=>`${te.sessionId}
${to(te.segment||te.command)}`,z=P.filter((te)=>te.decision!=="allow"),Q=z.filter((te)=>te.sessionId).reduce((te,ne)=>te.set(V(ne),(te.get(V(ne))??0)+1),new Map);return new Set(z.filter((te)=>te.failureStage||(Q.get(V(te))??0)>=2))}import{existsSync as qd,readdirSync as Vd,readFileSync as zd}from"node:fs";import{join as Jd}from"node:path";function Yt(P,V){try{return Vd(P,{withFileTypes:!0,encoding:"utf8"}).flatMap((z)=>{let Q=Jd(P,z.name);if(z.isDirectory())return Yt(Q,V);if(z.name.endsWith(".jsonl"))return[Q];return[]})}catch{if(V&&qd(P))V.count++;return[]}}var Wd=["segment","reason","sessionId","decision","agent","ruleId","failureStage"];function Kd(P){if(!P||typeof P!=="object"||Array.isArray(P))return!1;let V=P;if(typeof V.ts!=="string"||typeof V.command!=="string")return!1;return Wd.every((z)=>V[z]===void 0||typeof V[z]==="string")}function pn(P,V){try{return zd(P,"utf-8").split(`
`).filter(Boolean).flatMap((z)=>{try{let Q=JSON.parse(z);if(!Kd(Q)){if(V)V.count++;return[]}return[Q]}catch{if(V)V.count++;return[]}})}catch{if(V)V.count++;return[]}}function Ct(P){return Array.from(P,(V)=>{let z=V.charCodeAt(0);if(z<=31||z>=127&&z<=159)return`\\x${z.toString(16).padStart(2,"0")}`;return V}).join("")}function ou(P,V){let z=ee(P),Q=xt({label:"logs",booleans:{all:["--all"],suspect:["--suspect"],json:["--json"],pruneLegacy:["--prune-legacy"],dryRun:["--dry-run"]},values:{id:["--id"],limit:["--limit"],since:["--since"],agent:["--agent"],rule:["--rule"],session:["--session"],project:["--project"]}},V);if(Ht(Q.errors))return null;if(Q.values.id!==void 0&&!/^[a-f0-9]{16}$/.test(Q.values.id))return console.error("--id must be 16 hexadecimal characters"),null;let te=Q.values.limit===void 0?20:Ni(Q.values.limit);if(te===null)return console.error("--limit must be a positive number"),null;let ne=Q.values.since===void 0?Math.min(30,z):Ni(Q.values.since);if(ne===null||ne>z)return console.error(`--since must be a positive number of days no greater than ${z}`),null;let ce={limit:te,limitExplicit:Q.values.limit!==void 0,since:ne,sinceExplicit:Q.values.since!==void 0,all:Q.flags.all,json:Q.flags.json,suspect:Q.flags.suspect,pruneLegacy:Q.flags.pruneLegacy,dryRun:Q.flags.dryRun,id:Q.values.id,agent:Q.values.agent,rule:Q.values.rule,session:Q.values.session,project:Q.values.project===void 0?void 0:nu(Q.values.project)};if(ce.id&&(ce.agent!==void 0||ce.rule!==void 0||ce.session!==void 0||ce.project!==void 0||ce.suspect||ce.sinceExplicit||ce.limitExplicit))return console.error("--id cannot be combined with --agent, --rule, --session, --project, --suspect, --since, or --limit"),null;if(ce.pruneLegacy&&(ce.id!==void 0||ce.agent!==void 0||ce.rule!==void 0||ce.session!==void 0||ce.project!==void 0||ce.suspect||ce.all||ce.sinceExplicit||ce.limitExplicit))return console.error("--prune-legacy cannot be combined with --id, --agent, --rule, --session, --project, --suspect, --all, --since, or --limit"),null;if(ce.dryRun&&!ce.pruneLegacy)return console.error("--dry-run requires --prune-legacy"),null;return ce}async function Ui(P,V,z={}){let Q=ou(P,V);if(!Q)return 1;let te=z.logsDir??O(P);if(Q.pruneLegacy)return iu(te,Q.json,Q.dryRun);if(!te)return console.log(Q.json?"[]":Q.id?`No retained audit log entry found for id ${Ct(Q.id)}.`:"No audit log entries found."),0;B(P,te);let ne={count:0},ce=Yt(te,ne).flatMap((Ie)=>pn(Ie,ne).map((Xe)=>({entry:Xe,file:Ie})));if(ne.count>0)console.error(`warning: ${ne.count} audit log ${ne.count===1?"source":"sources"} could not be read; these results are incomplete`);if(Q.id)return cu(ce,Q,z.timeZone);let ye=Date.now()-Q.since*24*60*60*1000,he=ce.filter((Ie)=>du(Ie,Q,te,ye)),be=Q.suspect?ji(he.map((Ie)=>Ie.entry)):null,Le=(be?he.filter((Ie)=>be.has(Ie.entry)):he).sort((Ie,Xe)=>Date.parse(Xe.entry.ts)-Date.parse(Ie.entry.ts)).slice(0,Q.limit);if(Q.json)return console.log(JSON.stringify(Le.map((Ie)=>Ie.entry),null,2)),0;if(Le.length===0)return console.log("No audit log entries found."),0;for(let Ie of Le)console.log(fu(Ie.entry,z.timeZone));return 0}function iu(P,V,z){let Q=P?au(P).map((ye)=>eu(P,ye)):[];if(z)return su(Q,V);let te=[],ne=0,ce=0;for(let ye of Q){let he=Mi(ye,{throwIfNoEntry:!1})?.size??0,be=lu(ye);if(be){te.push(`${Hi(ye)}: ${be}`);continue}ne++,ce+=he}if(V)return console.log(JSON.stringify({removedFiles:ne,removedBytes:ce,failedFiles:te.length})),te.length===0?0:1;console.log(ne===0&&te.length===0?"No legacy audit log files found.":`Removed ${ne} legacy audit log ${ne===1?"file":"files"} (${Gi(ce)}).`);for(let ye of te)console.error(`Could not remove ${Ct(ye)}`);if(console.log("Nested v2 audit logs were not changed."),ne>0)console.log("This deletion cannot be undone.");return te.length===0?0:1}function su(P,V){let z=P.reduce((Q,te)=>Q+(Mi(te,{throwIfNoEntry:!1})?.size??0),0);if(V)return console.log(JSON.stringify({dryRun:!0,files:P.length,bytes:z})),0;if(console.log(P.length===0?"No legacy audit log files found.":`Would remove ${P.length} legacy audit log ${P.length===1?"file":"files"} (${Gi(z)}).`),console.log("Nested v2 audit logs are not included."),P.length>0)console.log("Run the same command without --dry-run to delete them.");return 0}function au(P){try{return Yd(P,{withFileTypes:!0}).filter((V)=>V.isFile()&&V.name.endsWith(".jsonl")).map((V)=>V.name)}catch{return[]}}function lu(P){try{return Zd(P),null}catch(V){return V instanceof Error?V.message:String(V)}}function Gi(P){let V=["B","KiB","MiB","GiB"],z=Math.min(Math.floor(Math.log2(Math.max(P,1))/10),V.length-1);return`${Math.round(P/1024**z*10)/10} ${V[z]}`}function cu(P,V,z){let Q=P.filter((ne)=>ne.entry.id===V.id);if(Q.length>1)return console.error(`Multiple audit log entries found for id ${Ct(V.id??"")}.`),1;if(V.json)return console.log(JSON.stringify(Q.map((ne)=>ne.entry),null,2)),0;let te=Q[0];if(!te)return console.log(`No retained audit log entry found for id ${Ct(V.id??"")}.`),0;return console.log(mu(te.entry,z)),0}function du(P,V,z,Q){if(!V.all&&P.entry.decision==="allow")return!1;if(Date.parse(P.entry.ts)<Q)return!1;if(V.agent!==void 0&&P.entry.agent!==V.agent)return!1;if(V.rule!==void 0&&P.entry.ruleId!==V.rule)return!1;if(V.session!==void 0&&!uu(P,z,V.session))return!1;if(V.project!==void 0&&!pu(P.entry.cwd,V.project))return!1;return!0}function uu(P,V,z){if(P.entry.sessionId===z)return!0;return Xd(P.file)===V&&Hi(P.file,".jsonl")===z}function pu(P,V){if(!P)return!1;let z=tu(V,P);return z!==".."&&!z.startsWith(`..${ru}`)&&!Qd(z)}function fu(P,V){let z=Ct(P.id??"-"),Q=Ct(P.decision??"deny"),te=P.cwd?`  [${Ct(P.cwd)}]`:"",ne=P.segment||P.command,ce=ne===P.command?"":"↳ ",ye=ne.length>50?`${ne.slice(0,50)}…`:ne;return`${z.padEnd(16)}  ${Ct(Bi(P.ts,V))}  ${Q.padEnd(5)}  ${Ct(P.agent??"-").padEnd(15)}  ${Ct(P.ruleId??"-").padEnd(20)}  ${ce}${Ct(ye)}${te}`}function mu(P,V){let z=(te)=>Ct(te===void 0||te===null||te===""?"-":te),Q=P.shape?`${P.agent??"-"} (shape: ${P.shape})`:P.agent??"-";return[`id:        ${z(P.id)}`,`ts:        ${z(Bi(P.ts,V))}`,`decision:  ${z(P.decision)}`,`agent:     ${z(Q)}`,`level:     ${z(P.level)}`,`tool:      ${z(P.toolName)}`,`rule:      ${z(P.ruleId)}`,`intent:    ${z(P.intent)}`,`stage:     ${z(P.failureStage)}`,`error:     ${z(P.errorCode)}`,`session:   ${z(P.sessionId)}`,`cwd:       ${z(P.cwd)}`,`version:   ${z(P.v)}`,`truncated: ${z(P.truncated===!0?"yes":void 0)}`,`reason:    ${z(P.reason)}`,`command:   ${z(P.command)}`,`segment:   ${z(P.segment)}`].join(`
`)}function Bi(P,V){let z=new Date(P);if(Number.isNaN(z.getTime()))return P;return new Intl.DateTimeFormat("sv-SE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23",timeZone:V}).format(z)}function Ni(P){let V=Number(P);return Number.isFinite(V)&&V>0?V:null}var qi={name:"doctor",aliases:["--doctor"],description:"Run diagnostic checks to verify installation and configuration",usage:"doctor [options]",options:[{flags:"--json",description:"Output diagnostics as JSON"},{flags:"--skip-update-check",description:"Skip npm registry version check"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net doctor","cc-safety-net doctor --json","cc-safety-net doctor --skip-update-check"]};var Vi={name:"explain",description:"Show step-by-step analysis trace of how a command would be analyzed",usage:"explain [options] <command>",argument:"<command>",options:[{flags:"--json",description:"Output analysis as JSON"},{flags:"--cwd",argument:"<path>",description:"Use custom working directory"},{flags:"-h, --help",description:"Show this help"}],examples:['cc-safety-net explain "git reset --hard"','cc-safety-net explain --json "rm -rf /"','cc-safety-net explain --cwd /tmp "git status"']};var zi={name:"gui",description:"Open the local policy editor GUI",usage:"gui [options]",options:[{flags:"--no-open",description:"Print the URL without opening a browser"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net gui","cc-safety-net gui --no-open"]};var Yn=[{id:"antigravity-cli",displayName:"Antigravity CLI",doctorOrder:3,runtime:{order:1,flags:["-ac","--agy-cli"],description:"Run as Antigravity CLI PreToolUse hook",legacyTopLevelFlags:[]},install:{order:2,flag:"--agy-cli",artifactKind:"hook config",probeCommand:["agy","--version"]}},{id:"claude-code",displayName:"Claude Code",doctorOrder:1,runtime:{order:2,displayName:"Coding CLI",flags:["-cc","--coding-cli"],legacyFlags:["--claude-code"],description:"Run as Coding CLI PreToolUse hook",legacyTopLevelFlags:["-cc","--claude-code"]},install:{order:3,flag:"--claude-code",artifactKind:"plugin",probeCommand:["claude","--version"]}},{id:"codex",displayName:"Codex",doctorOrder:4,runtime:{order:3,flags:["-cx","--codex"],description:"Run as a Codex PreToolUse hook",legacyTopLevelFlags:[]},install:{order:4,flag:"--codex",artifactKind:"plugin",probeCommand:["codex","--version"]}},{id:"copilot-cli",displayName:"GitHub Copilot CLI",doctorOrder:7,runtime:{order:6,flags:["-cp","--copilot-cli"],description:"Run as GitHub Copilot CLI PreToolUse hook",legacyTopLevelFlags:["-cp","--copilot-cli"]},install:{order:7,flag:"--copilot-cli",artifactKind:"plugin",probeCommand:["copilot","--binary-version"]}},{id:"gemini-cli",displayName:"Gemini CLI",doctorOrder:6,runtime:{order:5,flags:["-gc","--gemini-cli"],description:"Run as Gemini CLI BeforeTool hook",legacyTopLevelFlags:["-gc","--gemini-cli"]},install:{order:6,flag:"--gemini-cli",artifactKind:"extension",probeCommand:["gemini","--version"]}},{id:"grok-build",displayName:"Grok Build",doctorOrder:8,runtime:{order:7,flags:["-gb","--grok-build"],description:"Run as Grok Build PreToolUse hook",legacyTopLevelFlags:[]},install:{order:8,flag:"--grok-build",artifactKind:"hook config",probeCommand:["grok","--version"]}},{id:"hermes-agent",displayName:"Hermes Agent",doctorOrder:9,runtime:{order:8,flags:["-ha","--hermes-agent"],description:"Run as Hermes Agent pre_tool_call hook",legacyTopLevelFlags:[]},install:{order:9,flag:"--hermes-agent",artifactKind:"plugin",probeCommand:["hermes","--version"]}},{id:"kimi-code",displayName:"Kimi Code",doctorOrder:10,runtime:{order:9,flags:["-kc","--kimi-code"],description:"Run as Kimi Code PreToolUse hook",legacyTopLevelFlags:[]},install:{order:10,flag:"--kimi-code",artifactKind:"hook config",probeCommand:["kimi","--version"]}},{id:"openclaw",displayName:"OpenClaw",doctorOrder:11,install:{order:11,flag:"--openclaw",artifactKind:"plugin",probeCommand:["openclaw","--version"]}},{id:"opencode",displayName:"OpenCode",doctorOrder:12,install:{order:12,flag:"--opencode",artifactKind:"plugin",probeCommand:["opencode","--version"]}},{id:"pi",displayName:"Pi",doctorOrder:13,install:{order:13,flag:"--pi",artifactKind:"package",probeCommand:["pi","--version"]}},{id:"cursor",displayName:"Cursor",doctorOrder:5,runtime:{order:4,flags:["-cu","--cursor"],description:"Run as Cursor preToolUse hook",legacyTopLevelFlags:[]},install:{order:5,flag:"--cursor",artifactKind:"hook config",probeCommand:["cursor","--version"]}},{id:"amp",displayName:"Amp Code",doctorOrder:2,install:{order:1,flag:"--amp",artifactKind:"plugin",probeCommand:["amp","--version"]}}],Zn=Yn.slice().sort((P,V)=>P.doctorOrder-V.doctorOrder).map((P)=>P.id),Sn=Yn.filter((P)=>("runtime"in P)).slice().sort((P,V)=>P.runtime.order-V.runtime.order).map((P)=>({id:P.id,displayName:"displayName"in P.runtime?P.runtime.displayName:P.displayName,flags:P.runtime.flags,legacyFlags:"legacyFlags"in P.runtime?P.runtime.legacyFlags:[],description:P.runtime.description,legacyTopLevelFlags:P.runtime.legacyTopLevelFlags})),$t=Yn.slice().sort((P,V)=>P.install.order-V.install.order).map((P)=>({id:P.id,...P.install})).map(({order:P,...V})=>V),gu=Object.fromEntries(Yn.map((P)=>[P.id,P.displayName]));function bt(P){return gu[P]}var yu=Sn.map((P)=>({flags:P.flags.join(", "),description:P.description})),hu=Sn.flatMap((P)=>P.flags.map((V)=>`cc-safety-net hook ${V}`)),Ji={name:"hook",description:"Run as an agent CLI hook (reads JSON from stdin)",usage:"hook INTEGRATION_FLAG",options:[...yu,{flags:"-h, --help",description:"Show this help"}],examples:hu};var Wi={name:"install",description:"Install CC Safety Net into a coding agent CLI",usage:"install [TARGET_FLAG]",options:[...$t.map((P)=>({flags:P.flag,description:`Install ${bt(P.id)} ${P.artifactKind}`})),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net install",...$t.map((P)=>`cc-safety-net install ${P.flag}`)]},Ki={name:"uninstall",description:"Uninstall CC Safety Net from a coding agent CLI",usage:"uninstall [TARGET_FLAG]",options:[...$t.map((P)=>({flags:P.flag,description:`Uninstall ${bt(P.id)} ${P.artifactKind}`})),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net uninstall",...$t.map((P)=>`cc-safety-net uninstall ${P.flag}`)]},Yi={name:"update",description:"Update every installed CC Safety Net integration to the latest version",usage:"update",options:[{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net update"]};var Zi={name:"logs",description:"Browse audit log entries recorded by hooks",usage:"logs [options]",options:[{flags:"--id",argument:"<id>",description:"Show one entry from retained history by its 16-character id (not guaranteed once it is older than the configured retention)"},{flags:"--limit",argument:"<n>",description:"Maximum entries to print",default:"20"},{flags:"--since",argument:"<days>",description:"Only include entries newer than this many days (max: the configured audit retention, 1-365)",default:"30"},{flags:"--agent",argument:"<name>",description:"Filter by agent name"},{flags:"--rule",argument:"<ruleId>",description:"Filter by rule id"},{flags:"--session",argument:"<id>",description:"Filter by session id"},{flags:"--project",argument:"<path>",description:"Filter by project path"},{flags:"--suspect",description:"Only denials that look like false positives"},{flags:"--all",description:"Include allow entries"},{flags:"--prune-legacy",description:"Permanently delete all legacy root-level logs; nested logs are untouched"},{flags:"--dry-run",description:"With --prune-legacy, report what would be deleted and delete nothing"},{flags:"--json",description:"Output entries as JSON"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net logs --id 3fa9c2d1a70e8b42","cc-safety-net logs --agent claude-code","cc-safety-net logs --project . --since 7","cc-safety-net logs --suspect --since 7","cc-safety-net logs --json","cc-safety-net logs --prune-legacy --dry-run","cc-safety-net logs --prune-legacy"]};var Xn={name:"policy",description:"Check and apply project or user policy proposals",usage:"policy <subcommand>",subcommands:[{usage:"check <file>",description:"Validate a policy proposal and print its diff"},{usage:"apply <file>",description:"Apply a proposal after confirming in a terminal"}],options:[{flags:"-g, --global",description:"Use the user-scope policy instead of the project one"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net policy check proposal.json","cc-safety-net policy apply proposal.json","cc-safety-net policy apply proposal.json --global"]};var no=[{flags:"--ref",argument:"<ref>",description:"Use a branch, tag, or commit"},{flags:"--only",argument:"<rulebook...>",description:"Add only these repository rulebooks"},{flags:"-g, --global",description:"Use user-scope rule config"},{flags:"-h, --help",description:"Show this help"}],ro=["cc-safety-net rule add project-rules","cc-safety-net rule add acme/safety-rules","cc-safety-net rule add acme/safety-rules --only aws gcloud","cc-safety-net rule add acme/safety-rules --ref v2 --only aws","cc-safety-net rule add --only terraform aws"],fn={name:"rule",description:"Manage CC Safety Net rule config and rulebook sources",usage:"rule <subcommand>",subcommands:[{usage:"init [--example]",description:"Create inert rule config"},{usage:"add [source] [--ref <ref>] [--only <rulebook...>]",description:"Add rulebook sources and sync"},{usage:"remove <source>",description:"Remove a rulebook source and sync"},{usage:"update [source]",description:"Re-fetch and vendor remote rulebooks"},{usage:"sync",description:"Deprecated: migrate lock and cache leftovers"},{usage:"list",description:"List active rulebooks"},{usage:"wrapper add <command>",description:"Trust a transparent command wrapper"},{usage:"wrapper remove <command>",description:"Remove a transparent command wrapper"},{usage:"wrapper list",description:"List transparent command wrappers"},{usage:"migrate [--cleanup]",description:"Migrate legacy inline rules"},{usage:"doc",description:"Print the rulebook authoring guide"},{usage:"verify",description:"Validate rule config files"}],options:[{flags:"-g, --global",description:"Use user-scope rule config"},{flags:"--cleanup",description:"Delete legacy files after rule migrate verifies them"},{flags:"--delete-source",description:"Delete clean local source directory on remove"},{flags:"--example",description:"Create an inactive example rulebook with rule init"},...no.slice(0,2),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net rule init","cc-safety-net rule init --example","cc-safety-net rule wrapper add rtk",...ro,"cc-safety-net rule update","cc-safety-net rule migrate --cleanup","cc-safety-net rule verify"]};var Xi={name:"status",description:"Show what the runtime is enforcing right now",usage:"status",options:[{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net status"]};var Qi={name:"statusline",description:"Print status line with mode indicators for shell integration",usage:"statusline --claude-code",options:[{flags:"-cc, --claude-code",description:"Print status line for Claude Code"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net statusline -cc","cc-safety-net statusline --claude-code"]};var Qn=[Xi,qi,Zi,Vi,fn,Xn,Wi,Yi,Ki,Ji,zi,Qi];function vu(P){return P.aliases??[]}function er(P){let V=P.toLowerCase();return Qn.find((z)=>z.name.toLowerCase()===V||vu(z).some((Q)=>Q.toLowerCase()===V))}import{basename as bu}from"node:path";function tr(P,V=7,z=O(P)){let Q=Date.now()-V*24*60*60*1000,te=[],ne=new Set,ce=0,ye,he,be,Le;if(z)B(P,z);let Ie={count:0},Xe=z?Yt(z,Ie):[];for(let xe of Xe)for(let at of pn(xe,Ie)){if(at.decision==="allow")continue;let ct=new Date(at.ts).getTime();if(ct>=Q){if(ce++,ne.add(at.sessionId??bu(xe,".jsonl")),he===void 0||ct<=he)ye=at.ts,he=ct;if(Le===void 0||ct>Le)be=at.ts,Le=ct;Lu(te,at,ct)}}let et=te.map((xe)=>({timestamp:xe.ts,command:xe.command,reason:xe.reason,relativeTime:Fi(new Date(xe.ts))}));return{totalBlocked:ce,sessionCount:ne.size,recentEntries:et,oldestEntry:ye,newestEntry:be,unreadable:Ie.count}}function Lu(P,V,z){let Q=P.findIndex((te)=>z>new Date(te.ts).getTime());if(Q===-1){if(P.length<3)P.push(V);return}if(P.splice(Q,0,V),P.length>3)P.pop()}import{dirname as Tu}from"node:path";import{dirname as wu,join as ku,resolve as xu}from"node:path";var Cu="config.json";function Rt(P,V,z,Q){g(Su(P),`${JSON.stringify(V,null,2)}
`,z,Q)}function Su(P){return typeof P==="string"?re(P):P}function io(P){return{errors:ie(Pu(P),": "," "),ruleNames:new Set(_e(P).map((V)=>V.toLowerCase()))}}var Ru="must match pattern (letters, numbers, hyphens, underscores; max 64 chars)",es="must match pattern (letters, numbers, hyphens, underscores)";function Pu(P){if(!ts(P))return[e([],"Config must be an object")];return[...P.version===1?[]:[e(["version"],"must be 1")],...Eu(P.rules)]}function Eu(P){if(P===void 0)return[];if(!Array.isArray(P))return[e(["rules"],"must be an array")];return[...P.flatMap((V,z)=>ts(V)?Du(V,["rules",z]):[e(["rules",z],"must be an object")]),...Te(P)]}function Du(P,V){return[...oo(P.name,[...V,"name"],"required string",l,Ru),...oo(P.command,[...V,"command"],"required string",w,es),...P.subcommand===void 0?[]:oo(P.subcommand,[...V,"subcommand"],"must be a string if provided",w,es),...Au(P.block_args,[...V,"block_args"]),..._u(P.reason,[...V,"reason"]),...P.intent===void 0||Ce(P.intent)?[]:[e([...V,"intent"],we)]]}function oo(P,V,z,Q,te){if(typeof P!=="string")return[e(V,z)];return Q.test(P)?[]:[e(V,te)]}function Au(P,V){if(!Array.isArray(P))return[e(V,"required array")];if(P.length===0)return[e(V,"must have at least one element")];return P.flatMap((z,Q)=>{if(typeof z!=="string")return[e([...V,Q],"must be a string")];return z===""?[e([...V,Q],"must not be empty")]:[]})}function _u(P,V){if(typeof P!=="string")return[e(V,"required string")];if(P==="")return[e(V,"must not be empty")];return P.length>T?[e(V,`must be at most ${T} characters`)]:[]}function ts(P){return!!P&&typeof P==="object"&&!Array.isArray(P)}function so(P){let V=ns(P);if(!V.ok)return V.result;return io(V.parsed)}function ns(P){let V=[],z=new Set;try{let Q=typeof P==="string"?re(P):P,te=n(Q);if(te===null)return V.push(`File not found: ${Q.path}`),{ok:!1,result:{errors:V,ruleNames:z}};if(!te.trim())return V.push("Config file is empty"),{ok:!1,result:{errors:V,ruleNames:z}};return{ok:!0,parsed:JSON.parse(te)}}catch(Q){if(Q instanceof r)return V.push(Q.message),{ok:!1,result:{errors:V,ruleNames:z}};let te=Q instanceof Error?Q.message:String(Q);return V.push(Q instanceof SyntaxError?"Invalid JSON":te),{ok:!1,result:{errors:V,ruleNames:z}}}}function rs(P){return xu(P,".safety-net.json")}function Jt(P){let V=ns(P);if(!V.ok)return V.result;let z=Ve(V.parsed);return{errors:z.errors,ruleNames:z.sources}}function nr(P,V={}){return ku(wu(Se(P,V)),Cu)}function os(P,V,z){let Q;try{if(n(V)===null)return{path:P,exists:!1,valid:!1,ruleCount:0};Q=Jt(V),Q.errors.push(...j(P,z))}catch(te){if(!(te instanceof r))throw te;Q={errors:[te.message],ruleNames:new Set}}return{path:P,exists:!0,valid:Q.errors.length===0,ruleCount:Q.ruleNames.size,...Q.errors.length>0?{errors:Q.errors}:{}}}function $u(P,V){return{source:V,name:P.name,command:P.command,subcommand:P.subcommand,blockArgs:[...P.block_args],reason:P.reason}}function is(P,V){let z=M(P),Q=F(V),te=Tu(z),ne=J(P,{cwd:V,userConfigPath:z,projectConfigPath:Q,userConfigDir:te}),ce=K(P,{cwd:V,userConfigPath:z,projectConfigPath:Q,userConfigDir:te}),ye=new Map(ne.rulebooks.flatMap((he)=>he.rules.map((be)=>[be,he.source])));return{userConfig:os(z,ce.userConfigTarget,ce.userScope),projectConfig:os(Q,ce.projectConfigTarget,ce.projectScope),effectiveRules:ne.rules.map((he)=>$u(he,ye.get(he.name)??"project"))}}var Iu=[{flag:o.level,description:"Safety level preset: standard, strict, or paranoid",defaultBehavior:"standard"},{flag:o.strict,description:"Legacy; equivalent to safety.overrides.fail_closed",defaultBehavior:"permissive"},{flag:o.paranoid,description:"Legacy; equivalent to safety.overrides.paranoid_rm and paranoid_interpreters",defaultBehavior:"off"},{flag:o.paranoidRm,description:"Legacy; equivalent to safety.overrides.paranoid_rm",defaultBehavior:"off"},{flag:o.paranoidInterpreters,description:"Legacy; equivalent to safety.overrides.paranoid_interpreters",defaultBehavior:"off"},{flag:o.worktree,description:"Allow local git discards in linked worktrees",defaultBehavior:"off"},{flag:o.debug,description:"Print diagnostic messages to stderr",defaultBehavior:"off"},{flag:o.auditScope,description:"Command decisions recorded: all, or blocked (privacy-minimizing, denials only)",defaultBehavior:"all"}];function ss(P){return[...Iu.map((V)=>({name:V.flag.name,value:ve(V.flag,P.env),isSet:Qe(V.flag,P.env),legacyName:V.flag.legacyName,legacyValue:V.flag.legacyName?P.env.get(V.flag.legacyName):void 0,legacyIsSet:V.flag.legacyName?P.env.get(V.flag.legacyName)!==void 0:void 0,description:V.description,defaultBehavior:V.defaultBehavior})),{name:"CC_SAFETY_NET_HOME",value:P.env.get("CC_SAFETY_NET_HOME"),isSet:P.env.get("CC_SAFETY_NET_HOME")!==void 0,description:"Override user-scope config/cache directory",defaultBehavior:"~/.cc-safety-net"}]}var as={error:0,warning:1,info:2},Ou=["policy","config","audit"];function Fu(P){return P.map((V)=>{if(V==="ownership")return"is not owned by the current user";if(V==="permissions")return"has unsafe permissions";if(V==="symlink")return"is a symbolic link";return"is not a directory"}).join(" and ")}var ju=[{derive:(P)=>P.hooks.length>0&&P.hooks.every((V)=>!V.configured)?[{checkId:"integration.none-configured",severity:"error",title:"No integration configured",detail:"CC Safety Net is not connected to any supported coding-agent integration.",fixHint:"Run `cc-safety-net install` and configure at least one integration."}]:[]},{derive:(P)=>P.hooks.filter((V)=>V.inspectionStatus==="failed").map((V)=>{let z=bt(V.platform);return{checkId:"integration.inspection-failed",severity:"error",title:`${z} inspection failed`,detail:`Doctor could not verify the ${z} integration configuration.`,fixHint:`Correct the reported ${z} configuration error, then run \`cc-safety-net doctor\` again.`,integration:V.platform}})},{derive:(P)=>P.userConfig.exists&&!P.userConfig.valid?[{checkId:"config.user-invalid",severity:"error",title:"User configuration is invalid",detail:"Doctor could not load a valid user rules configuration.",fixHint:"Run `cc-safety-net rule verify`, correct the reported error, then rerun doctor.",path:P.userConfig.path}]:[]},{derive:(P)=>P.projectConfig.exists&&!P.projectConfig.valid?[{checkId:"config.project-invalid",severity:"error",title:"Project configuration is invalid",detail:"Doctor could not load a valid project rules configuration.",fixHint:"Run `cc-safety-net rule verify`, correct the reported error, then rerun doctor.",path:P.projectConfig.path}]:[]},{derive:(P)=>P.configState.state==="degraded"?[{checkId:"config.runtime-degraded",severity:"warning",title:"Runtime is enforcing a fallback configuration",detail:`The rejected candidate configuration is not active: ${P.configState.reason}`,fixHint:"Fix the file named in the reason, or run `cc-safety-net rule update` to vendor a remote source, then rerun doctor."}]:[]},{derive:(P)=>P.v2Leftovers&&P.v2Leftovers.length>0?[{checkId:"config.v2-leftovers",severity:"info",title:"Rulebook lock and cache leftovers detected",detail:`Files an earlier version left behind are no longer read: ${P.v2Leftovers.join(", ")}.`,fixHint:"Run `cc-safety-net rule sync` (add `--global` for user scope) to migrate them, then rerun doctor."}]:[]},{derive:(P)=>{let V=P.environment.find((z)=>z.name==="CC_SAFETY_NET_AUDIT_SCOPE");return He(V?.value)==="invalid"?[{checkId:"environment.audit-scope-invalid",severity:"warning",title:"Audit scope value is invalid",detail:"CC_SAFETY_NET_AUDIT_SCOPE is not `all` or `blocked`, so allowed command decisions are not recorded.",fixHint:"Set CC_SAFETY_NET_AUDIT_SCOPE to `all` or `blocked`, then restart the integration."}]:[]}},...Ou.map((P)=>({derive:(V)=>V.posture.directories.filter((z)=>z.kind===P&&z.status==="unsafe").map((z)=>({checkId:`posture.${P}-directory-unsafe`,severity:"error",title:`${P[0]?.toUpperCase()}${P.slice(1)} directory is unsafe`,detail:`The ${P} directory ${Fu(z.issues)}.`,fixHint:"Ensure this is a real directory owned by the current user with no group or other write access, then rerun doctor.",...z.path?{path:z.path}:{}}))})),{derive:(P)=>{let V=[...P.effectiveSafety.weakenedRuleOverrides].sort();return V.length>0?[{checkId:"posture.rule-overrides-weaken-preset",severity:"warning",title:"Rule overrides weaken the selected preset",detail:`Explicit overrides disable rules the resolved preset would enable: ${V.join(", ")}.`,fixHint:`Remove these \`off\` overrides or set them to \`on\`: ${V.join(", ")}.`}]:[]}}];function ls(P){return ju.flatMap((V,z)=>V.derive(P).map((Q,te)=>({finding:Q,catalogOrder:z,occurrence:te}))).sort((V,z)=>as[V.finding.severity]-as[z.finding.severity]||V.catalogOrder-z.catalogOrder||V.occurrence-z.occurrence).map((V)=>V.finding)}function Ut(){return Boolean(process.stdout.isTTY&&!process.env.NO_COLOR)}var Nu=(P)=>Ut()?`\x1B[32m${P}\x1B[0m`:P,Mu=(P)=>Ut()?`\x1B[33m${P}\x1B[0m`:P,Hu=(P)=>Ut()?`\x1B[34m${P}\x1B[0m`:P,Uu=(P)=>Ut()?`\x1B[36m${P}\x1B[0m`:P,Gu=(P)=>Ut()?`\x1B[31m${P}\x1B[0m`:P,Bu=(P)=>Ut()?`\x1B[2m${P}\x1B[0m`:P,qu=(P)=>Ut()?`\x1B[1m${P}\x1B[0m`:P,dt={green:Nu,yellow:Mu,blue:Hu,cyan:Uu,red:Gu,dim:Bu,bold:qu},Vu="\x1B[0m",zu=[39,82,198,226,208,51,196,46,201,214,93,154,220,27,49,190,200,33,129,227,45,160,63,118,123,202];function Ju(P){let V=P;return()=>(V=(V*1664525+1013904223)%4294967296,V/4294967296)}function Wu(P){let V=[...zu],z=Ju(P);for(let Q=V.length-1;Q>0;Q--){let te=Math.floor(z()*(Q+1)),ne=V[Q];V[Q]=V[te],V[te]=ne}return V}function Ku(P,V=0){if(!Ut())return"";let z=Wu(V);return`\x1B[38;5;${z[P%z.length]}m`}function cs(P,V,z=0){if(!Ut())return`"${P}"`;return`${Ku(V,z)}"${P}"${Vu}`}function rr(P){return P==="default"?"built-in default":`${P} policy`}var Yu=new RegExp("\x1B\\[[0-9;]*m","g"),ao=(P)=>P.replace(Yu,"").length;function Zt(P){let V=(P.headers??P.rows[0]??[]).map((ce,ye)=>{let he=Math.max(...P.rows.map((be)=>ao(be[ye]??"")));return Math.max(ao(ce),he)}),z=(ce,ye)=>ce+" ".repeat(Math.max(0,ye-ao(ce))),Q=(ce,ye)=>ye[0]+V.map((he)=>ce.repeat(he+2)).join(ye[1])+ye[2],te=(ce)=>`│ ${ce.map((ye,he)=>z(ye,V[he]??0)).join(" │ ")} │`,ne=P.headers?[`   ${te(P.headers)}`,`   ${Q("─",["├","┼","┤"])}`]:[];return[`   ${Q("─",["┌","┬","┐"])}`,...ne,...P.rows.map((ce)=>`   ${te(ce)}`),`   ${Q("─",["└","┴","┘"])}`].join(`
`)}function ds(P){let V=[];V.push("Hook Integration"),V.push(Zu(P));let z=[],Q=[];for(let te of P){let ne=bt(te.platform);if(te.errors&&te.errors.length>0)for(let ce of te.errors)if(te.configured)z.push({platform:ne,message:ce});else Q.push({platform:ne,message:ce})}for(let te of z)V.push(`   Warning (${te.platform}): ${te.message}`);for(let te of Q)V.push(dt.red(`   Error (${te.platform}): ${te.message}`));return V.join(`
`)}function Zu(P){let V=["Platform","Discovery","Configuration","Inspection"],z=P.map((Q)=>{let te=bt(Q.platform);if(Q.inspectionStatus==="not-inspected"){let he=dt.dim("Not inspected");return[te,he,he,he]}let ne=Q.detected?dt.green("Detected"):Q.inspectionStatus==="failed"?dt.red("Unknown"):dt.dim("Not detected"),ce=Q.configured?dt.green("Configured"):Q.detected?dt.yellow("Not configured"):Q.inspectionStatus==="failed"?dt.red("Unknown"):dt.dim("Not applicable"),ye=Q.inspectionStatus==="verified"?dt.green("Verified"):Q.inspectionStatus==="failed"?dt.red("Failed"):dt.dim("Not applicable");return[te,ne,ce,ye]});return Zt({headers:V,rows:z})}function us(P){let z=["Guard Engine Verification",`   Synthetic self-test: ${P.failed>0?dt.red(`${P.passed}/${P.total} FAIL`):dt.green(`${P.passed}/${P.total} passed`)}`],Q=P.results.filter((te)=>!te.passed);if(Q.length>0){z.push(""),z.push(dt.red("   Failures:"));for(let te of Q)z.push(dt.red(`   • ${te.description}`)),z.push(dt.red(`     expected ${te.expected}, got ${te.actual}`))}return z.join(`
`)}function Xu(P){if(P.length===0)return"   (no custom rules)";let V=["Source","Name","Command","Block Args"],z=P.map((Q)=>[Q.source,Q.name,Q.subcommand?`${Q.command} ${Q.subcommand}`:Q.command,Q.blockArgs.join(", ")]);return Zt({headers:V,rows:z})}function ps(P){let V=[];if(V.push("Configuration"),V.push(Qu(P.userConfig,P.projectConfig)),V.push(""),P.effectiveRules.length>0)V.push(`   Effective rules (${P.effectiveRules.length} total):`),V.push(Xu(P.effectiveRules));else V.push("   Effective rules: (none - using built-in rules only)");return V.join(`
`)}function Qu(P,V){let z=["Scope","Status"],Q=(ne)=>{if(!ne.exists)return dt.dim("N/A");if(!ne.valid)return dt.red(`Invalid (${ne.errors?.[0]??"unknown error"})`);return dt.green("Configured")},te=[["User",Q(P)],["Project",Q(V)]];return Zt({headers:z,rows:te})}function fs(P){let V=[];return V.push("Environment"),V.push(ep(P)),V.join(`
`)}function ms(P){let V=P.effectiveSafety.policyScopes,z=["Effective Safety",`   Selected preset: ${P.effectiveSafety.selectedPreset}${V?` (${rr(V.levelScope)})`:""}`,`   Effective: ${P.effectiveSafety.level}`],Q=[["fail_closed","fail_closed"],["paranoid_rm","paranoid_rm"],["paranoid_interpreters","paranoid_interpreters"]];for(let[te,ne]of Q){let ce=P.effectiveSafety.capabilities[te],ye=ce.enabled?dt.green("ON"):dt.dim("OFF"),he=ce.sources.length>0?` (${ce.sources.join(", ")})`:"";z.push(`   ${ne}: ${ye} via ${ce.source}${he}`)}if(V&&V.weakenings.length>0){z.push("   Project policy deltas:");for(let te of V.weakenings)z.push(`      ${te}`)}z.push(`   Stored rule customizations: ${P.effectiveSafety.ruleCounts.stored}`),z.push(`   Effective rule customizations: ${P.effectiveSafety.ruleCounts.effective}`);for(let[te,ne]of Object.entries(P.effectiveSafety.ruleOverrides))z.push(`   ${te}: ${ne}`);return z.join(`
`)}function gs(P){let V=["Findings"];if(P.length===0)return V.push("   No findings from inspected doctor facts."),V.join(`
...
`)}function ep(P){let V=["Variable","Status","Legacy"],z=P.map((Q)=>{let te=Q.isSet?dt.green("✓"):dt.dim("✗"),ne=Q.legacyName&&Q.legacyIsSet?`${Q.legacyName} ${dt.green("✓")}`:Q.legacyName??"";return[Q.name,te,ne]});return Zt({headers:V,rows:z})}function ys(P){let V=[];if(P.totalBlocked===0)V.push("Recent Activity"),V.push("   No blocked commands in the last 7 days"),V.push("   Tip: This is normal for new installations");else V.push(`Recent Activity · last 7 days (${P.totalBlocked} blocked / ${P.sessionCount} sessions)`),V.push(tp(P.recentEntries));if(P.unreadable>0)V.push(`   Warning: ${P.unreadable} audit log ${P.unreadable===1?"source":"sources"} could not be read; this summary is incomplete`);return V.join(`
`)}function tp(P){let V=["Time","Command"],z=P.map((Q)=>{let te=Ct(Q.command.replace(/\r\n|\r|\n/g," ↵ ").replace(/\t/g," ")),ne=te.length>40?`${te.slice(0,37)}...`:te;return[Q.relativeTime,ne]});return Zt({headers:V,rows:z})}function hs(P){let V=[];if(V.push("Update Check"),P.latestVersion===null&&!P.error)return V.push(or([["Status",dt.dim("Skipped")],["Installed",P.currentVersion]])),V.join(`
...
`)}function or(P){return Zt({rows:P})}function vs(P){let V=[];return V.push("System Info"),V.push(np(P)),V.join(`
`)}function np(P){let V=["Component","Version"],z=(ne)=>{if(ne===null)return dt.dim("not found");return ne},te=[{label:"cc-safety-net",value:P.version},...Zn.map((ne)=>({label:bt(ne),value:P.versions[ne]??null})),{label:"Node.js",value:P.nodeVersion},{label:"npm",value:P.npmVersion},{label:"Bun",value:P.bunVersion},{label:"Platform",value:P.platform}].map((ne)=>[ne.label,z(ne.value)]);return Zt({headers:V,rows:te})}function bs(P){if(P.findings.length===0)return dt.green(`
...
${P.findings.length} ${Q}: ${z.join(", ")}.`;if(V.error>0)return dt.red(te);if(V.warning>0)return dt.yellow(te);return dt.blue(te)}import{lstatSync as rp}from"node:fs";import{dirname as lo}from"node:path";function co(P,V){try{let z=rp(V);if(z.isSymbolicLink())return{kind:P,path:V,status:"unsafe",issues:["symlink"]};if(!z.isDirectory())return{kind:P,path:V,status:"unsafe",issues:["not-directory"]};if(process.platform==="win32"||typeof process.getuid!=="function")return{kind:P,path:V,status:"unknown",issues:[]};let Q=[...z.uid!==process.getuid()?["ownership"]:[],...(z.mode&18)!==0?["permissions"]:[]];return{kind:P,path:V,status:Q.length>0?"unsafe":"safe",issues:Q}}catch(z){if(typeof z==="object"&&z!==null&&"code"in z&&z.code==="ENOENT")return{kind:P,path:V,status:"not-applicable",issues:[]};return{kind:P,path:V,status:"unknown",issues:[]}}}function Ls(P,V){let z=O(P);return{directories:[co("policy",lo(lo(V))),co("config",lo(V)),...z?[co("audit",z)]:[{kind:"audit",status:"unknown",issues:[]}]]}}import{spawn as op}from"node:child_process";import{existsSync as ws}from"node:fs";import{delimiter as ip,extname as sp,join as ap}from"node:path";import{stripVTControlCharacters as ks}from"node:util";var Cs="2.4.2",lp=5000,cp="_CC_SAFETY_NET_TEST_SPAWN_PLATFORM";function wt(){return Cs}function uo(P,V){let z=P[V];if(z)return z;let Q=Object.keys(P).find((te)=>te.toLowerCase()===V.toLowerCase()&&!!P[te]);return Q?P[Q]:z}function dp(P){return(uo(P,"PATHEXT")||".COM;.EXE;.BAT;.CMD").split(";").filter((V)=>V.length>0)}function up(P,V){let z=sp(P)?[P]:[...dp(V).map((Q)=>`${P}${Q}`),P];if(P.includes("/")||P.includes("\\"))return z.find((Q)=>ws(Q))??P;return(uo(V,"PATH")??"").split(ip).flatMap((Q)=>z.map((te)=>ap(Q,te))).find((Q)=>ws(Q))??P}function xs(P){if(!/[\s"&|<>^]/.test(P))return P;return`"${P.replace(/"/g,'""')}"`}function Xt(P,V){let[z,...Q]=P,te=V[cp]==="win32"?"win32":process.platform;if(!z||te!=="win32")return{cmd:z??"",args:Q};let ne=up(z,V);if(!/\.(?:bat|cmd)$/i.test(ne))return{cmd:ne,args:Q};return{cmd:uo(V,"COMSPEC")??"cmd.exe",args:["/d","/c",["call",xs(ne),...Q.map(xs)].join(" ")]}}var mn=async(P,V=lp)=>{let z=await pp(P,{timeoutMs:V});if(z.code!==0)return null;return ks(z.stdout).trim()||ks(z.stderr).trim()||null};function pp(P,V){let[z,...Q]=P;if(!z)return Promise.resolve({code:null,stdout:"",stderr:""});return new Promise((te)=>{try{let ne=Xt([z,...Q],process.env),ce=op(ne.cmd,ne.args,{stdio:["ignore","pipe","pipe"]}),ye=!1,he="",be="";ce.stdout.on("data",(Xe)=>{he+=Xe.toString()}),ce.stderr.on("data",(Xe)=>{be+=Xe.toString()});let Le=(Xe)=>{if(ye)return;ye=!0,clearTimeout(Ie),te(Xe)},Ie=setTimeout(()=>{ce.kill(),Le({code:null,stdout:he,stderr:be})},V.timeoutMs);ce.on("close",(Xe)=>{Le({code:Xe,stdout:he,stderr:be})}),ce.on("error",()=>{Le({code:null,stdout:he,stderr:be})})}catch{te({code:null,stdout:"",stderr:""})}})}function ir(P){if(!P)return null;let V=/Claude Code\s+(\d+\.\d+\.\d+)/i.exec(P);if(V)return V[1]??null;let z=/v?(\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?)/i.exec(P);if(z)return z[1]??null;return P.split(`
`)[0]?.trim()||null}async function sr(P=mn){let[V,z,Q,te,ne,ce]=await Promise.all([Promise.all($t.map(async(ye)=>[ye.id,ir(await P([...ye.probeCommand]))])),P(["codex","plugin","list"],30000),P(["amp","plugins","list"],30000),P(["node","--version"]),P(["npm","--version"]),P(["bun","--version"])]);return{version:Cs,versions:Object.fromEntries(V),codexPluginListOutput:z,ampPluginListOutput:Q,nodeVersion:ir(te),npmVersion:ir(ne),bunVersion:ir(ce),platform:`${process.platform} ${process.arch}`}}function po(P,V){if(V==="dev")return!1;let z=P.split(".").map(Number),Q=V.split(".").map(Number),[te=0,ne=0,ce=0]=z,[ye=0,he=0,be=0]=Q;if(te!==ye)return te>ye;if(ne!==he)return ne>he;return ce>be}async function Wt(){let P=wt(),V=new AbortController,z=setTimeout(()=>V.abort(),3000);try{let Q=await fetch("https://registry.npmjs.org/cc-safety-net/latest",{signal:V.signal});if(!Q.ok)return{currentVersion:P,latestVersion:null,updateAvailable:!1,error:`npm registry returned ${Q.status}`};let te=await Q.json(),ne=po(te.version,P);return{currentVersion:P,latestVersion:te.version,updateAvailable:ne}}catch(Q){return{currentVersion:P,latestVersion:null,updateAvailable:!1,error:Q instanceof Error?Q.message:"Network error"}}finally{clearTimeout(z)}}import*as Ts from"node:readline";var Es=(P)=>`\x1B[${P}B`,fp=(P)=>`\x1B[${P}A`;var Ss=["░","▒","▓","╱","╲","┃","━","┏","┓","┗","┛","╋"];function mp(P){return new Promise((V)=>setTimeout(V,P))}function gp(P,V,z){if(!z)return V(P);if(z.aborted)return Promise.resolve();return new Promise((Q,te)=>{let ne=()=>z.removeEventListener("abort",ce),ce=()=>{ne(),Q()};z.addEventListener("abort",ce,{once:!0}),V(P).then(()=>{ne(),Q()},(ye)=>{ne(),te(ye)})})}function ar(P){return Math.max(0,Math.min(1,P))}function gn(P){return Math.max(0,Math.min(255,Math.round(P)))}function fo(P){return P<=0.0031308?12.92*P:1.055*P**0.4166666666666667-0.055}function yp(P,V,z){let Q=z*Math.PI/180,te=V*Math.cos(Q),ne=V*Math.sin(Q),ce=(P+0.3963377774*te+0.2158037573*ne)**3,ye=(P-0.1055613458*te-0.0638541728*ne)**3,he=(P-0.0894841775*te-1.291485548*ne)**3;return{blue:gn(fo(ar(-0.0041960863*ce-0.7034186147*ye+1.707614701*he))*255),green:gn(fo(ar(-1.2684380046*ce+2.6097574011*ye-0.3413193965*he))*255),red:gn(fo(ar(4.0767416621*ce-3.3077115913*ye+0.2309699292*he))*255)}}function mo(P,V){let z=(V*P*180/Math.PI%360+360)%360;return yp(0.72,0.15,z)}function Ds(P,V=0.1){let z=mo(V,P);return`\x1B[38;2;${z.red};${z.green};${z.blue}m`}function hp(P,V){return{blue:gn(P.blue+(255-P.blue)*V),green:gn(P.green+(255-P.green)*V),red:gn(P.red+(255-P.red)*V)}}function As(P,V,z){let Q=Math.imul(P+2654435769,2246822507)^Math.imul(V+3266489909,668265263)^Math.imul(z+374761393,2654435761),te=Q^Q>>>15,ne=Math.imul(te,739982445),ce=ne^ne>>>12,ye=Math.imul(ce,695872825);return((ye^ye>>>15)>>>0)/4294967296}function vp(P,V,z){let Q=Math.floor(As(P,V,z)*Ss.length);return Ss[Q]??"░"}function Rs(P){let V=ar(P);return V*V*V*(V*(V*6-15)+10)}function bp(P){if(P.length===0)return"";let V=[],z=!1,Q="";for(let te of P){let ne=`${te.red};${te.green};${te.blue}`;if(te.bold!==z)V.push(te.bold?"\x1B[1m":"\x1B[22m"),z=te.bold;if(ne!==Q)V.push(`\x1B[38;2;${ne}m`),Q=ne;V.push(te.character)}return`${V.join("")}\x1B[22m\x1B[39m`}function Lp(P,V,z,Q,te){return P.map((ne,ce)=>({...mo(z,Q+V+ce/te),bold:!1,character:ne}))}function wp(P,V,z,Q,te,ne,ce,ye){let he=Math.max(1,Q*0.75),be=Math.min(1,z/he),Le=te*Rs(be),Ie=Math.max(0,(z-he)/Math.max(1,Q-he)),Xe=(1-Rs(z/Q))*ye*2,et=0.35*Math.max(0,1-Ie*2),xe=be>=1,at=Math.min(P.length,Math.ceil(Le+2+1));return P.slice(0,at).map((ct,lt)=>{let pt=mo(ne,ce+V+lt/ye+Xe),mt=lt+As(V,lt,7919)*2-1;if(mt>Le+2)return{...pt,bold:!1,character:" "};let gt=Le-mt,ut=0.8*Math.exp(-(gt*gt)/12.5),vt=Math.min(0.9,ut+et),Dt=!xe&&mt>Le-4;return{...hp(pt,vt),bold:vt>0.3,character:Dt?vp(V,lt,z):ct}})}function Ps(P){return`\x1B[?2026h${P.map((V,z)=>`\x1B8${z>0?Es(z):""}${bp(V)}`).join("")}\x1B[?2026l`}async function go(P,V={}){if(!P)return;let z=V.output??process.stdout,Q=V.sleep??mp,te=V.seed??0,ne=P.split(`
...
`.repeat(ne.length-1)}${fp(ne.length-1)}`:""}\x1B7`);try{for(let Le=1;Le<=he;Le+=1){if(V.signal?.aborted)break;z.write(Ps(ne.map((Ie,Xe)=>wp(Ie,Xe,Le,he,ce,0.1,te,3)))),await gp(be,Q,V.signal)}}finally{if(z.write(Ps(ne.map((Le,Ie)=>Lp(Le,Ie,0.1,te,3)))),z.write("\x1B8"),ne.length>1)z.write(Es(ne.length-1));z.write(`
\x1B[0m\x1B[?25h`)}}var _s=["┏━┛┏━┛  ┏━┛┏━┃┏━┛┏━┛━┏┛┃ ┃  ┏━ ┏━┛━┏┛","┃  ┃    ━━┃┏━┃┏━┛┏━┛ ┃ ━┏┛  ┃ ┃┏━┛ ┃ ","━━┛━━┛  ━━┛┛ ┛┛  ━━┛ ┛  ┛   ┛ ┛━━┛ ┛ "].join(`
`);function kp(P){return Boolean(P.isTTY)}async function Rn(P={}){let V=P.output??process.stdout;if(!kp(V))return;let z=P.input??process.stdin,Q={output:V,seed:P.seed??Math.random()*8192,sleep:P.sleep};if(!z.isTTY||typeof z.setRawMode!=="function"){await go(_s,Q);return}let te=new AbortController,ne=z.readableFlowing===!0,ce=z.isRaw===!0,ye=!1,he=(be,Le)=>{if(Le.ctrl&&Le.name==="c")ye=!0;if(ye||Le.name==="return"||Le.name==="enter")te.abort()};Ts.emitKeypressEvents(z),z.on("keypress",he),z.setRawMode(!0),z.resume();try{await go(_s,{...Q,signal:te.signal})}finally{if(z.off("keypress",he),z.setRawMode(ce),!ne)z.pause()}if(!ye)return;if(P.onInterrupt){P.onInterrupt();return}process.kill(process.pid,"SIGINT")}import{createHash as Pp}from"node:crypto";import{existsSync as Os}from"node:fs";import{dirname as lr,join as Fs}from"node:path";import{dirname as $s,join as xp,resolve as Cp}from"node:path";var Sp="rule.lock";function Rp(P){return xp($s(P),Sp)}function Is(P={}){return Cp(P.cwd??process.cwd(),".safety-net.json")}function Pt(P,V){let z=V.global?V.userConfigPath??M(P,V):V.projectConfigPath??F(V.cwd??process.cwd()),Q=V.global?Be(P,V):ze(z,V.cwd??process.cwd()),te=Rp(z);return{configDir:$s(z),configPath:z,lockPath:te,filesystemScope:Q,configTarget:i(Q,z),lockTarget:i(Q,te)}}var Ep="`cc-safety-net rule sync` is deprecated: rulebooks are live files that need no synchronization. This run only migrates the lock and cache an earlier version left behind.",Dp="cache",Ap="rulebooks";function js(P,V={}){let z=Pt(P,V),Q=i(z.filesystemScope,Ms(z.configDir)),te=n(z.lockTarget);if(console.log(Ep),te===null&&!Os(Q.path))return console.log(`No v2 lock or cache leftovers found in ${lr(z.configDir)}; nothing to migrate.`),0;let ne=Op(te),ce=m(z.configTarget);if(!ce.config&&(n(z.configTarget)!==null||ne.size>0))return console.error(`Cannot migrate: the rules config in ${lr(z.configDir)} is missing or unreadable while v2 leftovers remain. Restore rule.json, then re-run rule sync.`),1;let ye=ce.config?.rules??[];for(let he of ye.flatMap((be)=>_p(be,ne,z,Q,V.global===!0)))console.log(he);return N(z.lockTarget),tt(Q),console.log(`Removed the v2 lock and cache under ${lr(z.configDir)}.`),0}function Ns(P,V){return[...new Set([{cwd:V},{cwd:V,global:!0}].flatMap((z)=>{let Q=Pt(P,z);return[Q.lockPath,Ms(Q.configDir)]}))].filter((z)=>Os(z))}function _p(P,V,z,Q,te){if(!S(P))return[];let ne=A(P).name,ce=i(z.filesystemScope,I(z.configDir,ne)),ye=n(ce);if(ye!==null&&Tp(ye,ne))return[];let he=V.get(P),be=he?$p(he,ne,Q.path,z.filesystemScope):null;if(be===null)return[`Could not migrate ${P} from the v2 cache. Run \`cc-safety-net rule update ${P}${te?" --global":""}\` to vendor it.`];if(g(ce,be),ye!==null)return[`Restored ${P} from the v2 cache over an invalid file.`];return[`Vendored ${P} from the v2 cache.`]}function Tp(P,V){let z=me(P);return!("problem"in z)&&z.rulebook.name===V}function $p(P,V,z,Q){let te=Fs(z,Ap,`${Ip(P)}--${P.digest.replace("sha256:","").slice(0,12)}`,ue),ne=n(i(Q,te));if(ne===null||Np(ne)!==P.digest)return null;let ce=me(ne);if("problem"in ce||ce.rulebook.name!==V)return null;return ne}function Ms(P){return Fs(lr(P),Dp)}function Ip(P){return([P.owner,P.repo,P.display_ref,P.name].every((Q)=>typeof Q==="string"&&Q!=="")?`${P.owner}/${P.repo}#${P.display_ref}/${P.name}`:P.spec).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")||"rulebook"}function Op(P){let V=P===null?null:jp(P),z=Hs(V)&&Array.isArray(V.rulebooks)?V.rulebooks:[];return new Map(z.filter(Fp).map((Q)=>[Q.spec,Q]))}function Fp(P){return Hs(P)&&typeof P.spec==="string"&&typeof P.digest==="string"}function Hs(P){return!!P&&typeof P==="object"}function jp(P){try{return JSON.parse(P)}catch{return null}}function Np(P){return`sha256:${Pp("sha256").update(P).digest("hex")}`}var Us="\r\x1B[2K",Mp="\x1B[?25l",Hp="\x1B[39m",Up="\x1B[?25h",Gp=100,Bp=0.55,qp=80,Gs=["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"];function Vp(P){return new Promise((V)=>setTimeout(V,P))}async function cr(P,V={}){let z=V.output??process.stdout;if(!z.isTTY)return P;let Q=V.sleep??Vp,te=!1,ne=P.then((ye)=>(te=!0,ye),(ye)=>{throw te=!0,ye});if(await Promise.race([ne.then(()=>!0),Q(Gp).then(()=>!1)]))return ne;z.write(Mp);try{for(let ye=0;!te;ye+=1)z.write(`${Us}${Ds(ye*Bp)}${Gs[ye%Gs.length]}${Hp} ${V.loadingMessage??"Loading…"}`),await Promise.race([ne,Q(qp)]);return await ne}finally{z.write(`${Us}${Up}`)}}async function Pn(P,V,z,Q={}){let te=V();if(P)await z();if(P&&te.ready)await cr(te.ready,Q);return te.finish()}import{stripVTControlCharacters as zp}from"node:util";var dr="amp plugins list",Jp=/^\s*[✓✗]\s+cc-safety-net(?:\.ts)?\s+\(User Plugins\)\s+(\S+)\s*$/;function Bs(P){if(!P.ampPluginListOutput)return{platform:"amp",status:"n/a"};let V=zp(P.ampPluginListOutput).split(`
`).map((z)=>Jp.exec(z)?.[1]).find((z)=>z!==void 0);if(!V)return{platform:"amp",status:"n/a"};if(V!=="active")return{platform:"amp",status:"disabled",method:dr,configPath:dr,errors:[`Amp personal plugin cc-safety-net is ${V}; run "plugins: reload" in Amp or reinstall with install --amp`]};return{platform:"amp",status:"configured",method:dr,configPath:dr}}import{existsSync as Yp,readFileSync as Zp}from"node:fs";import{isAbsolute as Z1,join as Kp,relative as X1}from"node:path";function En(P){return Kp(P,".gemini","config","hooks.json")}var Xp=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*(?:--agy-cli|-ac)(\s|["']|$)/;function Qp(P){if(!P||typeof P!=="object"||Array.isArray(P))return[];return Object.values(P).flatMap((V)=>{if(!V||typeof V!=="object"||Array.isArray(V))return[];let z=V,Q=z.PreToolUse;if(!Array.isArray(Q))return[];return Q.flatMap((te)=>{if(!te||typeof te!=="object"||Array.isArray(te))return[];let ne=te.hooks;if(!Array.isArray(ne))return[];return ne.flatMap((ce)=>{if(!ce||typeof ce!=="object"||Array.isArray(ce))return[];let ye=ce.command;if(typeof ye!=="string"||!Xp.test(ye))return[];return[{command:ye,enabled:z.enabled!==!1}]})})})}function qs(P){let V=En(P.environment.home);if(!Yp(V))return{platform:"antigravity-cli",status:"n/a",configPath:V};let z;try{z=Qp(JSON.parse(Zp(V,"utf-8")))}catch(Q){return{platform:"antigravity-cli",status:"n/a",configPath:V,errors:[`Failed to parse Antigravity hooks config ${V}: ${Q instanceof Error?Q.message:String(Q)}`]}}if(z.some((Q)=>Q.enabled))return{platform:"antigravity-cli",status:"configured",method:"hook config",configPath:V};if(z.length>0)return{platform:"antigravity-cli",status:"disabled",method:"hook config",configPath:V};return{platform:"antigravity-cli",status:"n/a",configPath:V}}import{join as Vs}from"node:path";import{existsSync as e2,lstatSync as t2,readFileSync as n2}from"node:fs";function Ft(P,V=(z)=>z){if(!e2(P))return{kind:"missing"};try{return{kind:"ok",value:JSON.parse(V(n2(P,"utf-8")))}}catch{return{kind:"unreadable"}}}function Lt(P){try{return t2(P)}catch{return}}function ur(P,V){let z=Lt(V);if(!z)return{platform:P,status:"n/a",configPath:V};if(!z.isSymbolicLink()&&z.isDirectory())return;return{platform:P,status:"n/a",configPath:V,errors:[`${V} is a symlink or not a directory; move or remove it before installing`]}}function ft(P,V){return typeof P==="object"&&P!==null?P[V]:void 0}var yo="cc-safety-net@cc-marketplace";function zs(P){return Vs(P.home,".claude","plugins","installed_plugins.json")}function Js(P,V){let z=ft(ft(P,"plugins"),V);return Array.isArray(z)&&z.length>0}function pr(P,V){let z=Ft(zs(P));return z.kind==="ok"&&Js(z.value,V)}function ho(P){let V=zs(P),z=Ft(V);if(z.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(z.kind==="missing")return{platform:"claude-code",status:"n/a"};if(!Js(z.value,yo))return{platform:"claude-code",status:"n/a"};let Q=Vs(P.home,".claude","settings.json"),te=Ft(Q);if(te.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(!(te.kind==="ok"&&ft(ft(te.value,"enabledPlugins"),yo)===!0))return{platform:"claude-code",status:"disabled",method:"plugin config",configPath:Q,errors:[`${yo} is installed but not enabled in Claude Code`]};return{platform:"claude-code",status:"configured",method:"plugin config",configPath:V}}function Ws(P){return ho(P.environment)}function Ks(P){if(!P.codexPluginListOutput)return{platform:"codex",status:"n/a"};let V=P.codexPluginListOutput.split(`
`).find((z)=>z.includes("https://github.com/kenryu42/cc-safety-net.git"));if(!V)return{platform:"codex",status:"n/a"};if(!V.includes("installed,"))return{platform:"codex",status:"n/a"};if(!V.includes("installed, enabled"))return{platform:"codex",status:"disabled",method:"codex plugin list",configPath:"codex plugin list",errors:["Codex plugin line for https://github.com/kenryu42/cc-safety-net.git must contain installed, enabled."]};return{platform:"codex",status:"configured",method:"codex plugin list",configPath:"codex plugin list"}}import{existsSync as vr,readdirSync as r2,readFileSync as o2}from"node:fs";import{join as St}from"node:path";function At(P){let V="",z=0,Q=!1,te=!1,ne=-1;while(z<P.length){let ce=P[z],ye=P[z+1];if(te){V+=ce,te=!1,z++;continue}if(ce==='"'&&!Q){Q=!0,ne=-1,V+=ce,z++;continue}if(ce==='"'&&Q){Q=!1,V+=ce,z++;continue}if(ce==="\\"&&Q){te=!0,V+=ce,z++;continue}if(Q){V+=ce,z++;continue}if(ce==="/"&&ye==="/"){while(z<P.length&&P[z]!==`
`)z++;continue}if(ce==="/"&&ye==="*"){z+=2;while(z<P.length-1){if(P[z]==="*"&&P[z+1]==="/"){z+=2;break}z++}continue}if(ce===","){ne=V.length,V+=ce,z++;continue}if(ce==="}"||ce==="]"){if(ne!==-1){let he=V.slice(ne+1);if(/^\s*$/.test(he))V=V.slice(0,ne)+he}ne=-1,V+=ce,z++;continue}if(!/\s/.test(ce))ne=-1;V+=ce,z++}return V}function vo(P,V,z){let Q=V+1,te=!1;while(Q<P.length){if(te){te=!1,Q++;continue}if(P[Q]==="\\"){te=!0,Q++;continue}if(P[Q]==='"')return Q+1;Q++}throw Error(z)}function bo(P,V,z){let Q=P[V],te=Q==="["?"]":"}",ne=0,ce=V;while(ce<P.length){let ye=z.skipComment?.(P,ce)??ce;if(ye!==ce){ce=ye;continue}if(P[ce]==='"'){ce=vo(P,ce,z.stringError);continue}if(P[ce]===Q)ne++;if(P[ce]===te){if(ne--,ne===0)return ce}ce++}throw Error(z.bracketError)}function Zs(P,V){let z=P.lastIndexOf(`
`,V)+1;return/^[ \t]*/.exec(P.slice(z))?.[0]??""}function mr(P,V){let z=V.end+(/^\s*/.exec(P.slice(V.end))?.[0].length??0);if(P[z]===","){let ce=P[z+1]===`
...
`,Q-1),ne=te!==-1&&/^\s*$/.test(P.slice(te+1,Q))?te:Q;return`${P.slice(0,ne)}${P.slice(V.end)}`}function fr(P,V){if(P.startsWith("//",V)){let z=P.indexOf(`
`,V+2);return z===-1?P.length:z+1}if(P.startsWith("/*",V)){let z=P.indexOf("*/",V+2);return z===-1?P.length:z+2}return V}function Ys(P,V){let z=V;while(z<P.length){if(/\s/.test(P[z]??"")){z++;continue}let Q=fr(P,z);if(Q===z)return z;z=Q}return z}function Xs(P,V,z){let Q=0,te=0;while(te<P.length){let ne=fr(P,te);if(ne!==te){te=ne;continue}if(P[te]==='"'){let ce=vo(P,te,z.stringError);if(Q===1&&JSON.parse(P.slice(te,ce))===V){let ye=Ys(P,ce),he=Ys(P,ye+1);if(P[ye]===":"&&P[he]==="[")return{start:he,end:bo(P,he,{skipComment:fr,...z})}}te=ce;continue}if(P[te]==="{"||P[te]==="[")Q++;if(P[te]==="}"||P[te]==="]")Q--;te++}return}function Qs(P,V,z){let Q=[],te=V.start+1;while(te<V.end){let ne=fr(P,te);if(ne!==te){te=ne;continue}if(P[te]==='"'){let ce=vo(P,te,z),ye=JSON.parse(P.slice(te,ce));if(typeof ye==="string")Q.push({range:{start:te,end:ce},value:ye});te=ce;continue}te++}return Q}var jt="cc-safety-net@cc-marketplace",gr=["cc-marketplace","cc-safety-net"],ea=["_direct","copilot-safety-net"],ta=["cc-marketplace","safety-net"],na="safety-net@cc-marketplace";function yr(P,V){let z=V.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(`(^|[^a-z0-9-])${z}([^a-z0-9-]|$)`,"m").test(P??"")}function ra(P){return yr(P,"cc-safety-net@cc-marketplace")}function oa(P){return yr(P,"cc-marketplace")}function ia(P){return yr(P,"copilot-safety-net")}function sa(P){return yr(P,"safety-net@cc-marketplace")}function Lo(P){if(!P?.includes("cc-safety-net"))return!1;return/(^|\s)hook\s+(?:[^\s]+\s+)*(--copilot-cli|-cp)(\s|$)/.test(P)}function la(P,V){if(!P)return null;let z=P.match(/(\d+)\.(\d+)\.(\d+)/);if(!z)return null;let Q=[Number(z[1]),Number(z[2]),Number(z[3])];for(let te=0;te<V.length;te++){let ne=Q[te]??0,ce=V[te]??0;if(ne!==ce)return ne>ce}return!0}function i2(P){return la(P,[0,0,422])}function s2(P){return la(P,[1,0,8])}function Dn(P){return P.env.get("COPILOT_HOME")||St(P.home,".copilot")}function wo(P){return(P.hooks?.preToolUse??[]).some((z)=>{if(z.type!=="command")return!1;return Lo(z.command)||Lo(z.bash)||Lo(z.powershell)})}function hr(P){return P===void 0||typeof P==="string"}function a2(P){if(!P||typeof P!=="object"||Array.isArray(P))return!1;let V=P;if(V.disableAllHooks!==void 0&&typeof V.disableAllHooks!=="boolean")return!1;if(V.hooks===void 0)return!0;if(!V.hooks||typeof V.hooks!=="object"||Array.isArray(V.hooks))return!1;let z=V.hooks.preToolUse;if(z===void 0)return!0;return Array.isArray(z)&&z.every((Q)=>Q!==null&&typeof Q==="object"&&!Array.isArray(Q)&&hr(Q.type)&&hr(Q.command)&&hr(Q.bash)&&hr(Q.powershell))}function ko(P,V){try{let z=JSON.parse(At(o2(P,"utf-8")));if(!a2(z)){V?.push(`Invalid hook config ${P}: hooks.preToolUse must be an array of hook objects`);return}return z}catch(z){V?.push(`Failed to parse ${P}: ${z instanceof Error?z.message:String(z)}`);return}}function ca(P,V){try{return r2(P).filter((z)=>z.endsWith(".json")).sort((z,Q)=>z.localeCompare(Q))}catch(z){return V?.push(`Failed to read ${P}: ${z instanceof Error?z.message:String(z)}`),[]}}function l2(P,V){if(!vr(P))return[];let z=[];for(let Q of ca(P,V)){let te=St(P,Q),ne=ko(te,V);if(ne&&wo(ne))z.push(te)}return z}function yn(P,V){if(!vr(P))return;let z=ko(P,V);if(!z)return;return{path:P,config:z}}function aa(P,V,z,Q){if(V){P.push(`GitHub Copilot CLI ${V} does not support ${z}; requires ${Q}+`);return}P.push(`GitHub Copilot CLI version unavailable; skipping ${z} because it requires ${Q}+`)}function c2(P){for(let V of P){if(V?.config.disableAllHooks===!0)return V.path;if(V?.config.disableAllHooks===!1)return}return}function d2(P,V,z,Q){let te=Dn(P),ne=St(V,".github","hooks"),ce=St(te,"hooks"),ye=St(V,".github","copilot"),he=St(V,".claude"),be=s2(z),Le=be===!0?Q:void 0,Ie=[yn(St(ye,"settings.local.json"),Le),yn(St(ye,"settings.json"),Le),yn(St(he,"settings.local.json"),Le),yn(St(he,"settings.json"),Le)],Xe=[yn(St(te,"settings.json"),Le),yn(St(te,"config.json"),Le)];if(be!==!1){let gt=c2([...Ie,...Xe]);if(gt){if(be===null)Q.push(`GitHub Copilot CLI version unavailable; treating disableAllHooks in ${gt} as active`);return{activeConfigPaths:[],disabledBy:gt}}}let et=l2(ne,Q),xe=i2(z),at=xe===!0?Q:void 0,ct=vr(ce)?ca(ce,at):[],lt=[];for(let gt of ct){let ut=St(ce,gt),vt=ko(ut,at);if(vt&&wo(vt))lt.push(ut)}if(xe!==!0&&lt.length>0)aa(Q,z,`user hook files in ${ce}`,"0.0.422"),lt.length=0;let pt=[];for(let gt of[...Ie,...Xe]){if(!gt)continue;if(!wo(gt.config))continue;if(be===!0){pt.push(gt);continue}aa(Q,z,"inline hook definitions in Copilot config files","1.0.8");break}let mt=(gt)=>gt.filter((ut)=>!!ut&&pt.includes(ut)).map((ut)=>ut.path);return{activeConfigPaths:[...mt(Ie),...et,...mt(Xe),...lt]}}function da(P){let V=[],z=d2(P.environment,P.cwd,P.copilotCliVersion,V);if(z.disabledBy)return{platform:"copilot-cli",status:"disabled",method:"hook config",configPath:z.disabledBy,configPaths:[z.disabledBy],errors:V.length>0?V:void 0};let Q=Dn(P.environment),te=St(Q,"installed-plugins",...gr),ne=vr(te),ce=St(Q,"settings.json"),ye=Ft(ce,At);if(ne&&ye.kind==="unreadable")return{platform:"copilot-cli",status:"not-inspected"};if(ne&&ye.kind==="ok"&&ft(ft(ye.value,"enabledPlugins"),jt)===!1)return{platform:"copilot-cli",status:"disabled",method:"plugin config",configPath:ce,errors:[`${jt} is installed but not enabled in Copilot CLI`]};if(ne||z.activeConfigPaths.length>0){let he=ne,be=z.activeConfigPaths[0];return{platform:"copilot-cli",status:"configured",method:he?"plugin config":"hook config",configPath:be??(he?te:void 0),configPaths:z.activeConfigPaths.length>0?z.activeConfigPaths:void 0,errors:V.length>0?V:void 0}}return{platform:"copilot-cli",status:"n/a",errors:V.length>0?V:void 0}}import{existsSync as L2,readFileSync as w2}from"node:fs";import{existsSync as ua,mkdirSync as f2,readFileSync as m2}from"node:fs";import{dirname as g2,join as y2}from"node:path";import{renameSync as u2,writeFileSync as p2}from"node:fs";function kt(P,V){let z=`${P}.${process.pid}.tmp`;p2(z,V),u2(z,P)}var Nt=Object.fromEntries(Sn.map((P)=>[P.id,`npx -y cc-safety-net hook ${P.flags[1]}`]));var An=Nt.cursor,pa=30;function Lr(P){return y2(P.home,".cursor","hooks.json")}function Qt(P){return typeof P==="object"&&P!==null&&!Array.isArray(P)}function xo(){return{command:An,timeout:pa,failClosed:!0}}function br(P){return Qt(P)&&P.command===An}function h2(P){return Object.keys(P).length===3&&P.command===An&&P.timeout===pa&&P.failClosed===!0}function v2(P){try{return JSON.parse(m2(P,"utf-8"))}catch(V){if(V instanceof SyntaxError)throw Error(`Failed to parse Cursor hooks config ${P}: ${V.message}`);throw V}}function fa(P){let V=v2(P);if(!Qt(V))throw Error(`Cursor hooks config ${P} must be a JSON object`);if(V.version!==1)throw Error(`Cursor hooks config ${P} must set "version": 1`);if(V.hooks!==void 0&&!Qt(V.hooks))throw Error(`Cursor hooks config ${P} "hooks" must be an object`);let z=Qt(V.hooks)?V.hooks.preToolUse:void 0;if(z!==void 0&&!Array.isArray(z))throw Error(`Cursor hooks config ${P} "hooks.preToolUse" must be an array`);return V}function ma(P){let V=Qt(P.hooks)?P.hooks.preToolUse:void 0;return Array.isArray(V)?V:[]}function b2(P){if(!P.some(br))return[...P,xo()];return P.reduce((V,z)=>{if(!br(z))return V.result.push(z),V;if(!V.inserted)V.result.push(xo()),V.inserted=!0;return V},{result:[],inserted:!1}).result}function ga(P,V,z){let Q=Qt(V.hooks)?V.hooks:{},te={...V,hooks:{...Q,preToolUse:z}};kt(P,`${JSON.stringify(te,null,2)}
`)}function ya(P){let V=Lr(P);if(!ua(V))return f2(g2(V),{recursive:!0}),kt(V,`${JSON.stringify({version:1,hooks:{preToolUse:[xo()]}},null,2)}
`),{path:V,alreadyInstalled:!1};let z=fa(V),Q=ma(z),te=Q.filter(br);if(Qt(z.hooks)&&Array.isArray(z.hooks.preToolUse)&&te.length===1&&te[0]!==void 0&&h2(te[0]))return{path:V,alreadyInstalled:!0};return ga(V,z,b2(Q)),{path:V,alreadyInstalled:!1}}function ha(P){let V=Lr(P);if(!ua(V))return{path:V,alreadyInstalled:!1};let z=fa(V),Q=ma(z),te=Q.filter((ne)=>!br(ne));if(te.length===Q.length)return{path:V,alreadyInstalled:!1};return ga(V,z,te),{path:V,alreadyInstalled:!0}}function k2(P){if(!P||typeof P!=="object"||Array.isArray(P))return[];let V=P.hooks;if(!V||typeof V!=="object"||Array.isArray(V))return[];let z=V.preToolUse;if(!Array.isArray(z))return[];return z.filter((Q)=>!!Q&&typeof Q==="object"&&!Array.isArray(Q)&&Q.command===An)}function x2(P){let V=[];if(P.length>1)V.push("Multiple managed cc-safety-net hooks found; reinstall to collapse duplicates");let z=P[0];if(z&&z.failClosed!==!0)V.push('Managed hook is missing "failClosed": true; reinstall to repair');if(z&&z.timeout!==30)V.push('Managed hook "timeout" is not 30; reinstall to repair');return V}function va(P){let V=Lr(P.environment);if(!L2(V))return{platform:"cursor",status:"n/a",configPath:V};let z;try{z=JSON.parse(w2(V,"utf-8"))}catch(ne){return{platform:"cursor",status:"n/a",configPath:V,errors:[`Failed to parse Cursor hooks config ${V}: ${ne instanceof Error?ne.message:String(ne)}`]}}let Q=k2(z);if(Q.length===0)return{platform:"cursor",status:"n/a",configPath:V};let te=x2(Q);return{platform:"cursor",status:"configured",method:"hook config",configPath:V,errors:te.length>0?te:void 0}}import{existsSync as C2}from"node:fs";import{join as Co}from"node:path";var So="gemini-safety-net";function Ro(P){let V=Co(P.home,".gemini","extensions"),z=Co(V,So);if(!C2(z))return{platform:"gemini-cli",status:"n/a"};let Q=Co(V,"extension-enablement.json"),te=Ft(Q);if(te.kind==="unreadable")return{platform:"gemini-cli",status:"not-inspected"};let ne=te.kind==="ok"?ft(ft(te.value,So),"overrides"):void 0;if(Array.isArray(ne)&&ne.some((ye)=>typeof ye==="string"&&ye.startsWith("!")))return{platform:"gemini-cli",status:"disabled",method:"extension config",configPath:Q,errors:[`${So} is disabled in Gemini CLI`]};return{platform:"gemini-cli",status:"configured",method:"extension config",configPath:z}}function ba(P){return Ro(P.environment)}import{existsSync as E2,readFileSync as D2}from"node:fs";import{existsSync as wa,mkdirSync as S2,readFileSync as ka,rmSync as R2}from"node:fs";import{dirname as P2,join as La}from"node:path";var _n=Nt["grok-build"],xr=30;function Cr(P){return La(P.env.get("GROK_HOME")??La(P.home,".grok"),"hooks","cc-safety-net.json")}function en(P){return typeof P==="object"&&P!==null&&!Array.isArray(P)}function wr(){return{hooks:[{type:"command",command:_n,timeout:xr}]}}function xa(P){return en(P)&&P.command===_n}function Ca(P){return P.flatMap((V)=>{if(!en(V)||!Array.isArray(V.hooks))return[V];let z=V.hooks.filter((Q)=>!xa(Q));if(z.length===V.hooks.length)return[V];return z.length===0?[]:[{...V,hooks:z}]})}function Sa(P){try{let V=JSON.parse(P);return en(V)?V:null}catch{return null}}function Ra(P){let V=en(P.hooks)?P.hooks.PreToolUse:void 0;return Array.isArray(V)?V:[]}function kr(P,V,z){let Q=en(V.hooks)?V.hooks:{};kt(P,`${JSON.stringify({...V,hooks:{...Q,PreToolUse:z}},null,2)}
`)}function Pa(P){let V=Cr(P);if(!wa(V))return S2(P2(V),{recursive:!0}),kr(V,{},[wr()]),{path:V,alreadyInstalled:!1};let z=Sa(ka(V,"utf-8"));if(!z)return kr(V,{},[wr()]),{path:V,alreadyInstalled:!1};let Q=Ra(z),te=Q.filter((ne)=>en(ne)&&Array.isArray(ne.hooks)&&ne.hooks.some(xa));if(te.length===1&&JSON.stringify(te[0])===JSON.stringify(wr()))return{path:V,alreadyInstalled:!0};return kr(V,z,[...Ca(Q),wr()]),{path:V,alreadyInstalled:!1}}function Ea(P){let V=Cr(P);if(!wa(V))return{path:V,alreadyInstalled:!1};let z=Sa(ka(V,"utf-8"));if(!z)return{path:V,alreadyInstalled:!1};let Q=Ra(z),te=Ca(Q);if(JSON.stringify(te)===JSON.stringify(Q))return{path:V,alreadyInstalled:!1};let ne=en(z.hooks)?z.hooks:{};if(te.length===0&&Object.keys(z).length===1&&Object.keys(ne).length===1)return R2(V),{path:V,alreadyInstalled:!0};return kr(V,z,te),{path:V,alreadyInstalled:!0}}function Tn(P){return!!P&&typeof P==="object"&&!Array.isArray(P)}function A2(P){if(!Tn(P)||!Tn(P.hooks))return[];let V=P.hooks.PreToolUse;if(!Array.isArray(V))return[];return V.filter((z)=>Tn(z)&&Array.isArray(z.hooks)&&z.hooks.some((Q)=>Tn(Q)&&Q.command===_n))}function _2(P){let z=(Array.isArray(P.hooks)?P.hooks.filter(Tn):[]).find((Q)=>Q.command===_n);return[...P.matcher===void 0||P.matcher===""||P.matcher==="*"?[]:['Managed hook has a "matcher" that narrows coverage; reinstall to repair'],...z?.type==="command"?[]:['Managed hook "type" is not "command"; reinstall to repair'],...z?.timeout===xr?[]:[`Managed hook "timeout" is not ${xr}; reinstall to repair`]]}function Da(P){let V=Cr(P.environment);if(!E2(V))return{platform:"grok-build",status:"n/a",configPath:V};let z;try{z=JSON.parse(D2(V,"utf-8"))}catch(ne){return{platform:"grok-build",status:"n/a",configPath:V,errors:[`Failed to parse Grok Build hooks config ${V}: ${ne instanceof Error?ne.message:String(ne)}`]}}let Q=A2(z)[0];if(!Q)return{platform:"grok-build",status:"n/a",configPath:V};let te=_2(Q);return{platform:"grok-build",status:"configured",method:"hook config",configPath:V,errors:te.length>0?te:void 0}}import{readFileSync as ja}from"node:fs";import{join as Na}from"node:path";var Tt="cc-safety-net",Po="# cc-safety-net managed Hermes Agent plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --hermes-agent",T2=30;function Aa(P){return`${Po}
# version: ${P}
`}function $2(P){return`${Aa(P)}name: ${Tt}
version: "${P}"
description: "Block destructive commands and secret-file access before Hermes runs a tool."
author: "cc-safety-net"
provides_hooks:
  - pre_tool_call
`}function I2(P){return`${Aa(P)}"""CC Safety Net guard for Hermes Agent.
...
`}function $n(P){return[{name:"__init__.py",content:I2(P)},{name:"plugin.yaml",content:$2(P)}]}import{mkdirSync as O2,readdirSync as F2,readFileSync as j2,rmSync as Eo}from"node:fs";import{join as tn}from"node:path";var N2="__pycache__";function Do(P){let V=P.env.get("HERMES_HOME")?.trim();return V?V:tn(P.home,".hermes")}function Ao(P){return tn(Do(P),"plugins",Tt)}function _o(P){return P.startsWith(Po)}function To(P,V){let z=Ao(P),Q=Lt(z);if(Q&&(Q.isSymbolicLink()||!Q.isDirectory()))throw Error(`Refusing to ${V} ${z}: not a regular directory. Move or remove it and rerun ${V==="install"?"install":"uninstall"} --hermes-agent.`);return z}function _a(P,V){let z=Lt(P);if(!z)return;if(z.isSymbolicLink()||!z.isFile())throw Error(`Refusing to ${V} ${P}: not a regular file. Move or remove it.`);let Q=j2(P,"utf-8");if(!_o(Q))throw Error(`Refusing to ${V} unmanaged file at ${P}. Move or remove it.`);return Q}function Ta(P){let V=To(P,"install"),z=$n(wt());if(z.map((te)=>_a(tn(V,te.name),"overwrite")).every((te,ne)=>te===z[ne]?.content))return{path:V,alreadyInstalled:!0};return O2(V,{recursive:!0}),z.forEach((te)=>{kt(tn(V,te.name),te.content)}),{path:V,alreadyInstalled:!1}}function $o(P){let V=To(P,"remove");if(!Lt(V))return[];return $n(wt()).filter((z)=>_a(tn(V,z.name),"remove")!==void 0)}function $a(P){let V=To(P,"remove");if(!Lt(V))return{path:V,alreadyInstalled:!1};let z=$o(P);if(z.forEach((Q)=>{Eo(tn(V,Q.name))}),Eo(tn(V,N2),{recursive:!0,force:!0}),F2(V).length===0)Eo(V,{recursive:!0});return{path:V,alreadyInstalled:z.length>0}}var Sr="hermes-agent",Ia=/^([^\s#][^:]*):/,M2=/^\s+([A-Za-z_][\w-]*):/,Oa=/^\s+-\s*(.*)$/;function H2(P){return P.trim().replace(/^(["'])(.*)\1$/,"$2")}function U2(P){let V=P.split(/\r?\n/),z=V.findIndex((ne)=>Ia.exec(ne)?.[1]?.trim()==="plugins");if(z===-1)return[];let Q=V.slice(z+1),te=Q.findIndex((ne)=>Ia.test(ne));return te===-1?Q:Q.slice(0,te)}function Fa(P,V){let z=U2(P),Q=z.findIndex((ce)=>M2.exec(ce)?.[1]===V);if(Q===-1)return[];let te=z.slice(Q+1),ne=te.findIndex((ce)=>!Oa.test(ce));return(ne===-1?te:te.slice(0,ne)).map((ce)=>H2(Oa.exec(ce)?.[1]??""))}function G2(P){try{return ja(Na(Do(P),"config.yaml"),"utf-8")}catch{return}}function Io(P){let V=G2(P)??"";return Fa(V,"enabled").includes(Tt)&&!Fa(V,"disabled").includes(Tt)}function Ma(P){return/^# version:\s*(.+)$/m.exec(P)?.[1]?.trim()}function B2(P,V){let z=Lt(P);if(!z)return{error:`${V.name} is missing from ${P}; run install --hermes-agent`};if(z.isSymbolicLink()||!z.isFile())return{error:`${P} is a symlink or not a regular file; move or remove it`};try{let Q=ja(P,"utf-8");if(!_o(Q))return{error:`Unmanaged ${V.name} occupies ${P}; move or remove it`};if(Ma(Q)===wt()&&Q!==V.content)return{error:`Modified ${V.name} occupies ${P}; run install --hermes-agent to restore it`};return{content:Q}}catch(Q){return{error:`Failed to read ${P}: ${Q instanceof Error?Q.message:String(Q)}`}}}function Ha(P){let V=Ao(P.environment),z=ur(Sr,V);if(z)return z;let Q=$n(wt()).map((ye)=>B2(Na(V,ye.name),ye)),te=Q.flatMap((ye)=>("error"in ye)?[ye.error]:[]);if(te.length>0)return{platform:Sr,status:"n/a",configPath:V,errors:te};let ne=Q.some((ye)=>("content"in ye)&&Ma(ye.content)!==wt()),ce=ne?["Installed Hermes Agent plugin is outdated; run install --hermes-agent to update"]:[];if(!Io(P.environment))return{platform:Sr,status:"disabled",method:"plugin directory",configPath:V,errors:[`${Tt} is not enabled in Hermes; run \`hermes plugins enable ${Tt}\``,...ce]};return{platform:Sr,status:"configured",method:"plugin directory",configPath:V,errors:ne?ce:void 0}}import{existsSync as q2,readFileSync as V2}from"node:fs";import{join as Ua}from"node:path";var z2=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*--kimi-code(\s|["']|$)/;function J2(P){return Ua(P.env.get("KIMI_CODE_HOME")||Ua(P.home,".kimi-code"),"config.toml")}function In(P){let V=J2(P.environment);if(!q2(V))return{platform:"kimi-code",status:"n/a",configPath:V};try{if(!z2.test(V2(V,"utf-8")))return{platform:"kimi-code",status:"n/a",configPath:V}}catch(z){return{platform:"kimi-code",status:"n/a",configPath:V,errors:[`Failed to read ${V}: ${z instanceof Error?z.message:String(z)}`]}}return{platform:"kimi-code",status:"configured",method:"hook config",configPath:V}}import{readFileSync as Ya}from"node:fs";import{join as Fn}from"node:path";var ht="cc-safety-net",It="index.js",hn="openclaw.plugin.json",vn="package.json";var Rr="// cc-safety-net managed OpenClaw plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --openclaw";import{existsSync as Y2,lstatSync as Z2,readdirSync as X2,readFileSync as Q2}from"node:fs";import{dirname as Ba,join as Gt}from"node:path";import{fileURLToPath as ef}from"node:url";import{spawn as W2}from"node:child_process";function K2(P){return P.join(" ")}function Oo(P,V,z){return[`Failed to run ${K2(P)}${V===null?"":` (exit ${V})`}.`,z.trim()].filter(Boolean).join(`
`)}function Fo(P){let V={stdout:"",stderr:""};return P.stdout.setEncoding("utf-8"),P.stderr.setEncoding("utf-8"),P.stdout.on("data",(z)=>{V.stdout+=z}),P.stderr.on("data",(z)=>{V.stderr+=z}),V}function _t(P,V){return new Promise((z,Q)=>{let te=Xt([...P],process.env),ne=W2(te.cmd,te.args,{stdio:["ignore","pipe","pipe"]}),ce=Fo(ne),ye=()=>[ce.stdout,ce.stderr].filter(Boolean).join(`
`),he=V?.timeoutMs??120000,be=setTimeout(()=>{ne.kill(),Q(Error(Oo(P,null,`Timed out after ${he}ms.
${ye()}`.trim())))},he);ne.on("error",(Le)=>{clearTimeout(be),Q(Error(Oo(P,null,`${Le.message}
${ye()}`.trim())))}),ne.on("close",(Le)=>{if(clearTimeout(be),Le!==0){Q(Error(Oo(P,Le,ye())));return}z(V?.stdoutOnly?ce.stdout:ye())})})}async function jo(P){for(let V of P)await _t(V)}async function Ga(P){for(let V of P)try{await _t(V)}catch(z){console.warn(z instanceof Error?z.message:String(z))}}var No=Gt("openclaw",ht),tf=[It,hn,vn];function Mo(P,V){if(P==="~")return V;if(P.startsWith("~/")||P.startsWith("~\\"))return Gt(V,P.slice(2));return P}function qa(P){let V=P.env.get("OPENCLAW_STATE_DIR")?.trim();if(V)return Mo(V,P.home);let z=P.env.get("OPENCLAW_CONFIG_PATH")?.trim();return z?Ba(Mo(z,P.home)):Gt(P.home,".openclaw")}function Va(P){let V=P.env.get("OPENCLAW_CONFIG_PATH")?.trim();return V?Mo(V,P.home):Gt(qa(P),"openclaw.json")}function Ho(P){return Gt(qa(P),"extensions",ht)}function nf(P){let V=X2(P);if(V.length===0)return!0;if(V.some((te)=>!tf.includes(te)))return!1;let z=Gt(P,It),Q=Lt(z);return Q!==void 0&&!Q.isSymbolicLink()&&Q.isFile()&&Q2(z,"utf-8").startsWith(Rr)}function Uo(P){let V=Ho(P),z=Lt(V);if(!z)return;if(!z.isSymbolicLink()&&z.isDirectory()&&nf(V))return;throw Error(`Refusing to modify ${V}: it does not hold a cc-safety-net managed OpenClaw plugin. Move or remove it, then run the command again.`)}function za(){let P=Ba(ef(import.meta.url));return[Gt(P,No),Gt(P,"..",No),Gt(P,"..","..","..","dist",No)]}function Go(P=za()){return P.find((V)=>Y2(V)&&Z2(V).isDirectory())}function rf(P=za()){let V=Go(P);if(!V)throw Error("Packaged OpenClaw plugin directory not found. Reinstall cc-safety-net and try again.");return V}function Ja(P=rf()){return[["openclaw","plugins","install",P,"--force"],["openclaw","plugins","enable",ht]]}function of(P){let V=(()=>{try{return JSON.parse(P)}catch{return}})(),z=ft(ft(V,"plugin"),"status");return typeof z==="string"?z:void 0}async function Wa(){let P=of(await _t(["openclaw","plugins","inspect",ht,"--runtime","--json"],{stdoutOnly:!0}));if(P==="loaded")return;throw Error(`${P===void 0?`The ${ht} plugin's load state could not be verified: OpenClaw's runtime inspect report was unreadable.`:`OpenClaw reports the ${ht} plugin with status "${P}".`} Run \`openclaw plugins inspect ${ht} --runtime\` for details.`)}var Pr="openclaw",On=`run \`openclaw plugins enable ${ht}\``;function bn(P,V){let z=Fn(P,V),Q=Lt(z);if(!Q)return{error:`${V} is missing from ${z}; run install --openclaw`};if(Q.isSymbolicLink()||!Q.isFile())return{error:`${z} is a symlink or not a regular file; move or remove it`};try{return{content:Ya(z,"utf-8")}}catch(te){return{error:`Failed to read ${z}: ${te instanceof Error?te.message:String(te)}`}}}function Za(P){try{return JSON.parse(At(P))}catch{return}}function sf(P){let V=bn(P,hn);if("error"in V)return V.error;if(ft(Za(V.content),"id")===ht)return;return`${Fn(P,hn)} is not a valid ${ht} manifest; run install --openclaw`}function af(P){let V=bn(P,vn);if("error"in V)return V.error;let z=ft(ft(Za(V.content),"openclaw"),"extensions");if(Array.isArray(z)&&z.includes(`./${It}`))return;return`${Fn(P,vn)} does not point OpenClaw at ${It}; run install --openclaw`}function Ka(P){return Array.isArray(P)?P.filter((V)=>typeof V==="string"):[]}function lf(P){let V=Va(P);if(!Lt(V))return`${ht} is not enabled; ${On}`;let z=(()=>{try{return JSON.parse(At(Ya(V,"utf-8")))}catch{return}})();if(z===void 0)return`Failed to read ${V}; fix it, then ${On}`;let Q=ft(z,"plugins");if(ft(Q,"enabled")===!1)return`plugins.enabled is false in ${V}; no OpenClaw plugin loads`;let te=ft(ft(ft(Q,"entries"),ht),"enabled");if(Ka(ft(Q,"deny")).includes(ht)||te===!1)return`${ht} is disabled in ${V}; ${On}`;let ne=Ka(ft(Q,"allow"));if(ne.length>0&&!ne.includes(ht))return`plugins.allow in ${V} does not list ${ht}; add it, then ${On}`;if(ne.includes(ht)||te===!0)return;return`${ht} is not enabled; ${On}`}function Xa(P){return/^\/\/ version:\s*(.+)$/m.exec(P)?.[1]?.trim()}function cf(P,V,z){if(z===void 0)return[];let Q=bn(z,It);if("error"in Q||Xa(Q.content)!==V)return[];return[It,hn,vn].flatMap((te)=>{let ne=bn(P,te),ce=bn(z,te);if("error"in ne||"error"in ce||ne.content===ce.content)return[];return[`Modified ${te} occupies ${Fn(P,te)}; run install --openclaw to restore it`]})}function Qa(P){let V=Ho(P.environment),z=ur(Pr,V);if(z)return z;let Q=bn(V,It),ne=["error"in Q?Q.error:Q.content.startsWith(Rr)?void 0:`Unmanaged ${It} occupies ${Fn(V,It)}; move or remove it`,sf(V),af(V)].filter((Le)=>Le!==void 0),ce="content"in Q?Xa(Q.content):void 0,ye=ne.length>0?ne:cf(V,ce,Go());if(ye.length>0)return{platform:Pr,status:"n/a",configPath:V,errors:ye};let he=ce===wt()?[]:["Installed OpenClaw plugin is outdated; run install --openclaw to update"],be=lf(P.environment);if(be)return{platform:Pr,status:"disabled",method:"plugin directory",configPath:V,errors:[be,...he]};return{platform:Pr,status:"configured",method:"plugin directory",configPath:V,errors:he.length>0?he:void 0}}import{existsSync as gf,readFileSync as yf}from"node:fs";import{join as hf}from"node:path";import{existsSync as Er,readFileSync as Bo,rmSync as df}from"node:fs";import{join as Bt}from"node:path";import{pathToFileURL as uf}from"node:url";var Dr="cc-safety-net",nn=`${Dr}@latest`,nl=["opencode.json","opencode.jsonc"],el="CCSafetyNetPlugin",tl={stringError:"Unterminated string in OpenCode config",bracketError:"Unmatched plugin array in OpenCode config"};function _r(P){return Bt(P.env.get("XDG_CONFIG_HOME")||Bt(P.home,".config"),"opencode")}function pf(P){return Bt(_r(P),nl[0])}function rl(P){return nl.map((V)=>Bt(_r(P),V))}function ol(P){return Bt(P.env.get("XDG_CACHE_HOME")||Bt(P.home,".cache"),"opencode","packages",nn)}function il(P){df(ol(P),{recursive:!0,force:!0})}async function sl(P){let V=(await _t(["opencode","--version"],{stdoutOnly:!0})).trim(),z=/^(?:opencode\s+)?v?(\d+)\.(\d+)\.(\d+)(?:[-+].*)?$/.exec(V),Q=Number(z?.[1]),te=Number(z?.[2]),ne=Number(z?.[3]);if(!z||Q!==1&&Q!==2||Q===1&&(te<18||te===18&&ne<29)||Q===2&&te===0&&ne<6)throw Error(`OpenCode 1.18.29+ or 2.0.6+ is required; found ${V||"an unknown version"}.`);if(Q===2){for(let ce of rl(P)){if(!Er(ce))continue;let ye=qo(Bo(ce,"utf-8"),ce);if(["plugin","plugins"].some((be)=>{let Le=ft(ye,be);return Array.isArray(Le)&&Le.some((Ie)=>Ar(Ie)&&(typeof Ie==="string"?Ie:ft(Ie,"package"))!==nn)}))throw Error(`Change the cc-safety-net package spec in ${ce} to ${nn}, preserving its options, then retry. Adding another spec would create a duplicate plugin ID.`)}return{commands:[["opencode","plugin","add",nn],["opencode","plugin","update",nn]],afterInstall:async()=>{let ce=await _t(["opencode","plugin","list"],{stdoutOnly:!0});if(/^cc-safety-net\s+\S+\s+cc-safety-net@latest\s*$/m.test(ce))return;throw Error("OpenCode did not load cc-safety-net from cc-safety-net@latest. Run `opencode plugin list` for details.")}}}return il(P),{commands:[["opencode","plugin","-g","-f",nn]],afterInstall:()=>ff(P)}}async function ff(P){let V=Bt(ol(P),"node_modules",Dr),z=Bt(V,"package.json");if(!Er(z))throw Error(`The OpenCode plugin cache at ${V} is missing its package, so OpenCode would load nothing and fail open. Run \`opencode plugin -g -f ${nn}\` for details.`);let Q=ft(JSON.parse(Bo(z,"utf-8")),"main");if(typeof Q!=="string")throw Error(`The cached OpenCode plugin at ${V} declares no "main" entry.`);let te=Bt(V,Q);if(typeof(await import(uf(te).href))[el]==="function")return;throw Error(`The cached OpenCode plugin at ${te} does not export a callable ${el}, so OpenCode would load nothing and fail open.`)}function qo(P,V){try{return JSON.parse(At(P))}catch(z){if(z instanceof SyntaxError)throw Error(`Failed to parse OpenCode config ${V}: ${z.message}`);throw z}}function Ar(P){let V=typeof P==="string"?P:ft(P,"package");return typeof V==="string"&&(V===Dr||V.startsWith(`${Dr}@`))}function Vo(P){return["plugin","plugins"].some((V)=>{let z=ft(P,V);return Array.isArray(z)&&z.some(Ar)})}function mf(P,V){let Q=["plugin","plugins"].flatMap((te)=>{let ne=Xs(P,te,tl);if(!ne)return[];if(te==="plugin")return Qs(P,ne,tl.stringError).filter((Le)=>Ar(Le.value)).map((Le)=>Le.range);let ce=[],ye=0,he=ne.start+1,be=P.slice(ne.start+1,ne.end).matchAll(/\/\/[^\n]*|\/\*[\s\S]*?\*\/|"(?:\\.|[^"\\])*"|[^"\s{}[\],/]+|[{}[\],]/g);for(let Le of be){if(Le[0].startsWith("//")||Le[0].startsWith("/*"))continue;let Ie=ne.start+1+Le.index;if(ye===0)he=Ie;if(Le[0]==="{"||Le[0]==="[")ye++;if(Le[0]==="}"||Le[0]==="]")ye--;if(ye!==0||Le[0]===",")continue;let Xe=Ie+Le[0].length;if(Ar(JSON.parse(At(P.slice(he,Xe)))))ce.push({start:he,end:Xe})}return ce}).sort((te,ne)=>te.start-ne.start).reverse().reduce((te,ne)=>{let ce=/^(?:\s|\/\/[^\n]*(?:\n|$)|\/\*[\s\S]*?\*\/)*,/.exec(te.slice(ne.end));if(ce?.[0].includes("/")){let ye=ne.end+ce[0].length-1;return te.slice(0,ne.start)+te.slice(ne.end,ye)+te.slice(ye+1)}return mr(te,ne)},P);return qo(Q,V),Q}function al(P){il(P);let V=rl(P),z=V.find((ne)=>Er(ne)),Q=[],te=[];for(let ne of V){if(!Er(ne))continue;try{let ce=Bo(ne,"utf-8");if(!Vo(qo(ce,ne)))continue;kt(ne,mf(ce,ne)),te.push(ne)}catch(ce){Q.push(ce instanceof Error?ce.message:String(ce))}}if(Q.length>0)throw Error(Q.join(`
`));return{path:te[0]??z??pf(P),alreadyInstalled:te.length>0}}function ll(P){let V=[],z=_r(P.environment),Q=["opencode.json","opencode.jsonc"];for(let te of Q){let ne=hf(z,te);if(gf(ne))try{let ce=yf(ne,"utf-8"),ye=At(ce),he=JSON.parse(ye);if(Vo(he))return{platform:"opencode",status:"configured",method:"plugin array",configPath:ne,errors:V.length>0?V:void 0}}catch(ce){V.push(`Failed to parse ${te}: ${ce instanceof Error?ce.message:String(ce)}`)}}return{platform:"opencode",status:"n/a",errors:V.length>0?V:void 0}}import{join as vf}from"node:path";function zo(P){return vf(P.home,".pi","agent","settings.json")}function Jo(P){if(typeof P!=="string")return!1;return P==="npm:cc-safety-net"||P.startsWith("npm:cc-safety-net@")}function cl(P){let V=zo(P.environment),z=Ft(V);if(z.kind==="unreadable")return{platform:"pi",status:"not-inspected"};if(z.kind==="missing")return{platform:"pi",status:"n/a"};let Q=ft(z.value,"packages");if(!Array.isArray(Q))return{platform:"pi",status:"n/a"};let te=Q.find((ye)=>Jo(typeof ye==="string"?ye:ft(ye,"source")));if(te===void 0)return{platform:"pi",status:"n/a"};let ne=ft(te,"extensions");if(Array.isArray(ne)&&ne.some((ye)=>typeof ye==="string"&&ye.startsWith("-")))return{platform:"pi",status:"disabled",method:"package config",configPath:V,errors:["npm:cc-safety-net is installed but its extension is disabled in Pi settings"]};return{platform:"pi",status:"configured",method:"package config",configPath:V}}var bf={amp:Bs,"antigravity-cli":qs,"claude-code":Ws,codex:Ks,"copilot-cli":da,cursor:va,"gemini-cli":ba,"grok-build":Da,"hermes-agent":Ha,"kimi-code":In,openclaw:Qa,opencode:ll,pi:cl};function Ln(P,V,z){let Q={...z,cwd:V,environment:P};return Zn.map((te)=>Lf(bf[te](Q)))}function Lf(P){if(P.status==="not-inspected")return{platform:P.platform,detected:!1,configured:!1,inspectionStatus:"not-inspected"};return{platform:P.platform,detected:P.status!=="n/a",configured:P.status==="configured",inspectionStatus:P.status!=="n/a"?"verified":P.errors&&P.errors.length>0?"failed":"not-applicable",method:P.method,configPath:P.configPath,configPaths:P.configPaths,errors:P.errors}}import{join as wf}from"node:path";var kf=Object.freeze([{command:"git reset --hard",description:"git reset --hard",expectBlocked:!0},{command:"rm -rf /",description:"rm -rf /",expectBlocked:!0},{command:"rm -rf ./node_modules",description:"rm in cwd (safe)",expectBlocked:!1}]),xf=Object.freeze({state:"ready",diagnostics:Object.freeze([]),ruleMetadata:Object.freeze({}),policy:Object.freeze({rules:Object.freeze([]),transparentWrappers:Object.freeze([]),safety:Object.freeze({}),worktreeMode:!1,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:Object.freeze({}),destructiveCommandAllowPaths:Object.freeze([]),secretProtection:Object.freeze({enabled:!0,disabledRules:Object.freeze([]),denyPaths:Object.freeze([]),allowPaths:Object.freeze([])})})}),Cf={strict:!1,paranoidRm:!1,paranoidInterpreters:!1,worktreeMode:!1,effectiveLevel:"standard",capabilities:{fail_closed:{enabled:!1,source:"preset",sources:[]},paranoid_rm:{enabled:!1,source:"preset",sources:[]},paranoid_interpreters:{enabled:!1,source:"preset",sources:[]}}};function dl(P){let V=wf(P.tmpdir,"cc-safety-net-self-test"),z=kf.map((Q)=>{let te=D(P,d("self-test",{command:Q.command},{kind:"command",shell:"auto"},{configCwd:V,executionCwd:V},Q.command),{guard:{dependencies:{loadPolicySnapshot:()=>xf,getModes:()=>Cf,findPolicyMutation:()=>null}},audit:{agent:"self-test",getSessionId:()=>{return}}}),ne=Q.expectBlocked?"blocked":"allowed",ce=te.decision.kind==="deny"?"blocked":"allowed";return{command:Q.command,description:Q.description,expected:ne,actual:ce,passed:ne===ce,reason:te.decision.kind==="deny"?te.decision.reason:void 0,ruleId:te.decision.kind==="deny"?te.decision.ruleId:void 0}});return{passed:z.filter((Q)=>Q.passed).length,failed:z.filter((Q)=>!Q.passed).length,total:z.length,results:z}}function Wo(P){let V=xt({label:"doctor",booleans:{json:["--json"],skipUpdateCheck:["--skip-update-check"]}},P);if(Ht(V.errors))return null;return{json:V.flags.json,skipUpdateCheck:V.flags.skipUpdateCheck}}async function ul(P,V={}){let z=await Pn(!V.json,()=>{let Q=Sf(P,V);return{ready:Q,finish:()=>Q}},()=>Rn(),{loadingMessage:"Checking system status…"});if(V.json)console.log(JSON.stringify(z,null,2));else Rf(z);return z.engineSelfTest.failed>0||z.findings.some((Q)=>Q.severity==="error")?1:0}async function Sf(P,V){let z=V.cwd??process.cwd(),Q=await sr(),te=Ln(P,z,{ampPluginListOutput:Q.ampPluginListOutput,codexPluginListOutput:Q.codexPluginListOutput,copilotCliVersion:Q.versions["copilot-cli"]}),ne=is(P,z),ce=ss(P),ye=E(P,{cwd:z}),he=ye.policy,be=L(he,P.env),Le=G(he,be.capabilities),Ie=tr(P,7),Xe=Ns(P,z),et=V.skipUpdateCheck?{currentVersion:wt(),latestVersion:null,updateAvailable:!1}:await Wt(),xe={hooks:te,engineSelfTest:dl(P),userConfig:ne.userConfig,projectConfig:ne.projectConfig,configState:Ne(ye),effectiveRules:ne.effectiveRules,environment:ce,effectiveSafety:{selectedPreset:he.safety.level??"standard",level:be.effectiveLevel,capabilities:be.capabilities,ruleOverrides:he.destructiveCommandRuleOverrides,weakenedRuleOverrides:Object.entries(Le).filter(([,at])=>at.source==="rule_override"&&at.override==="off"&&at.inheritedEnabled&&at.changesInherited).map(([at])=>at),ruleCounts:{stored:Object.keys(he.destructiveCommandRuleOverrides).length,effective:Object.values(Le).filter((at)=>at.changesInherited).length},...ye.policyScopes?{policyScopes:ye.policyScopes}:{}},...Xe.length>0?{v2Leftovers:Xe}:{},posture:Ls(P,ne.userConfig.path),activity:Ie,update:et,system:Q};return{...xe,findings:ls(xe)}}function Rf(P){console.log(),console.log(ds(P.hooks)),console.log(),console.log(us(P.engineSelfTest)),console.log(),console.log(ps(P)),console.log(),console.log(fs(P.environment)),console.log(),console.log(ms(P)),console.log(),console.log(gs(P.findings)),console.log(),console.log(ys(P.activity)),console.log(),console.log(vs(P.system)),console.log(),console.log(hs(P.update)),console.log(bs(P))}import{existsSync as Pf}from"node:fs";var Ef=/^[A-Za-z0-9_@%+=:,./-]+$/,pl="Usage: cc-safety-net explain [--json] [--cwd <path>] <command>";function Ko(P){let V=xt({label:"explain",booleans:{json:["--json"]},values:{cwd:["--cwd"]},positionals:"tail"},P);if(Ht(V.errors))return console.error(pl),console.error("Pass -- before a command that starts with dashes."),null;if(V.values.cwd!==void 0&&!Pf(V.values.cwd))return console.error(`Error: --cwd path does not exist: ${V.values.cwd}`),null;let z=V.positionals.length===1?V.positionals[0]:V.positionals.map((Q)=>Ef.test(Q)?Q:`'${Q.replaceAll("'","'\\''")}'`).join(" ");if(!z)return console.error("Error: No command provided"),console.error(pl),null;return{json:V.flags.json,cwd:V.values.cwd,command:z}}function fl(P){if(P)return{dh:"=",dv:"|",dtl:"+",dtr:"+",dbl:"+",dbr:"+",h:"-",v:"|",tl:"+",tr:"+",bl:"+",br:"+",sh:"="};return{dh:"═",dv:"║",dtl:"╔",dtr:"╗",dbl:"╚",dbr:"╝",h:"─",v:"│",tl:"┌",tr:"┐",bl:"└",br:"┘",sh:"━"}}function ml(P,V){let Q=V-18;return[`${P.dtl}${P.dh.repeat(V)}${P.dtr}`,`${P.dv}  Command Analysis${" ".repeat(Q)}${P.dv}`,`${P.dbl}${P.dh.repeat(V)}${P.dbr}`]}function Yo(P){return JSON.stringify(P)}function gl(P,V=0){return`[${P.map((Q,te)=>cs(Q,te,V)).join(",")}]`}function jn(P,V,z=70){let Q=P.split(" "),te=[],ne="";for(let ce of Q)if(ne&&ne.length+ce.length+1>z)te.push(ne),ne=ce;else ne=ne?`${ne} ${ce}`:ce;if(ne)te.push(ne);return te.map((ce,ye)=>ye===0?ce:`${V}${ce}`)}function yl(P,V,z){let Q=[];switch(P.type){case"parse":return null;case"env-strip":return Q.push(""),Q.push(`STEP ${V} ${z.h} Strip environment variables`),Q.push(`  Removed: ${P.envVars.map((te)=>`${te}=<redacted>`).join(", ")}`),Q.push(`  Tokens:  ${Yo(P.output)}`),{lines:Q,incrementStep:!0};case"leading-tokens-stripped":return Q.push(""),Q.push(`STEP ${V} ${z.h} Strip wrappers`),Q.push(`  Removed: ${P.removed.join(", ")}`),Q.push(`  Tokens:  ${Yo(P.output)}`),{lines:Q,incrementStep:!0};case"shell-wrapper":return Q.push(""),Q.push(`STEP ${V} ${z.h} Detect shell wrapper`),Q.push(`  Wrapper: ${P.wrapper} -c`),Q.push(`  Inner:   ${P.innerCommand}`),{lines:Q,incrementStep:!0};case"interpreter":{if(Q.push(""),Q.push(`STEP ${V} ${z.h} Detect interpreter`),Q.push(`  Interpreter: ${P.interpreter}`),Q.push(`  Code:        ${P.codeArg}`),P.paranoidBlocked)Q.push("  Result:      ✗ BLOCKED (paranoid mode)");return{lines:Q,incrementStep:!0}}case"busybox":return Q.push(""),Q.push(`STEP ${V} ${z.h} Busybox wrapper`),Q.push(`  Subcommand: ${P.subcommand}`),{lines:Q,incrementStep:!0};case"transparent-wrapper":return Q.push(""),Q.push(`STEP ${V} ${z.h} Transparent wrapper`),Q.push(`  Wrapper: ${P.wrapper}`),Q.push(`  Tokens:  ${Yo(P.output)}`),{lines:Q,incrementStep:!0};case"recurse":return{lines:[],incrementStep:!1};case"rule-check":{if(Q.push(""),Q.push(`STEP ${V} ${z.h} Match rules`),Q.push(`  Rule:   ${P.rule}()`),P.matched)Q.push("  Result: MATCHED");else Q.push("  Result: No match");return{lines:Q,incrementStep:!0}}case"worktree-relaxation":return Q.push(""),Q.push(`STEP ${V} ${z.h} Worktree relaxation`),Q.push(`  Mode:   ${o.worktree.name}`),Q.push(`  Git cwd: ${P.gitCwd}`),Q.push("  Result: Allowed local discard in linked worktree"),{lines:Q,incrementStep:!0};case"tmpdir-check":return null;case"fallback-scan":{if(P.embeddedCommandFound)return Q.push(""),Q.push(`STEP ${V} ${z.h} Fallback scan`),Q.push(`  Found: ${P.embeddedCommandFound}`),{lines:Q,incrementStep:!0};return null}case"custom-rules-check":{if(P.rulesChecked){if(Q.push(""),Q.push(`STEP ${V} ${z.h} Custom rules`),P.matched)Q.push("  Result: MATCHED");else Q.push("  Result: No match");return{lines:Q,incrementStep:!0}}return null}case"cwd-change":return null;case"dangerous-text":{if(P.matched)return Q.push(""),Q.push(`STEP ${V} ${z.h} Dangerous text check`),Q.push(`  Token:  ${P.token}`),Q.push("  Result: MATCHED"),{lines:Q,incrementStep:!0};return null}case"strict-unparseable":return Q.push(""),Q.push(`STEP ${V} ${z.h} Strict mode check`),Q.push(`  Command: ${P.rawCommand}`),Q.push("  Result:  ✗ UNPARSEABLE"),{lines:Q,incrementStep:!0};case"segment-skipped":return null;case"error":return Q.push(""),Q.push(`ERROR: ${P.message}`),{lines:Q,incrementStep:!1};default:return P}}function Zo(P,V){let z=fl(V?.asciiOnly??!1),Q=58,te=[],ne=1;te.push(...ml(z,58)),te.push("");let ce=P.trace.steps.find((xe)=>xe.type==="error");if(ce&&ce.type==="error"){te.push("ERROR"),te.push(`  ${ce.message}`),te.push(""),te.push("RESULT"),te.push(`  Status: ${P.result==="blocked"?dt.red("BLOCKED"):dt.green("ALLOWED")}`),te.push(""),te.push("CONFIG");let xe=P.configSource??"none";return te.push(`  Path: ${xe}`),te.join(`
`)}let ye=P.trace.steps.find((xe)=>xe.type==="parse");if(ye&&ye.type==="parse"){te.push("INPUT"),te.push(`  ${ye.input}`),te.push(""),te.push(`STEP ${ne} ${z.h} Split shell commands`),ne++;for(let xe=0;xe<ye.segments.length;xe++){let at=ye.segments[xe];if(at){let ct=Math.random();te.push(`  Segment ${xe+1}: ${gl(at,ct)}`)}}}let he=P.trace.segments,be=he.length>1;for(let xe of he){if(be){te.push("");let pt="";if(ye&&ye.type==="parse"){let eo=ye.segments[xe.index];if(eo)pt=eo.join(" ")}let mt=54,gt=pt,ut=` Segment ${xe.index+1}: `,vt=" ";if(pt){if(ut.length+pt.length+vt.length>mt){let Gd=mt-ut.length-vt.length;gt=`${pt.substring(0,Gd-1)}…`}}let Dt=pt?`${ut}${gt}${vt}`:` Segment ${xe.index+1} `,Hd=pt?`${ut}${dt.cyan(gt)}${vt}`:Dt,Ii=58-Dt.length,Oi=Math.floor(Ii/2),Ud=Ii-Oi;te.push(`${z.sh.repeat(Oi)}${Hd}${z.sh.repeat(Ud)}`)}if(xe.steps.find((pt)=>pt.type==="segment-skipped")){te.push(""),te.push("  (skipped — prior segment blocked)");continue}let ct=!1,lt=!1;for(let pt of xe.steps){let mt=yl(pt,ne,z);if(mt){if(lt=!0,pt.type==="recurse"){te.push("");let gt=" RECURSING ",ut=58-gt.length-4;te.push(`  ${z.tl}${z.h}${gt}${z.h.repeat(ut)}`),te.push(`  ${z.v}`),ct=!0;continue}for(let gt of mt.lines)if(ct)te.push(`  ${z.v} ${gt}`);else te.push(gt);if(mt.incrementStep)ne++}}if(ct)te.push(`  ${z.v}`),te.push(`  ${z.bl}${z.h.repeat(56)}`),ct=!1;if(!lt)te.push(""),te.push(`  ${dt.green("✓")} Allowed (no matching rules)`)}if(te.push(""),te.push("RESULT"),P.result==="blocked"){if(te.push(`  Status: ${dt.red("BLOCKED")}`),P.customRule){if(te.push(`  Rule: ${P.customRule.id}`),P.customRule.rulebook)te.push(`  Rulebook: ${P.customRule.rulebook.name} ${P.customRule.rulebook.version}`);if(P.customRule.source)te.push(`  Source: ${P.customRule.source}`);if(P.customRule.override)te.push(`  Override: reason ${P.customRule.override.reason}`)}if(P.reason){let xe=jn(P.reason,"          ");te.push(`  Reason: ${xe[0]}`);for(let at=1;at<xe.length;at++)te.push(xe[at]??"")}}else te.push(`  Status: ${dt.green("ALLOWED")}`);te.push(""),te.push("CONFIG");let Le=P.configSource??"none",Ie=P.configValid?"":" (invalid)";te.push(`  Path: ${Le}${Ie}`);let Xe=P.safetyPresetScope;te.push(`  Safety preset: ${P.selectedPreset??"standard"}${Xe?` (${rr(Xe)})`:""}`),te.push(`  Effective capabilities: ${P.effectiveLevel}`);let et=Object.entries(P.destructiveCommandRuleOverrides??{});if(te.push(`  Rule customizations: ${et.length}`),P.ruleActivation)te.push(`  Rule activation: ${P.ruleActivation.id} — ${P.ruleActivation.enabled?"on":"off"} via ${P.ruleActivation.source}`);return te.join(`
`)}function Xo(P){return JSON.stringify(P,null,2)}import{resolve as $f}from"node:path";var Df=["AKIA","ASIA","ghp_","gho_","ghu_","ghs_","ghr_","github_pat_","glpat-","xox","npm_","pypi-","rk_","sk-","sk_","gsk_","xai-","pplx-","bastn_","tgp_v1_","flp_","wfr_","fw_","fwp_","tp-","psk-"];function hl(P){let V=0,z={allocateSegment(){return V++},getNextSegmentIndex(){return V},recordGlobal(Q){P.record({kind:"step",scope:"global",step:Q})},recordSegment(Q,te=z.currentSegmentIndex){if(te===void 0)return;P.record({kind:"step",scope:"segment",segmentIndex:te,step:Q})}};return z}function vl(P={}){let V=[],z=P.maxEvents??512,Q={maxTextLength:P.maxTextLength??2048,maxListLength:P.maxListLength??128,maxObjectProperties:P.maxObjectProperties??P.maxListLength??128,maxDepth:P.maxDepth??16},te,ne=new Set;return{record(ce){if(te)return;if(!ce||V.length>=z)return;try{V.push(ti(Af(ce,Q,ne)))}catch{}},finish(){if(te)return te;return te=ti({events:Object.freeze(V)}),te}}}function Af(P,V,z){if(P.kind!=="step")throw TypeError("invalid trace event");let{scope:Q,step:te}=P;Tr(te,z,V);let ne=Qo(te,V,z);if(Q==="global")return{kind:"step",scope:"global",step:ne};if(Q!=="segment")throw TypeError("invalid trace event scope");return{kind:"step",scope:"segment",segmentIndex:P.segmentIndex,step:ne}}function Tr(P,V,z,Q=0,te=new WeakSet){if(typeof P==="string"){let ye=P.slice(0,z.maxTextLength);if(!Ge(ye))return;for(let he of Ze(ye))for(let be of he.match(/[^\s"'()$]+/g)??[])V.add(bl(be));return}if(!P||typeof P!=="object"||Q>=z.maxDepth||te.has(P))return;if(te.add(P),Array.isArray(P)){let ye=Math.min(P.length,z.maxListLength);for(let he=0;he<ye;he++)Tr(P[he],V,z,Q+1,te);return}let ne=0,ce=new Set;for(let ye in P){if(!Object.hasOwn(P,ye))continue;if(ne>=z.maxObjectProperties)break;ne++,Tr(ye,V,z);let he=ei(ye,z,V);if(ce.has(he))continue;ce.add(he),Tr(P[ye],V,z,Q+1,te)}}function Qo(P,V,z,Q=0,te=new WeakSet){if(typeof P==="string")return ei(P,V,z);if(!P||typeof P!=="object")return P;if(Q>=V.maxDepth)return;if(te.has(P))return;if(te.add(P),Array.isArray(P)){let ye=[],he=Math.min(P.length,V.maxListLength);for(let be=0;be<he;be++)ye.push(Qo(P[be],V,z,Q+1,te));return ye}let ne={},ce=0;for(let ye in P){if(!Object.hasOwn(P,ye))continue;if(ce>=V.maxObjectProperties)break;ce++;let he=ei(ye,V,z);if(Object.hasOwn(ne,he))continue;Object.defineProperty(ne,he,{value:Qo(P[ye],V,z,Q+1,te),enumerable:!0,configurable:!0,writable:!0})}return ne}function ei(P,V,z){let Q=P.slice(0,V.maxTextLength),te=Ge(Q)?Ue(Q):Q,ne=z.size>0?Tf(te,z):te;return(_f(ne)?ke(ne):ne).slice(0,V.maxTextLength)}function _f(P){return P.includes("PRIVATE KEY")||P.includes("://")||P.includes("eyJ")||P.includes(":")&&/(?:authorization|cookie|x-api-key|api-key|(?:^|\s)(?:-u|--user)(?:\s|=))/i.test(P)||P.length>=14&&Df.some((V)=>P.includes(V))||P.length>=49&&/\b[a-f0-9]{32}\.[A-Za-z0-9]{16}\b/.test(P)}function Tf(P,V){return P.replace(/[^\s"'()$]+/g,(z)=>V.has(bl(z))?"<redacted>":z)}function bl(P){let V=2166136261,z=2166136261;for(let Q=0;Q<P.length;Q++)V=Math.imul(V^P.charCodeAt(Q),16777619),z=Math.imul(z^P.charCodeAt(P.length-Q-1),16777619);return`${V>>>0}:${z>>>0}:${P.length}`}function ti(P){if(P&&typeof P==="object"&&!Object.isFrozen(P)){for(let V of Object.values(P))ti(V);Object.freeze(P)}return P}function Nn(P,V={},z){let Q=$f(V.cwd??process.cwd()),te=V.policySnapshot??E(z,{cwd:Q,userConfigDir:V.userConfigDir}),ne=L(te.policy,z.env),ce=Fe({policySnapshot:te,effectiveCapabilities:ne.capabilities,strict:ne.strict,paranoidRm:ne.paranoidRm,paranoidInterpreters:ne.paranoidInterpreters,worktreeMode:ne.worktreeMode}),ye={effectiveLevel:ce.effectiveLevel,selectedPreset:te.policy.safety.level??"standard",...te.policyScopes?{safetyPresetScope:te.policyScopes.levelScope}:{},effectiveCapabilities:ce.effectiveCapabilities,destructiveCommandRuleOverrides:te.policy.destructiveCommandRuleOverrides},{configSource:he,configValid:be}=Of(z,{cwd:Q,userConfigDir:V.userConfigDir});if(!P||!P.trim())return{trace:{steps:[{type:"error",message:"No command provided"}],segments:[]},result:"allowed",configSource:he,configValid:be,...ye};let Le=h(P,"auto");if(Le.status==="limited")throw new y;let Ie=Le.dialect==="powershell"?h(P,"posix"):Le,Xe=st(Ie),et=vl(),xe=hl(et);xe.recordGlobal({type:"parse",input:P,segments:Xe.map((Dt)=>[...Dt])});let at=d("Bash",{command:P},{kind:"command",shell:"auto"},{configCwd:Q,executionCwd:Q},P),ct=W(at,{environment:z,trace:xe,dependencies:{loadPolicySnapshot:()=>te}}),lt=ct.decision.kind==="deny"?ct.decision:null;if(lt&&(ct.stage==="policy-protection"||ct.stage==="secret-protection")){let Dt=If(lt);return{trace:{steps:[],segments:[{index:0,steps:[{type:"rule-check",rule:Dt.rule,matched:!0,reason:lt.reason}]}]},result:"blocked",reason:k(lt.reason),segment:k(Ll(lt,P)),...Dt.ruleId?{ruleId:k(Dt.ruleId)}:{},configSource:he,configValid:be,...ye}}let pt=xe.getNextSegmentIndex();if(lt&&pt>0&&pt<Xe.length)xe.recordSegment({type:"segment-skipped",index:pt,reason:"prior-segment-blocked"},pt);let mt=et.finish(),gt=lt?.ruleId??Ff(at,te,ne,z),ut=U.find((Dt)=>Dt.id===gt&&Dt.activationCapability),vt=ut?ce.policy.effectiveDestructiveCommandRules[ut.id]:void 0;return{trace:Nf(mt),result:lt?"blocked":"allowed",reason:lt?k(lt.reason):void 0,segment:lt?k(Ll(lt,P)):void 0,ruleId:lt?.ruleId?k(lt.ruleId):void 0,customRule:jf(Mf(lt?.ruleId,te)),configSource:he,configValid:be,...ye,...ut&&vt?{ruleActivation:{id:ut.id,...vt}}:{}}}function Ll(P,V){return P.evidence?.segment??V}function If(P){if(P.reason===$e)return{ruleId:"policy-protection",rule:"policy-protection:findPolicyConfigMutationTargetInSemanticFacts"};if(P.reason===Me)return{ruleId:"policy-apply-protection",rule:"policy-apply-protection:findPolicyApplyInvocationInSemanticFacts"};if(P.reason===x)return{ruleId:"git-metadata-protection",rule:"git-metadata-protection:findGitMetadataMutationTargetInSemanticFacts"};return{ruleId:P.ruleId,rule:"secret-protection:findSensitiveTargetInSemanticFacts"}}function Of(P,V){let z=F(V.cwd),Q=M(P,V),te=K(P,{cwd:V.cwd,userConfigDir:V.userConfigDir});try{if(n(te.projectConfigTarget)!==null){if(Jt(te.projectConfigTarget).errors.length===0)return{configSource:z,configValid:!0};return{configSource:z,configValid:!1}}}catch(ne){if(ne instanceof r)return{configSource:z,configValid:!1};throw ne}try{if(n(te.userConfigTarget)!==null){let ne=Jt(te.userConfigTarget);return{configSource:Q,configValid:ne.errors.length===0}}return{configSource:null,configValid:!0}}catch(ne){if(ne instanceof r)return{configSource:Q,configValid:!1};throw ne}}function Ff(P,V,z,Q){let te=V.policy,ne=Ee({...te,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:{...te.destructiveCommandRuleOverrides,...Object.fromEntries(U.flatMap((ye)=>ye.activationCapability?[[ye.id,"on"]]:[]))}},V.state==="degraded"?{diagnostics:V.diagnostics,reason:V.reason}:void 0),ce=W(P,{environment:Q,dependencies:{loadPolicySnapshot:()=>ne,getModes:()=>({...z,strict:!0,paranoidRm:!0,paranoidInterpreters:!0}),findSensitiveTarget:()=>null}});return ce.decision.kind==="deny"?ce.decision.ruleId:void 0}function jf(P){if(!P)return;return{id:k(P.id),...P.rulebook?{rulebook:{name:k(P.rulebook.name),version:k(P.rulebook.version)}}:{},...P.source?{source:k(P.source)}:{},...P.override?{override:{type:"reason",reason:k(P.override.reason)}}:{}}}function Nf(P){let V=P.events.flatMap((Q)=>Q.kind==="step"&&Q.scope==="global"?[Q.step]:[]),z=new Map;for(let Q of P.events){if(Q.kind!=="step"||Q.scope!=="segment")continue;let te=z.get(Q.segmentIndex)??{index:Q.segmentIndex,steps:[]};te.steps.push(Q.step),z.set(Q.segmentIndex,te)}return{steps:V,segments:[...z.values()]}}function Mf(P,V){let z=P?.replace(/^custom\./,"");if(!z||!V.policy.rules.some((Q)=>Q.name===z))return;return V.ruleMetadata[z]??Object.freeze({id:z})}function wl(P){return new Promise((V)=>{process.stdout.write(`${P}
`,()=>V())})}async function kl(P,V){let z=Ko(V);if(!z)return 1;try{let Q=Nn(z.command,{cwd:z.cwd},P),te=!!process.env.NO_COLOR||!process.stdout.isTTY;return await wl(z.json?Xo(Q):Zo(Q,{asciiOnly:te})),0}catch(Q){let te=Hf(Q instanceof p?Q.cause:Q);if(te===void 0)throw Q;if(z.json)return await wl(JSON.stringify({error:te})),1;return console.error(te),1}}function Hf(P){if(P instanceof y)return P.message;if(P instanceof f)return P.message;if(P instanceof s&&a[P.kind].errorCode==="path-canonicalization-limit")return"Path canonicalization work limit exceeded.";return}var xl="2.4.2",Ot="  ",rn="cc-safety-net";function Cl(P){return P.argument?`${P.flags} ${P.argument}`:P.flags}function Uf(P){return Math.max(...P.map((V)=>Cl(V).length))}function Gf(P){return Math.max(...P.map((V)=>V.usage.length))}function Bf(P){return Math.max(...P.map((V)=>`${rn} ${V.usage}`.length))}function qf(P,V){let z=`${rn} ${P.usage}`;return`${Ot}${z.padEnd(V+2)}${P.description}`}function qt(P,V){return`${Ot}${P.padEnd(Math.max(40,P.length+2))}${V}`}function wn(P,V=console.log){let z=[];if(z.push(`${rn} ${P.name}`),z.push(""),z.push(`${Ot}${P.description}`),z.push(""),z.push("USAGE:"),z.push(`${Ot}${rn} ${P.usage}`),z.push(""),P.subcommands&&P.subcommands.length>0){z.push("SUBCOMMANDS:");let Q=Gf(P.subcommands);for(let te of P.subcommands)z.push(`${Ot}${te.usage.padEnd(Q+2)}${te.description}`);z.push("")}if(P.options.length>0){z.push("OPTIONS:");let Q=Uf(P.options);for(let te of P.options){let ne=Cl(te),ce=te.default?`${te.description} (default: ${te.default})`:te.description;z.push(`${Ot}${ne.padEnd(Q+2)}${ce}`)}z.push("")}if(P.examples&&P.examples.length>0){z.push("EXAMPLES:");for(let Q of P.examples)z.push(`${Ot}${Q}`)}V(z.join(`
`))}function ni(){let P=Bf(Qn),V=[];V.push(`${rn} v${xl}`),V.push(""),V.push("Blocks destructive commands and secret access."),V.push(""),V.push("COMMANDS:");for(let z of Qn)V.push(qf(z,P));V.push(""),V.push("GLOBAL OPTIONS:"),V.push(`${Ot}-h, --help       Show help (use with command for command-specific help)`),V.push(`${Ot}-V, --version    Show version`),V.push(""),V.push("HELP:"),V.push(`${Ot}${rn} help <command>     Show help for a specific command`),V.push(`${Ot}${rn} <command> --help   Show help for a specific command`),V.push(""),V.push("ENVIRONMENT VARIABLES:"),V.push(qt(`${o.level.name}=standard|strict|paranoid`,"Set session safety level")),V.push(qt(`${o.worktree.name}=1`,"Allow local git discards in linked worktrees")),V.push(qt(`${o.debug.name}=1`,"Print diagnostic messages to stderr")),V.push(qt(`${o.auditScope.name}=all|blocked`,"Record all command decisions, or denials only")),V.push(qt("CC_SAFETY_NET_HOME","Override rule config home directory")),V.push(""),V.push("LEGACY ENVIRONMENT VARIABLES (STILL SUPPORTED):"),V.push(qt(`${o.strict.name}=1`,"Force safety.overrides.fail_closed on")),V.push(qt(`${o.paranoid.name}=1`,"Force paranoid_rm and paranoid_interpreters on")),V.push(qt(`${o.paranoidRm.name}=1`,"Force safety.overrides.paranoid_rm on")),V.push(qt(`${o.paranoidInterpreters.name}=1`,"Force safety.overrides.paranoid_interpreters on")),V.push(""),V.push("Documentation:        https://ccsafetynet.com/docs"),console.log(V.join(`
`))}function Sl(){console.log(xl)}function Mn(P,V=console.log){let z=er(P);if(!z)return!1;if(z.name.toLowerCase()!==P.toLowerCase())return!1;return wn(z,V),!0}import{existsSync as gi,readFileSync as wc}from"node:fs";import{join as fi}from"node:path";import*as Kt from"node:readline";function Vf(P){return P==="install"?"Install":"Uninstall"}function zf(P){return P==="install"?"Installing":"Uninstalling"}function Jf(P){return P==="install"?"into":"from"}function El(P){return P?.available===!0}function Wf(P,V){let z=new Set(V);return P.filter((Q)=>z.has(Q.target)).map((Q)=>Q.target)}function Rl(P,V,z){if(P.length===0||P.every((Q)=>!Q.available))return V;return Array.from({length:P.length},(Q,te)=>te+1).map((Q)=>(V+Q*z+P.length)%P.length).find((Q)=>El(P[Q]))}function Kf(P,V,z){if(z.ctrl&&z.name==="c")return"interrupt";if(z.name==="escape"||V==="q")return"abort";if(P==="install"&&(V==="u"||V==="U"))return"update";if(z.name==="up"||V==="k")return"up";if(z.name==="down"||V==="j")return"down";if(z.name==="space"||V===" ")return"toggle";if(z.name==="return"||z.name==="enter")return"confirm";return null}function Yf(P){return{cursor:P.findIndex((V)=>V.available),selected:[]}}function Zf(P,V,z){if(z==="confirm"||z==="update"||z==="abort"||z==="interrupt")return{state:P,done:z};if(z==="up")return{state:{...P,cursor:Rl(V,P.cursor,-1)}};if(z==="down")return{state:{...P,cursor:Rl(V,P.cursor,1)}};let Q=V[P.cursor];if(!El(Q))return{state:P};let te=P.selected.includes(Q.target)?P.selected.filter((ne)=>ne!==Q.target):Wf(V,[...P.selected,Q.target]);return{state:{...P,selected:te}}}var Dl="◉",Al="◯",_l=">",Tl=" ";function Xf(P,V,z,Q={}){let te=Q.color!==!1,ne=te?dt.dim:(he)=>he,ce=te?dt.green:(he)=>he,ye=te?dt.bold:(he)=>he;return["",`${Vf(P)} CC Safety Net ${Jf(P)}:`,"",...V.map((he,be)=>{let Le=z.selected.includes(he.target),Ie=be===z.cursor,Xe=Le?Dl:Al,et=Ie?_l:Tl,xe=he.available?"":` (${he.unavailableReason??"not installed"})`,at=`${Xe} ${he.label}${xe}`,ct=!he.available?ne(at):Le?ce(at):Ie?ye(at):at;return`${et} ${ct}`}),"",P==="install"?"Space: select  Enter: confirm  u: update installed  Up/Down: move  q/Esc: cancel":V.some((he)=>he.available)?"Space: select  Enter: confirm  Up/Down: move  q/Esc: cancel":`No selectable integrations found for ${P}. q/Esc: close`].join(`
`)}var Pl=["global-hook","plugin"];function Qf(P,V,z={}){let Q=z.color!==!1?dt.bold:(ne)=>ne;return["","Install the Kimi Code integration as:","",...[`Global hook — ${V?"already installed; selecting it reports the current state":"write the hook into ~/.kimi-code/config.toml now"}`,"Native Kimi plugin — print the steps to run inside Kimi Code"].map((ne,ce)=>{let ye=ce===P,he=`${ye?Dl:Al} ${ne}`;return`${ye?_l:Tl} ${ye?Q(he):he}`}),"","Enter: confirm  Up/Down: move  q/Esc: cancel"].join(`
`)}function $l(P){let{input:V,output:z}=P;Kt.emitKeypressEvents(V);let Q=V.isRaw===!0;V.setRawMode(!0),V.resume();let te=0,ne=()=>{if(te===0)return;Kt.moveCursor(z,0,-te),Kt.cursorTo(z,0),Kt.clearScreenDown(z)},ce=()=>{ne();let ye=P.render();z.write(`${ye}
`),te=ye.split(`
`).length};return new Promise((ye)=>{let he=(Le)=>{V.off("keypress",be),V.setRawMode(Q),V.pause(),ne(),ye(Le)};function be(Le,Ie){P.onKey(Le,Ie,{finish:he,draw:ce})}V.on("keypress",be),ce()})}function Il(P={}){let V=0;return $l({input:P.input??process.stdin,output:P.output??process.stdout,render:()=>Qf(V,P.globalHookInstalled===!0),onKey:(z,Q,te)=>{if(Q.ctrl&&Q.name==="c"){te.finish(null),(P.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return}if(Q.name==="escape"||z==="q")return te.finish(null);if(Q.name==="return"||Q.name==="enter")return te.finish(Pl[V]);if(Q.name==="up"||Q.name==="down"||z==="k"||z==="j")V=(V+1)%Pl.length,te.draw()}})}function ri(P=process.stdin,V=process.stdout){return Boolean(P.isTTY&&V.isTTY&&typeof P.setRawMode==="function")}function Ol(P,V,z={}){let Q=z.output??process.stdout,te=Yf(V);return $l({input:z.input??process.stdin,output:Q,render:()=>Xf(P,V,te),onKey:(ne,ce,ye)=>{let he=Kf(P,ne,ce);if(!he)return;let be=Zf(te,V,he);if(te=be.state,be.done==="interrupt"){ye.finish(null),(z.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return}if(be.done==="abort")return ye.finish(null);if(be.done==="update")return ye.finish("update");if(be.done==="confirm"){if(te.selected.length===0){Q.write("\x07"),ye.draw();return}ye.finish([...te.selected]),Q.write(`${zf(P)} selected integrations...
`);return}ye.draw()}})}import{existsSync as Fl,lstatSync as tm,mkdirSync as nm,mkdtempSync as rm,readdirSync as om,readFileSync as xn,rmSync as Ir}from"node:fs";import{basename as im,dirname as sm,join as Et}from"node:path";import{fileURLToPath as am}from"node:url";var oi="// cc-safety-net managed Amp plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --amp",on="cc-safety-net",sn="cc-safety-net/index.ts";import{spawn as em}from"node:child_process";var ii=(P,V)=>{let z=Xt([...P],process.env);return new Promise((Q)=>{let te=em(z.cmd,z.args,{cwd:V,stdio:["ignore","pipe","pipe"]}),ne=Fo(te),ce=!1,ye=setTimeout(()=>{ce=!0,te.kill()},120000);te.on("error",(he)=>{clearTimeout(ye),Q({status:null,errorCode:he.code,stdout:ne.stdout,stderr:[he.message,ne.stderr].filter(Boolean).join(`
`)})}),te.on("close",(he)=>{clearTimeout(ye),Q({status:ce?null:he,errorCode:ce?"ETIMEDOUT":void 0,stdout:ne.stdout,stderr:ne.stderr})})})};var kn="cc-safety-net.ts",si=Et("amp",sn);function lm(P){return Et(P.home,".config","amp","plugins","cc-safety-net.ts")}function cm(){let P=sm(am(import.meta.url));return[Et(P,si),Et(P,"..",si),Et(P,"..","..","..","dist",si)]}function dm(P=cm()){let V=P.find((z)=>Fl(z)&&tm(z).isFile());if(!V)throw Error("Packaged Amp plugin artifact not found. Reinstall cc-safety-net and try again.");return V}function jl(P){try{return JSON.parse(P)}catch{return}}function Or(P){return P.subarray(0,Buffer.byteLength(oi)).toString("utf-8")===oi}async function Hn(P,V,z){let Q=await P(V,z);if(Q.status===0)return Q;throw Error([`Failed to run ${V.join(" ")}${Q.status===null?"":` (exit ${Q.status})`}.`,[Q.stdout,Q.stderr].filter(Boolean).join(`
`).trim()].filter(Boolean).join(`
`))}async function Nl(P){let V=await P(["amp","plugins","repositories","--json"]);if(V.status===null)throw Error(`${V.errorCode==="ENOENT"?'Amp CLI not found. Install the amp CLI, sign in with "amp login", and rerun install --amp.':`amp plugins repositories --json did not finish (${V.errorCode??"terminated"}). Check that the amp CLI responds and rerun install --amp.`}
...
`)}`.trim());let z=jl(V.stdout),Q=(Array.isArray(z)?z:[]).filter((te)=>ft(te,"scope")==="user"&&ft(te,"exists")===!0&&ft(te,"viewerCanWrite")===!0).map((te)=>ft(te,"cloneRef")).find((te)=>typeof te==="string"&&te.length>0);if(!Q)throw Error('Your Amp account has no writable Personal Plugins repository. Sign in with "amp login", open Amp once to create it, and rerun install --amp.');return Q}async function Ml(P,V,z){let Q=rm(Et(V.tmpdir,"cc-safety-net-amp-"));try{return await Hn(P,["amp","clone","user-plugins",Q]),await z(Q)}finally{Ir(Q,{recursive:!0,force:!0})}}function ai(P){return`rerun ${P==="overwrite"?"install":"uninstall"} --amp`}function Hl(P,V,z){let Q=Et(P,V),te=Lt(Q);if(!te)return;if(te.isSymbolicLink()||!te.isFile())throw Error(`Refusing to ${z} ${V} in your Amp personal plugins repository: not a regular file. Remove it there and ${ai(z)}.`);let ne=xn(Q);if(Or(ne))return ne;throw Error(`Refusing to ${z} unmanaged file ${V} in your Amp personal plugins repository. Remove it there and ${ai(z)}.`)}function Ul(P,V){let z=Et(P,on),Q=Lt(z);if(!Q)return;if(Q.isSymbolicLink()||!Q.isDirectory())throw Error(`Refusing to ${V} ${on} in your Amp personal plugins repository: not a regular directory. Remove it there and ${ai(V)}.`);return Hl(P,sn,V)}function um(P){let V=Et(P,kn),z=Lt(V);if(!z||z.isSymbolicLink()||!z.isFile())return;let Q=xn(V);return Or(Q)?Q:void 0}async function Gl(P,V,z,Q){if(await Hn(P,z,V),(await Hn(P,["git","status","--porcelain"],V)).stdout.trim()==="")return!1;return await Hn(P,["git","-c","commit.gpgsign=false","-c","user.name=cc-safety-net","-c","user.email=cc-safety-net@localhost","commit","-m",Q],V),await Hn(P,["git","push","origin","HEAD"],V),!0}function $r(P,V){pm(P,V),fm(P,V)}function Bl(P,V){if(V==="keep")return;throw Error(`Local Amp plugin ${P} is not a managed copy and masks the personal plugin. Remove it and rerun install --amp.`)}function pm(P,V){let z=lm(P),Q=Lt(z);if(!Q)return;if(!Q.isSymbolicLink()&&Q.isFile()&&Or(xn(z))){Ir(z);return}Bl(z,V)}function fm(P,V){let z=Et(P.home,".config","amp","plugins",on),Q=Lt(z);if(!Q)return;if(!Q.isSymbolicLink()&&Q.isDirectory()&&mm(z)){Ir(z,{recursive:!0});return}Bl(z,V)}function mm(P){let V=im(sn);if(om(P).join("\x00")!==V)return!1;let z=Et(P,V),Q=Lt(z);return!!Q&&!Q.isSymbolicLink()&&Q.isFile()&&Or(xn(z))}function gm(P){let V=u(P);if(!Fl(V))return"";let z=jl(xn(V,"utf-8"));if(!z||typeof z!=="object"||Array.isArray(z))return"";return`;globalThis.__CC_SAFETY_NET_EMBEDDED_POLICY__ = ${JSON.stringify(C(z,P.home))};
`}async function ql(P,V=dm(),z=ii){let Q=Buffer.concat([xn(V),Buffer.from(gm(P),"utf-8")]),te=await Nl(z);return Ml(z,P,async(ne)=>{let ce=`${te}/${on}`,ye=Ul(ne,"overwrite"),he=Hl(ne,kn,"overwrite");if(ye?.equals(Q)&&!he)return $r(P,"fail"),{path:ce,alreadyInstalled:!0};if(nm(Et(ne,on),{recursive:!0}),kt(Et(ne,sn),Q),he)Ir(Et(ne,kn));let be=await Gl(z,ne,["git","add","--",sn,...he?[kn]:[]],`chore: update cc-safety-net plugin to v${wt()}`);return $r(P,"fail"),{path:ce,alreadyInstalled:!be}})}async function Vl(P,V=ii){let z=await Nl(V);return Ml(V,P,async(Q)=>{let te=Ul(Q,"remove"),ne=um(Q),ce=`${z}/${ne&&!te?kn:on}`;if(!te&&!ne)return $r(P,"keep"),{path:ce,alreadyInstalled:!1};return await Gl(V,Q,["git","rm","--",...te?[sn]:[],...ne?[kn]:[]],`chore: remove cc-safety-net plugin v${wt()}`),$r(P,"keep"),{path:ce,alreadyInstalled:!0}})}import{existsSync as zl,mkdirSync as ym,readFileSync as hm}from"node:fs";import{dirname as vm}from"node:path";var li=Nt["antigravity-cli"],an="cc-safety-net";function ln(P){return Boolean(P)&&typeof P==="object"&&!Array.isArray(P)}function jr(){return{PreToolUse:[{hooks:[{type:"command",command:li,timeout:30}]}]}}function Jl(P){try{let V=JSON.parse(hm(P,"utf-8"));if(!V||typeof V!=="object"||Array.isArray(V))throw Error("Antigravity hooks config must be a JSON object");return V}catch(V){if(V instanceof SyntaxError)throw Error(`Failed to parse Antigravity hooks config ${P}: ${V.message}`);throw V}}function Wl(P){let V=P[an];if(V===void 0){let Q=jr();return P[an]=Q,{definition:Q,preToolUse:Q.PreToolUse??[]}}if(!ln(V))throw Error(`Antigravity hooks config entry "${an}" must be an object`);let z=Array.isArray(V.PreToolUse)?V.PreToolUse:[];return V.PreToolUse=z,{definition:V,preToolUse:z}}function Kl(P){if(!Array.isArray(P.PreToolUse))return!1;return P.PreToolUse.some((V)=>ln(V)&&Array.isArray(V.hooks)&&V.hooks.some((z)=>ln(z)&&z.command===li))}function bm(P){return Object.values(P).some((V)=>ln(V)&&V.enabled!==!1&&Kl(V))}function Lm(P){if(P[an]===void 0)return!1;let V=Wl(P);if(V.definition.enabled!==!1||!Kl(V.definition))return!1;return V.definition.enabled=!0,!0}function wm(P){if(P[an]===void 0){P[an]=jr();return}let V=Wl(P);V.definition.enabled=!0,V.preToolUse.push(jr().PreToolUse?.[0]??{hooks:[]})}function km(P){let V=!1;for(let z of Object.values(P)){if(!ln(z)||!Array.isArray(z.PreToolUse))continue;z.PreToolUse=z.PreToolUse.flatMap((Q)=>{if(!ln(Q)||!Array.isArray(Q.hooks))return[Q];let te=Q.hooks.filter((ne)=>!ln(ne)||ne.command!==li);if(te.length!==Q.hooks.length)V=!0;return te.length===0?[]:[{...Q,hooks:te}]})}return V}function Fr(P,V){kt(P,`${JSON.stringify(V,null,2)}
`)}function Yl(P){let V=En(P.home);if(ym(vm(V),{recursive:!0}),!zl(V))return Fr(V,{[an]:jr()}),{path:V,alreadyInstalled:!1};let z=Jl(V);if(bm(z))return{path:V,alreadyInstalled:!0};if(Lm(z))return Fr(V,z),{path:V,alreadyInstalled:!1};return wm(z),Fr(V,z),{path:V,alreadyInstalled:!1}}function Zl(P){let V=En(P.home);if(!zl(V))return{path:V,alreadyInstalled:!1};let z=Jl(V);if(!km(z))return{path:V,alreadyInstalled:!1};return Fr(V,z),{path:V,alreadyInstalled:!0}}import{existsSync as xm,readdirSync as Cm,rmSync as Sm}from"node:fs";import{join as Rm}from"node:path";function Xl(P,V=process.platform,z){if(!xm(P))return;let Q=V==="win32"?/^bunx-\d+-cc-safety-net@/:new RegExp(`^bunx-${process.getuid?.()??0}-cc-safety-net@`);Cm(P).filter((te)=>te!==z&&Q.test(te)).forEach((te)=>{Sm(Rm(P,te),{recursive:!0,force:!0})})}import{spawn as Pm}from"node:child_process";var Mt=$t.map((P)=>({target:P.id,flag:P.flag,label:bt(P.id),probeCommand:P.probeCommand}));function ci(P){let V=new Set(P);return Mt.map((z)=>z.target).filter((z)=>V.has(z))}async function Ql(P,V){for(let z of P)await V(z)}var Em=5000;function di(P,V=Em){return new Promise((z)=>{let Q=Xt([...P],process.env),te=Pm(Q.cmd,Q.args,{env:process.env,stdio:"ignore"}),ne=!1,ce=(he)=>{if(ne)return;ne=!0,clearTimeout(ye),z(he)},ye=setTimeout(()=>{te.kill(),ce(!1)},V);te.on("error",()=>ce(!1)),te.on("close",(he)=>ce(he===0))})}function ec(P=di,V={}){let z=new Set(V.configuredTargets??[]);return Promise.all(Mt.map(async(Q)=>({target:Q.target,flag:Q.flag,label:Q.label,...nc(V.action,await P(Q.probeCommand),z.has(Q.target))})))}function tc(P,V){let z=new Set(V.configuredTargets??[]);return P.map((Q)=>({...Q,...nc(V.action,Q.available,z.has(Q.target))}))}function nc(P,V,z){if(P==="uninstall")return z?{available:!0}:{available:!1,unavailableReason:"not installed"};if(P==="install"&&z)return{available:!1,unavailableReason:"already installed"};if(!V)return{available:!1,unavailableReason:"CLI not installed"};return{available:!0}}import{existsSync as rc,readdirSync as Dm,rmSync as Am}from"node:fs";import{join as Cn}from"node:path";function Nr(P,V=process.platform){let z=Cn(P.env.get("npm_config_cache")||(V==="win32"?Cn(P.env.get("LOCALAPPDATA")||Cn(P.home,"AppData","Local"),"npm-cache"):Cn(P.home,".npm")),"_npx");if(!rc(z))return;Dm(z).filter((Q)=>rc(Cn(z,Q,"node_modules","cc-safety-net"))).forEach((Q)=>{Am(Cn(z,Q),{recursive:!0,force:!0})})}import{existsSync as cc,mkdirSync as Tm,readFileSync as dc}from"node:fs";import{dirname as $m,join as lc}from"node:path";function _m(P,V){if(P[V]!=="#")return V;let z=P.indexOf(`
`,V+1);return z===-1?P.length:z+1}function ui(P,V,z){let Q=new RegExp(`^(\\s*)${V}\\s*=\\s*\\[`),te=0;for(let ne of P.split(`
`)){if(/^\s*\[/.test(ne))return;let ce=Q.exec(ne);if(ce){let ye=te+ce[0].lastIndexOf("[");return{start:ye,end:bo(P,ye,{skipComment:_m,...z})}}te+=ne.length+1}return}function oc(P,V,z){let Q=P.slice(0,V.end).trimEnd(),te=Zs(P,V.end),ne=te===""?"     ":`${te}  `,ce=!Q.endsWith("[")&&!Q.endsWith(",");return`${Q}${ce?",":""}
${ne}${z}${P.slice(V.end)}`}function ic(P,V,z){let Q=P.indexOf(z,V.start);if(Q===-1||Q>V.end)return P;return mr(P,{start:Q,end:Q+z.length})}function sc(P,V){let z=new RegExp(`^\\s*${V}\\s*=\\s*\\[\\s*]\\s*(?:#.*)?$`),Q=P.split(`
`),te=Q.findIndex((ye)=>/^\s*\[/.test(ye)),ne=te===-1?Q:Q.slice(0,te),ce=te===-1?[]:Q.slice(te);return[...ne.filter((ye)=>!z.test(ye)),...ce].join(`
`)}function ac(P,V,z){let Q=new RegExp(`^\\s*\\[\\[${V}]]\\s*$`,"m");return P.split(/(?=^\s*\[)/m).filter((te)=>!Q.test(te)||!te.includes(z)).join("").trimEnd()}var Un=Nt["kimi-code"],pi=`[[hooks]]
event = "PreToolUse"
command = "${Un}"`,uc=`{ event = "PreToolUse", command = "${Un}" }`,pc={stringError:"Unterminated string in Kimi Code config",bracketError:"Unmatched hooks array in Kimi Code config"};function fc(P){return lc(P.env.get("KIMI_CODE_HOME")??lc(P.home,".kimi-code"),"config.toml")}function Im(P){let V=ui(P,"hooks",pc);if(V&&P.slice(V.start+1,V.end).trim())return oc(P,V,uc);let z=sc(P,"hooks").trimEnd();if(z==="")return`${pi}
`;return`${z}

${pi}
`}function mc(P){let V=fc(P);if(Tm($m(V),{recursive:!0}),!cc(V))return kt(V,`${pi}
`),{path:V,alreadyInstalled:!1};let z=dc(V,"utf-8");if(z.includes(Un))return{path:V,alreadyInstalled:!0};return kt(V,Im(z)),{path:V,alreadyInstalled:!1}}function gc(P){let V=fc(P);if(!cc(V))return{path:V,alreadyInstalled:!1};let z=dc(V,"utf-8");if(!z.includes(Un))return{path:V,alreadyInstalled:!1};let Q=ui(z,"hooks",pc),te=Q?ic(z,Q,uc):`${ac(z,"hooks",Un)}
`;return kt(V,te),{path:V,alreadyInstalled:!0}}var mi="safety-net@cc-marketplace",yc=new Set(["claude-code","codex","copilot-cli","gemini-cli","hermes-agent","openclaw","opencode","pi"]),hc=new Set(["antigravity-cli","cursor","grok-build","hermes-agent","kimi-code"]);function yi(P){return/^\s*safety-net@cc-marketplace[^a-z0-9-][^\n]*installed,/m.test(P??"")}function kc(P){return/^\s*cc-safety-net[^a-z0-9-][^\n]*installed,/m.test(P??"")}function Om(P){return/^Marketplace `cc-marketplace`\s*$/m.test(P??"")}var xc={"claude-code":{installCommands:(P)=>{let V=pr(P,"cc-safety-net@cc-marketplace");return{commands:[...V?[["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","update","cc-safety-net@cc-marketplace"]]:[["claude","plugin","marketplace","add","kenryu42/cc-marketplace"],["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","install","cc-safety-net@cc-marketplace"]],...ho(P).status==="disabled"?[["claude","plugin","enable","cc-safety-net@cc-marketplace"]]:[]],cleanupCommands:pr(P,mi)?[["claude","plugin","uninstall",mi]]:[],update:V}},uninstallCommands:[["claude","plugin","uninstall","cc-safety-net@cc-marketplace"],["claude","plugin","marketplace","remove","cc-marketplace"]]},codex:{installCommands:async(P,V)=>{let z=V??await _t(["codex","plugin","list"]),Q=kc(z);return{commands:[Q||Om(z)?["codex","plugin","marketplace","upgrade","cc-marketplace"]:["codex","plugin","marketplace","add","kenryu42/cc-marketplace"],["codex","plugin","add","cc-safety-net@cc-marketplace"]],cleanupCommands:yi(z)?[["codex","plugin","remove","safety-net@cc-marketplace"]]:[],update:Q}},uninstallCommands:[["codex","plugin","remove","cc-safety-net@cc-marketplace"],["codex","plugin","marketplace","remove","cc-marketplace"]],postInstallMessage:"Start Codex, open `/hooks`, select the cc-safety-net PreToolUse hook, and press `t` to trust it."},"copilot-cli":{installCommands:async()=>{let P=await _t(["copilot","plugin","list"]),V=[...ia(P)?[["copilot","plugin","uninstall","copilot-safety-net"]]:[],...sa(P)?[["copilot","plugin","uninstall",na]]:[]];if(ra(P))return{commands:[["copilot","plugin","marketplace","update","cc-marketplace"],["copilot","plugin","update",jt]],cleanupCommands:V,update:!0};return{commands:[oa(await _t(["copilot","plugin","marketplace","list"]))?["copilot","plugin","marketplace","update","cc-marketplace"]:["copilot","plugin","marketplace","add","kenryu42/cc-marketplace"],["copilot","plugin","install",jt]],cleanupCommands:V}},uninstallCommands:[["copilot","plugin","uninstall","cc-safety-net@cc-marketplace"],["copilot","plugin","marketplace","remove","cc-marketplace"]]},"gemini-cli":{installCommands:(P)=>{let V=Ro(P);if(V.status==="configured")return{commands:[["gemini","extensions","update","gemini-safety-net"]],update:!0};if(V.status==="disabled")return{commands:[["gemini","extensions","update","gemini-safety-net"],["gemini","extensions","enable","gemini-safety-net"]],update:!0};return{commands:[["gemini","extensions","install","https://github.com/kenryu42/gemini-safety-net","--consent"]]}},uninstallCommands:[["gemini","extensions","uninstall","gemini-safety-net"]]},openclaw:{beforeInstall:Uo,installCommands:()=>({commands:Ja()}),uninstallCommands:[["openclaw","plugins","uninstall",ht,"--force"]],postInstallMessage:["Restart the OpenClaw Gateway to apply the change.","If plugins.allow is set in openclaw.json, it must also list cc-safety-net."].join(`
`)},opencode:{installCommands:sl},pi:{installCommands:[["pi","install","npm:cc-safety-net"]],uninstallCommands:[["pi","uninstall","npm:cc-safety-net"]]}};function Cc(P,V=(z)=>z){try{let z=JSON.parse(V(wc(P,"utf-8")));if(!z||typeof z!=="object"||Array.isArray(z))throw Error(`Settings file ${P} must be a JSON object`);return z}catch(z){if(z instanceof SyntaxError)throw Error(`Failed to parse ${P}: ${z.message}`);throw z}}function Fm(P){let V=fi(Dn(P),"settings.json");if(!gi(V))return;let z=Cc(V,At),Q=z.enabledPlugins;if(!Q||typeof Q!=="object"||Array.isArray(Q))return;if(Q[jt]!==!1)return;let te=wc(V,"utf-8"),ne=te.replace(new RegExp(`("${jt}"\\s*:\\s*)false`),"$1true");return Q[jt]=!0,kt(V,ne!==te?ne:`${JSON.stringify(z,null,2)}
`),`Enabled ${jt} plugin in ${V}`}function jm(P){let V=zo(P);if(!gi(V))return;let z=Cc(V);if(!Array.isArray(z.packages))return;let Q=z.packages.find((te)=>!!te&&typeof te==="object"&&!Array.isArray(te)&&Jo(te.source)&&("extensions"in te));if(!Q)return;return delete Q.extensions,kt(V,`${JSON.stringify(z,null,2)}
`),`Enabled npm:cc-safety-net extensions in ${V}`}function vc(P,V){let z=xt({label:V,booleans:Object.fromEntries(Mt.map((ne)=>[ne.target,[ne.flag]]))},P),Q=z.errors[0];if(Q)throw Error(Q);let te=Mt.filter((ne)=>z.flags[ne.target]).map((ne)=>ne.target);if(te.length!==1)throw Error(`Choose exactly one ${V} target: ${Mt.map((ne)=>ne.flag).join(", ")}`);return te[0]}async function Sc(P,V=mn){let[z,Q,te]=await Promise.all([V(["amp","plugins","list"],30000),V(["codex","plugin","list"],30000),V(["copilot","--binary-version"])]);return{codexPluginListOutput:Q,hooks:Ln(P,process.cwd(),{ampPluginListOutput:z,codexPluginListOutput:Q,copilotCliVersion:te})}}async function Nm(P,V,z=mn){let Q=await Sc(P,z);return Q.hooks.filter((te)=>V==="install"?te.configured:te.detected||te.inspectionStatus==="not-inspected").filter((te)=>te.platform!=="codex"||!yi(Q.codexPluginListOutput)||kc(Q.codexPluginListOutput)).map((te)=>te.platform)}function Mm(P,V,z,Q){if(z.length>0)return{finish:async()=>[vc(z,V)]};if(!Q.selectTargets&&!ri(Q.input,Q.output))return{finish:async()=>[vc(z,V)]};let te=Q.detectConfiguredTargets??(()=>Nm(P,V,Q.fetchVersion)),ne=Promise.all([ec(Q.probeTargets),te()]);return{ready:ne,finish:async()=>{let[ce,ye]=await ne,he=tc(ce,{action:V,configuredTargets:ye}),be=Q.selectTargets?await Q.selectTargets(V,Lc(V,he)):await Ol(V,Lc(V,he),{input:Q.input,output:Q.output});if(be==="update")return be;if(!be||be.length===0)return null;return ci(be)}}}async function Hm(P,V,z=!1,Q){let te=xc[P];te.beforeInstall?.(V);let ne=typeof te.installCommands==="function"?await te.installCommands(V,Q):{commands:te.installCommands};return await jo(ne.commands),await Ga(ne.cleanupCommands??[]),await ne.afterInstall?.(),[`${ne.update||z?"Updated":"Installed"} ${bt(P)} integration`,te.postInstallMessage].filter(Boolean).join(`
`)}async function Um(P){let V=xc[P];if(!V.uninstallCommands)throw Error(`${bt(P)} uninstall is not supported`);return await jo(V.uninstallCommands),`Uninstalled ${bt(P)} integration`}function Gm(P){let V=al(P);return V.alreadyInstalled?`Uninstalled OpenCode plugin from ${V.path}`:`OpenCode plugin not installed in ${V.path}`}var Rc={"antigravity-cli":{install:Yl,uninstall:Zl},cursor:{install:ya,uninstall:ha},"grok-build":{install:Pa,uninstall:Ea},"kimi-code":{install:mc,uninstall:gc}};function Bm(P,V,z,Q=!1){if(P==="install"&&!Q)Nr(z);let te=Rc[V][P](z),ne=bt(V),ce=P!=="install"?"Uninstalled":Q?"Updated":"Installed";return P==="install"&&te.alreadyInstalled?Q?`${ne} hook up to date in ${te.path}`:`${ne} hook already installed in ${te.path}`:P==="uninstall"&&!te.alreadyInstalled?`${ne} hook not installed in ${te.path}`:`${ce} ${ne} hook ${P==="install"?"in":"from"} ${te.path}`}var Pc={amp:{install:ql,uninstall:Vl,restartNote:'Amp personal plugins apply to every Amp session, including Orb threads. Restart Amp or run "plugins: reload" to apply the change.'},"hermes-agent":{install:Ta,uninstall:$a,afterInstall:async(P)=>{let V=Io(P);return await _t(["hermes","plugins","enable",Tt,"--no-allow-tool-override"]),!V},beforeUninstall:async(P)=>{$o(P);try{await _t(["hermes","plugins","disable",Tt])}catch(V){console.warn(`${V instanceof Error?V.message:String(V)}
Removing the plugin files anyway; ${Tt} may still be listed in the Hermes config.`)}},restartNote:"Restart Hermes to apply the change."}};async function qm(P,V,z,Q=!1){let te=Pc[V];if(P==="uninstall")await te.beforeUninstall?.(z);let ne=P==="install"?await te.install(z):await te.uninstall(z),ce=P==="install"&&await te.afterInstall?.(z),ye=bt(V),he=!ce&&(P==="install"&&ne.alreadyInstalled||P==="uninstall"&&!ne.alreadyInstalled);return[he?P==="install"?`${ye} plugin ${Q?"up to date":"already installed"} at ${ne.path}`:`${ye} plugin not installed at ${ne.path}`:`${P!=="install"?"Uninstalled":Q?"Updated":"Installed"} ${ye} plugin ${P==="install"?"at":"from"} ${ne.path}`,he?void 0:te.restartNote].filter(Boolean).join(`
`)}var Vm={"copilot-cli":{afterInstall:Fm},"hermes-agent":{beforeInstall:(P,V)=>{if(!V)Nr(P)}},openclaw:{afterInstall:async()=>{await Wa();return},beforeUninstall:Uo},pi:{afterInstall:jm}};function zm(P){return P in Rc}function Jm(P){return P in Pc}var bc=["Install CC Safety Net as a native Kimi Code plugin:","","  1. Start Kimi Code and run: /plugins install https://github.com/kenryu42/cc-safety-net","     Confirm the trust prompt; it defaults to cancel.","  2. Run /reload, or start a new session.","","Note: Kimi Code hooks are fail-open. When the hook process cannot start, crashes, or times","out, Kimi Code allows the tool call."].join(`
`);function Wm(P){if(In({environment:P,cwd:process.cwd()}).status!=="configured")return bc;return[bc,"",dt.red(["CAUTION: the global Kimi Code hook is installed and will run alongside the plugin.","After the plugin is active, remove it with: cc-safety-net uninstall --kimi-code"].join(`
`))].join(`
`)}function Lc(P,V){return V.map((z)=>P==="install"&&z.target==="kimi-code"&&z.unavailableReason==="already installed"?{...z,available:!0,unavailableReason:void 0,label:`${z.label} (global hook installed)`}:z)}function Km(P,V){if(P.selectKimiInstallMethod)return P.selectKimiInstallMethod();if(!ri(P.input,P.output))return Promise.resolve("global-hook");return Il({input:P.input,output:P.output,globalHookInstalled:In({environment:V,cwd:process.cwd()}).status==="configured"})}async function Ec(P,V,z,Q=!1,te){let ne=Vm[V];if(P==="install")ne?.beforeInstall?.(z,Q);if(P==="uninstall")ne?.beforeUninstall?.(z);if(zm(V))return Bm(P,V,z,Q);if(Jm(V))return qm(P,V,z,Q);if(P==="uninstall")return V==="opencode"?Gm(z):Um(V);return[await Hm(V,z,Q,te),await ne?.afterInstall?.(z)].filter(Boolean).join(`
`)}function Ym(P){let V=xt({label:"update"},P).errors[0];if(V)throw Error(V)}async function Zm(P,V=mn){let z=await Sc(P,V),Q=fi(Dn(P),"installed-plugins");return{targets:ci([...z.hooks.filter((ne)=>ne.platform!=="copilot-cli"&&ne.detected).map((ne)=>ne.platform),...[gr,ta,ea].flatMap((ne)=>gi(fi(Q,...ne))?["copilot-cli"]:[]),...pr(P,mi)?["claude-code"]:[],...yi(z.codexPluginListOutput)?["codex"]:[]]),codexPluginListOutput:z.codexPluginListOutput}}async function Xm(P){let V=c(),z=P.output??process.stdout,Q=(P.scriptPath??process.argv[1]??"").split(/[\\/]/),te=Q.find((et)=>/^bunx-\d+-/.test(et)),ne=te!==void 0||Q.includes("_npx")?null:(P.checkLatestVersion??Wt)(),ce=async()=>{let et=ne&&await ne;if(et?.updateAvailable)z.write(`
...
`)},ye=Zm(V,P.fetchVersion??mn).then(async(et)=>{let xe=new Set(et.targets);return{targets:et.targets,codexPluginListOutput:et.codexPluginListOutput,available:new Map(await Promise.all(Mt.filter((at)=>xe.has(at.target)&&yc.has(at.target)).map(async(at)=>[at.target,await di(at.probeCommand)])))}}),he=await Pn(P.showBanner??!0,()=>({ready:ye,finish:()=>ye}),()=>Rn({input:P.input??process.stdin,output:z}),{loadingMessage:"Checking installed integrations…",output:z}),be=await Promise.resolve().then(()=>(Xl(V.tmpdir,process.platform,te),null)).catch((et)=>Gn(et));if(he.targets.length===0){if(z.write("No installed integrations found. Run `cc-safety-net install` to set one up.\n"),be!==null)console.error(be);return await ce(),be===null?0:1}let Le=he.targets.some((et)=>hc.has(et))?await Promise.resolve().then(()=>(Nr(V),null)).catch((et)=>Gn(et)):null,Ie=await cr(Promise.all(he.targets.map((et)=>{if(yc.has(et)&&!he.available.get(et))return Promise.resolve({message:`${bt(et)} not found; skipped`,failed:!1});if(Le!==null&&hc.has(et))return Promise.resolve({message:Le,failed:!0});return Ec("install",et,V,!0,he.codexPluginListOutput).then((xe)=>({message:xe,failed:!1}),(xe)=>({message:Gn(xe),failed:!0}))})),{loadingMessage:`Updating ${he.targets.length} integration${he.targets.length===1?"":"s"}…`,output:z}),Xe=be===null?Ie:[...Ie,{message:be,failed:!0}];return Xe.forEach((et)=>{et.failed?console.error(et.message):z.write(`${et.message}
`)}),await ce(),Xe.some((et)=>et.failed)?1:0}function hi(P,V={}){return Promise.resolve().then(()=>Ym(P)).then(()=>Xm(V)).catch((z)=>(console.error(Gn(z)),1))}async function Bn(P,V,z={}){try{let Q=c(),te=await Pn(!0,()=>Mm(Q,P,V,z),()=>Rn({input:z.input??process.stdin,output:z.output??process.stdout}),{loadingMessage:P==="install"?"Checking available integrations…":"Checking installed integrations…",output:z.output??process.stdout});if(!te)return(z.output??process.stdout).write(`Cancelled: nothing was ${P}ed.
`),0;if(te==="update")return(z.runUpdate??(()=>hi([],{fetchVersion:z.fetchVersion,input:z.input,output:z.output,showBanner:!1})))();let ne=z.output??process.stdout;return await Ql(te,async(ce)=>{if(ce==="kimi-code"&&P==="install"){let he=await Km(z,Q);if(he===null){ne.write(`Cancelled: Kimi Code integration was not installed.
`);return}if(he==="plugin"){ne.write(`${Wm(Q)}
`);return}}let ye=await cr(Ec(P,ce,Q),{loadingMessage:`${P==="install"?"Installing":"Uninstalling"} ${bt(ce)} integration…`,output:ne});ne.write(`${ye}
...
Check that every parent path component is a directory.`;return V}import{mkdirSync as og}from"node:fs";import{dirname as ig}from"node:path";import{createInterface as sg}from"node:readline";import{existsSync as Ac,readFileSync as Qm}from"node:fs";function cn(P,V){let z=Ye(P,V);return{policy:z.policy,errors:ie(qe(z.issues,Je,(Q)=>Q.kind==="custom")," "," ")}}function qn(P,V){return cn(P,V).errors}function Dc(P,V){return{"safety.level":P.safety.level,...vi("safety.overrides",P.safety.overrides),"workflow.worktree_mode":String(P.workflow.worktree_mode),"destructive_command_protection.enabled":String(P.destructive_command_protection.enabled),...vi("destructive_command_protection.overrides",P.destructive_command_protection.overrides),"destructive_command_protection.allow_paths":bi(P.destructive_command_protection.allow_paths),"secret_protection.enabled":String(P.secret_protection.enabled),...vi("secret_protection.overrides",P.secret_protection.overrides),"secret_protection.deny_paths":bi(P.secret_protection.deny_paths),"secret_protection.allow_paths":bi(P.secret_protection.allow_paths),...V?{"audit.retention_days":String(P.audit.retention_days)}:{}}}function Mr(P,V,z){let Q=Dc(P,z),te=Dc(V,z);return[...new Set([...Object.keys(Q),...Object.keys(te)])].flatMap((ne)=>Q[ne]===te[ne]?[]:[{field:ne,before:Q[ne],after:te[ne]}])}function Vn(P,V){let z=u(P,V);if(!Ac(z))return{baseline:C(globalThis.__CC_SAFETY_NET_EMBEDDED_POLICY__,P.home),diagnostics:[]};let Q=dn(z),te=cn(Q.value,P.home);return{baseline:te.policy,diagnostics:Q.errors.length>0?Q.errors:te.errors}}function dn(P){if(!Ac(P))return{errors:[`${P}: file not found`]};try{return{value:JSON.parse(Qm(P,"utf-8")),errors:[]}}catch(V){let z=V instanceof Error?V.message:String(V);return{errors:[`${P}: ${V instanceof SyntaxError?`Invalid JSON: ${z}`:z}`]}}}function Hr(P,V){let z=eg(P)?P:{};return{version:V.version,...Object.fromEntries(["safety","workflow","destructive_command_protection","secret_protection"].filter((Q)=>z[Q]!==void 0).map((Q)=>[Q,z[Q]]))}}function vi(P,V){return Object.fromEntries(Object.entries(V).flatMap(([z,Q])=>Q===void 0?[]:[[`${P}.${z}`,String(Q)]]))}function bi(P){return P.length===0?"(none)":P.join(", ")}function eg(P){return!!P&&typeof P==="object"&&!Array.isArray(P)}import{chmodSync as tg,existsSync as _c,mkdirSync as ng,readFileSync as Tc}from"node:fs";import{dirname as rg}from"node:path";function $c(P,V={}){let z=u(P,V);if(!_c(z))return{path:z,exists:!1,raw:"",policy:H(),errors:[]};let Q=Tc(z,"utf-8");if(!Q.trim())return{path:z,exists:!0,raw:Q,policy:H(),errors:["Config file is empty"]};try{let te=cn(JSON.parse(Q),P.home);return{path:z,exists:!0,raw:Q,policy:te.policy,errors:te.errors}}catch(te){return{path:z,exists:!0,raw:Q,policy:H(),errors:[`Invalid JSON: ${te instanceof Error?te.message:String(te)}`]}}}function Vt(P,V,z={}){let Q=u(P,z),te=cn(V,P.home);if(te.errors.length>0)return{path:Q,policy:H(),errors:te.errors};let ne=te.policy;return ng(rg(Q),{recursive:!0,mode:448}),g(re(Q),`${JSON.stringify(ne,null,2)}
`,384),tg(Q,384),{path:Q,policy:ne,errors:[]}}function Ic(P,V){let z=cn(V,P.home);if(z.errors.length>0)return{errors:z.errors};return{preview:Ae(z.policy,P.env),errors:[]}}function Oc(P,V={}){let z=u(P,V);if(!_c(z))return Vt(P,Z,V);let Q=Tc(z,"utf-8");if(!Q.trim())return Vt(P,Z,V);try{return Vt(P,C(JSON.parse(Q),P.home),V)}catch{return Vt(P,Z,V)}}var Fc=new Set(["check","apply"]),jc="(unset)";async function Mc(P,V,z={}){let Q=xt({label:"policy",booleans:{global:["-g","--global"]},positionals:"list"},V),te=Q.positionals[0],ne=[...Q.errors,...te&&!Fc.has(te)?[`Unknown policy subcommand: ${te}`]:[],...te&&Fc.has(te)&&!Q.positionals[1]?[`policy ${te} requires a file`]:[],...Q.positionals.slice(2).map((xe)=>`Unexpected policy argument: ${xe}`)];if(ne.length>0){for(let xe of ne)console.error(xe);return 1}let ce=Q.positionals[1];if(!te||!ce)return wn(Xn,console.error),1;let ye=Q.flags.global?u(P):b(z.cwd??process.cwd()),he=dn(ce),be=[...he.errors,...qn(he.value,P.home).map((xe)=>`${ce}: ${xe}`),...!Q.flags.global&&cg(he.value)&&he.value.audit!==void 0?[`${ce}: audit settings are user scope only; remove the audit section from a project proposal`]:[]];if(be.length>0){for(let xe of be)console.error(xe);return 1}let Le=C(he.value,P.home);if(console.log(`Scope: ${Q.flags.global?"user":"project"} (${ye})`),console.log(`Proposal: ${ce}`),Q.flags.global)Nc(C(dn(ye).value,P.home),Le,!0);if(!Q.flags.global){let xe=Vn(P).baseline;console.log("Effective policy (user + project merged):"),Nc(Y(xe,ae(dn(ye).value,P.home).policy).policy,Y(xe,ae(he.value,P.home).policy).policy,!1)}if(te==="check")return 0;let Ie=z.input??process.stdin,Xe=z.output??process.stdout;if(!Ie.isTTY||!Xe.isTTY)return console.error("policy apply confirms interactively; run this yourself in a terminal:"),console.error(`  cc-safety-net policy apply ${ce}${Q.flags.global?" --global":""}`),1;if(!await ag(`Apply this policy to ${ye}? [y/N] `,Ie,Xe))return console.log("Cancelled; nothing was written."),0;return lg(P,ye,he.value,Le,Q.flags.global),console.log(`Policy applied: ${ye}`),0}function ag(P,V,z){let Q=sg({input:V,output:z,terminal:!1});return new Promise((te)=>{Q.once("close",()=>te(!1)),Q.question(P,(ne)=>{te(/^y(es)?$/i.test(ne.trim())),Q.close()})})}function lg(P,V,z,Q,te){if(te){Vt(P,Q);return}og(ig(V),{recursive:!0}),Rt(V,Hr(z,Q))}function Nc(P,V,z){let Q=Mr(P,V,z);if(Q.length===0){console.log("No changes.");return}console.log(`Changes (${Q.length}):`);for(let te of Q)console.log(`  ${te.field}: ${te.before??jc} -> ${te.after??jc}`)}function cg(P){return!!P&&typeof P==="object"&&!Array.isArray(P)}import{join as ky}from"node:path";var Hc="# Custom Rules Reference\n\nAgent reference for generating CC Safety Net rulebook configuration.\n\n## Config Locations\n\n| Scope | Config path | Rulebook path | Priority |\n|-------|-------------|---------------|----------|\n| User | `~/.cc-safety-net/rules/rule.json` | `~/.cc-safety-net/rules/<rulebook-name>/rulebook.json` | First |\n| Project | `.cc-safety-net/rules/rule.json` | `.cc-safety-net/rules/<rulebook-name>/rulebook.json` | Second |\n| GitHub source | Listed in a local `rule.json` | Vendored into the consumer's `<rulebook-name>/rulebook.json` by `rule add` | Source order |\n\nEvery rulebook is a live file: the runtime reads it on each tool call, so an edit applies to the next command with no publishing step.\n\nUser scope is evaluated before project scope; within a scope, sources apply in `rules` array order. A duplicate active rulebook name keeps the first claim and ignores the later rulebook with a warning, so a user-scoped name shadows a project-scoped one.\n\nUse `cc-safety-net rule init` to create an inert local config. Use `--global` for user scope. Use `cc-safety-net rule init --example` to also create an inactive example rulebook. `CC_SAFETY_NET_HOME` overrides the `~/.cc-safety-net` user root.\n\nLegacy inline `.safety-net.json` and `~/.cc-safety-net/config.json` files are not loaded at runtime. Convert them with `cc-safety-net rule migrate`.\n\n## rule.json Schema\n\n```json\n{\n  \"version\": 1,\n  \"rules\": [\"project-rules\", \"owner/repo#main/team-rules\"],\n  \"overrides\": {\n    \"project-rules/block-docker-system-prune\": {\n      \"reason\": \"Use targeted Docker cleanup commands.\"\n    },\n    \"team-rules/block-npm-global\": \"off\"\n  },\n  \"transparent_wrappers\": [\"rtk\"]\n}\n```\n\n- `version`: Required. Must be `1`.\n- `$schema`: Optional. `cc-safety-net rule verify` inserts it into a valid `rule.json` that lacks it.\n- `rules`: Optional array of rulebook source strings. Missing `rules` is treated as `[]`.\n- `overrides`: Optional object keyed by `<rulebook-name>/<rule-name>`.\n- `overrides` values are either `\"off\"` to disable a rule or an object with a required `reason` (replacement block reason) and an optional `intent` (one of `hard_stop`, `use_alternative`, `scope_down`, `manual_only`, `stop_and_explain`).\n- A project override cannot target a user-scoped rule: only that override is ignored, the user rule keeps its configured state, and `rule verify` reports the diagnostic as a failure.\n- `transparent_wrappers`: Optional array of command names that transparently execute a visible child command.\n- Transparent wrappers have no built-in defaults. Configure only wrappers you intentionally trust, such as `\"rtk\"`.\n- Use `cc-safety-net rule wrapper add rtk` to configure RTK without manually editing `rule.json`.\n\n## Rulebook Sources\n\n- Local sources are bare rulebook names such as `project-rules`; the rulebook file is `.cc-safety-net/rules/project-rules/rulebook.json`.\n- Run `cc-safety-net rule add owner/repo` to add every rulebook currently present on the repository's default branch.\n- Use `--only` to select one or more rulebooks while preserving their order: `cc-safety-net rule add owner/repo --only aws gcloud`.\n- Use `--ref` to select a branch, tag, or commit instead of the default branch: `cc-safety-net rule add owner/repo --ref v2 --only aws`.\n- GitHub sources are stored in canonical form as `owner/repo#ref/<rulebook-name>`. That form remains valid in `rule.json` and as direct CLI input.\n- GitHub refs may contain `/`-separated path segments, such as `feature/rulebook-v2`.\n- The GitHub source name, the repository directory name, and the rulebook `name` must match exactly.\n- Rulebook source strings must be unique in a config.\n\n## rulebook.json Schema\n\n```json\n{\n  \"rulebook_version\": 1,\n  \"name\": \"project-rules\",\n  \"version\": \"1.0.0\",\n  \"description\": \"Project-specific CC Safety Net rules.\",\n  \"author\": \"project\",\n  \"allowed_commands\": [\"docker\"],\n  \"rules\": [\n    {\n      \"name\": \"block-docker-system-prune\",\n      \"command\": \"docker\",\n      \"subcommand\": \"system\",\n      \"block_args\": [\"prune\"],\n      \"reason\": \"Use targeted cleanup instead.\"\n    }\n  ],\n  \"tests\": [\n    {\n      \"command\": \"docker system prune\",\n      \"expect\": \"blocked\",\n      \"rule\": \"block-docker-system-prune\"\n    },\n    {\n      \"command\": \"docker ps\",\n      \"expect\": \"allowed\"\n    }\n  ]\n}\n```\n\n### Rulebook Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `rulebook_version` | Yes | Must be `1` or `2` |\n| `name` | Yes | `^[a-zA-Z][a-zA-Z0-9_-]{0,63}$` |\n| `version` | Yes | Non-empty string |\n| `description` | No | Free text; not type-checked at runtime |\n| `author` | No | Free text; not type-checked at runtime |\n| `allowed_commands` | Yes | Unique command names matching `^[a-zA-Z][a-zA-Z0-9_-]*$` |\n| `rules` | Yes | Array of rule objects |\n| `tests` | No | Array of fixtures |\n\n### Rule Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `name` | Yes | Unique within the rulebook (case-insensitive); same pattern as rulebook `name` |\n| `command` | Yes | Must be listed in `allowed_commands`; basename only, not path |\n| `subcommand` | No | Same pattern as `command`; omit to match any subcommand |\n| `intent` | No | One of `hard_stop`, `use_alternative`, `scope_down`, `manual_only`, `stop_and_explain` |\n| `block_args` | Yes | Non-empty array of non-empty strings |\n| `reason` | Yes | Non-empty string, max 256 chars |\n\n### Rule Fields (`rulebook_version` 2)\n\nVersion 2 replaces `subcommand` and `block_args` with an exact-token `match` object. Version 1 rulebooks keep their fields and their behavior; a client that does not support version 2 rejects the rulebook instead of applying broader version 1 semantics.\n\n```json\n{\n  \"name\": \"block-terraform-apply-destroy\",\n  \"command\": \"terraform\",\n  \"match\": {\n    \"command_path\": [\"apply\"],\n    \"any_args\": [\"-destroy\", \"--destroy\"]\n  },\n  \"reason\": \"Review a destroy plan first with 'terraform plan -destroy'.\",\n  \"intent\": \"use_alternative\"\n}\n```\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `name` | Yes | Same as version 1 |\n| `command` | Yes | Same as version 1 |\n| `match.command_path` | Yes | Non-empty array of non-empty command words |\n| `match.any_args` | No | Non-empty array of unique non-empty argument tokens |\n| `match.exclude_args` | No | Non-empty array of unique non-empty argument tokens |\n| `intent` | No | Same as version 1 |\n| `reason` | Yes | Same as version 1 |\n\n### Matching Behavior (`rulebook_version` 2)\n\n- **Command**: Normalized to lowercase basename, as in version 1.\n- **Command path**: After recognized global options and their values are skipped, the next command words must equal `command_path` exactly. AWS, gcloud, and Azure CLI value-taking global options are built in; Terraform's `-chdir=dir` is `=`-joined and is skipped with its own token.\n- **Unrecognized options**: A token starting with `-` that is not a recognized global option is skipped without consuming a value, so an unlisted value-taking option with a separate value (`--newflag value`) makes the rule miss. This fails open deliberately; document such gaps in the rulebook.\n- **`any_args`**: At least one listed token must appear literally among the arguments.\n- **`exclude_args`**: Any listed token appearing literally among the arguments prevents the match, which is how a safe preview such as `aws s3 rm --dryrun` stays allowed.\n- **No short-option expansion**: Arguments compare as exact tokens, so list every accepted spelling (`\"-destroy\"` and `\"--destroy\"`).\n- **Literal and case-sensitive**: No regex, glob, or substring matching. The first matching rule wins.\n- Release channels are separate rules: `gcloud beta compute instances delete` needs its own `command_path`.\n\n### Test Fixture Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `command` | Yes | Non-empty shell command string |\n| `expect` | Yes | `\"blocked\"` or `\"allowed\"` |\n| `rule` | Required for blocked fixtures | Rule name expected to block the command |\n\nFixtures are optional documentation of intended behavior. Version 1 fixtures are shape-validated only. Version 2 fixtures are evaluated against the rulebook's own rules when a source is fetched by `rule add` or `rule update`, and by `rule verify`; a failing fixture rejects that source before it is written. Loading a rulebook does not re-evaluate fixtures. CC Safety Net never executes fixture commands; they are analyzer inputs only.\n\n## Matching Behavior\n\nThe subcommand, argument, and option rules below describe `rulebook_version` 1 rules; version 2 rules match as described in Matching Behavior (`rulebook_version` 2). Execution order and transparent wrappers apply to both.\n\n- **Command**: Normalized to lowercase basename with any trailing `.exe` removed (`/usr/bin/git` → `git`).\n- **Subcommand**: The first command token after recognized Git and Docker global options and their values; `--` ends option parsing. An unrecognized option without `=` may consume the following token as its value.\n- **Arguments**: Each `block_args` value is compared literally against every command token, including expanded short options. The command is blocked if **any** item matches.\n- **Short options**: Expanded (`-Ap` matches `-A`).\n- **Long options**: Exact match (`--all-files` does not match `--all`).\n- **Execution order**: Built-in rules first, then custom rulebooks. Custom rules only add restrictions.\n- **Transparent wrappers**: A configured wrapper such as `rtk` lets `rtk git commit` be analyzed as `git commit` only when `git` is protected by built-in analyzers or active custom rules. `rtk -- git commit` is also supported.\n\n## Workflow\n\n1. Run `cc-safety-net rule init` or create `rule.json` manually.\n2. Optionally run `cc-safety-net rule init --example` to create an inactive example rulebook.\n3. Use `cc-safety-net rule wrapper add rtk` for trusted transparent wrappers.\n4. Run `cc-safety-net rule add <source>` after creating or choosing a rulebook source; add `--only <rulebook...>` or `--ref <ref>` for repository selection. The command adds the selected sources and syncs them.\n5. Edit a local rulebook whenever you like: the edit is enforced on the next command, so there is nothing to run afterwards.\n6. Run `cc-safety-net rule update [source]` to re-fetch remote sources and rewrite the vendored copies; the command prints what changed. A source with an ordinary update failure keeps its vendored copy while the other selected sources still update. Resource-limit failures remain fatal for the whole update.\n7. Run `cc-safety-net rule verify` to validate config, local rulebooks, and shareable GitHub-source rulebook directories in the current repository (it does not fetch remote content).\n8. Run `cc-safety-net rule list` to inspect active rulebooks and transparent wrappers.\n\nA missing or invalid rulebook file makes that source inactive, and an unreadable or invalid `rule.json` makes every source in its scope inactive. Inactive sources stop applying their rules while other custom rules and all built-in protections stay active. Fix the file named in the diagnostic, or run `cc-safety-net rule update` when a remote source has not been vendored yet. Run `cc-safety-net status` to see degraded sources.\n";function Ur(P,V){if(!P.ok){Vc(P);return}Bc(P,V)}function Gc(P,V,z){if(P.ok)console.log(z);if(!P.add){Ur(P,`Added rulebook source: ${V}`);return}if(!P.ok){Vc(P);return}if(P.add.added.length>0)console.log(`Added ${P.add.added.length} ${P.add.added.length===1?"rulebook":"rulebooks"} from ${P.add.source} at ${P.add.ref}:`),P.add.added.forEach((Q)=>{console.log(`  - ${Q}`)});if(P.add.alreadyConfigured.length>0)console.log(`Rulebooks already configured from ${P.add.source} at ${P.add.ref}: ${P.add.alreadyConfigured.join(", ")}`);if(P.add.commits.length>0)console.log(`Vendored at ${P.add.commits.map((Q)=>Q.slice(0,7)).join(", ")}.`);Bc(P,"Rule config updated.")}function Bc(P,V){for(let z of P.changes??[])console.log(z);console.log(V),console.log(""),dg(P.entries)}function dg(P){if(P.length===0){console.log("Active rulebooks: (none)");return}console.log(`Active rulebooks (${P.length}):`);for(let V of P)console.log(`  - ${V.name} ${V.version} (${ug(V.ruleCount)})`),console.log(`    Source: ${V.spec}`)}function ug(P){return`${P} ${P===1?"rule":"rules"}`}function qc(P){un("Active sources",P.rulebooks,(V)=>[`[${V.source}] ${V.name} ${V.version}`,`  Source: ${V.spec}`]),un("Active rules",P.rules,(V)=>[`[${fg(P,V.name)}] ${V.name}`,...pg(V),`  Reason: ${V.reason}`]),un("Disabled rules",Uc(P,"off"),(V)=>[V.key]),un("Reason overrides",Uc(P,"reason"),(V)=>[V.key,`  Reason: ${V.value.reason}`]),un("Transparent wrappers",P.transparent_wrappers,(V)=>[V]),un("Issues",P.errors,(V)=>[V]),un("Warnings",P.warnings,(V)=>[V])}function un(P,V,z){if(V.length===0){console.log(`${P}: (none)`);return}console.log(`${P} (${V.length}):`);for(let Q of V){let[te,...ne]=z(Q);console.log(`  - ${te}`);for(let ce of ne)console.log(`    ${ce}`)}}function pg(P){if(!P.match)return[`  Command: ${P.subcommand?`${P.command} ${P.subcommand}`:P.command}`,`  Block args: ${P.block_args.join(", ")}`];return[`  Command: ${[P.command,...P.match.command_path].join(" ")}`,...P.match.any_args?[`  Any args: ${P.match.any_args.join(", ")}`]:[],...P.match.exclude_args?[`  Exclude args: ${P.match.exclude_args.join(", ")}`]:[]]}function fg(P,V){return P.rulebooks.find((z)=>z.rules.includes(V))?.source??"project"}function Uc(P,V){return Object.entries({...P.userConfig?.overrides??{},...P.projectConfig?.overrides??{}}).filter((z)=>{if(V==="off")return z[1]==="off";return!!z[1]&&typeof z[1]==="object"}).map(([z,Q])=>({key:z,value:Q}))}function Vc(P){for(let V of P.errors)console.error(V)}import{dirname as md,join as Kr}from"node:path";import{join as Ci,resolve as Sg}from"node:path";function Li(P){let V=m(P);if(V.errors.length>0)return{ok:!1,result:{ok:!1,errors:V.errors,entries:[]}};return{ok:!0,config:V.config??it}}function zc(P){Rt(P,{version:1,rules:[],overrides:{},transparent_wrappers:[]})}function Jc(P){Rt(P,{rulebook_version:1,name:"example-rules",version:"1.0.0",description:"Project-specific CC Safety Net rules.",author:"project",allowed_commands:["docker"],rules:[{name:"block-docker-system-prune",command:"docker",subcommand:"system",block_args:["prune"],reason:"Use targeted cleanup instead."}],tests:[{command:"docker system prune",expect:"blocked",rule:"block-docker-system-prune"}]})}import{dirname as Vr}from"node:path";var mg="custom.";function Gr(P){if(P.rulebook_version!==2)return[];let V=P.rules.map((z)=>({name:z.name,command:z.command,block_args:[],match:z.match,reason:z.reason,intent:z.intent}));return(P.tests??[]).flatMap((z,Q)=>{let te=wi(h(z.command));if(te.length===0)return[`tests[${Q}]: could not parse fixture command: ${z.command}`];let ne=te.reduce((ce,ye)=>ce??_(ye,V)?.id.slice(mg.length),void 0);if(z.expect==="blocked"){if(ne===z.rule)return[];let ce=ne?`"${ne}" matched first`:"no rule matched";return[`tests[${Q}]: expected "${z.rule}" to block "${z.command}" but ${ce}`]}return ne?[`tests[${Q}]: expected "${z.command}" to be allowed but "${ne}" matched`]:[]})}function wi(P){return P.nodes.flatMap((V)=>{if(V.kind==="group"||V.kind==="function")return wi(V.body);if(V.kind!=="command")return[];let z=ge(X(V.dialect,V.words)).words.map(t);return[...z.length>0?[z]:[],...V.nested.flatMap((Q)=>wi(Q))]})}var Br=Object.freeze({concurrency:4,maxRequests:131,maxResponseBytes:67108864});function qr(P={}){return{requests:0,responseBytes:0,maxRequests:P.maxRequests??Br.maxRequests,maxResponseBytes:P.maxResponseBytes??Br.maxResponseBytes}}function zt(P){return{controller:new AbortController,budget:qr(),resolveUrl:P}}function Wc(P){return P instanceof Error&&P.message==="Rule synchronization exceeds CC Safety Net's safe resource limits."}function Kc(P){if(P.requests>=P.maxRequests)throw Error("Rule synchronization exceeds CC Safety Net's safe resource limits.");P.requests++}function Yc(P,V){if(V>P.maxResponseBytes-P.responseBytes)throw P.responseBytes+=V,Error("Rule synchronization exceeds CC Safety Net's safe resource limits.");P.responseBytes+=V}var Qc=Object.freeze({timeoutMs:15000,metadataBytes:524288,commitBytes:262144,treeBytes:16777216,rawBytes:4194304});async function Zc(P,V,z=v(Vr(Vr(V)),"rules policy"),Q=zt()){if(S(P))return vg(P,Q);return hg(P,V,z)}async function ed(P,V,z,Q,te,ne){if(!S(P))return Zc(P,V,z,Q);let ce=te?null:gg(P,V,z);if(ce)return ce;if(!te&&!ne)throw Error(`${P} is not vendored; run rule update ${P} to vendor it`);return Zc(P,V,z,Q)}function gg(P,V,z=v(Vr(Vr(V)),"rules policy")){let Q=A(P),te=I(V,Q.name),ne=n(i(z,te));if(ne===null)return null;let ce=se(ki(ne,`Invalid rulebook ${te}.`));if(ce.name!==Q.name)throw Error(`rulebook name "${ce.name}" in ${te} must match "${Q.name}"`);return{spec:P,rulebook:ce,content:ne}}async function td(P,V={}){if(!q(P))throw Error(`Invalid GitHub repository source: ${P}`);let[z,Q]=P.split("/");if(!z||!Q)throw Error(`Invalid GitHub repository source: ${P}`);if(V.ref!==void 0&&!oe(V.ref))throw Error(`GitHub rulebook refs must use valid path segments: ${V.ref}`);let te=V.operation??zt(),ne=V.ref??await yg(z,Q,P,te),ce=await rd(z,Q,ne,P,te),ye=await zr(`https://api.github.com/repos/${z}/${Q}/git/trees/${ce}?recursive=1`,"tree",te),he=ye.response;if(!he.ok)throw Error(`Failed to inspect ${P}: GitHub tree returned ${he.status}`);let be=JSON.parse(ye.content);if(!Array.isArray(be?.tree))throw Error(`Failed to inspect ${P}: unexpected GitHub tree response`);let Le=be.tree,Ie=[...new Set(Le.flatMap((Xe)=>{if(!Xe||typeof Xe!=="object")return[];let et=Xe;if(et.type!=="blob"||typeof et.path!=="string")return[];let xe=et.path.match(rt);return xe?.[1]?[xe[1]]:[]}))].sort();if(Ie.length===0)throw Error(`No rulebooks found in ${P} under ${de}/`);return{source:P,owner:z,repo:Q,ref:ne,commit:ce,names:Ie}}async function yg(P,V,z,Q){let te=await zr(`https://api.github.com/repos/${P}/${V}`,"metadata",Q),ne=te.response;if(!ne.ok)throw Error(`Failed to inspect ${z}: GitHub returned ${ne.status}`);let ye=JSON.parse(te.content)?.default_branch;if(typeof ye!=="string"||ye==="")throw Error(`Failed to inspect ${z}: missing default branch`);if(!oe(ye))throw Error(`GitHub returned an invalid default branch: ${ye}`);return ye}function hg(P,V,z){ot(P);let Q=I(V,P),te=n(i(z,Q));if(te===null)throw Error(`Rulebook source not found: ${P}`);let ne=nd(ki(te,"Invalid local rulebook source."));if(ne.name!==P)throw Error(`rulebook name "${ne.name}" must match local source "${P}"`);return{spec:P,rulebook:ne,content:te}}async function vg(P,V){let z=A(P),Q=await rd(z.owner,z.repo,z.ref,P,V),te=await zr(`https://raw.githubusercontent.com/${z.owner}/${z.repo}/${Q}/${z.path}`,"raw",V),ne=te.response;if(!ne.ok)throw Error(`Failed to fetch ${P}: GitHub raw returned ${ne.status}`);let ce=te.content,ye=nd(ki(ce,"Invalid GitHub rulebook response."));if(ye.name!==z.name)throw Error(`rulebook name "${ye.name}" must match GitHub source "${z.name}"`);return{spec:P,rulebook:ye,content:ce}}function nd(P){let V=se(P),z=Gr(V);if(z.length>0)throw Error(z.join("; "));return V}function ki(P,V){try{return JSON.parse(P)}catch{throw Error(V)}}async function rd(P,V,z,Q,te){let ne=await zr(`https://api.github.com/repos/${P}/${V}/commits/${encodeURIComponent(z)}`,"commit",te),ce=ne.response;if(!ce.ok)throw Error(`Failed to resolve ${Q}: GitHub returned ${ce.status}`);let ye=JSON.parse(ne.content);if(typeof ye?.sha!=="string"||ye.sha==="")throw Error(`Failed to resolve commit for ${Q}`);return ye.sha}async function bg(P,V,z={}){if(z.signal?.aborted)throw z.signal.reason;let Q=z.budget??qr(),te=new AbortController,ne=()=>te.abort(z.signal?.reason);z.signal?.addEventListener("abort",ne,{once:!0});let ce=!1,ye=setTimeout(()=>{if(te.signal.aborted)return;ce=!0,te.abort()},z.timeoutMs??Qc.timeoutMs);try{if(z.signal?.aborted)throw z.signal.reason;Kc(Q);let he=await fetch(P,{signal:te.signal,redirect:"error"});if(!he.ok)return od(he),{response:he,content:""};return{response:he,content:await Lg(he,V,Q,()=>te.abort())}}catch(he){if(ce)throw Error("GitHub request timed out",{cause:he});if(z.signal?.aborted)throw z.signal.reason;throw he}finally{clearTimeout(ye),z.signal?.removeEventListener("abort",ne)}}function zr(P,V,z){return bg(z.resolveUrl?.(P)??P,V,{budget:z.budget,signal:z.controller.signal})}async function Lg(P,V,z=qr(),Q){let te=Qc[`${V}Bytes`],ne=Number(P.headers.get("content-length"));if(Number.isFinite(ne)&&ne>te)throw od(P),Error(`GitHub ${V} response exceeds ${te} bytes`);if(!P.body)return"";let ce=P.body.getReader(),ye=[],he=0;while(!0){let be=await ce.read();if(be.done)break;try{Yc(z,be.value.byteLength)}catch(Le){throw Q?.(),Xc(ce),Le}if(he+=be.value.byteLength,he>te)throw Q?.(),Xc(ce),Error(`GitHub ${V} response exceeds ${te} bytes`);ye.push(Buffer.from(be.value))}return Buffer.concat(ye,he).toString("utf-8")}function od(P){if(!P.body)return;id(()=>P.body?.cancel())}function Xc(P){id(()=>P.cancel())}function id(P){try{Promise.resolve(P()).catch(()=>{})}catch{}}var wg=/^([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+)#(.+)$/;function sd(P,V){let z=cd(P.rules,V);if(z.length>0)return{ok:!0,specs:z};return ld(P.rules,V)}function ad(P,V){let z=cd(P,V);if(z.length>0)return{ok:!0,specs:z};let Q=xg(P,V);if(Q.length>0)return{ok:!0,specs:Q};let te=Cg(P,V);if(!te.ok)return te;if(te.specs.length>0)return{ok:!0,specs:te.specs};return ld(P,V)}function ld(P,V){let z=P.filter((Q)=>xi(Q)?.name===V);if(z.length===1)return{ok:!0,specs:z};return kg(V,z)}function kg(P,V){return{ok:!1,result:{ok:!1,errors:V.length===0?[`No configured rulebook matches ${P}`]:[`Ambiguous rulebook match ${P}: ${V.join(", ")}`],entries:[]}}}function cd(P,V){return P.filter((z)=>z===V)}function xg(P,V){let z=V.match(wg),Q=z?.[1],te=z?.[2],ne=z?.[3];if(!Q||!te||!ne||!oe(ne))return[];return dd(P,(ce)=>ce.owner===Q&&ce.repo===te&&ce.ref===ne)}function Cg(P,V){if(!q(V))return{ok:!0,specs:[]};let[z,Q]=V.split("/"),te=dd(P,(ce)=>ce.owner===z&&ce.repo===Q);if(new Set(te.map((ce)=>xi(ce)?.ref).filter((ce)=>!!ce)).size<2)return{ok:!0,specs:te};return{ok:!1,result:{ok:!1,errors:[`Multiple refs are configured for ${V}. Use an explicit ref:`,`  cc-safety-net rule remove ${V}#<ref>`],entries:[]}}}function xi(P){try{return A(P)}catch{return null}}function dd(P,V){return P.filter((z)=>{let Q=xi(z);return Q?V(Q):!1})}async function Wr(P,V={}){let z=Si(V);return Rg(P,z,await Jr(P,z,zt()))}function Rg(P,V,z){if(!z.ok)return z;let Q=Pt(P,V),te=[...new Set(j(Q.configPath,Q.filesystemScope))];if(te.length===0)return z;return{ok:!1,errors:te,entries:z.entries}}async function Jr(P,V,z,Q={},te=new Set,ne=new Set){try{let ce=Pt(P,V),ye=Li(ce.configTarget);if(!ye.ok)return ye.result;let he=ye.config,be=V.only?sd(he,V.only):{ok:!0,specs:he.rules};if(!be.ok)return be.result;let Le=new Set([...V.refresh?be.specs:[],...te]),Ie=(ut)=>ed(ut,ce.configDir,ce.filesystemScope,z,Le.has(ut),!V.refresh||Le.has(ut)),Xe=await jg(he.rules,V.refresh?(ut)=>Ie(ut).then((vt)=>({ok:!0,item:vt})).catch((vt)=>{if(Wc(vt))throw vt;return{ok:!1,spec:ut,message:vt instanceof Error?vt.message:String(vt)}}):async(ut)=>({ok:!0,item:await Ie(ut)}),z),et=Xe.filter((ut)=>!ut.ok),xe=Xe.filter((ut)=>ut.ok).map((ut)=>ut.item),at=xe.flatMap((ut)=>Pg(ut,he.rules)),ct=xe.flatMap((ut)=>Eg(ut,ne,ce)),lt=new Set([...at,...ct].map((ut)=>ut.spec)),pt=[...et,...at,...ct],mt=[],gt=Ag(mt,()=>xe.flatMap((ut)=>lt.has(ut.spec)||pt.length>0&&ne.has(ut.spec)?[]:Dg(ut,ce,Q,mt)));return{ok:pt.length===0,errors:pt.map((ut)=>`Failed to update ${ut.spec}: ${ut.message}`),entries:xe.map(Tg),changes:gt}}catch(ce){return Jn(ce)}}function Pg(P,V){if(!S(P.spec))return[];let z=Pe(P.spec),Q=V.filter((te)=>te!==P.spec&&Pe(te).toLowerCase()===z.toLowerCase());if(Q.length===0)return[];return[{ok:!1,spec:P.spec,message:`rulebook name "${z}" is also claimed by ${Q.join(", ")}; rename one of them`}]}function Eg(P,V,z){if(!V.has(P.spec)||!S(P.spec))return[];let Q=I(z.configDir,P.rulebook.name),te=n(i(z.filesystemScope,Q));if(te===null||te===P.content)return[];return[{ok:!1,spec:P.spec,message:`${Q} already exists and no configured source claims it; remove or rename the file, then re-run rule add`}]}function Dg(P,V,z,Q){if(!S(P.spec))return[];let te=I(V.configDir,P.rulebook.name),ne=i(V.filesystemScope,te),ce=n(ne);if(ce===P.content)return[];return Q?.push({target:ne,previous:ce}),g(ne,P.content,void 0,z._testAfterPolicyRename),_g(P,ce)}function Ag(P,V){try{return V()}catch(z){for(let Q of[...P].reverse()){if(Q.previous===null){N(Q.target);continue}g(Q.target,Q.previous)}throw z}}function _g(P,V){if(V===null)return[`Vendored ${P.spec} (${P.rulebook.version})`];let z=me(V),Q="problem"in z?null:z.rulebook,te=new Map(Q?.rules.map((ce)=>[ce.name,JSON.stringify(ce)])??[]),ne=new Set(P.rulebook.rules.map((ce)=>ce.name));return[`Updated ${P.spec} (${Q?.version??"unreadable"} -> ${P.rulebook.version})`,...[...ne].filter((ce)=>!te.has(ce)).map((ce)=>`  + ${ce}`),...[...te.keys()].filter((ce)=>!ne.has(ce)).map((ce)=>`  - ${ce}`),...P.rulebook.rules.filter((ce)=>{let ye=te.get(ce.name);return ye!==void 0&&ye!==JSON.stringify(ce)}).map((ce)=>`  ~ ${ce.name}`)]}function Tg(P){return{spec:P.spec,name:P.rulebook.name,version:P.rulebook.version,ruleCount:P.rulebook.rules.length}}async function ud(P,V,z={}){return $g(P,V,Mg(z),zt())}async function $g(P,V,z,Q,te={}){let ne=null,ce=!1;try{let ye=Pt(P,z),he=n(ye.configTarget);ne={target:ye.configTarget,content:he};let be=Li(ye.configTarget);if(!be.ok)return be.result;let Le=be.config,Ie=q(V);Ig(V,z,Ie);let Xe=Ie?await td(V,{ref:z.ref,operation:Q}):null,et=Xe?Og(Xe,z.rulebooks):[],xe=Xe?et.map((mt)=>Fg(Le.rules,Xe,mt)??`${V}#${Xe.ref}/${mt}`):[V],at=xe.filter((mt)=>!Le.rules.includes(mt)),ct=[...Le.rules,...at];if(ct.length>pe)return Ng();if(ct.length!==Le.rules.length)ce=!0,Rt(ye.configTarget,{version:1,rules:ct,overrides:Le.overrides??{},transparent_wrappers:Le.transparent_wrappers??[]},void 0,te._testAfterPolicyRename);let lt=await Jr(P,z,Q,te,new Set(at),new Set(at));if(!lt.ok)zn(ye.configTarget,he);if(!lt.ok||!Xe)return lt;let pt=et.filter((mt,gt)=>at.includes(xe[gt]??""));return{...lt,add:{source:V,ref:Xe.ref,selected:et,added:pt,alreadyConfigured:et.filter((mt)=>!pt.includes(mt)),commits:at.length>0?[Xe.commit]:[]}}}catch(ye){if(ce&&ne)try{zn(ne.target,ne.content)}catch(he){return Jn(he)}return Jn(ye)}}function Ig(P,V,z){if(!z&&V.rulebooks!==void 0)throw Error("--only can only select rulebooks from an owner/repo source");if(!z&&V.ref)throw Error(`--ref can only select a ref for an owner/repo source: ${P}`);if(V.rulebooks?.length===0)throw Error("--only requires at least one rulebook name");let Q=V.rulebooks?.filter((te)=>!l.test(te))??[];if(Q.length>0)throw Error(`Invalid rulebook names: ${Q.join(", ")}`)}function Og(P,V){let z=V?[...new Set(V)]:P.names,Q=z.filter((te)=>!P.names.includes(te));if(Q.length>0)throw Error(`Rulebooks not found in ${P.source} at ${P.ref}: ${Q.join(", ")}
Available rulebooks: ${P.names.join(", ")}`);return z}function Fg(P,V,z){let Q=`${V.source}#${V.ref}/${z}`;if(P.includes(Q))return Q;let te=`${V.source}#${V.commit}/${z}`;return P.find((ne)=>ne===te)}async function jg(P,V,z=zt()){if(P.length>pe)throw Error(fe);let Q=Array(P.length),te=0,ne,ce=Array.from({length:Math.min(P.length,Br.concurrency)},async()=>{while(!ne){let ye=te;if(ye>=P.length)return;te++;try{Q[ye]=await V(P[ye],ye,z.controller.signal)}catch(he){if(!ne)ne={value:he},te=P.length,z.controller.abort(he);return}}});if(await Promise.all(ce),ne)throw ne.value;return Q}function Ng(){return{ok:!1,errors:[fe],entries:[]}}function Si(P){return{cwd:P.cwd,userConfigDir:P.userConfigDir,userConfigPath:P.userConfigPath,projectConfigPath:P.projectConfigPath,global:P.global,only:P.only,refresh:P.refresh}}function Mg(P){return{...Si(P),ref:P.ref,rulebooks:P.rulebooks}}function Hg(P){return{...Si(P),deleteSource:P.deleteSource}}async function pd(P,V,z={}){try{return await Ug(P,V,Hg(z),{})}catch(Q){return Jn(Q)}}async function Ug(P,V,z,Q){let te=Pt(P,z),ne=m(te.configTarget);if(ne.errors.length>0)return{ok:!1,errors:ne.errors,entries:[]};if(!ne.config)return{ok:!1,errors:[`No config found at ${te.configPath}`],entries:[]};let ce=ad(ne.config.rules,V);if(!ce.ok)return ce.result;let ye=z.deleteSource?Gg(te.configDir,ce.specs,te.filesystemScope):{ok:!0,dirs:[]};if(!ye.ok)return ye.result;let he=n(te.configTarget);if(he===null)return Jn(Error("Rules config is unavailable."));try{Rt(te.configTarget,{version:1,rules:ne.config.rules.filter((Ie)=>!ce.specs.includes(Ie)),overrides:ne.config.overrides??{},transparent_wrappers:ne.config.transparent_wrappers??[]},void 0,Q._testAfterPolicyRename)}catch(Ie){throw zn(te.configTarget,he),Ie}let be=await Jr(P,z,zt(),Q);if(!be.ok)return zn(te.configTarget,he),be;let Le=Bg(ye.dirs,Q,te.filesystemScope);if(!Le.ok){zn(te.configTarget,he);let Ie=await Jr(P,z,zt(),Q);if(!Ie.ok)return{ok:!1,errors:[...Le.result.errors,...Ie.errors],entries:Ie.entries};return Le.result}return be}function Gg(P,V,z){let Q=V.flatMap((ye)=>l.test(ye)?[]:["--delete-source can only delete local rulebook sources"]),te=V.map((ye)=>Ci(P,ye)),ne=Q.length>0?[]:te.flatMap((ye)=>fd(ye,z)),ce=[...Q,...ne];return ce.length>0?{ok:!1,result:{ok:!1,errors:ce,entries:[]}}:{ok:!0,dirs:te}}function fd(P,V){let z=Sg(P),Q=i(V,z),te=le(Q);if(!te)return[`Local rulebook source directory not found: ${P}`];let ne=te.find((ce)=>ce.name==="rulebook.json");if(!ne)return[`Local rulebook source directory is missing rulebook.json: ${P}`];if(ne.kind!=="file")throw new r(V.label);if(n(i(V,Ci(z,"rulebook.json"))),te.length>1)return[`Local rulebook source directory contains extra files: ${P}. delete manually if you really want to remove the directory.`];return[]}function Bg(P,V,z){let Q=P.flatMap((te)=>{try{if(!le(i(z,te)))return[];let ne=fd(te,z);if(ne.length>0)return ne;return qg(te,V,z),[]}catch(ne){return[`Failed to delete local rulebook source ${te}: ${ne instanceof Error?ne.message:String(ne)}`]}});return Q.length>0?{ok:!1,result:{ok:!1,errors:Q,entries:[]}}:{ok:!0}}function qg(P,V,z){if(V._testDeleteLocalSourceDir){V._testDeleteLocalSourceDir(P);return}N(i(z,Ci(P,ue))),nt(i(z,P))}function zn(P,V){if(V===null){N(P);return}g(P,V)}function Jn(P){return{ok:!1,errors:[P instanceof Error?P.message:String(P)],entries:[]}}var Vg=".safety-net.json",zg="~/.cc-safety-net/config.json";async function hd(P,V){return[await gd(P,{legacyPath:Is({cwd:V.cwd}),configPath:F(V.cwd),defaultRulebookName:"project-rules",migratedFrom:Vg,cleanup:V.cleanup,syncOptions:{cwd:V.cwd}}),await gd(P,{legacyPath:nr(P),configPath:M(P),defaultRulebookName:"user-rules",migratedFrom:zg,cleanup:V.cleanup,syncOptions:{cwd:V.cwd,global:!0}})].every((Q)=>Q)?0:1}async function gd(P,V){let z=Pt(P,V.syncOptions),Q=i(z.filesystemScope,V.legacyPath),te=n(Q);if(te===null)return console.log(`No legacy config found at ${V.legacyPath}`),!0;let ne=Wg(te);if(!ne.ok){for(let et of ne.errors)console.error(et);return!1}let ce=m(z.configTarget);if(ce.errors.length>0){for(let et of ce.errors)console.error(et);return!1}let ye=ce.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},he=Kg(md(V.configPath),ye.rules,V.defaultRulebookName,V.migratedFrom,z.filesystemScope),be=Kr(md(V.configPath),he,"rulebook.json"),Le=i(z.filesystemScope,be),Ie=[yd(z.configTarget),yd(Le)],Xe=await Jg(P,V,z.configTarget,Le,he,ne.config.rules,ye.rules.includes(he)?ye.rules:[...ye.rules,he],ye.overrides??{},ye.transparent_wrappers??[]);if(!Xe.ok){Xg(Ie);for(let et of Xe.errors)console.error(et);return!1}if(!V.cleanup)return console.log(`Migrated legacy config at ${V.legacyPath}. Legacy file is no longer used.`),!0;if(!Zg(z.configTarget,Le,he,V.migratedFrom,ne.config.rules))return console.error(`Migration cleanup verification failed for ${V.legacyPath}`),!1;return N(Q),console.log(`Deleted legacy config at ${V.legacyPath}`),!0}async function Jg(P,V,z,Q,te,ne,ce,ye,he){try{return Rt(z,{version:1,rules:ce,overrides:ye,transparent_wrappers:he}),Rt(Q,Yg(te,V.migratedFrom,ne)),await Wr(P,V.syncOptions)}catch(be){return{ok:!1,errors:[be instanceof Error?be.message:String(be)]}}}function Wg(P){try{let V=JSON.parse(P),z=io(V);if(z.errors.length>0)return{ok:!1,errors:z.errors};return{ok:!0,config:{version:1,rules:V.rules??[]}}}catch{return{ok:!1,errors:["Invalid JSON"]}}}function Kg(P,V,z,Q,te){let ne=V.find((ce)=>Qg(i(te,Kr(P,ce,"rulebook.json")))===Q);if(ne)return ne;if(n(i(te,Kr(P,z,"rulebook.json")))===null)return z;for(let ce=2;;ce++){let ye=`${z}-${ce}`;if(n(i(te,Kr(P,ye,"rulebook.json")))===null)return ye}}function Yg(P,V,z){return{rulebook_version:1,name:P,version:"1.0.0",description:"Migrated CC Safety Net rules.",author:"project",migrated_from:V,allowed_commands:[...new Set(z.map((Q)=>Q.command))],rules:z,tests:z.map((Q)=>({command:[Q.command,Q.subcommand,Q.block_args[0]].filter(Boolean).join(" "),expect:"blocked",rule:Q.name}))}}function Zg(P,V,z,Q,te){if(!m(P).config?.rules.includes(z))return!1;try{let ce=n(V);if(ce===null)return!1;let ye=JSON.parse(ce);return ye.migrated_from===Q&&JSON.stringify(ye.rules)===JSON.stringify(te)}catch{return!1}}function yd(P){return{target:P,content:n(P)}}function Xg(P){for(let V of P){if(V.content===null){N(V.target);continue}g(V.target,V.content)}}function Qg(P){let V=n(P);if(V===null)return null;try{let z=JSON.parse(V);return typeof z.migrated_from==="string"?z.migrated_from:null}catch{return null}}import{mkdir as ey,readFile as ty,writeFile as ny}from"node:fs/promises";import{dirname as ry,join as oy}from"node:path";var iy=86400000,sy=604800000;async function bd(P,V=Date.now()){if(P.env.get("CC_SAFETY_NET_NO_UPDATE_CHECK"))return null;let z=je(P);if(!z)return null;let Q=oy(z,".cc-safety-net","update-check.json"),te=await ay(Q,V);if(!te.lastCheck||V-te.lastCheck>iy){let ye=await Wt();if(te.lastCheck=V,ye.latestVersion)te.latestVersion=ye.latestVersion;if(!await vd(Q,te))return null;if(ye.error)return null}let ne=te.latestVersion,ce=wt();if(!ne||!po(ne,ce))return null;if(te.notifiedVersion===ne&&te.notifiedAt!==void 0&&V-te.notifiedAt<sy)return null;if(te.notifiedVersion=ne,te.notifiedAt=V,!await vd(Q,te))return null;return`UPDATE_AVAILABLE: cc-safety-net v${ne} is available (running v${ce}). Ask the user once whether to run \`npx -y cc-safety-net@latest update\`; continue the current task either way and do not raise this again.`}async function ay(P,V){let z=await ty(P,"utf8").then((ne)=>JSON.parse(ne)).catch(()=>{return});if(!z||typeof z!=="object"||Array.isArray(z))return{};let Q=z,te=(ne)=>typeof ne==="number"&&Number.isFinite(ne)&&ne<=V?ne:void 0;return{lastCheck:te(Q.lastCheck),latestVersion:typeof Q.latestVersion==="string"?Q.latestVersion:void 0,notifiedVersion:typeof Q.notifiedVersion==="string"?Q.notifiedVersion:void 0,notifiedAt:te(Q.notifiedAt)}}async function vd(P,V){return ey(ry(P),{recursive:!0,mode:448}).then(()=>ny(P,JSON.stringify(V),{mode:384})).then(()=>!0).catch(()=>!1)}import{join as ly,resolve as Ri}from"node:path";var Ld="CC Safety Net Config",cy="═".repeat(Ld.length),dy="https://raw.githubusercontent.com/kenryu42/cc-safety-net/main/assets/cc-safety-net.schema.json",uy=new Set(["rule.json","rule.lock","cache"]);function wd(P,V={}){try{return py(P,V)}catch(z){if(z instanceof r)return console.error(z.message),1;throw z}}function py(P,V){let z=V.cwd??process.cwd(),Q=K(P,{cwd:z}),te=nr(P),ne=rs(z),ce=Ri(z,de),ye=i(Q.userScope,te),he=i(Q.projectScope,ne),be=!1,Le=!1,Ie=[],Xe=[],et=fy(i(Q.projectScope,ce));if(gy(),n(Q.userConfigTarget)!==null){let xe=Jt(Q.userConfigTarget);if(xe.errors.push(...j(Q.userConfigPath,Q.userScope)),Ie.push({scope:"User",path:Q.userConfigPath,result:xe,schema:"rules",target:Q.userConfigTarget}),xe.errors.length>0)be=!0}if(n(ye)!==null)if(Le=!0,n(Q.userConfigTarget)!==null)Xe.push(Yr("user","cleanup"));else{let xe=so(ye);if(Ie.push({scope:"User",path:te,result:xe,schema:"legacy",inactive:!0,target:ye}),Xe.push(Yr("user",xe.errors.length>0?"fix-or-delete":"migrate")),xe.errors.length>0)be=!0}if(n(Q.projectConfigTarget)!==null){let xe=Jt(Q.projectConfigTarget);if(xe.errors.push(...j(Q.projectConfigPath,Q.projectScope)),Ie.push({scope:"Project",path:Ri(Q.projectConfigPath),result:xe,schema:"rules",target:Q.projectConfigTarget}),xe.errors.length>0)be=!0;if(n(he)!==null)Le=!0,Xe.push(Yr("project","cleanup"))}else if(n(he)!==null){Le=!0,be=!0;let xe=so(he);Ie.push({scope:"Project",path:Ri(ne),result:xe,schema:"legacy",inactive:!0,target:he}),Xe.push(Yr("project",xe.errors.length>0?"fix-or-delete":"migrate"))}if(et?.result.errors.length)be=!0;if(Ie.length===0&&!et)return console.log(`
No config files found. Using built-in rules only.`),0;for(let xe of Ie)if(xe.inactive)hy(xe.scope,xe.path,xe.result);else if(xe.result.errors.length>0)vy(xe.scope,xe.path,xe.result.errors);else{if(xe.schema==="rules"&&wy(xe.target))console.log(`
Added $schema to ${xe.scope.toLowerCase()} config.`);yy(xe.scope,xe.path,xe.result,xe.schema)}for(let xe of Xe)console.error(`
${dt.red(xe)}`);if(et)if(et.result.errors.length>0)Ly(et.path,et.result.errors);else by(et.path,et.result);if(be)return console.error(`
Config validation failed.`),1;return console.log(Le?`
Configs valid with warnings.`:`
All configs valid.`),0}function Yr(P,V){let z=`legacy ${P} config`;if(V==="cleanup")return`Warning: Legacy ${P} config is no longer needed. Run \`npx -y cc-safety-net rule migrate --cleanup\` to clean it up safely.`;if(V==="migrate")return`Warning: Legacy ${P} config is ignored by CC Safety Net. Run \`npx -y cc-safety-net rule migrate\`.`;return`Warning: Legacy ${P} config is no longer supported. Fix or delete the ${z}, then run \`npx -y cc-safety-net rule migrate\`.`}function fy(P){if(le(P)===null)return null;let V=my(P);if(V.ruleNames.size===0&&V.errors.length===0)return null;return{path:P.path,result:V}}function my(P){let V=[],z=new Set,Q=(le(P)??[]).filter((te)=>!uy.has(te.name)).sort((te,ne)=>te.name.localeCompare(ne.name));if(Q.length===0)return{errors:V,ruleNames:z};for(let te of Q){if(!l.test(te.name)){V.push(`rulebook directory names must match ${l}: ${te.name}`);continue}if(te.kind!=="directory"){V.push(`${te.name} must be a rulebook directory`);continue}let ne=i(P.scope,ly(P.path,te.name,"rulebook.json")),ce=n(ne);if(ce===null){V.push(`${te.name}/rulebook.json is required`);continue}try{let ye;try{ye=JSON.parse(ce)}catch{V.push(`${te.name}/rulebook.json: invalid JSON`);continue}let he=se(ye);if(he.name!==te.name){V.push(`rulebook name "${he.name}" must match folder "${te.name}"`);continue}let be=Gr(he);if(be.length>0){V.push(...be.map((Le)=>`${te.name}/rulebook.json: ${Le}`));continue}z.add(te.name)}catch(ye){V.push(ye instanceof Error?`${te.name}/rulebook.json: ${ye.message}`:`${te.name}/rulebook.json: ${String(ye)}`)}}return{errors:V,ruleNames:z}}function gy(){console.log(Ld),console.log(cy)}function yy(P,V,z,Q){if(console.log(`
✓ ${P} config: ${V}`),console.log(`  Schema: ${Q==="rules"?"rulebook sources":"legacy inline rules"}`),z.ruleNames.size>0){console.log(`  ${Q==="rules"?"Sources":"Rules"}:`);let te=1;for(let ne of z.ruleNames)console.log(`    ${te}. ${ne}`),te++}else console.log(`  ${Q==="rules"?"Sources":"Rules"}: (none)`)}function hy(P,V,z){if(console.error(`
✗ Legacy ${P.toLowerCase()} config: ${V}`),console.error("  Schema: legacy inline rules"),console.error("  Status: ignored by CC Safety Net"),z.errors.length>0){console.error("  Errors:");let Q=1;for(let te of z.errors)for(let ne of te.split("; "))console.error(`    ${Q}. ${ne}`),Q++;return}if(z.ruleNames.size>0){console.error("  Rules:");let Q=1;for(let te of z.ruleNames)console.error(`    ${Q}. ${te}`),Q++;return}console.error("  Rules: (none)")}function vy(P,V,z){kd(`${P} config`,V,z)}function by(P,V){console.log(`
✓ GitHub source rules: ${P}`),console.log("  Rulebooks:");let z=1;for(let Q of V.ruleNames)console.log(`    ${z}. ${Q}`),z++}function Ly(P,V){kd("GitHub source rules",P,V)}function kd(P,V,z){console.error(`
✗ ${P}: ${V}`),console.error("  Errors:");let Q=1;for(let te of z)for(let ne of te.split("; "))console.error(`    ${Q}. ${ne}`),Q++}function wy(P){try{let V=n(P);if(V===null)return!1;let z=JSON.parse(V);if(z.$schema)return!1;return g(P,JSON.stringify({$schema:dy,...z},null,2)),!0}catch(V){if(V instanceof r)throw V;return!1}}var xd=new Set(["init","add","remove","update","sync","list","wrapper","migrate","doc","verify"]),xy=new Set(["add","remove","list"]),Cy="cc-safety-net/rulebooks";async function Cd(P,V){try{return await Sy(P,V)}catch(z){if(z instanceof r)return console.error(z.message),1;throw z}}async function Sy(P,V){let z=Py(V),Q=z.help?Ry(z.positionals):null;if(Q)return wn(Q),0;if(z.errors.length>0){for(let ye of z.errors)console.error(ye);return 1}let te=z.positionals[0];if(!te)return wn(fn,console.error),1;let ne=z.positionals[1],ce={global:z.global};if(te==="init"){let ye=Pt(P,ce);_y(ye.configTarget);let he=ky(ye.configDir,"example-rules","rulebook.json"),be=i(ye.filesystemScope,he);if(z.example&&n(be)===null)Jc(be);let Le=j(ye.configPath,ye.filesystemScope);for(let Ie of Le)console.error(Ie);if(Le.length>0)return 1;return console.log("Rule config initialized."),0}if(te==="add"){let ye=Sd(z);if(!ye)return console.error("rule add requires a source (pass --only <rulebook...> to select from cc-safety-net/rulebooks)"),1;let he=Pt(P,ce),be=await ud(P,ye,{...ce,ref:z.ref,rulebooks:z.only.length>0?z.only:void 0});return Gc(be,ye,`Scope: ${z.global?"user":"project"} (${he.configDir})`),be.ok?0:1}if(te==="remove"){if(!ne)return console.error("rule remove requires a source"),1;let ye=await pd(P,ne,{...ce,deleteSource:z.deleteSource});return Ur(ye,`Removed rulebook source: ${ne}`),ye.ok?0:1}if(te==="update"){let ye=await Wr(P,{...ce,only:ne,refresh:!0});return Ur(ye,"Rule config updated."),ye.ok?0:1}if(te==="sync")return js(P,{global:z.global});if(te==="list"){let ye=J(P,{cwd:process.cwd()});return qc(ye),ye.errors.length>0?1:0}if(te==="wrapper")return Ty(P,z);if(te==="migrate")return hd(P,{cleanup:z.cleanup,cwd:process.cwd()});if(te==="doc"){console.log(Hc);let ye=await bd(P);if(ye)console.error(ye);return 0}if(te==="verify")return wd(P);return 1}function Ry(P){if(P.length===0)return fn;let V=fn.subcommands.filter((Q)=>Q.usage.split(" ")[0]===P[0]);if(V.length===0)return null;if(P.length===1&&V.length>1)return{name:`rule ${P[0]}`,description:`Subcommands of rule ${P[0]}`,usage:`rule ${P[0]} <subcommand>`,subcommands:V,options:[]};let z=P.length===1?V[0]:V.find((Q)=>Q.usage.split(" ")[1]===P[1]);if(!z)return null;return{name:`rule ${P[0]}`,description:z.description,usage:`rule ${z.usage}`,options:P[0]==="add"?no:[],examples:P[0]==="add"?ro:void 0}}function Py(P){let V=xt({label:"rule",booleans:{global:["-g","--global"],cleanup:["--cleanup"],deleteSource:["--delete-source"],example:["--example"]},values:{ref:["--ref"]},lists:{only:["--only"]},positionals:"list"},P),z={...V.flags,ref:V.values.ref,only:V.lists.only??[],help:V.help,positionals:V.positionals,errors:V.errors};return Ey(z),z}function Ey(P){let[V]=P.positionals;if(V&&!xd.has(V))P.errors.push(`Unknown rule subcommand: ${V}`);if(P.deleteSource&&V!=="remove")if(V&&xd.has(V))P.errors.push(`Unknown option for rule ${V}: --delete-source`);else P.errors.push("--delete-source is only valid with 'rule remove'");if(P.cleanup&&V!=="migrate")P.errors.push(Wn(V,"--cleanup"));if(P.example&&V!=="init")P.errors.push(Wn(V,"--example"));if(P.ref&&V!=="add")P.errors.push(Wn(V,"--ref"));if(P.only.length>0&&V!=="add")P.errors.push(Wn(V,"--only"));if(V==="add")Dy(P);if(V==="migrate"){if(P.global)P.errors.push(Wn(V,"--global"));if(P.positionals.length>1)P.errors.push(`Unexpected rule migrate argument: ${P.positionals[1]}`)}else if(V==="wrapper")Ay(P);else if(P.positionals.length>2)P.errors.push(`Unexpected rule argument: ${P.positionals[2]}`);if(V==="list"&&P.global)P.errors.push("Unknown option for rule list: --global")}function Sd(P){if(P.positionals[1])return P.positionals[1];if(P.ref||P.only.length>0)return Cy;return}function Dy(P){let V=Sd(P);if(!V)return;if((P.ref||P.only.length>0)&&!q(V)){if(P.ref)P.errors.push(`--ref can only select a ref for an owner/repo source: ${V}`);if(P.only.length>0)P.errors.push("--only can only select rulebooks from an owner/repo source");return}if(P.ref&&!oe(P.ref))P.errors.push(`--ref must use valid path segments: ${P.ref}`);let z=P.only.filter((Q)=>!l.test(Q));if(z.length>0)P.errors.push(`Invalid rulebook names: ${z.join(", ")}`)}function Wn(P,V){return P?`Unknown option for rule ${P}: ${V}`:`Unknown option for rule: ${V}`}function Ay(P){let V=P.positionals[1],z=P.positionals[2];if(!V){P.errors.push("rule wrapper requires add, remove, or list");return}if(!xy.has(V)){P.errors.push(`Unknown rule wrapper action: ${V}`);return}if(V==="list"){if(z)P.errors.push(`Unexpected rule wrapper argument: ${z}`);return}if(!z){P.errors.push(`rule wrapper ${V} requires a command`);return}if(P.positionals.length>3)P.errors.push(`Unexpected rule wrapper argument: ${P.positionals[3]}`)}function _y(P){if(n(P)===null){zc(P);return}let V=m(P);if(!V.config)return;Rt(P,{version:1,rules:V.config.rules,overrides:V.config.overrides??{},transparent_wrappers:V.config.transparent_wrappers??[]})}async function Ty(P,V){let z=V.positionals[1],Q=V.positionals[2],te=Pt(P,{global:V.global}).configTarget;if(z==="list"){let he=m(te);if(he.errors.length>0){for(let be of he.errors)console.error(be);return 1}return $y(he.config?.transparent_wrappers??[]),0}if(!Q||!w.test(Q))return console.error("transparent wrapper must match command pattern"),1;if(Re(Q))return console.error(`reserved command "${Q}" cannot be a wrapper`),1;let ne=m(te);if(ne.errors.length>0){for(let he of ne.errors)console.error(he);return 1}let ce=ne.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},ye=z==="add"?[...new Set([...ce.transparent_wrappers??[],Q])]:(ce.transparent_wrappers??[]).filter((he)=>he!==Q);return Rt(te,{version:1,rules:ce.rules,overrides:ce.overrides??{},transparent_wrappers:ye}),console.log(z==="add"?`Added transparent wrapper: ${Q}`:`Removed transparent wrapper: ${Q}`),0}function $y(P){if(P.length===0){console.log("Transparent wrappers: (none)");return}console.log(`Transparent wrappers (${P.length}):`);for(let V of P)console.log(`  - ${V}`)}import{sep as My}from"node:path";import{existsSync as Iy,readFileSync as Oy}from"node:fs";import{join as Fy}from"node:path";async function jy(P){if(P.isTTY)return null;return(await We(P).catch(()=>null))?.trim()||null}function Ny(P){let V=P.env.get("CLAUDE_SETTINGS_PATH");if(V)return V;return Fy(P.home,".claude","settings.json")}function Pi(P){let V=Ny(P);if(!Iy(V))return!1;try{let z=Oy(V,"utf-8"),Q=JSON.parse(z);if(!Q.enabledPlugins)return!1;let te="cc-safety-net@cc-marketplace";if(!(te in Q.enabledPlugins))return!1;return Q.enabledPlugins[te]===!0}catch(z){if(R(o.debug,P.env))console.error(`CC Safety Net debug: failed to read Claude settings: ${V}: ${z instanceof Error?z.message:String(z)}`);return!1}}async function Ei(P,V=process.stdin){let z=Pi(P),Q;if(!z)Q="\uD83D\uDEE1️ CC Safety Net ❌";else{let ne=E(P,{cwd:process.cwd()}),ce=ne.policy,ye=L(ce,P.env),he=Object.values(G(ce,ye.capabilities)).some((Ie)=>Ie.changesInherited),be={standard:"✅",strict:"\uD83D\uDD12",paranoid:"\uD83D\uDC41️",custom:"\uD83D\uDD27"}[he?"custom":ye.effectiveLevel],Le=(ne.policyScopes?.weakenings.length??0)>0?"\uD83D\uDD3B":"";Q=`\uD83D\uDEE1️ CC Safety Net ${be}${ye.worktreeMode?"\uD83C\uDF33":""}${Le}${ne.state==="degraded"?"⚠️":""}`}let te=await jy(V);if(te&&!te.startsWith("{"))console.log(`${te} | ${Q}`);else console.log(Q)}function Rd(P){let V=E(P,{cwd:process.cwd()}),z=V.policy,Q=L(z,P.env),te=!!process.env.NO_COLOR||!process.stdout.isTTY,ne=Math.min(process.stdout.columns||80,100),ce=te?"ok":"✔",ye=te?"OFF":"✘",he=(at,ct)=>{let lt=`  ${at.padEnd(13)}${ct}`;return(lt.length>ne?`${lt.slice(0,ne-1)}…`:lt).replaceAll(ye,dt.red(ye))},be=Object.values(G(z,Q.capabilities)).some((at)=>at.changesInherited),Le=(at)=>at===P.home||at.startsWith(`${P.home}${My}`)?`~${at.slice(P.home.length)}`:at,Ie={ready:dt.green,degraded:dt.yellow}[V.state],Xe=V.policyScopes?.weakenings??[],et=[...Pi(P)?[]:["plugin cc-safety-net@cc-marketplace is disabled in Claude Code; nothing is enforced in Claude Code until it is re-enabled. Other integrations are not affected."],...V.diagnostics],xe=te?"-":"·";console.log([`${te?"":"\uD83D\uDEE1️  "}CC Safety Net — ${Ie(V.state)}`,"",he("Protection",`destructive ${z.destructiveCommandProtectionEnabled?ce:ye}   secrets ${z.secretProtection.enabled?ce:ye}`),he("Level",be?`${Q.effectiveLevel} (customised)`:Q.effectiveLevel),he("Rules",z.rules.length===0?"none active":`${z.rules.length} active`),he("Policy",Le(u(P))),...V.policyScopes?[he("Project",Le(b(process.cwd())))]:[],...Q.worktreeMode?[he("Worktree","relaxations active")]:[],"",...Xe.length===0?[]:["  Project policy",...Xe.flatMap((at)=>jn(at,"      ",ne-6).map((ct,lt)=>lt===0?`    ${ct}`:ct)),""],...et.length===0?["  Everything configured is active."]:["  Not active",...et.flatMap((at)=>jn(at,"      ",ne-6).map((ct,lt)=>lt===0?`    ${xe} ${ct}`:ct)),"","  Full report: cc-safety-net doctor"]].join(`
`))}import{spawn as jd}from"node:child_process";import{randomBytes as Ky}from"node:crypto";import{existsSync as Yy}from"node:fs";import{createServer as Zy}from"node:http";import{Writable as Xy}from"node:stream";var Zr=500;function Hy(P){let V=P.filter((te)=>te.decision!=="allow"),z=P.filter((te)=>te.decision==="allow"),Q=Math.min(V.length,Math.max(Zr-z.length,Math.ceil(Zr/2)));return[...V.slice(0,Q),...z.slice(0,Zr-Q)]}function Pd(P,V,z=O(P)){if(z)B(P,z);let Q=(ct)=>new Date(ct.getFullYear(),ct.getMonth(),ct.getDate()).getTime(),te=Q(new Date),ne=new Date(te);ne.setDate(ne.getDate()-(V-1));let ce=ne.getTime(),ye=[],he={count:0};for(let ct of z?Yt(z,he):[])for(let lt of pn(ct,he)){let pt=new Date(lt.ts).getTime();if(!Number.isFinite(pt))continue;if(pt>=ce)ye.push(lt)}ye.sort((ct,lt)=>new Date(lt.ts).getTime()-new Date(ct.ts).getTime());let be=Array.from({length:V},()=>0),Le=Array.from({length:V},()=>0),Ie={},Xe={},et={},xe=0,at=0;for(let ct of ye){let lt=ct.agent||"unknown";Ie[lt]=(Ie[lt]??0)+1;let pt=Math.round((te-Q(new Date(ct.ts)))/86400000),mt=V-1-pt,gt=pt>=0&&pt<V;if(gt)Le[mt]=(Le[mt]??0)+1;if(ct.decision!=="allow"){if(xe++,ct.ruleId)Xe[ct.ruleId]=(Xe[ct.ruleId]??0)+1;let ut=to(ct.segment||ct.command);if(ut)et[ut]=(et[ut]??0)+1;if(ct.failureStage)at++;if(gt)be[mt]=(be[mt]??0)+1}}return{days:V,logsDir:z,homeDir:P.home,totalInWindow:ye.length,truncated:ye.length>Zr,unreadable:he.count,counts:{blocked:xe,allowed:ye.length-xe,agents:Ie,blockedByDay:be,analyzedByDay:Le,rules:Xe,commands:et,errors:at},entries:Hy(ye).sort((ct,lt)=>new Date(lt.ts).getTime()-new Date(ct.ts).getTime())}}import{spawn as Uy}from"node:child_process";import{existsSync as Gy,statSync as Ed}from"node:fs";import{delimiter as By,join as qy}from"node:path";var Vy=120000,Xr="Choose the project folder",zy=`try
  return POSIX path of (choose folder with prompt "${Xr}")
on error number -128
  return ""
end try`,Jy=`Add-Type -AssemblyName System.Windows.Forms
$dialog = New-Object System.Windows.Forms.FolderBrowserDialog
$dialog.Description = '${Xr}'
if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { [Console]::Out.Write($dialog.SelectedPath) }`,Dd=[{binary:"zenity",args:["--file-selection","--directory",`--title=${Xr}`]},{binary:"kdialog",args:["--getexistingdirectory",".","--title",Xr]}],Ad=(P,V)=>(V.PATH??"").split(By).some((z)=>{if(z.length===0)return!1;try{let Q=Ed(qy(z,P));return Q.isFile()&&(Q.mode&73)!==0}catch{return!1}});function Di(P,V){if(P==="darwin"||P==="win32")return!0;if(P!=="linux")return!1;if(!V.DISPLAY&&!V.WAYLAND_DISPLAY)return!1;return Dd.some((z)=>Ad(z.binary,V))}function Wy(P,V){if(P==="darwin")return{cmd:"osascript",args:["-e",zy]};if(P==="win32")return{cmd:"powershell.exe",args:["-NoProfile","-STA","-Command",Jy]};let z=Dd.find((Q)=>Ad(Q.binary,V));return z?{cmd:z.binary,args:z.args}:null}function Ai(P=process.platform,V=process.env){let z=Wy(P,V);if(!z)return Promise.resolve({error:"No folder dialog is available on this system"});return new Promise((Q)=>{let te=Uy(z.cmd,z.args,{env:V,stdio:["ignore","pipe","pipe"]}),ne="",ce=!1,ye=(be)=>{if(ce)return;ce=!0,clearTimeout(he),Q(be)},he=setTimeout(()=>{te.kill(),ye({error:"The folder dialog timed out"})},Vy);te.stdout.on("data",(be)=>{ne+=be.toString()}),te.on("error",()=>ye({error:`Could not open the folder dialog (${z.cmd})`})),te.on("close",()=>{let be=ne.trim().replace(/\/+$/,"");if(!be)return ye({cancelled:!0});if(!Gy(be)||!Ed(be).isDirectory())return ye({error:"That selection is not a folder on disk"});ye({path:be})})})}var _d=`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
`;var Td='<script id="ccsn-data" type="application/json">';function $d(P){return _d.replace(Td,()=>Td+JSON.stringify({token:P}).replaceAll("<","\\u003c"))}var Qr="kenryu42/cc-safety-net",Qy=`https://github.com/${Qr}`,$i=1e4,eh=7,th="The project draft directory changed; reload the draft before applying.",nh="audit settings are user scope only; remove the audit section from a project proposal";async function Nd(P,V={}){let z=xt({label:"gui",booleans:{noOpen:["--no-open"]}},P),Q=V.log??console.log,te=V.error??console.error;if(z.errors.length>0){for(let ce of z.errors)te(ce);return te("Usage: cc-safety-net gui [--no-open]"),1}let ne=await rh(c,V);if(Q(`CC Safety Net policy GUI: ${ne.url}`),!z.flags.noOpen)try{await(V.openBrowser??mh)(ne.url)}catch(ce){te(`Failed to open browser: ${ce instanceof Error?ce.message:String(ce)}`),te(`Open this URL manually: ${ne.url}`)}if(V.keepAlive===!1)return await ne.close(),0;return await fh(ne),0}async function rh(P,V={}){let z=Ky(24).toString("base64url"),Q={dir:null,revision:0},te=Zy((ye,he)=>{oh(P,ye,he,z,V,Q)});await new Promise((ye,he)=>{te.once("error",he),te.listen(0,"127.0.0.1",()=>{te.off("error",he),ye()})});let ce=`http://127.0.0.1:${te.address().port}`;return{origin:ce,token:z,url:`${ce}/?token=${encodeURIComponent(z)}`,close:()=>ph(te)}}async function oh(P,V,z,Q,te,ne){let ce=P(),ye=new URL(V.url??"/","http://127.0.0.1");if(V.method==="GET"&&ye.pathname==="/favicon.ico"){z.writeHead(204,{"cache-control":"no-store"}),z.end();return}if(!ch(V,ye,Q)){yt(z,403,{error:"Forbidden"});return}if(V.method==="GET"&&ye.pathname==="/"){uh(z,$d(Q));return}if(V.method==="GET"&&ye.pathname==="/api/policy"){let he=$c(ce,te),be=E(ce,_i(te));yt(z,200,{...he,configState:Ne(be),...be.policyScopes?{projectPolicy:{path:b(te.cwd??process.cwd()),weakenings:be.policyScopes.weakenings}}:{},destructiveCommandRules:U,secretPatterns:Ke,version:wt(),preview:he.errors.length>0?null:Ae(he.policy,ce.env)});return}if(V.method==="POST"&&ye.pathname==="/api/policy/preview"){let he=await Kn(V);if(!he.ok){yt(z,he.status,{errors:[he.error]});return}let be=Ic(ce,he.value);yt(z,be.errors.length>0?400:200,be);return}if(V.method==="POST"&&ye.pathname==="/api/policy/explain"){let he=await Kn(V);if(!he.ok){yt(z,he.status,{errors:[he.error]});return}let be=he.value;if(be===null||typeof be.command!=="string"){yt(z,400,{errors:["command must be a string"]});return}let Le=qn(be.policy,ce.home);if(Le.length>0){yt(z,400,{errors:Le});return}yt(z,200,ah(ce,be.command,be.policy,te));return}if(V.method==="POST"&&ye.pathname==="/api/policy"){let he=await Kn(V);if(!he.ok){yt(z,he.status,{errors:[he.error]});return}let be=Vt(ce,he.value,te);yt(z,be.errors.length>0?400:200,be);return}if(V.method==="POST"&&ye.pathname==="/api/reset"){yt(z,200,Vt(ce,Z,te));return}if(V.method==="POST"&&ye.pathname==="/api/repair"){yt(z,200,Oc(ce,te));return}if(V.method==="POST"&&ye.pathname==="/api/policy/project/choose-directory"){let he=await(te.chooseDirectory??Ai)();if("path"in he)ne.dir=he.path,ne.revision+=1;yt(z,200,{cancelled:"cancelled"in he,..."error"in he?{error:he.error}:{}});return}if(V.method==="GET"&&ye.pathname==="/api/policy/project"){let he=Md(ne,te),be=Id(he,ce.home),Le=Vn(ce,te);yt(z,200,{path:b(he),revision:ne.revision,baseline:Le.baseline,userPolicyDiagnostics:Le.diagnostics,projection:be.projection,projectionDiagnostics:be.diagnostics,canPickDirectory:Di(process.platform,process.env)});return}if(V.method==="POST"&&ye.pathname==="/api/policy/project/diff"){let he=await Od(ce,V,z,ne,te);if(!he)return;let be=Id(he.dir,ce.home),Le=Vn(ce,te).baseline,Ie=Y(Le,ae(he.proposal,ce.home).policy);yt(z,200,{rows:Mr(Y(Le,be.projection).policy,Ie.policy,!1),weakenings:Ie.weakenings,existingFileDiagnostics:be.diagnostics});return}if(V.method==="POST"&&ye.pathname==="/api/policy/project/apply"){let he=await Od(ce,V,z,ne,te);if(!he)return;let be=sh(he.dir,he.proposal,ce.home);yt(z,be.errors.length>0?500:200,be);return}if(V.method==="GET"&&ye.pathname==="/api/activity"){let he=ee(ce,te),be=lh(ye.searchParams.get("days"),he);if(be===null){yt(z,400,{error:`days must be an integer between 1 and ${he}`});return}yt(z,200,Pd(ce,be,te.activityLogsDir));return}if(V.method==="POST"&&ye.pathname==="/api/rules/choose-directory"){yt(z,200,await Ai());return}if(V.method==="GET"&&ye.pathname==="/api/rules"){let he=J(ce,_i(te)),be=new Map(he.rules.map((Le)=>[Le.name,Le]));yt(z,200,{projectPath:te.cwd??process.cwd(),canPickDirectory:Di(process.platform,process.env),rulebooks:he.rulebooks.map((Le)=>({source:Le.source,spec:Le.spec,name:Le.name,version:Le.version,rules:Le.rules.flatMap((Ie)=>{let Xe=be.get(Ie);if(!Xe)return[];return[{name:Xe.name,command:Xe.command,subcommand:Xe.subcommand,block_args:Xe.block_args,reason:Xe.reason}]})})),errors:he.errors,warnings:he.warnings});return}if(V.method==="GET"&&ye.pathname==="/api/star/context"){yt(z,200,await(te.fetchStarContext??(()=>Lh(ce,{logsDir:te.activityLogsDir})))());return}if(V.method==="POST"&&ye.pathname==="/api/star"){let he=await(te.starRepo??gh)();yt(z,200,he.ok?{ok:!0}:{ok:!1,fallbackUrl:Qy});return}if(V.method==="GET"&&ye.pathname==="/api/integrations"){yt(z,200,await(te.fetchIntegrations??(()=>yh(ce)))());return}if(V.method==="GET"&&ye.pathname==="/api/health"){yt(z,200,await(te.fetchHealth??vh)());return}if(V.method==="POST"&&(ye.pathname==="/api/install"||ye.pathname==="/api/uninstall")){let he=await Kn(V);if(!he.ok){yt(z,he.status,{errors:[he.error]});return}let be=he.value?.target;if(typeof be!=="string"||!Mt.some((Ie)=>Ie.target===be)){yt(z,400,{error:"unknown target"});return}let Le=ye.pathname==="/api/install"?"install":"uninstall";yt(z,200,await(te.runIntegration??bh)(Le,be));return}yt(z,404,{error:"Not found"})}function _i(P){return{...P,cwd:P.cwd??process.cwd()}}function Md(P,V){return P.dir??V.cwd??process.cwd()}function Id(P,V){let z=b(P),Q=Yy(z)?dn(z):{value:void 0,errors:[]},te=ae(Q.value,V);return{projection:te.policy,diagnostics:[...Q.errors,...te.diagnostics]}}async function Od(P,V,z,Q,te){let ne=Md(Q,te),ce=Q.revision,ye=await Kn(V);if(!ye.ok)return yt(z,ye.status,{errors:[ye.error]}),null;let he=ye.value;if(typeof he?.revision!=="number")return yt(z,400,{errors:["revision must be a number"]}),null;if(he.revision!==ce)return yt(z,409,{errors:[th]}),null;let be=ih(he.proposal,P.home);if(be.length>0)return yt(z,400,{errors:be}),null;return{dir:ne,proposal:he.proposal}}function ih(P,V){let z=qn(P,V);if(z.length>0)return z;return P?.audit===void 0?[]:[nh]}function sh(P,V,z){let Q=b(P),te=Hr(V,C(V,z));try{return g(i(v(P,"project policy"),Q),`${JSON.stringify(te,null,2)}
`),{path:Q,errors:[]}}catch(ne){return{path:Q,errors:[ne instanceof Error?ne.message:String(ne)]}}}function ah(P,V,z,Q){let te=C(z,P.home),ne=E(P,_i(Q)),ce=Ee({rules:ne.policy.rules,transparentWrappers:ne.policy.transparentWrappers,safety:De(te.safety),worktreeMode:te.workflow.worktree_mode,destructiveCommandProtectionEnabled:te.destructive_command_protection.enabled,destructiveCommandRuleOverrides:te.destructive_command_protection.overrides,destructiveCommandAllowPaths:te.destructive_command_protection.allow_paths,secretProtection:{enabled:te.secret_protection.enabled,disabledRules:Oe(te.secret_protection.overrides),denyPaths:te.secret_protection.deny_paths,allowPaths:te.secret_protection.allow_paths}});return Nn(V,{policySnapshot:ce,cwd:Q.cwd,userConfigDir:Q.userConfigDir},P)}function lh(P,V){if(P===null)return Math.min(eh,V);let z=Number(P);if(!Number.isInteger(z)||z<1||z>V)return null;return z}function ch(P,V,z){if(V.searchParams.get("token")!==z)return!1;if(P.method!=="POST")return!0;return P.headers["x-cc-safety-net-token"]===z}var dh=1048576;async function Kn(P){let V=[],z=0;for await(let Q of P){let te=Q;if(z+=te.byteLength,z>dh)return{ok:!1,status:413,error:"Request body is too large"};V.push(te)}try{return{ok:!0,value:JSON.parse(Buffer.concat(V).toString("utf-8")||"{}")}}catch(Q){return{ok:!1,status:400,error:`Invalid JSON: ${Q instanceof Error?Q.message:String(Q)}`}}}function uh(P,V){P.writeHead(200,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}),P.end(V)}function yt(P,V,z){P.writeHead(V,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}),P.end(JSON.stringify(z))}function ph(P){return new Promise((V,z)=>{P.close((Q)=>Q?z(Q):V())})}function fh(P){return new Promise((V)=>{let z=()=>{process.off("SIGINT",Q),process.off("SIGTERM",Q)},Q=()=>{z(),P.close().then(V)};process.once("SIGINT",Q),process.once("SIGTERM",Q)})}function mh(P){let V=process.platform==="darwin"?"open":process.platform==="win32"?"cmd":"xdg-open",z=process.platform==="win32"?["/c","start","",P]:[P];return new Promise((Q,te)=>{let ne=jd(V,z,{detached:!0,stdio:"ignore"}),ce=(he)=>{ne.off("spawn",ye),te(he)},ye=()=>{ne.off("error",ce),ne.unref(),Q()};ne.once("error",ce),ne.once("spawn",ye)})}async function gh(P="gh",V=$i){return{ok:await Ti(P,["api","-X","PUT",`/user/starred/${Qr}`],V)===0}}async function yh(P,V={}){let z=await sr(V.fetcher),Q=hh(P,z);return{targets:$t.map((te)=>{let ne=Q.find((ce)=>ce.platform===te.id);return{target:te.id,label:bt(te.id),version:z.versions[te.id]??null,status:ne?.configured?"active":ne?.detected?"disabled":ne?.inspectionStatus==="not-inspected"?"not-inspected":"not-installed"}}),system:{version:z.version,nodeVersion:z.nodeVersion,platform:z.platform}}}function hh(P,V){return Ln(P,process.cwd(),{ampPluginListOutput:V.ampPluginListOutput,codexPluginListOutput:V.codexPluginListOutput,copilotCliVersion:V.versions["copilot-cli"]})}async function vh(P={}){let V=await(P.checkUpdates??Wt)();return{update:{latestVersion:V.latestVersion??null,updateAvailable:V.updateAvailable}}}var Fd=Promise.resolve();function bh(P,V,z={}){let Q=async()=>{let ne=[],{log:ce,error:ye}=console;console.log=(...he)=>ne.push(he.map(String).join(" ")),console.error=console.log;try{return{ok:await Bn(P,[],{selectTargets:async()=>[V],output:new Xy({write(be,Le,Ie){ne.push(String(be).replace(/\n$/,"")),Ie()}}),...z})===0,output:ne.join(`
`)}}finally{console.log=ce,console.error=ye}},te=Fd.then(Q);return Fd=te.then(()=>{return},()=>{return}),te}async function Lh(P,V={}){let[z,Q,te]=await Promise.all([wh(V.command),kh(V.fetchRepo),Promise.resolve(tr(P,ee(P),V.logsDir).totalBlocked)]);return{starred:z,starCount:Q,blockedTotal:te}}async function wh(P="gh",V=$i){if(await Ti(P,["auth","status"],V)!==0)return null;let z=await Ti(P,["api",`/user/starred/${Qr}`],V);if(z===0)return!0;if(z===null)return null;return!1}function Ti(P,V,z){return new Promise((Q)=>{let te=jd(P,V,{stdio:"ignore",windowsHide:!0}),ne=!1,ce,ye=(he)=>{if(ne)return;if(ne=!0,ce)clearTimeout(ce);Q(he)};te.once("error",()=>ye(null)),te.once("close",ye),ce=setTimeout(()=>{te.kill(),ye(null)},z)})}async function kh(P=fetch){try{let V=await P(`https://api.github.com/repos/${Qr}`,{headers:{accept:"application/vnd.github+json"},signal:AbortSignal.timeout($i)});if(!V.ok)return null;let z=await V.json();return typeof z.stargazers_count==="number"?z.stargazers_count:null}catch{return null}}function xh(P){if(P[0]!=="help")return!1;let V=P[1];if(!V)ni(),process.exit(0);if(Mn(V))process.exit(0);console.error(`Unknown command: ${V}`),console.error("Run 'cc-safety-net --help' for available commands."),process.exit(1)}var Ch={hook:async()=>{console.error("hook requires exactly one integration flag. Try: cc-safety-net hook --kimi-code"),Mn("hook",console.error),process.exit(1)},install:async(P)=>{process.exit(await Bn("install",P))},update:async(P)=>{process.exit(await hi(P))},uninstall:async(P)=>{process.exit(await Bn("uninstall",P))},rule:async(P)=>{process.exit(await Cd(c(),P))},policy:async(P)=>{process.exit(await Mc(c(),P))},status:async(P)=>{if(Ht(xt({label:"status"},P).errors))process.exit(1);Rd(c())},statusline:async(P)=>{let V=xt({label:"statusline",booleans:{claudeCode:["-cc","--claude-code"]}},P);if(V.errors.length===0&&V.flags.claudeCode){await Ei(c());return}if(Ht(V.errors),!V.flags.claudeCode)console.error("statusline requires --claude-code (-cc)");Mn("statusline",console.error),process.exit(1)},doctor:async(P)=>{let V=Wo(P);if(!V)process.exit(1);let z=await ul(c(),{json:V.json,skipUpdateCheck:V.skipUpdateCheck});process.exit(z)},logs:async(P)=>{process.exit(await Ui(c(),P))},gui:async(P)=>{process.exit(await Nd(P))},explain:async(P)=>{process.exit(await kl(c(),P))}};async function Sh(P){let V=xt({label:"cc-safety-net",booleans:{version:["-V","--version"]},positionals:"list"},P);if(xh(P))return;let z=P[0],Q=z?er(z):void 0;if(V.help&&Q&&Q.name!=="rule")Mn(Q.name),process.exit(0);if(!z||V.help&&!Q)ni(),process.exit(0);if(V.flags.version)Sl(),process.exit(0);if(Q){await Ch[Q.name](P.slice(1));return}if(z==="--statusline"){await Ei(c());return}console.error(z.startsWith("-")?`Unknown option: ${z}`:`Unknown command: ${z}`),console.error("Run 'cc-safety-net --help' for usage."),process.exit(1)}export{Sh as runCli};
