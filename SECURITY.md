...
For every non-shell interpreter's inline, heredoc, or stdin code, standard mode treats string literals as inert diagnostic or test data only when the code as a whole contains no recognizable filesystem, command-execution, or eval marker; one such marker anywhere in the code keeps every literal in it as a candidate. Strict and paranoid modes keep every literal unconditionally and also inspect inside literal text. Configured deny paths and their descendants are never relaxed, and opaque or custom access wrappers remain outside standard mode's non-adversarial guarantee.
