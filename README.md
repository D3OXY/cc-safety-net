...
Linked-worktree mode relaxes only local discard. Git discards inside a temp-root repository unrelated to the workspace are allowed like temp deletes, as is `git worktree remove --force` of a temp-root path unrelated to the workspace. See [Modes](https://ccsafetynet.com/docs/configuration/modes).
